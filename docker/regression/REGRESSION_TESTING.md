...
The repo provides four npm scripts that wrap the docker-compose commands:
...
#    empty) this also runs `regression:gen-forms` to generate Angular entity
#    forms against a temporary DB — that step takes ~5 minutes but only
#    happens when the form output is missing. Subsequent builds skip it.
npm run regression:build

# 3. Run the full stack — creates docker/regression/test-results/run-{TIMESTAMP}/
#    Fast path: just `docker compose up`, no rebuild work, no codegen.
...
The underlying commands live in [`package.json`](../../package.json) under `regression:*` if you need to invoke `docker compose` directly.

> **Why two separate commands?** The Angular entity forms used by `mjexplorer`
> are themselves generated by `mj codegen`, which needs a live SQL Server with
> the AssociationDB schema installed. That setup can't run inside a Docker
> `build` step (no compose network during a build). So `regression:build`
> spawns a temporary stack, runs codegen, captures the output to
> `.docker-generated/`, tears down, and then builds the real images. This is
> slow (~5 min) but cached; `regression:up` skips it entirely.
...
├── gen-forms.sh                  # Convenience wrapper: `npm run regression:gen-forms`
...
# (c) Against the regression DB from your host (after `npm run regression:up`)
