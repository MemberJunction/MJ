...
import type { EntityFieldInfo, EntityInfo, RunQueryResult, RunViewResult } from '@memberjunction/global';
import { ComponentLibraryEntity, ComponentMetadataEngine } from '@memberjunction/core-entities';
import type { UserInfo } from '@memberjunction/global';
          ...
          if (t.isIdentifier(init.callee.property) && init.callee.property.name === 'then') {
            ...
            if (t.isCallExpression(obj) && t.isMemberExpression(obj.callee)) {
              if (
                ...
                (obj.callee.property.name === methodName || obj.callee.property.name === methodName + 's')
              ) {
                isFromMethod = true;
              }
            }
          ...
          CallExpression(path: NodePath<t.CallExpression>) {
            ...
            if (t.isIdentifier(callee) && (callee.name.includes('load') || callee.name.includes('fetch'))) {
              ...
              if (funcParent) {
                ...
                if (
                  funcName &&
                  (funcName.includes('Sort') ||
                    funcName.includes('Filter') ||
                    funcName.includes('handleSort') ||
                    funcName.includes('handleFilter'))
                ) {
                  violations.push({
                    ...
                    .map((p) => {
                      ...
                      if (providedName) {
                        ...
                        const existingProp = paramValue.properties.find(
                          (prop) =>
                            t.isObjectProperty(prop) && t.isIdentifier(prop.key) && prop.key.name.toLowerCase() === p.name.toLowerCase()
                        ) as t.ObjectProperty | undefined;
        ...
        traverse(ast, {
          MemberExpression(path: NodePath<t.MemberExpression>) {
            ...
            if (t.isIdentifier(path.node.object) && t.isIdentifier(path.node.property)) {
              ...
              const isFromRunQuery =
                path.scope.hasBinding(objName) && ComponentLinter.isVariableFromRunQueryOrView(path, objName, 'RunQuery');
              const isFromRunView =
                path.scope.hasBinding(objName) && ComponentLinter.isVariableFromRunQueryOrView(path, objName, 'RunView');
          ...
          VariableDeclarator(path: NodePath<t.VariableDeclarator>) {
            ...
            if (t.isObjectPattern(path.node.id) && t.isIdentifier(path.node.init)) {
              ...
              // Only check if we can definitively trace this to RunQuery or RunView
              const isFromRunQuery =
                path.scope.hasBinding(sourceName) && ComponentLinter.isVariableFromRunQueryOrView(path, sourceName, 'RunQuery');
              const isFromRunView =
                path.scope.hasBinding(sourceName) && ComponentLinter.isVariableFromRunQueryOrView(path, sourceName, 'RunView');
                ...
                traverse(
                  path.node,
                  {
                    MemberExpression(innerPath: NodePath<t.MemberExpression>) {
                      if (t.isIdentifier(innerPath.node.object) && innerPath.node.object.name === varName) {
...

    {
      ...
      test: (ast: t.File, componentName: string, componentSpec?: ComponentSpec) => {
        ...
        traverse(ast, {
          ...
          MemberExpression(path: NodePath<t.MemberExpression>) {
            if (t.isIdentifier(path.node.object) && t.isIdentifier(path.node.property) && path.node.property.name === 'length') {
              const objName = path.node.object.name;
  ...
  if (${resultInfo.varName}?.Success) {
...

    {
      ...
      test: (ast: t.File, componentName: string, componentSpec?: ComponentSpec) => {
        ...
        for (const dep of componentSpec.dependencies) {
          const requiredProps = dep.properties?.filter((p) => p.required)?.map((p) => p.name) || [];
        ...
        const standardProps = new Set(['styles', 'utilities', 'components', 'callbacks', 'savedUserSettings', 'onSaveUserSettings']);
        ...
        const checkFieldNullability = (propertyName: string): FieldNullabilityResult => {
          ...
          try {
            ...
            for (const dataReqEntity of componentSpec.dataRequirements.entities) {
              ...
              const fullEntity = options.utilities.md?.Entities.find(
                (e: EntityInfo) => e.Name && e.Name.toLowerCase() === entityName.toLowerCase()
              );
              ...
              if (fullEntity && fullEntity.Fields && Array.isArray(fullEntity.Fields)) {
                ...
                const field = fullEntity.Fields.find(
                  (f: EntityFieldInfo) => f.Name && f.Name.trim().toLowerCase() === propertyName.trim().toLowerCase()
                );
            ...
            if (t.isCallExpression(expr) && t.isMemberExpression(expr.callee) && t.isIdentifier(expr.callee.property)) {
              ...
              if (formattingMethods.has(methodName)) {
                ...
                if (t.isMemberExpression(callee.object) && t.isIdentifier(callee.object.property)) {
                  ...
                  if (
                    grandParent &&
                    t.isLogicalExpression(grandParent) &&
              ...
              for (const prop of configObj.properties) {
                if (
                  ...
                  if (requiredEntities.size > 0 && !requiredEntities.has(usedEntity)) {
                    ...
                    if (entityFields) {
                      // Check Fields array
                      const fieldsProperty = configObj.properties.find(
                        (p) => t.isObjectProperty(p) && t.isIdentifier(p.key) && p.key.name === 'Fields'
                      );
                        ...
                        for (const fieldElement of fieldsProperty.value.elements) {
                          if (t.isStringLiteral(fieldElement)) {
                            ...
                            if (/COUNT\s*\(|SUM\s*\(|AVG\s*\(|MAX\s*\(|MIN\s*\(/i.test(fieldName)) {
                              ...
                              const isAllowed =
                                entityFields.displayFields.has(fieldName) ||
                      ...
                      // Check OrderBy field
                      const orderByProperty = configObj.properties.find(
                        (p) => t.isObjectProperty(p) && t.isIdentifier(p.key) && p.key.name === 'OrderBy'
                      );
  ...
  private static checkRefElementType(
    ...
    traverse(ast, {
      JSXElement(path: NodePath<t.JSXElement>) {
        ...
        const refAttr = openingElement.attributes.find(
          (attr) => t.isJSXAttribute(attr) && t.isJSXIdentifier(attr.name) && attr.name.name === 'ref'
        );
