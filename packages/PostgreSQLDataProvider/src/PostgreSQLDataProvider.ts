...
SELECT * FROM delete_result`;
    ...
    private getCRUDFunctionName(type: 'create' | 'update' | 'delete', entityInfo: EntityInfo): string {
        // PG CRUD functions are generated by CodeGen's PostgreSQLCodeGenProvider as
        // fn_<verb>_<snake_case_table> (e.g. fn_create_workspace). The baseline
        // converter still emits spCreate*/spUpdate*/spDelete* shells, but those aren't
        // the authoritative runtime targets on PG — the fn_* variants are what CodeGen
        // keeps in sync with schema. Honor the entity's custom sproc override first if
        // one is set in metadata (rare), then fall back to the fn_* convention.
        const table = this.toSnakeCase(entityInfo.BaseTable);
        switch (type) {
            case 'create':
                if (entityInfo.spCreate?.length > 0) return entityInfo.spCreate;
                return `fn_create_${table}`;
            case 'update':
                if (entityInfo.spUpdate?.length > 0) return entityInfo.spUpdate;
                return `fn_update_${table}`;
            case 'delete':
                if (entityInfo.spDelete?.length > 0) return entityInfo.spDelete;
                return `fn_delete_${table}`;
        }
    }
