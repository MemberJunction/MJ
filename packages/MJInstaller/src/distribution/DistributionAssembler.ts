...
export interface AssembleOptions {
  ...
  SourceDir: string;
  /** When true, include the full `migrations/` tree (offline bundles). Install omits it. */
...
const SQLSCRIPTS_IGNORE: readonly string[] = [
  '_all_entities.sql',
  '_all_entities.permissions.sql',
  'install/**',
  'internal_only/**',
  'generated/**',
];
 ...
 *
 * @param includeMigrations - Add the `migrations/` tree (offline bundles).
 */
export function distributionSourcePaths(includeMigrations = false): string[] {
...
export class DistributionAssembler {
  ...
  async Plan(opts: AssembleOptions): Promise<WriteOp[]> {
    const mappings = this.mappingsFor(opts.IncludeMigrations ?? false);
  ...
  /** The mappings to include for this assembly (migrations are conditional). */
  private mappingsFor(includeMigrations: boolean): DirMapping[] {
    const mappings = [...BASE_MAPPINGS];
    if (includeMigrations) {
      mappings.push({ SourceRel: 'migrations', DestRel: 'migrations', Kind: 'migrations' });
    }
    return mappings;
  }
  ...
  private ignoreFor(kind: MappingKind): readonly string[] {
    switch (kind) {
      case 'server': return SERVER_IGNORE;
      case 'angular': return ANGULAR_IGNORE;
      case 'sqlscripts': return SQLSCRIPTS_IGNORE;
      case 'migrations': return MIGRATIONS_IGNORE;
    }
  }
