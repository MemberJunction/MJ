 ...
 * 2. Regenerate class registration manifest `.ts` files via the `mj:manifest` script.
...
export class CodeGenPhase {
  ...
  async Run(context: CodeGenContext): Promise<CodeGenResult> {
    ...
    if (firstResult.Success) {
      ...
      await this.regenerateManifestsAndRebuild(context.Dir, pm, emitter, context.Fast ?? false);
    ...
    throw new InstallerError(
      ...
      `Run "${pm.Name} run build" then "mj codegen" manually to see full error output. Verify database connectivity and that migrations have been applied.`
    );
  }
  ...
  /**
   * Execute `mj codegen` (via the resolved CLI) and return structured results.
   *
   ...
   */
  private async rebuildPackages(dir: string, pm: PackageManagerCommands, emitter: InstallerEventEmitter): Promise<void> {
    emitter.Emit('step:progress', {
      Type: 'step:progress',
      Phase: 'codegen',
    ...
    if (result.TimedOut) {
      throw new InstallerError(
        ...
        `Run "${pm.Name} run build" manually at the repo root, then re-run "mj codegen".`
      );
    }
   ...
   *  2. Regenerate manifest .ts files using the `mj:manifest` script (which reads
   ...
   */
  private async regenerateManifestsAndRebuild(dir: string, pm: PackageManagerCommands, emitter: InstallerEventEmitter, fast: boolean): Promise<void> {
    ...
    if (isDistribution) {
      ...
      await this.applyKnownIssuePatches(dir, pm, emitter);
      return;
    }
    ...
    await this.forceRebuildCodegenOutputPackages(dir, pm, emitter);
    ...
    const manifestRegenOk = await this.regenerateManifests(dir, pm, emitter);
    ...
    await this.forceRebuildManifestPackages(dir, pm, emitter);
   ...
   */
  private async forceRebuildCodegenOutputPackages(dir: string, pm: PackageManagerCommands, emitter: InstallerEventEmitter): Promise<void> {
    const filters = [
    ...
    const turbo = pm.Exec('turbo', ['build', '--force', '--log-order=stream', ...filters]);
   ...
   */
  private async forceRebuildManifestPackages(dir: string, pm: PackageManagerCommands, emitter: InstallerEventEmitter): Promise<void> {
    ...
    for (const pkg of CodeGenPhase.MANIFEST_PACKAGE_BUILDS) {
      ...
      // Run the build script in the package directory. This invokes the
      ...
      const pkgBuild = pm.RunScript('build');
      const result = await this.processRunner.Run(
  ...
  /**
   * Regenerate class registration manifest `.ts` files by running the `mj:manifest` script.
   *
   ...
   */
  private async regenerateManifests(dir: string, pm: PackageManagerCommands, emitter: InstallerEventEmitter): Promise<boolean> {
    const manifest = pm.RunScript('mj:manifest');
    const result = await this.processRunner.Run(manifest.Cmd, manifest.Args, {
      Cwd: dir,
  ...
  private async patchStaleManifestImports(dir: string, emitter: InstallerEventEmitter): Promise<void> {
    ...
    const renameMap = await this.buildEntityRenameMap(dir);
    if (renameMap.size === 0) {
      emitter.Emit('warn', {
        Type: 'warn',
        Phase: 'codegen',
   ...
   */
  private async buildEntityRenameMap(dir: string): Promise<Map<string, string>> {
   ...
   */
  private async applyKnownIssuePatches(dir: string, pm: PackageManagerCommands, emitter: InstallerEventEmitter): Promise<void> {
    ...
    for (const pkgRelDir of packagesToRebuild) {
      await this.rebuildSinglePackage(dir, pkgRelDir, pm, emitter);
    }
  }

  /**
   * Rebuild a single package by running its own build script in its directory.
   */
  private async rebuildSinglePackage(dir: string, pkgRelDir: string, pm: PackageManagerCommands, emitter: InstallerEventEmitter): Promise<void> {
    ...
    const singleBuild = pm.RunScript('build');
    const result = await this.processRunner.Run(
