...
- **HTTP Range Streaming**: Serve large audio/video via `GetObjectStream` without buffering whole files in memory (Box/AWS/Azure). Capability is introspectable via `SupportsStreaming`. See [docs/STREAMING.md](docs/STREAMING.md).
...
| [Google Cloud Storage](https://cloud.google.com/storage) | `Google Cloud Storage` | No | Yes | Simulated | No |
| [Google Drive](https://developers.google.com/drive/api/guides/about-sdk) | `Google Drive Storage` | Yes (content) | Yes | Native | No |
| [Microsoft SharePoint](https://learn.microsoft.com/en-us/sharepoint/dev/) | `SharePoint` | Yes (content) | Yes | Native | No |
| [Dropbox](https://www.dropbox.com/developers/documentation) | `Dropbox` | Yes (content) | Yes | Native | No |
...
| `get SupportsStreaming` | `boolean` | Whether the provider supports `GetObjectStream` (Box/AWS/Azure: `true`; others: `false`) |
