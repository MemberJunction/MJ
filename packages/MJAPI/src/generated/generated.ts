...
import { AccountInsightEntity, AccountStatusEntity, AccountTypeEntity, AccountEntity, ActivityEntity, ActivityTypeEntity, ContactRelationshipEntity, ContactEntity, DealProductEntity, DealEntity, EventReviewTaskEntity, EventEntity, IndustryEntity, InvoiceLineItemEntity, InvoiceEntity, LegislativeFindingsEntity, PaymentEntity, ProductEntity, RelationshipTypeEntity, SpeakerEntity, SubmissionNotificationEntity, SubmissionReviewEntity, SubmissionSpeakerEntity, SubmissionEntity } from 'mj_generatedentities';
...
//****************************************************************************
// ENTITY CLASS for Legislative Findings
//****************************************************************************
@ObjectType()
export class demoLegislativeFindings_ {
    @Field() 
    @MaxLength(16)
    ID: string;
...
//****************************************************************************
// INPUT TYPE for Legislative Findings
//****************************************************************************
@InputType()
export class CreatedemoLegislativeFindingsInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdatedemoLegislativeFindingsInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Legislative Findings
//****************************************************************************
@ObjectType()
export class RundemoLegislativeFindingsViewResult {
    @Field(() => [demoLegislativeFindings_])
    Results: demoLegislativeFindings_[];
...
@Resolver(demoLegislativeFindings_)
export class demoLegislativeFindingsResolver extends ResolverBase {
    @Query(() => RundemoLegislativeFindingsViewResult)
    async RundemoLegislativeFindingsViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RundemoLegislativeFindingsViewResult)
    async RundemoLegislativeFindingsViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RundemoLegislativeFindingsViewResult)
    async RundemoLegislativeFindingsDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Legislative Findings';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => demoLegislativeFindings_, { nullable: true })
    async demoLegislativeFindings(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<demoLegislativeFindings_ | null> {
        this.CheckUserReadPermissions('Legislative Findings', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [demo].[vwLegislativeFindings] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Legislative Findings', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = this.MapFieldNamesToCodeNames('Legislative Findings', rows && rows.length > 0 ? rows[0] : {})
        return result;
    }
    
    @Mutation(() => demoLegislativeFindings_)
    async CreatedemoLegislativeFindings(
        @Arg('input', () => CreatedemoLegislativeFindingsInput) input: CreatedemoLegislativeFindingsInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Legislative Findings', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => demoLegislativeFindings_)
    async UpdatedemoLegislativeFindings(
        @Arg('input', () => UpdatedemoLegislativeFindingsInput) input: UpdatedemoLegislativeFindingsInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Legislative Findings', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => demoLegislativeFindings_)
    async DeletedemoLegislativeFindings(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Legislative Findings', key, options, provider, userPayload, pubSub);
    }
