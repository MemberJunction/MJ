...
import { AccreditingBodyEntity, AdvocacyActionEntity, BikeEntity, BoardMemberEntity, BoardPositionEntity, CampaignMemberEntity, CampaignEntity, CertificateEntity, CertificationRenewalEntity, CertificationRequirementEntity, CertificationTypeEntity, CertificationEntity, ChapterMembershipEntity, ChapterOfficerEntity, ChapterEntity, CommitteeMembershipEntity, CommitteeEntity, CompetitionEntryEntity, CompetitionJudgeEntity, CompetitionEntity, ContinuingEducationEntity, CourseEntity, EmailClickEntity, EmailSendEntity, EmailTemplateEntity, EnrollmentEntity, EventRegistrationEntity, EventSessionEntity, EventEntity, ForumCategoryEntity, ForumModerationEntity, ForumPostEntity, ForumThreadEntity, GovernmentContactEntity, InvoiceLineItemEntity, InvoiceEntity, LegislativeBodyEntity, LegislativeIssueEntity, LocationEntity, MemberFollowEntity, MemberEntity, MembershipTypeEntity, MembershipEntity, OrganizationEntity, PaymentEntity, PolicyPositionEntity, PostAttachmentEntity, PostReactionEntity, PostTagEntity, ProductAwardEntity, ProductCategoryEntity, ProductEntity, RegulatoryCommentEntity, ResourceCategoryEntity, ResourceDownloadEntity, ResourceRatingEntity, ResourceTagEntity, ResourceVersionEntity, ResourceEntity, Rider_StatsEntity, RiderEntity, SegmentEntity, WeatherEntity } from 'mj_generatedentities';
...
//****************************************************************************
// ENTITY CLASS for Bikes
//****************************************************************************
@ObjectType({ description: `Stores bicycles owned by riders` })
export class MJBikingAppBike_ {
    @Field({description: `Unique identifier for the bike`}) 
    @MaxLength(16)
    bike_id: string;
        
    @Field({description: `Foreign key to the rider who owns this bike`}) 
    @MaxLength(16)
    rider_id: string;
        
    @Field({description: `User-defined name for the bike`}) 
    @MaxLength(200)
    name: string;
        
    @Field({nullable: true, description: `Type of bicycle: road, mountain, gravel, hybrid, bmx, electric, touring, or cyclocross`}) 
    @MaxLength(40)
    bike_type?: string;
        
    @Field(() => Float, {nullable: true, description: `Weight of the bike in kilograms (0-50)`}) 
    weight_kg?: number;
        
    @Field(() => Float, {nullable: true, description: `Wheel diameter in inches (12-36)`}) 
    wheel_size_in?: number;
        
    @Field(() => Float, {description: `Total distance traveled on this bike in meters`}) 
    total_distance_m: number;
        
    @Field({nullable: true, description: `Date and time of the last service or maintenance`}) 
    @MaxLength(8)
    last_serviced?: Date;
    ...
    Rider_Stats_bike_idArray: MJBikingAppRiderStats_[]; // Link to Rider_Stats
    
}

//****************************************************************************
// INPUT TYPE for Bikes
//****************************************************************************
@InputType()
export class CreateMJBikingAppBikeInput {
    @Field({ nullable: true })
    bike_id?: string;
...
//****************************************************************************
@InputType()
export class UpdateMJBikingAppBikeInput {
    @Field()
    bike_id: string;
...
//****************************************************************************
@ObjectType()
export class RunMJBikingAppBikeViewResult {
    @Field(() => [MJBikingAppBike_])
    Results: MJBikingAppBike_[];
...
@Resolver(MJBikingAppBike_)
export class MJBikingAppBikeResolver extends ResolverBase {
    @Query(() => RunMJBikingAppBikeViewResult)
    async RunMJBikingAppBikeViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppBikeViewResult)
    async RunMJBikingAppBikeViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppBikeViewResult)
    async RunMJBikingAppBikeDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Bikes';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => MJBikingAppBike_, { nullable: true })
    async MJBikingAppBike(@Arg('bike_id', () => String) bike_id: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<MJBikingAppBike_ | null> {
        this.CheckUserReadPermissions('Bikes', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwBikes] WHERE [bike_id]='${bike_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Bikes', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Bikes', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @FieldResolver(() => [MJBikingAppRiderStats_])
    async Rider_Stats_bike_idArray(@Root() mjbikingappbike_: MJBikingAppBike_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Rider _ Stats', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwRider_Stats] WHERE [bike_id]='${mjbikingappbike_.bike_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Rider _ Stats', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Rider _ Stats', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @Mutation(() => MJBikingAppBike_)
    async CreateMJBikingAppBike(
        @Arg('input', () => CreateMJBikingAppBikeInput) input: CreateMJBikingAppBikeInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Bikes', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => MJBikingAppBike_)
    async UpdateMJBikingAppBike(
        @Arg('input', () => UpdateMJBikingAppBikeInput) input: UpdateMJBikingAppBikeInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Bikes', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => MJBikingAppBike_)
    async DeleteMJBikingAppBike(@Arg('bike_id', () => String) bike_id: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'bike_id', Value: bike_id}]);
        return this.DeleteRecord('Bikes', key, options, provider, userPayload, pubSub);
    }
...
//****************************************************************************
@ObjectType({ description: `Stores geographic points of interest visited by riders` })
export class MJBikingAppLocation_ {
    @Field({description: `Unique identifier for the location`}) 
    @MaxLength(16)
    location_id: string;
        
    @Field({description: `Foreign key to the rider who discovered/saved this location`}) 
    @MaxLength(16)
    rider_id: string;
        
    @Field({description: `User-defined name for the location`}) 
    @MaxLength(300)
    name: string;
        
    @Field(() => Float, {description: `Geographic latitude coordinate (-90 to 90 degrees)`}) 
    latitude: number;
        
    @Field(() => Float, {description: `Geographic longitude coordinate (-180 to 180 degrees)`}) 
    longitude: number;
        
    @Field(() => Float, {nullable: true, description: `Elevation above sea level in meters (-500 to 9000)`}) 
    elevation_m?: number;
        
    @Field({nullable: true, description: `Type of terrain: road, gravel, singletrack, doubletrack, paved_trail, urban, or mountain`}) 
    @MaxLength(40)
    terrain_type?: string;
        
    @Field({nullable: true, description: `Current surface condition: dry, wet, muddy, icy, sandy, loose, or packed`}) 
    @MaxLength(40)
    surface_condition?: string;
        
    @Field(() => Float, {nullable: true, description: `Difficulty rating for the location (1.0 to 10.0)`}) 
    difficulty_rating?: number;
        
    @Field(() => Int, {description: `Number of times the rider has visited this location`}) 
    visit_count: number;
        
    @Field({description: `UTC timestamp when the location was first visited`}) 
    @MaxLength(8)
    first_visited: Date;
    ...
    @Field(() => [MJBikingAppWeather_])
    Weathers_location_idArray: MJBikingAppWeather_[]; // Link to Weathers
    
    @Field(() => [MJBikingAppRiderStats_])
    Rider_Stats_location_idArray: MJBikingAppRiderStats_[]; // Link to Rider_Stats
...
//****************************************************************************
@InputType()
export class CreateMJBikingAppLocationInput {
    @Field({ nullable: true })
    location_id?: string;
...
//****************************************************************************
@InputType()
export class UpdateMJBikingAppLocationInput {
    @Field()
    location_id: string;
...
//****************************************************************************
@ObjectType()
export class RunMJBikingAppLocationViewResult {
    @Field(() => [MJBikingAppLocation_])
    Results: MJBikingAppLocation_[];
...
@Resolver(MJBikingAppLocation_)
export class MJBikingAppLocationResolver extends ResolverBase {
    @Query(() => RunMJBikingAppLocationViewResult)
    async RunMJBikingAppLocationViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppLocationViewResult)
    async RunMJBikingAppLocationViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppLocationViewResult)
    async RunMJBikingAppLocationDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Locations';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => MJBikingAppLocation_, { nullable: true })
    async MJBikingAppLocation(@Arg('location_id', () => String) location_id: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<MJBikingAppLocation_ | null> {
        this.CheckUserReadPermissions('Locations', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwLocations] WHERE [location_id]='${location_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Locations', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Locations', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @FieldResolver(() => [MJBikingAppWeather_])
    async Weathers_location_idArray(@Root() mjbikingapplocation_: MJBikingAppLocation_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Weathers', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwWeathers] WHERE [location_id]='${mjbikingapplocation_.location_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Weathers', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Weathers', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @FieldResolver(() => [MJBikingAppRiderStats_])
    async Rider_Stats_location_idArray(@Root() mjbikingapplocation_: MJBikingAppLocation_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Rider _ Stats', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwRider_Stats] WHERE [location_id]='${mjbikingapplocation_.location_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Rider _ Stats', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Rider _ Stats', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @Mutation(() => MJBikingAppLocation_)
    async CreateMJBikingAppLocation(
        @Arg('input', () => CreateMJBikingAppLocationInput) input: CreateMJBikingAppLocationInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
    ...
    @Mutation(() => MJBikingAppLocation_)
    async UpdateMJBikingAppLocation(
        @Arg('input', () => UpdateMJBikingAppLocationInput) input: UpdateMJBikingAppLocationInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
    ...
    @Mutation(() => MJBikingAppLocation_)
    async DeleteMJBikingAppLocation(@Arg('location_id', () => String) location_id: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'location_id', Value: location_id}]);
...
//****************************************************************************
// ENTITY CLASS for Rider _ Stats
//****************************************************************************
@ObjectType({ description: `Stores performance metrics and physiological data from ride sessions` })
export class MJBikingAppRiderStats_ {
    @Field({description: `Unique identifier for the stats record`}) 
    @MaxLength(16)
    stats_id: string;
        
    @Field({description: `Foreign key to the rider who recorded these stats`}) 
    @MaxLength(16)
    rider_id: string;
        
    @Field({nullable: true, description: `Foreign key to the location where the ride took place (optional)`}) 
    @MaxLength(16)
    location_id?: string;
        
    @Field({nullable: true, description: `Foreign key to the bike used for the ride (optional)`}) 
    @MaxLength(16)
    bike_id?: string;
        
    @Field(() => Float, {nullable: true, description: `Average speed during the ride in meters per second (0-50)`}) 
    avg_speed_mps?: number;
        
    @Field(() => Float, {nullable: true, description: `Maximum speed during the ride in meters per second (0-80, must be >= avg_speed)`}) 
    max_speed_mps?: number;
        
    @Field(() => Int, {nullable: true, description: `Average heart rate during the ride in beats per minute (30-250)`}) 
    avg_heart_rate_bpm?: number;
        
    @Field(() => Int, {nullable: true, description: `Maximum heart rate during the ride in beats per minute (30-250, must be >= avg_heart_rate)`}) 
    max_heart_rate_bpm?: number;
        
    @Field(() => Int, {nullable: true, description: `Pedaling cadence in revolutions per minute (0-200)`}) 
    cadence_rpm?: number;
        
    @Field(() => Int, {nullable: true, description: `Power output in watts (0-2500)`}) 
    power_watts?: number;
        
    @Field(() => Float, {description: `Total distance covered in meters`}) 
    distance_m: number;
        
    @Field(() => Float, {nullable: true, description: `Total elevation gained during the ride in meters (0-10000)`}) 
    elevation_gain_m?: number;
        
    @Field(() => Int, {description: `Total duration of the ride in seconds (must be > 0)`}) 
    duration_seconds: number;
        
    @Field(() => Int, {nullable: true, description: `Estimated calories burned during the ride (0-20000)`}) 
    calories_burned?: number;
        
    @Field(() => Int, {nullable: true, description: `Self-reported perceived effort rating (1-10)`}) 
    effort_rating?: number;
        
    @Field({description: `UTC timestamp when the ride stats were recorded`}) 
    @MaxLength(8)
    recorded_at: Date;
    ...
    @Field({nullable: true}) 
    @MaxLength(300)
    location_id_Virtual?: string;
        
    @Field({nullable: true}) 
    @MaxLength(200)
    bike_id_Virtual?: string;
        
}

//****************************************************************************
// INPUT TYPE for Rider _ Stats
//****************************************************************************
@InputType()
export class CreateMJBikingAppRiderStatsInput {
    @Field({ nullable: true })
    stats_id?: string;
...
//****************************************************************************
@InputType()
export class UpdateMJBikingAppRiderStatsInput {
    @Field()
    stats_id: string;
...
//****************************************************************************
// RESOLVER for Rider _ Stats
//****************************************************************************
@ObjectType()
export class RunMJBikingAppRiderStatsViewResult {
    @Field(() => [MJBikingAppRiderStats_])
    Results: MJBikingAppRiderStats_[];
...
@Resolver(MJBikingAppRiderStats_)
export class MJBikingAppRiderStatsResolver extends ResolverBase {
    @Query(() => RunMJBikingAppRiderStatsViewResult)
    async RunMJBikingAppRiderStatsViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppRiderStatsViewResult)
    async RunMJBikingAppRiderStatsViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppRiderStatsViewResult)
    async RunMJBikingAppRiderStatsDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Rider _ Stats';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => MJBikingAppRiderStats_, { nullable: true })
    async MJBikingAppRiderStats(@Arg('stats_id', () => String) stats_id: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<MJBikingAppRiderStats_ | null> {
        this.CheckUserReadPermissions('Rider _ Stats', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwRider_Stats] WHERE [stats_id]='${stats_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Rider _ Stats', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Rider _ Stats', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => MJBikingAppRiderStats_)
    async CreateMJBikingAppRiderStats(
        @Arg('input', () => CreateMJBikingAppRiderStatsInput) input: CreateMJBikingAppRiderStatsInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Rider _ Stats', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => MJBikingAppRiderStats_)
    async UpdateMJBikingAppRiderStats(
        @Arg('input', () => UpdateMJBikingAppRiderStatsInput) input: UpdateMJBikingAppRiderStatsInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Rider _ Stats', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => MJBikingAppRiderStats_)
    async DeleteMJBikingAppRiderStats(@Arg('stats_id', () => String) stats_id: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'stats_id', Value: stats_id}]);
        return this.DeleteRecord('Rider _ Stats', key, options, provider, userPayload, pubSub);
    }
    
}

//****************************************************************************
// ENTITY CLASS for Riders
//****************************************************************************
@ObjectType({ description: `Stores cyclist profile information and preferences for the Trailbloom app` })
export class MJBikingAppRider_ {
    @Field({description: `Unique identifier for the rider`}) 
    @MaxLength(16)
    rider_id: string;
        
    @Field({description: `Unique username for the rider account`}) 
    @MaxLength(100)
    username: string;
        
    @Field({description: `Email address for the rider account`}) 
    @MaxLength(510)
    email: string;
        
    @Field(() => Float, {nullable: true, description: `Rider body weight in kilograms (0-300)`}) 
    weight_kg?: number;
        
    @Field(() => Int, {nullable: true, description: `Self-reported fitness level on a scale of 1-10`}) 
    fitness_level?: number;
        
    @Field({nullable: true, description: `Preferred riding terrain: road, gravel, mountain, urban, or mixed`}) 
    @MaxLength(40)
    preferred_terrain?: string;
        
    @Field({nullable: true, description: `JSON object containing aggregated lifetime statistics`}) 
    lifetime_stats?: string;
        
    @Field({description: `UTC timestamp when the rider account was created`}) 
    @MaxLength(8)
    created_at: Date;
    ...
    @Field(() => [MJBikingAppBike_])
    Bikes_rider_idArray: MJBikingAppBike_[]; // Link to Bikes
    
    @Field(() => [MJBikingAppLocation_])
    Locations_rider_idArray: MJBikingAppLocation_[]; // Link to Locations
    
}

//****************************************************************************
// INPUT TYPE for Riders
//****************************************************************************
@InputType()
export class CreateMJBikingAppRiderInput {
    @Field({ nullable: true })
    rider_id?: string;
...
//****************************************************************************
@InputType()
export class UpdateMJBikingAppRiderInput {
    @Field()
    rider_id: string;
...
//****************************************************************************
@ObjectType()
export class RunMJBikingAppRiderViewResult {
    @Field(() => [MJBikingAppRider_])
    Results: MJBikingAppRider_[];
...
@Resolver(MJBikingAppRider_)
export class MJBikingAppRiderResolver extends ResolverBase {
    @Query(() => RunMJBikingAppRiderViewResult)
    async RunMJBikingAppRiderViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppRiderViewResult)
    async RunMJBikingAppRiderViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppRiderViewResult)
    async RunMJBikingAppRiderDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Riders';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => MJBikingAppRider_, { nullable: true })
    async MJBikingAppRider(@Arg('rider_id', () => String) rider_id: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<MJBikingAppRider_ | null> {
        this.CheckUserReadPermissions('Riders', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwRiders] WHERE [rider_id]='${rider_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Riders', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Riders', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @FieldResolver(() => [MJBikingAppRiderStats_])
    async Rider_Stats_rider_idArray(@Root() mjbikingapprider_: MJBikingAppRider_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Rider _ Stats', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwRider_Stats] WHERE [rider_id]='${mjbikingapprider_.rider_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Rider _ Stats', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Rider _ Stats', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @FieldResolver(() => [MJBikingAppBike_])
    async Bikes_rider_idArray(@Root() mjbikingapprider_: MJBikingAppRider_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Bikes', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwBikes] WHERE [rider_id]='${mjbikingapprider_.rider_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Bikes', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Bikes', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @FieldResolver(() => [MJBikingAppLocation_])
    async Locations_rider_idArray(@Root() mjbikingapprider_: MJBikingAppRider_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Locations', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwLocations] WHERE [rider_id]='${mjbikingapprider_.rider_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Locations', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Locations', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @Mutation(() => MJBikingAppRider_)
    async CreateMJBikingAppRider(
        @Arg('input', () => CreateMJBikingAppRiderInput) input: CreateMJBikingAppRiderInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Riders', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => MJBikingAppRider_)
    async UpdateMJBikingAppRider(
        @Arg('input', () => UpdateMJBikingAppRiderInput) input: UpdateMJBikingAppRiderInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Riders', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => MJBikingAppRider_)
    async DeleteMJBikingAppRider(@Arg('rider_id', () => String) rider_id: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'rider_id', Value: rider_id}]);
        return this.DeleteRecord('Riders', key, options, provider, userPayload, pubSub);
    }
...
//****************************************************************************
// ENTITY CLASS for Weathers
//****************************************************************************
@ObjectType({ description: `Stores weather condition snapshots at locations` })
export class MJBikingAppWeather_ {
    @Field({description: `Unique identifier for the weather record`}) 
    @MaxLength(16)
    weather_id: string;
        
    @Field({description: `Foreign key to the location where weather was observed`}) 
    @MaxLength(16)
    location_id: string;
        
    @Field(() => Float, {nullable: true, description: `Temperature in Celsius (-90 to 60)`}) 
    temperature_c?: number;
        
    @Field(() => Float, {nullable: true, description: `Relative humidity percentage (0-100)`}) 
    humidity_pct?: number;
        
    @Field(() => Float, {nullable: true, description: `Wind speed in meters per second (0-120)`}) 
    wind_speed_mps?: number;
        
    @Field(() => Float, {nullable: true, description: `Wind direction in degrees from north (0-360)`}) 
    wind_direction_deg?: number;
        
    @Field({nullable: true, description: `Type of precipitation: none, drizzle, rain, heavy_rain, snow, sleet, hail, or fog`}) 
    @MaxLength(40)
    precipitation_type?: string;
        
    @Field(() => Int, {nullable: true, description: `Cloud cover percentage (0-100)`}) 
    cloud_cover_pct?: number;
        
    @Field(() => Float, {nullable: true, description: `Visibility in kilometers (0-100)`}) 
    visibility_km?: number;
        
    @Field({description: `UTC timestamp when the weather observation was recorded`}) 
    @MaxLength(8)
    observed_at: Date;
...
//****************************************************************************
// INPUT TYPE for Weathers
//****************************************************************************
@InputType()
export class CreateMJBikingAppWeatherInput {
    @Field({ nullable: true })
    weather_id?: string;
...
//****************************************************************************
@InputType()
export class UpdateMJBikingAppWeatherInput {
    @Field()
    weather_id: string;
...
//****************************************************************************
// RESOLVER for Weathers
//****************************************************************************
@ObjectType()
export class RunMJBikingAppWeatherViewResult {
    @Field(() => [MJBikingAppWeather_])
    Results: MJBikingAppWeather_[];
...
@Resolver(MJBikingAppWeather_)
export class MJBikingAppWeatherResolver extends ResolverBase {
    @Query(() => RunMJBikingAppWeatherViewResult)
    async RunMJBikingAppWeatherViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppWeatherViewResult)
    async RunMJBikingAppWeatherViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunMJBikingAppWeatherViewResult)
    async RunMJBikingAppWeatherDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Weathers';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => MJBikingAppWeather_, { nullable: true })
    async MJBikingAppWeather(@Arg('weather_id', () => String) weather_id: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<MJBikingAppWeather_ | null> {
        this.CheckUserReadPermissions('Weathers', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [MJ_Biking_App].[vwWeathers] WHERE [weather_id]='${weather_id}' ` + this.getRowLevelSecurityWhereClause(provider, 'Weathers', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Weathers', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => MJBikingAppWeather_)
    async CreateMJBikingAppWeather(
        @Arg('input', () => CreateMJBikingAppWeatherInput) input: CreateMJBikingAppWeatherInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Weathers', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => MJBikingAppWeather_)
    async UpdateMJBikingAppWeather(
        @Arg('input', () => UpdateMJBikingAppWeatherInput) input: UpdateMJBikingAppWeatherInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Weathers', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => MJBikingAppWeather_)
    async DeleteMJBikingAppWeather(@Arg('weather_id', () => String) weather_id: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'weather_id', Value: weather_id}]);
        return this.DeleteRecord('Weathers', key, options, provider, userPayload, pubSub);
    }
