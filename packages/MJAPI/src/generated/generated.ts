...
import { AccreditingBodyEntity, AdvocacyActionEntity, BoardMemberEntity, BoardPositionEntity, CampaignMemberEntity, CampaignEntity, CertificateEntity, CertificationRenewalEntity, CertificationRequirementEntity, CertificationTypeEntity, CertificationEntity, ChapterMembershipEntity, ChapterOfficerEntity, ChapterEntity, CommitteeMembershipEntity, CommitteeEntity, CompetitionEntryEntity, CompetitionJudgeEntity, CompetitionEntity, ContinuingEducationEntity, CourseEntity, EmailClickEntity, EmailSendEntity, EmailTemplateEntity, EnrollmentEntity, EventRegistrationEntity, EventSessionEntity, EventEntity, ForumCategoryEntity, ForumModerationEntity, ForumPostEntity, ForumThreadEntity, GovernmentContactEntity, InvoiceLineItemEntity, InvoiceEntity, LegislativeBodyEntity, LegislativeIssueEntity, ListeningHistoryEntity, MemberFollowEntity, MemberEntity, MembershipTypeEntity, MembershipEntity, OrganizationEntity, PaymentEntity, PlaylistSongEntity, PlaylistEntity, PolicyPositionEntity, PostAttachmentEntity, PostReactionEntity, PostTagEntity, ProductAwardEntity, ProductCategoryEntity, ProductEntity, RegulatoryCommentEntity, ResourceCategoryEntity, ResourceDownloadEntity, ResourceRatingEntity, ResourceTagEntity, ResourceVersionEntity, ResourceEntity, SegmentEntity, SongJournalEntity, SongEntity, UserYearSummaryEntity } from 'mj_generatedentities';
...
//****************************************************************************
// ENTITY CLASS for Listening Histories
//****************************************************************************
@ObjectType({ description: `Tracks each time a user listens to a song.` })
export class spotifyListeningHistory_ {
    @Field({description: `Unique identifier for the listening event.`}) 
    @MaxLength(16)
    ID: string;
        
    @Field({description: `User who listened to the song.`}) 
    @MaxLength(16)
    UserID: string;
        
    @Field({description: `Song that was listened to.`}) 
    @MaxLength(16)
    SongID: string;
        
    @Field({description: `Timestamp when the song was listened to.`}) 
    @MaxLength(10)
    ListenedAt: Date;
        
    @Field(() => Int, {nullable: true, description: `How long the user listened in seconds (may be less than song duration).`}) 
    PlayDuration?: number;
        
    @Field(() => Float, {nullable: true, description: `Percentage of the song that was played (0-100).`}) 
    CompletionPercentage?: number;
        
    @Field({nullable: true, description: `Platform where the song was played (Web, Mobile, Desktop, Other).`}) 
    @MaxLength(100)
    Source?: string;
...
//****************************************************************************
// INPUT TYPE for Listening Histories
//****************************************************************************
@InputType()
export class CreatespotifyListeningHistoryInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdatespotifyListeningHistoryInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Listening Histories
//****************************************************************************
@ObjectType()
export class RunspotifyListeningHistoryViewResult {
    @Field(() => [spotifyListeningHistory_])
    Results: spotifyListeningHistory_[];
...
@Resolver(spotifyListeningHistory_)
export class spotifyListeningHistoryResolver extends ResolverBase {
    @Query(() => RunspotifyListeningHistoryViewResult)
    async RunspotifyListeningHistoryViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifyListeningHistoryViewResult)
    async RunspotifyListeningHistoryViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifyListeningHistoryViewResult)
    async RunspotifyListeningHistoryDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Listening Histories';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => spotifyListeningHistory_, { nullable: true })
    async spotifyListeningHistory(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<spotifyListeningHistory_ | null> {
        this.CheckUserReadPermissions('Listening Histories', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwListeningHistories] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Listening Histories', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Listening Histories', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Query(() => [spotifyListeningHistory_])
    async AllListeningHistories(@Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Listening Histories', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwListeningHistories]` + this.getRowLevelSecurityWhereClause(provider, 'Listening Histories', userPayload, EntityPermissionType.Read, ' WHERE');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Listening Histories', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => spotifyListeningHistory_)
    async CreatespotifyListeningHistory(
        @Arg('input', () => CreatespotifyListeningHistoryInput) input: CreatespotifyListeningHistoryInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Listening Histories', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => spotifyListeningHistory_)
    async UpdatespotifyListeningHistory(
        @Arg('input', () => UpdatespotifyListeningHistoryInput) input: UpdatespotifyListeningHistoryInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Listening Histories', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => spotifyListeningHistory_)
    async DeletespotifyListeningHistory(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Listening Histories', key, options, provider, userPayload, pubSub);
    }
...
//****************************************************************************
// ENTITY CLASS for Playlist Songs
//****************************************************************************
@ObjectType({ description: `Junction table linking playlists to songs with ordering.` })
export class spotifyPlaylistSong_ {
    @Field({description: `Unique identifier for the playlist-song relationship.`}) 
    @MaxLength(16)
    ID: string;
        
    @Field({description: `Playlist this song belongs to.`}) 
    @MaxLength(16)
    PlaylistID: string;
        
    @Field({description: `Song included in the playlist.`}) 
    @MaxLength(16)
    SongID: string;
        
    @Field(() => Int, {description: `Order of the song in the playlist (starting from 1).`}) 
    SequenceNumber: number;
        
    @Field(() => Boolean, {description: `Whether this song was discovered via web search by an AI agent.`}) 
    AddedFromWeb: boolean;
        
    @Field({nullable: true, description: `Reason why this song was included (e.g., "Currently #3 on Billboard Hot 100").`}) 
    @MaxLength(1000)
...
//****************************************************************************
// INPUT TYPE for Playlist Songs
//****************************************************************************
@InputType()
export class CreatespotifyPlaylistSongInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdatespotifyPlaylistSongInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Playlist Songs
//****************************************************************************
@ObjectType()
export class RunspotifyPlaylistSongViewResult {
    @Field(() => [spotifyPlaylistSong_])
    Results: spotifyPlaylistSong_[];
...
@Resolver(spotifyPlaylistSong_)
export class spotifyPlaylistSongResolver extends ResolverBase {
    @Query(() => RunspotifyPlaylistSongViewResult)
    async RunspotifyPlaylistSongViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifyPlaylistSongViewResult)
    async RunspotifyPlaylistSongViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifyPlaylistSongViewResult)
    async RunspotifyPlaylistSongDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Playlist Songs';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => spotifyPlaylistSong_, { nullable: true })
    async spotifyPlaylistSong(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<spotifyPlaylistSong_ | null> {
        this.CheckUserReadPermissions('Playlist Songs', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwPlaylistSongs] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Playlist Songs', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Playlist Songs', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => spotifyPlaylistSong_)
    async CreatespotifyPlaylistSong(
        @Arg('input', () => CreatespotifyPlaylistSongInput) input: CreatespotifyPlaylistSongInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Playlist Songs', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => spotifyPlaylistSong_)
    async UpdatespotifyPlaylistSong(
        @Arg('input', () => UpdatespotifyPlaylistSongInput) input: UpdatespotifyPlaylistSongInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Playlist Songs', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => spotifyPlaylistSong_)
    async DeletespotifyPlaylistSong(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Playlist Songs', key, options, provider, userPayload, pubSub);
    }
    
}

//****************************************************************************
// ENTITY CLASS for Playlists
//****************************************************************************
@ObjectType({ description: `Stores user-created or AI-generated music playlists.` })
export class spotifyPlaylist_ {
    @Field({description: `Unique identifier for the playlist.`}) 
    @MaxLength(16)
    ID: string;
        
    @Field({description: `User who owns this playlist.`}) 
    @MaxLength(16)
    UserID: string;
        
    @Field({description: `Display name for the playlist.`}) 
    @MaxLength(510)
    Name: string;
        
    @Field({nullable: true, description: `Optional description of the playlist theme or purpose.`}) 
    Description?: string;
        
    @Field({nullable: true, description: `Primary genre of songs in this playlist.`}) 
    @MaxLength(200)
    Genre?: string;
        
    @Field(() => Boolean, {description: `Indicates whether this playlist was created by an AI agent.`}) 
    CreatedByAI: boolean;
        
    @Field({nullable: true, description: `Notes from the AI agent about how the playlist was curated.`}) 
    AIGenerationNotes?: string;
        
    @Field(() => Boolean, {description: `Whether this playlist is visible to other users.`}) 
    IsPublic: boolean;
        
    @Field(() => Int, {nullable: true, description: `Total duration of all songs in the playlist (in seconds).`}) 
    ...
    @Field(() => [spotifyPlaylistSong_])
    PlaylistSongs_PlaylistIDArray: spotifyPlaylistSong_[]; // Link to PlaylistSongs
    
}

//****************************************************************************
// INPUT TYPE for Playlists
//****************************************************************************
@InputType()
export class CreatespotifyPlaylistInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdatespotifyPlaylistInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Playlists
//****************************************************************************
@ObjectType()
export class RunspotifyPlaylistViewResult {
    @Field(() => [spotifyPlaylist_])
    Results: spotifyPlaylist_[];
...
@Resolver(spotifyPlaylist_)
export class spotifyPlaylistResolver extends ResolverBase {
    @Query(() => RunspotifyPlaylistViewResult)
    async RunspotifyPlaylistViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifyPlaylistViewResult)
    async RunspotifyPlaylistViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifyPlaylistViewResult)
    async RunspotifyPlaylistDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Playlists';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => spotifyPlaylist_, { nullable: true })
    async spotifyPlaylist(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<spotifyPlaylist_ | null> {
        this.CheckUserReadPermissions('Playlists', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwPlaylists] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Playlists', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Playlists', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @FieldResolver(() => [spotifyPlaylistSong_])
    async PlaylistSongs_PlaylistIDArray(@Root() spotifyplaylist_: spotifyPlaylist_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Playlist Songs', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwPlaylistSongs] WHERE [PlaylistID]='${spotifyplaylist_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Playlist Songs', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Playlist Songs', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @Mutation(() => spotifyPlaylist_)
    async CreatespotifyPlaylist(
        @Arg('input', () => CreatespotifyPlaylistInput) input: CreatespotifyPlaylistInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Playlists', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => spotifyPlaylist_)
    async UpdatespotifyPlaylist(
        @Arg('input', () => UpdatespotifyPlaylistInput) input: UpdatespotifyPlaylistInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Playlists', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => spotifyPlaylist_)
    async DeletespotifyPlaylist(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Playlists', key, options, provider, userPayload, pubSub);
    }
...
//****************************************************************************
// ENTITY CLASS for Song Journals
//****************************************************************************
@ObjectType({ description: `User journal entries reflecting on emotional connection to songs` })
export class spotifySongJournal_ {
    @Field() 
    @MaxLength(16)
    ID: string;
        
    @Field({description: `User who wrote the journal entry`}) 
    @MaxLength(16)
    UserID: string;
        
    @Field({description: `Song the journal entry is about`}) 
    @MaxLength(16)
    SongID: string;
        
    @Field({description: `User written reflection describing how the song made them feel`}) 
    JournalEntry: string;
        
    @Field(() => Int, {description: `Year the song was most listened to (e.g., 2024)`}) 
    ReflectionYear: number;
        
    @Field(() => Int, {description: `Month the song was most listened to (1-12)`}) 
    ...
    @Field({nullable: true, description: `AI-detected emotion from the journal entry (e.g., Joy, Nostalgia, Melancholy, Energy, Peace, Love)`}) 
    @MaxLength(200)
...
//****************************************************************************
// INPUT TYPE for Song Journals
//****************************************************************************
@InputType()
export class CreatespotifySongJournalInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdatespotifySongJournalInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Song Journals
//****************************************************************************
@ObjectType()
export class RunspotifySongJournalViewResult {
    @Field(() => [spotifySongJournal_])
    Results: spotifySongJournal_[];
...
@Resolver(spotifySongJournal_)
export class spotifySongJournalResolver extends ResolverBase {
    @Query(() => RunspotifySongJournalViewResult)
    async RunspotifySongJournalViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifySongJournalViewResult)
    async RunspotifySongJournalViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifySongJournalViewResult)
    async RunspotifySongJournalDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Song Journals';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => spotifySongJournal_, { nullable: true })
    async spotifySongJournal(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<spotifySongJournal_ | null> {
        this.CheckUserReadPermissions('Song Journals', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwSongJournals] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Song Journals', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Song Journals', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => spotifySongJournal_)
    async CreatespotifySongJournal(
        @Arg('input', () => CreatespotifySongJournalInput) input: CreatespotifySongJournalInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Song Journals', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => spotifySongJournal_)
    async UpdatespotifySongJournal(
        @Arg('input', () => UpdatespotifySongJournalInput) input: UpdatespotifySongJournalInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Song Journals', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => spotifySongJournal_)
    async DeletespotifySongJournal(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Song Journals', key, options, provider, userPayload, pubSub);
    }
    
}

//****************************************************************************
// ENTITY CLASS for Songs
//****************************************************************************
@ObjectType({ description: `Stores information about songs in the music library.` })
export class spotifySong_ {
    ...
    @Field({description: `Name of the artist or band.`}) 
    @MaxLength(510)
    Artist: string;
        
    @Field({nullable: true, description: `Album name where the song appears.`}) 
    @MaxLength(510)
    Album?: string;
        
    @Field(() => Int, {description: `Duration of the song in seconds.`}) 
    Duration: number;
        
    @Field({nullable: true, description: `Music genre classification.`}) 
    @MaxLength(200)
    Genre?: string;
        
    @Field(() => Int, {nullable: true, description: `Year the song was released.`}) 
    ReleaseYear?: number;
        
    @Field({nullable: true, description: `Spotify unique identifier for the track.`}) 
    @MaxLength(200)
    SpotifyID?: string;
        
    @Field({nullable: true, description: `URL to the album artwork image.`}) 
    @MaxLength(1000)
    AlbumArtURL?: string;
    ...
    @Field(() => [spotifyListeningHistory_])
    ListeningHistories_SongIDArray: spotifyListeningHistory_[]; // Link to ListeningHistories
    
    @Field(() => [spotifyPlaylistSong_])
    PlaylistSongs_SongIDArray: spotifyPlaylistSong_[]; // Link to PlaylistSongs
    
    @Field(() => [spotifySongJournal_])
    SongJournals_SongIDArray: spotifySongJournal_[]; // Link to SongJournals
    
}

//****************************************************************************
// INPUT TYPE for Songs
//****************************************************************************
@InputType()
export class CreatespotifySongInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdatespotifySongInput {
    @Field()
    ID: string;
...
//****************************************************************************
@ObjectType()
export class RunspotifySongViewResult {
    @Field(() => [spotifySong_])
    Results: spotifySong_[];
...
@Resolver(spotifySong_)
export class spotifySongResolver extends ResolverBase {
    @Query(() => RunspotifySongViewResult)
    async RunspotifySongViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifySongViewResult)
    async RunspotifySongViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifySongViewResult)
    async RunspotifySongDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Songs';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => spotifySong_, { nullable: true })
    async spotifySong(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<spotifySong_ | null> {
        this.CheckUserReadPermissions('Songs', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwSongs] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Songs', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Songs', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Query(() => [spotifySong_])
    async AllSongs(@Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Songs', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwSongs]` + this.getRowLevelSecurityWhereClause(provider, 'Songs', userPayload, EntityPermissionType.Read, ' WHERE');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Songs', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @FieldResolver(() => [spotifyListeningHistory_])
    async ListeningHistories_SongIDArray(@Root() spotifysong_: spotifySong_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Listening Histories', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwListeningHistories] WHERE [SongID]='${spotifysong_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Listening Histories', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Listening Histories', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
    ...
    @FieldResolver(() => [spotifyPlaylistSong_])
    async PlaylistSongs_SongIDArray(@Root() spotifysong_: spotifySong_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Playlist Songs', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwPlaylistSongs] WHERE [SongID]='${spotifysong_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Playlist Songs', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Playlist Songs', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @FieldResolver(() => [spotifySongJournal_])
    async SongJournals_SongIDArray(@Root() spotifysong_: spotifySong_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Song Journals', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwSongJournals] WHERE [SongID]='${spotifysong_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Song Journals', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Song Journals', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @Mutation(() => spotifySong_)
    async CreatespotifySong(
        @Arg('input', () => CreatespotifySongInput) input: CreatespotifySongInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Songs', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => spotifySong_)
    async UpdatespotifySong(
        @Arg('input', () => UpdatespotifySongInput) input: UpdatespotifySongInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Songs', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => spotifySong_)
    async DeletespotifySong(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Songs', key, options, provider, userPayload, pubSub);
    }
    
}

//****************************************************************************
// ENTITY CLASS for User Year Summaries
//****************************************************************************
@ObjectType({ description: `Stores annual "Wrapped" summary statistics for each user.` })
export class spotifyUserYearSummary_ {
    @Field({description: `Unique identifier for the summary record.`}) 
    @MaxLength(16)
    ID: string;
        
    @Field({description: `User this summary belongs to.`}) 
    @MaxLength(16)
    UserID: string;
        
    @Field(() => Int, {description: `Year this summary covers.`}) 
    Year: number;
        
    @Field(() => Int, {description: `Total number of songs listened to during the year.`}) 
    TotalListens: number;
        
    @Field(() => Int, {description: `Total minutes spent listening to music during the year.`}) 
    TotalMinutesListened: number;
        
    @Field(() => Int, {description: `Number of different artists listened to during the year.`}) 
    UniqueArtists: number;
        
    @Field(() => Int, {description: `Number of different songs listened to during the year.`}) 
    UniqueSongs: number;
        
    @Field({nullable: true, description: `Most frequently listened to genre during the year.`}) 
    @MaxLength(200)
    TopGenre?: string;
        
    @Field({nullable: true, description: `Most frequently listened to artist during the year.`}) 
    @MaxLength(510)
    TopArtist?: string;
        
    @Field({nullable: true, description: `Most frequently listened to song during the year.`}) 
    @MaxLength(510)
    TopSong?: string;
        
    @Field({nullable: true, description: `JSON blob containing additional summary data and statistics.`}) 
    SummaryData?: string;
...
//****************************************************************************
// INPUT TYPE for User Year Summaries
//****************************************************************************
@InputType()
export class CreatespotifyUserYearSummaryInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdatespotifyUserYearSummaryInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for User Year Summaries
//****************************************************************************
@ObjectType()
export class RunspotifyUserYearSummaryViewResult {
    @Field(() => [spotifyUserYearSummary_])
    Results: spotifyUserYearSummary_[];
...
@Resolver(spotifyUserYearSummary_)
export class spotifyUserYearSummaryResolver extends ResolverBase {
    @Query(() => RunspotifyUserYearSummaryViewResult)
    async RunspotifyUserYearSummaryViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifyUserYearSummaryViewResult)
    async RunspotifyUserYearSummaryViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunspotifyUserYearSummaryViewResult)
    async RunspotifyUserYearSummaryDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'User Year Summaries';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => spotifyUserYearSummary_, { nullable: true })
    async spotifyUserYearSummary(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<spotifyUserYearSummary_ | null> {
        this.CheckUserReadPermissions('User Year Summaries', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwUserYearSummaries] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'User Year Summaries', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('User Year Summaries', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Query(() => [spotifyUserYearSummary_])
    async AllUserYearSummaries(@Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('User Year Summaries', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [spotify].[vwUserYearSummaries]` + this.getRowLevelSecurityWhereClause(provider, 'User Year Summaries', userPayload, EntityPermissionType.Read, ' WHERE');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('User Year Summaries', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => spotifyUserYearSummary_)
    async CreatespotifyUserYearSummary(
        @Arg('input', () => CreatespotifyUserYearSummaryInput) input: CreatespotifyUserYearSummaryInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('User Year Summaries', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => spotifyUserYearSummary_)
    async UpdatespotifyUserYearSummary(
        @Arg('input', () => UpdatespotifyUserYearSummaryInput) input: UpdatespotifyUserYearSummaryInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('User Year Summaries', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => spotifyUserYearSummary_)
    async DeletespotifyUserYearSummary(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('User Year Summaries', key, options, provider, userPayload, pubSub);
    }
