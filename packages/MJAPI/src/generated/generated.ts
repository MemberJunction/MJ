...
import { AccreditingBodyEntity, AdvocacyActionEntity, BoardMemberEntity, BoardPositionEntity, CampaignMemberEntity, CampaignEntity, CertificateEntity, CertificationRenewalEntity, CertificationRequirementEntity, CertificationTypeEntity, CertificationEntity, ChapterMembershipEntity, ChapterOfficerEntity, ChapterEntity, CommitteeMembershipEntity, CommitteeEntity, CompetitionEntryEntity, CompetitionJudgeEntity, CompetitionEntity, ContinuingEducationEntity, CourseEntity, CuisineTypeEntity, EmailClickEntity, EmailSendEntity, EmailTemplateEntity, EnrollmentEntity, EventRegistrationEntity, EventSessionEntity, EventEntity, ForumCategoryEntity, ForumModerationEntity, ForumPostEntity, ForumThreadEntity, GovernmentContactEntity, GroupVisitMemberEntity, GroupVisitEntity, InvoiceLineItemEntity, InvoiceEntity, LegislativeBodyEntity, LegislativeIssueEntity, MemberFollowEntity, MemberEntity, Member__FoodieEntity, MembershipTypeEntity, MembershipEntity, OrganizationEntity, PaymentEntity, PolicyPositionEntity, PostAttachmentEntity, PostReactionEntity, PostTagEntity, ProductAwardEntity, ProductCategoryEntity, ProductEntity, RegulatoryCommentEntity, ResourceCategoryEntity, ResourceDownloadEntity, ResourceRatingEntity, ResourceTagEntity, ResourceVersionEntity, ResourceEntity, RestaurantTagEntity, RestaurantVisitEntity, RestaurantEntity, SegmentEntity, WishListEntity } from 'mj_generatedentities';
...
//****************************************************************************
// ENTITY CLASS for Cuisine Types
//****************************************************************************
@ObjectType({ description: `Stores types of cuisine (Italian, Mexican, Japanese, etc.)` })
export class FoodieCuisineType_ {
    @Field({description: `Unique identifier for the cuisine type`}) 
    @MaxLength(16)
    ID: string;
        
    @Field({description: `Name of the cuisine type`}) 
    @MaxLength(100)
    Name: string;
        
    @Field({nullable: true, description: `Description of the cuisine type`}) 
    @MaxLength(510)
    Description?: string;
    ...
    @Field(() => [FoodieRestaurant_])
    Restaurants_CuisineTypeIDArray: FoodieRestaurant_[]; // Link to Restaurants
    
}

//****************************************************************************
// INPUT TYPE for Cuisine Types
//****************************************************************************
@InputType()
export class CreateFoodieCuisineTypeInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdateFoodieCuisineTypeInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Cuisine Types
//****************************************************************************
@ObjectType()
export class RunFoodieCuisineTypeViewResult {
    @Field(() => [FoodieCuisineType_])
    Results: FoodieCuisineType_[];
...
@Resolver(FoodieCuisineType_)
export class FoodieCuisineTypeResolver extends ResolverBase {
    @Query(() => RunFoodieCuisineTypeViewResult)
    async RunFoodieCuisineTypeViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieCuisineTypeViewResult)
    async RunFoodieCuisineTypeViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieCuisineTypeViewResult)
    async RunFoodieCuisineTypeDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Cuisine Types';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => FoodieCuisineType_, { nullable: true })
    async FoodieCuisineType(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<FoodieCuisineType_ | null> {
        this.CheckUserReadPermissions('Cuisine Types', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwCuisineTypes] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Cuisine Types', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Cuisine Types', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @FieldResolver(() => [FoodieRestaurant_])
    async Restaurants_CuisineTypeIDArray(@Root() foodiecuisinetype_: FoodieCuisineType_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Restaurants', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwRestaurants] WHERE [CuisineTypeID]='${foodiecuisinetype_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Restaurants', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Restaurants', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @Mutation(() => FoodieCuisineType_)
    async CreateFoodieCuisineType(
        @Arg('input', () => CreateFoodieCuisineTypeInput) input: CreateFoodieCuisineTypeInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Cuisine Types', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => FoodieCuisineType_)
    async UpdateFoodieCuisineType(
        @Arg('input', () => UpdateFoodieCuisineTypeInput) input: UpdateFoodieCuisineTypeInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Cuisine Types', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => FoodieCuisineType_)
    async DeleteFoodieCuisineType(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Cuisine Types', key, options, provider, userPayload, pubSub);
    }
...
//****************************************************************************
// ENTITY CLASS for Group Visit Members
//****************************************************************************
@ObjectType({ description: `Junction table linking members to group visits with payment tracking` })
export class FoodieGroupVisitMember_ {
    @Field({description: `Unique identifier for the group visit member record`}) 
    @MaxLength(16)
    ID: string;
        
    @Field({description: `Foreign key to the GroupVisit table`}) 
    @MaxLength(16)
    GroupVisitID: string;
        
    @Field({description: `Foreign key to the Member table`}) 
    @MaxLength(16)
    MemberID: string;
        
    @Field(() => Float, {nullable: true, description: `Amount paid by this member for the group visit`}) 
    AmountPaid?: number;
...
//****************************************************************************
// INPUT TYPE for Group Visit Members
//****************************************************************************
@InputType()
export class CreateFoodieGroupVisitMemberInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdateFoodieGroupVisitMemberInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Group Visit Members
//****************************************************************************
@ObjectType()
export class RunFoodieGroupVisitMemberViewResult {
    @Field(() => [FoodieGroupVisitMember_])
    Results: FoodieGroupVisitMember_[];
...
@Resolver(FoodieGroupVisitMember_)
export class FoodieGroupVisitMemberResolver extends ResolverBase {
    @Query(() => RunFoodieGroupVisitMemberViewResult)
    async RunFoodieGroupVisitMemberViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieGroupVisitMemberViewResult)
    async RunFoodieGroupVisitMemberViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieGroupVisitMemberViewResult)
    async RunFoodieGroupVisitMemberDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Group Visit Members';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => FoodieGroupVisitMember_, { nullable: true })
    async FoodieGroupVisitMember(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<FoodieGroupVisitMember_ | null> {
        this.CheckUserReadPermissions('Group Visit Members', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwGroupVisitMembers] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Group Visit Members', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Group Visit Members', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => FoodieGroupVisitMember_)
    async CreateFoodieGroupVisitMember(
        @Arg('input', () => CreateFoodieGroupVisitMemberInput) input: CreateFoodieGroupVisitMemberInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Group Visit Members', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => FoodieGroupVisitMember_)
    async UpdateFoodieGroupVisitMember(
        @Arg('input', () => UpdateFoodieGroupVisitMemberInput) input: UpdateFoodieGroupVisitMemberInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Group Visit Members', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => FoodieGroupVisitMember_)
    async DeleteFoodieGroupVisitMember(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Group Visit Members', key, options, provider, userPayload, pubSub);
    }
    
}

//****************************************************************************
// ENTITY CLASS for Group Visits
//****************************************************************************
@ObjectType({ description: `Stores information about group visits to restaurants` })
export class FoodieGroupVisit_ {
    @Field({description: `Unique identifier for the group visit`}) 
    @MaxLength(16)
    ID: string;
        
    @Field({description: `Name or description of the group visit event`}) 
    @MaxLength(200)
    Name: string;
        
    @Field({description: `Foreign key to the Restaurant table`}) 
    @MaxLength(16)
    RestaurantID: string;
        
    @Field({description: `Date and time of the group visit`}) 
    @MaxLength(8)
    VisitDate: Date;
        
    @Field(() => Float, {nullable: true, description: `Total cost of the group visit`}) 
    TotalCost?: number;
        
    @Field(() => Int, {nullable: true, description: `Overall group consensus rating from 1 to 5 stars`}) 
    GroupRating?: number;
        
    @Field({nullable: true, description: `Notes about the group visit`}) 
    Notes?: string;
    ...
    @Field(() => [FoodieGroupVisitMember_])
    GroupVisitMembers_GroupVisitIDArray: FoodieGroupVisitMember_[]; // Link to GroupVisitMembers
    
}

//****************************************************************************
// INPUT TYPE for Group Visits
//****************************************************************************
@InputType()
export class CreateFoodieGroupVisitInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdateFoodieGroupVisitInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Group Visits
//****************************************************************************
@ObjectType()
export class RunFoodieGroupVisitViewResult {
    @Field(() => [FoodieGroupVisit_])
    Results: FoodieGroupVisit_[];
...
@Resolver(FoodieGroupVisit_)
export class FoodieGroupVisitResolver extends ResolverBase {
    @Query(() => RunFoodieGroupVisitViewResult)
    async RunFoodieGroupVisitViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieGroupVisitViewResult)
    async RunFoodieGroupVisitViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieGroupVisitViewResult)
    async RunFoodieGroupVisitDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Group Visits';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => FoodieGroupVisit_, { nullable: true })
    async FoodieGroupVisit(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<FoodieGroupVisit_ | null> {
        this.CheckUserReadPermissions('Group Visits', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwGroupVisits] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Group Visits', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Group Visits', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @FieldResolver(() => [FoodieGroupVisitMember_])
    async GroupVisitMembers_GroupVisitIDArray(@Root() foodiegroupvisit_: FoodieGroupVisit_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Group Visit Members', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwGroupVisitMembers] WHERE [GroupVisitID]='${foodiegroupvisit_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Group Visit Members', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Group Visit Members', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @Mutation(() => FoodieGroupVisit_)
    async CreateFoodieGroupVisit(
        @Arg('input', () => CreateFoodieGroupVisitInput) input: CreateFoodieGroupVisitInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Group Visits', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => FoodieGroupVisit_)
    async UpdateFoodieGroupVisit(
        @Arg('input', () => UpdateFoodieGroupVisitInput) input: UpdateFoodieGroupVisitInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Group Visits', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => FoodieGroupVisit_)
    async DeleteFoodieGroupVisit(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Group Visits', key, options, provider, userPayload, pubSub);
    }
...
//****************************************************************************
// ENTITY CLASS for Members__Foodie
//****************************************************************************
@ObjectType({ description: `Stores information about members who track restaurants` })
export class FoodieMember_ {
    @Field({description: `Unique identifier for the member`}) 
    @MaxLength(16)
    ID: string;
        
    @Field({description: `Name of the member`}) 
    @MaxLength(200)
    Name: string;
        
    @Field({description: `Email address of the member`}) 
    @MaxLength(510)
    Email: string;
        
    @Field({description: `Date when the member joined the tracking system`}) 
    @MaxLength(8)
    JoinDate: Date;
        
    @Field({nullable: true, description: `Dietary restrictions or preferences (vegetarian, vegan, gluten-free, etc.)`}) 
    @MaxLength(1000)
    ...
    @Field(() => [FoodieWishList_])
    WishLists_MemberIDArray: FoodieWishList_[]; // Link to WishLists
    
    @Field(() => [FoodieRestaurantVisit_])
    RestaurantVisits_MemberIDArray: FoodieRestaurantVisit_[]; // Link to RestaurantVisits
    
    @Field(() => [FoodieGroupVisitMember_])
    GroupVisitMembers_MemberIDArray: FoodieGroupVisitMember_[]; // Link to GroupVisitMembers
    
}

//****************************************************************************
// INPUT TYPE for Members__Foodie
//****************************************************************************
@InputType()
export class CreateFoodieMemberInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdateFoodieMemberInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Members__Foodie
//****************************************************************************
@ObjectType()
export class RunFoodieMemberViewResult {
    @Field(() => [FoodieMember_])
    Results: FoodieMember_[];
...
@Resolver(FoodieMember_)
export class FoodieMemberResolver extends ResolverBase {
    @Query(() => RunFoodieMemberViewResult)
    async RunFoodieMemberViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieMemberViewResult)
    async RunFoodieMemberViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieMemberViewResult)
    async RunFoodieMemberDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Members__Foodie';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => FoodieMember_, { nullable: true })
    async FoodieMember(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<FoodieMember_ | null> {
        this.CheckUserReadPermissions('Members__Foodie', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwMembers__Foodie] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Members__Foodie', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Members__Foodie', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @FieldResolver(() => [FoodieWishList_])
    async WishLists_MemberIDArray(@Root() foodiemember_: FoodieMember_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Wish Lists', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwWishLists] WHERE [MemberID]='${foodiemember_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Wish Lists', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Wish Lists', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @FieldResolver(() => [FoodieRestaurantVisit_])
    async RestaurantVisits_MemberIDArray(@Root() foodiemember_: FoodieMember_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Restaurant Visits', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwRestaurantVisits] WHERE [MemberID]='${foodiemember_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Restaurant Visits', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Restaurant Visits', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @FieldResolver(() => [FoodieGroupVisitMember_])
    async GroupVisitMembers_MemberIDArray(@Root() foodiemember_: FoodieMember_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Group Visit Members', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwGroupVisitMembers] WHERE [MemberID]='${foodiemember_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Group Visit Members', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Group Visit Members', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @Mutation(() => FoodieMember_)
    async CreateFoodieMember(
        @Arg('input', () => CreateFoodieMemberInput) input: CreateFoodieMemberInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Members__Foodie', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => FoodieMember_)
    async UpdateFoodieMember(
        @Arg('input', () => UpdateFoodieMemberInput) input: UpdateFoodieMemberInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Members__Foodie', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => FoodieMember_)
    async DeleteFoodieMember(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Members__Foodie', key, options, provider, userPayload, pubSub);
    }
...
//****************************************************************************
// ENTITY CLASS for Restaurant Tags
//****************************************************************************
@ObjectType({ description: `Stores tags and categories for restaurants (casual dining, outdoor seating, etc.)` })
export class FoodieRestaurantTag_ {
    @Field({description: `Unique identifier for the tag record`}) 
    @MaxLength(16)
    ID: string;
    ...
    @Field({description: `Name of the tag (e.g., Casual Dining, Outdoor Seating, Good for Groups)`}) 
    @MaxLength(100)
    TagName: string;
...
//****************************************************************************
// INPUT TYPE for Restaurant Tags
//****************************************************************************
@InputType()
export class CreateFoodieRestaurantTagInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdateFoodieRestaurantTagInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Restaurant Tags
//****************************************************************************
@ObjectType()
export class RunFoodieRestaurantTagViewResult {
    @Field(() => [FoodieRestaurantTag_])
    Results: FoodieRestaurantTag_[];
...
@Resolver(FoodieRestaurantTag_)
export class FoodieRestaurantTagResolver extends ResolverBase {
    @Query(() => RunFoodieRestaurantTagViewResult)
    async RunFoodieRestaurantTagViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieRestaurantTagViewResult)
    async RunFoodieRestaurantTagViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieRestaurantTagViewResult)
    async RunFoodieRestaurantTagDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Restaurant Tags';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => FoodieRestaurantTag_, { nullable: true })
    async FoodieRestaurantTag(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<FoodieRestaurantTag_ | null> {
        this.CheckUserReadPermissions('Restaurant Tags', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwRestaurantTags] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Restaurant Tags', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Restaurant Tags', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => FoodieRestaurantTag_)
    async CreateFoodieRestaurantTag(
        @Arg('input', () => CreateFoodieRestaurantTagInput) input: CreateFoodieRestaurantTagInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Restaurant Tags', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => FoodieRestaurantTag_)
    async UpdateFoodieRestaurantTag(
        @Arg('input', () => UpdateFoodieRestaurantTagInput) input: UpdateFoodieRestaurantTagInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Restaurant Tags', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => FoodieRestaurantTag_)
    async DeleteFoodieRestaurantTag(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Restaurant Tags', key, options, provider, userPayload, pubSub);
    }
    
}

//****************************************************************************
// ENTITY CLASS for Restaurant Visits
//****************************************************************************
@ObjectType({ description: `Stores individual member visits to restaurants with ratings and reviews` })
export class FoodieRestaurantVisit_ {
    @Field({description: `Unique identifier for the visit record`}) 
    @MaxLength(16)
    ID: string;
    ...
    @Field({description: `Date and time of the restaurant visit`}) 
    @MaxLength(8)
    VisitDate: Date;
        
    @Field(() => Int, {nullable: true, description: `Rating from 1 to 5 stars`}) 
    Rating?: number;
        
    @Field({nullable: true, description: `Member comments and review of the visit`}) 
    Comments?: string;
        
    @Field({nullable: true, description: `Description of dishes ordered during the visit`}) 
    DishesOrdered?: string;
        
    @Field(() => Boolean, {nullable: true, description: `Indicates whether the member would return to this restaurant`}) 
    WouldReturn?: boolean;
        
    @Field({nullable: true, description: `Comma-separated URLs to photos from the visit`}) 
    PhotoURLs?: string;
...
//****************************************************************************
// INPUT TYPE for Restaurant Visits
//****************************************************************************
@InputType()
export class CreateFoodieRestaurantVisitInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdateFoodieRestaurantVisitInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Restaurant Visits
//****************************************************************************
@ObjectType()
export class RunFoodieRestaurantVisitViewResult {
    @Field(() => [FoodieRestaurantVisit_])
    Results: FoodieRestaurantVisit_[];
...
@Resolver(FoodieRestaurantVisit_)
export class FoodieRestaurantVisitResolver extends ResolverBase {
    @Query(() => RunFoodieRestaurantVisitViewResult)
    async RunFoodieRestaurantVisitViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieRestaurantVisitViewResult)
    async RunFoodieRestaurantVisitViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieRestaurantVisitViewResult)
    async RunFoodieRestaurantVisitDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Restaurant Visits';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => FoodieRestaurantVisit_, { nullable: true })
    async FoodieRestaurantVisit(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<FoodieRestaurantVisit_ | null> {
        this.CheckUserReadPermissions('Restaurant Visits', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwRestaurantVisits] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Restaurant Visits', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Restaurant Visits', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => FoodieRestaurantVisit_)
    async CreateFoodieRestaurantVisit(
        @Arg('input', () => CreateFoodieRestaurantVisitInput) input: CreateFoodieRestaurantVisitInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Restaurant Visits', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => FoodieRestaurantVisit_)
    async UpdateFoodieRestaurantVisit(
        @Arg('input', () => UpdateFoodieRestaurantVisitInput) input: UpdateFoodieRestaurantVisitInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Restaurant Visits', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => FoodieRestaurantVisit_)
    async DeleteFoodieRestaurantVisit(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Restaurant Visits', key, options, provider, userPayload, pubSub);
    }
    
}

//****************************************************************************
// ENTITY CLASS for Restaurants
//****************************************************************************
@ObjectType({ description: `Stores information about restaurants` })
export class FoodieRestaurant_ {
    @Field({description: `Unique identifier for the restaurant`}) 
    @MaxLength(16)
    ID: string;
    ...
    @Field({nullable: true, description: `Street address of the restaurant`}) 
    @MaxLength(400)
    StreetAddress?: string;
        
    @Field({nullable: true, description: `City where the restaurant is located`}) 
    @MaxLength(200)
    City?: string;
        
    @Field({nullable: true, description: `State where the restaurant is located`}) 
    @MaxLength(4)
    State?: string;
        
    @Field({nullable: true, description: `Zip code of the restaurant location`}) 
    @MaxLength(20)
    ZipCode?: string;
        
    @Field({nullable: true, description: `Phone number of the restaurant`}) 
    @MaxLength(40)
    Phone?: string;
        
    @Field({nullable: true, description: `Website URL of the restaurant`}) 
    @MaxLength(510)
    Website?: string;
        
    @Field({nullable: true, description: `Foreign key to the CuisineType table`}) 
    @MaxLength(16)
    CuisineTypeID?: string;
        
    @Field({nullable: true, description: `Price range indicator ($ to $$$$)`}) 
    @MaxLength(20)
    PriceRange?: string;
        
    @Field(() => Float, {nullable: true, description: `Latitude coordinate for mapping`}) 
    Latitude?: number;
        
    @Field(() => Float, {nullable: true, description: `Longitude coordinate for mapping`}) 
    Longitude?: number;
        
    @Field({nullable: true, description: `Hours of operation text (e.g., Mon-Fri 11am-10pm)`}) 
    @MaxLength(1000)
    ...
    @Field(() => [FoodieWishList_])
    WishLists_RestaurantIDArray: FoodieWishList_[]; // Link to WishLists
    
    @Field(() => [FoodieRestaurantVisit_])
    RestaurantVisits_RestaurantIDArray: FoodieRestaurantVisit_[]; // Link to RestaurantVisits
    
    @Field(() => [FoodieRestaurantTag_])
    RestaurantTags_RestaurantIDArray: FoodieRestaurantTag_[]; // Link to RestaurantTags
    
    @Field(() => [FoodieGroupVisit_])
    GroupVisits_RestaurantIDArray: FoodieGroupVisit_[]; // Link to GroupVisits
    
}

//****************************************************************************
// INPUT TYPE for Restaurants
//****************************************************************************
@InputType()
export class CreateFoodieRestaurantInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdateFoodieRestaurantInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Restaurants
//****************************************************************************
@ObjectType()
export class RunFoodieRestaurantViewResult {
    @Field(() => [FoodieRestaurant_])
    Results: FoodieRestaurant_[];
...
@Resolver(FoodieRestaurant_)
export class FoodieRestaurantResolver extends ResolverBase {
    @Query(() => RunFoodieRestaurantViewResult)
    async RunFoodieRestaurantViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieRestaurantViewResult)
    async RunFoodieRestaurantViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieRestaurantViewResult)
    async RunFoodieRestaurantDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Restaurants';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => FoodieRestaurant_, { nullable: true })
    async FoodieRestaurant(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<FoodieRestaurant_ | null> {
        this.CheckUserReadPermissions('Restaurants', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwRestaurants] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Restaurants', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Restaurants', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @FieldResolver(() => [FoodieWishList_])
    async WishLists_RestaurantIDArray(@Root() foodierestaurant_: FoodieRestaurant_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Wish Lists', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwWishLists] WHERE [RestaurantID]='${foodierestaurant_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Wish Lists', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Wish Lists', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
    ...
    @FieldResolver(() => [FoodieRestaurantVisit_])
    async RestaurantVisits_RestaurantIDArray(@Root() foodierestaurant_: FoodieRestaurant_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Restaurant Visits', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwRestaurantVisits] WHERE [RestaurantID]='${foodierestaurant_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Restaurant Visits', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Restaurant Visits', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @FieldResolver(() => [FoodieRestaurantTag_])
    async RestaurantTags_RestaurantIDArray(@Root() foodierestaurant_: FoodieRestaurant_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Restaurant Tags', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwRestaurantTags] WHERE [RestaurantID]='${foodierestaurant_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Restaurant Tags', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Restaurant Tags', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @FieldResolver(() => [FoodieGroupVisit_])
    async GroupVisits_RestaurantIDArray(@Root() foodierestaurant_: FoodieRestaurant_, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine) {
        this.CheckUserReadPermissions('Group Visits', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwGroupVisits] WHERE [RestaurantID]='${foodierestaurant_.ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Group Visits', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.ArrayMapFieldNamesToCodeNames('Group Visits', rows, this.GetUserFromPayload(userPayload));
        return result;
    }
        
    @Mutation(() => FoodieRestaurant_)
    async CreateFoodieRestaurant(
        @Arg('input', () => CreateFoodieRestaurantInput) input: CreateFoodieRestaurantInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Restaurants', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => FoodieRestaurant_)
    async UpdateFoodieRestaurant(
        @Arg('input', () => UpdateFoodieRestaurantInput) input: UpdateFoodieRestaurantInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Restaurants', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => FoodieRestaurant_)
    async DeleteFoodieRestaurant(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Restaurants', key, options, provider, userPayload, pubSub);
    }
...
//****************************************************************************
// ENTITY CLASS for Wish Lists
//****************************************************************************
@ObjectType({ description: `Stores restaurants that members want to try in the future` })
export class FoodieWishList_ {
    @Field({description: `Unique identifier for the wishlist entry`}) 
    @MaxLength(16)
    ID: string;
    ...
    @Field({description: `Priority level for trying this restaurant (Low, Medium, High)`}) 
    @MaxLength(40)
    Priority: string;
        
    @Field({nullable: true, description: `Name of the person who suggested this restaurant`}) 
    @MaxLength(200)
    SuggestedBy?: string;
        
    @Field({nullable: true, description: `Additional notes about why this restaurant is on the wishlist`}) 
    @MaxLength(1000)
    Notes?: string;
        
    @Field({description: `Date when the restaurant was added to the wishlist`}) 
    @MaxLength(8)
    AddedDate: Date;
...
//****************************************************************************
// INPUT TYPE for Wish Lists
//****************************************************************************
@InputType()
export class CreateFoodieWishListInput {
    @Field({ nullable: true })
    ID?: string;
...
//****************************************************************************
@InputType()
export class UpdateFoodieWishListInput {
    @Field()
    ID: string;
...
//****************************************************************************
// RESOLVER for Wish Lists
//****************************************************************************
@ObjectType()
export class RunFoodieWishListViewResult {
    @Field(() => [FoodieWishList_])
    Results: FoodieWishList_[];
...
@Resolver(FoodieWishList_)
export class FoodieWishListResolver extends ResolverBase {
    @Query(() => RunFoodieWishListViewResult)
    async RunFoodieWishListViewByID(@Arg('input', () => RunViewByIDInput) input: RunViewByIDInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByIDGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieWishListViewResult)
    async RunFoodieWishListViewByName(@Arg('input', () => RunViewByNameInput) input: RunViewByNameInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        return super.RunViewByNameGeneric(input, provider, userPayload, pubSub);
    }

    @Query(() => RunFoodieWishListViewResult)
    async RunFoodieWishListDynamicView(@Arg('input', () => RunDynamicViewInput) input: RunDynamicViewInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        input.EntityName = 'Wish Lists';
        return super.RunDynamicViewGeneric(input, provider, userPayload, pubSub);
    }
    @Query(() => FoodieWishList_, { nullable: true })
    async FoodieWishList(@Arg('ID', () => String) ID: string, @Ctx() { dataSources, userPayload, providers }: AppContext, @PubSub() pubSub: PubSubEngine): Promise<FoodieWishList_ | null> {
        this.CheckUserReadPermissions('Wish Lists', userPayload);
        const provider = GetReadOnlyProvider(providers, { allowFallbackToReadWrite: true });
        const connPool = GetReadOnlyDataSource(dataSources, { allowFallbackToReadWrite: true });
        const sSQL = `SELECT * FROM [Foodie].[vwWishLists] WHERE [ID]='${ID}' ` + this.getRowLevelSecurityWhereClause(provider, 'Wish Lists', userPayload, EntityPermissionType.Read, 'AND');
        const rows = await SQLServerDataProvider.ExecuteSQLWithPool(connPool, sSQL, undefined, this.GetUserFromPayload(userPayload));
        const result = await this.MapFieldNamesToCodeNames('Wish Lists', rows && rows.length > 0 ? rows[0] : {}, this.GetUserFromPayload(userPayload));
        return result;
    }
    
    @Mutation(() => FoodieWishList_)
    async CreateFoodieWishList(
        @Arg('input', () => CreateFoodieWishListInput) input: CreateFoodieWishListInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.CreateRecord('Wish Lists', input, provider, userPayload, pubSub)
    }
        
    @Mutation(() => FoodieWishList_)
    async UpdateFoodieWishList(
        @Arg('input', () => UpdateFoodieWishListInput) input: UpdateFoodieWishListInput,
        @Ctx() { providers, userPayload }: AppContext,
        @PubSub() pubSub: PubSubEngine
    ) {
        const provider = GetReadWriteProvider(providers);
        return this.UpdateRecord('Wish Lists', input, provider, userPayload, pubSub);
    }
    
    @Mutation(() => FoodieWishList_)
    async DeleteFoodieWishList(@Arg('ID', () => String) ID: string, @Arg('options___', () => DeleteOptionsInput) options: DeleteOptionsInput, @Ctx() { providers, userPayload }: AppContext, @PubSub() pubSub: PubSubEngine) {
        const provider = GetReadWriteProvider(providers);
        const key = new CompositeKey([{FieldName: 'ID', Value: ID}]);
        return this.DeleteRecord('Wish Lists', key, options, provider, userPayload, pubSub);
    }
