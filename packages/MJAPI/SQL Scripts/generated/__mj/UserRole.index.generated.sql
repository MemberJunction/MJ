-----------------------------------------------------------------
-- SQL Code Generation
-- Entity: MJ: User Roles
-- Item: Index for Foreign Keys
--
-- This was generated by the MemberJunction CodeGen tool.
-- This file should NOT be edited by hand.
-----------------------------------------------------------------
-- Index for foreign key UserID in table UserRole
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IDX_AUTO_MJ_FKEY_UserRole_UserID' 
    AND object_id = OBJECT_ID('[__mj].[UserRole]')
)
CREATE INDEX IDX_AUTO_MJ_FKEY_UserRole_UserID ON [__mj].[UserRole] ([UserID]);

-- Index for foreign key RoleID in table UserRole
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IDX_AUTO_MJ_FKEY_UserRole_RoleID' 
    AND object_id = OBJECT_ID('[__mj].[UserRole]')
)
CREATE INDEX IDX_AUTO_MJ_FKEY_UserRole_RoleID ON [__mj].[UserRole] ([RoleID]);