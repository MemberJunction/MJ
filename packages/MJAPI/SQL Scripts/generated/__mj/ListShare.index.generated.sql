-----------------------------------------------------------------
-- SQL Code Generation
-- Entity: MJ: List Shares
-- Item: Index for Foreign Keys
--
-- This was generated by the MemberJunction CodeGen tool.
-- This file should NOT be edited by hand.
-----------------------------------------------------------------
-- Index for foreign key ListID in table ListShare
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IDX_AUTO_MJ_FKEY_ListShare_ListID' 
    AND object_id = OBJECT_ID('[__mj].[ListShare]')
)
CREATE INDEX IDX_AUTO_MJ_FKEY_ListShare_ListID ON [__mj].[ListShare] ([ListID]);

-- Index for foreign key UserID in table ListShare
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IDX_AUTO_MJ_FKEY_ListShare_UserID' 
    AND object_id = OBJECT_ID('[__mj].[ListShare]')
)
CREATE INDEX IDX_AUTO_MJ_FKEY_ListShare_UserID ON [__mj].[ListShare] ([UserID]);