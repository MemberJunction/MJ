-----------------------------------------------------------------
-- SQL Code Generation
-- Entity: MJ: AI Prompts
-- Item: fnAIPromptResultSelectorPromptID_GetRootID
--
-- This was generated by the MemberJunction CodeGen tool.
-- This file should NOT be edited by hand.
-----------------------------------------------------------------
------------------------------------------------------------
----- ROOT ID FUNCTION FOR: [AIPrompt].[ResultSelectorPromptID]
------------------------------------------------------------
IF OBJECT_ID('[__mj].[fnAIPromptResultSelectorPromptID_GetRootID]', 'IF') IS NOT NULL
    DROP FUNCTION [__mj].[fnAIPromptResultSelectorPromptID_GetRootID];
GO

CREATE FUNCTION [__mj].[fnAIPromptResultSelectorPromptID_GetRootID]
(
    @RecordID uniqueidentifier,
    @ParentID uniqueidentifier
)
RETURNS TABLE
AS
RETURN
(
    WITH CTE_RootParent AS (
        SELECT
            [ID],
            [ResultSelectorPromptID],
            [ID] AS [RootParentID],
            0 AS [Depth]
        FROM
            [__mj].[AIPrompt]
        WHERE
            [ID] = COALESCE(@ParentID, @RecordID)

        UNION ALL

        SELECT
            c.[ID],
            c.[ResultSelectorPromptID],
            c.[ID] AS [RootParentID],
            p.[Depth] + 1 AS [Depth]
        FROM
            [__mj].[AIPrompt] c
        INNER JOIN
            CTE_RootParent p ON c.[ID] = p.[ResultSelectorPromptID]
        WHERE
            p.[Depth] < 100
    )
    SELECT TOP 1
        [RootParentID] AS RootID
    FROM
        CTE_RootParent
    WHERE
        [ResultSelectorPromptID] IS NULL
    ORDER BY
        [RootParentID]
);
GO
