-----------------------------------------------------------------
-- SQL Code Generation
-- Entity: MJ: Projects
-- Item: Index for Foreign Keys
--
-- This was generated by the MemberJunction CodeGen tool.
-- This file should NOT be edited by hand.
-----------------------------------------------------------------
-- Index for foreign key EnvironmentID in table Project
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IDX_AUTO_MJ_FKEY_Project_EnvironmentID' 
    AND object_id = OBJECT_ID('[__mj].[Project]')
)
CREATE INDEX IDX_AUTO_MJ_FKEY_Project_EnvironmentID ON [__mj].[Project] ([EnvironmentID]);

-- Index for foreign key ParentID in table Project
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IDX_AUTO_MJ_FKEY_Project_ParentID' 
    AND object_id = OBJECT_ID('[__mj].[Project]')
)
CREATE INDEX IDX_AUTO_MJ_FKEY_Project_ParentID ON [__mj].[Project] ([ParentID]);