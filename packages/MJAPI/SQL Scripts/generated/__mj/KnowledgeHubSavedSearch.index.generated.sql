-----------------------------------------------------------------
-- SQL Code Generation
-- Entity: MJ: Knowledge Hub Saved Searches
-- Item: Index for Foreign Keys
--
-- This was generated by the MemberJunction CodeGen tool.
-- This file should NOT be edited by hand.
-----------------------------------------------------------------
-- Index for foreign key UserID in table KnowledgeHubSavedSearch
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IDX_AUTO_MJ_FKEY_KnowledgeHubSavedSearch_UserID' 
    AND object_id = OBJECT_ID('[__mj].[KnowledgeHubSavedSearch]')
)
CREATE INDEX IDX_AUTO_MJ_FKEY_KnowledgeHubSavedSearch_UserID ON [__mj].[KnowledgeHubSavedSearch] ([UserID]);