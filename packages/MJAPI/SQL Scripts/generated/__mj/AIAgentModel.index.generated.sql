-----------------------------------------------------------------
-- SQL Code Generation
-- Entity: MJ: AI Agent Models
-- Item: Index for Foreign Keys
--
-- This was generated by the MemberJunction CodeGen tool.
-- This file should NOT be edited by hand.
-----------------------------------------------------------------
-- Index for foreign key AgentID in table AIAgentModel
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IDX_AUTO_MJ_FKEY_AIAgentModel_AgentID' 
    AND object_id = OBJECT_ID('[__mj].[AIAgentModel]')
)
CREATE INDEX IDX_AUTO_MJ_FKEY_AIAgentModel_AgentID ON [__mj].[AIAgentModel] ([AgentID]);

-- Index for foreign key ModelID in table AIAgentModel
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IDX_AUTO_MJ_FKEY_AIAgentModel_ModelID' 
    AND object_id = OBJECT_ID('[__mj].[AIAgentModel]')
)
CREATE INDEX IDX_AUTO_MJ_FKEY_AIAgentModel_ModelID ON [__mj].[AIAgentModel] ([ModelID]);