...
export default class ExportSampleQueries extends Command {
  ...
  static examples = [
    '$ db-auto-doc export-sample-queries --input ./output/sample-queries.json --output ./metadata/queries/.queries.json',
    '$ db-auto-doc export-sample-queries --input ./sample-queries.json --output ./metadata/queries/.queries.json --separate-sql-files',
    '$ db-auto-doc export-sample-queries --input ./sample-queries.json --output ./queries/.queries.json --category "Database Documentation"',
    '$ db-auto-doc export-sample-queries --input ./sample-queries.json --output ./queries/.queries.json --status Pending --min-confidence 0.8'
  ];
  ...
  async run(): Promise<void> {
    ...
    try {
      // Load sample queries
      spinner.start('Loading sample queries');
      const inputPath = path.resolve(flags.input);
      const queriesJson = await fs.readFile(inputPath, 'utf-8');
      const sampleQueries: SampleQuery[] = JSON.parse(queriesJson);
      spinner.succeed(`Loaded ${sampleQueries.length} sample queries`);
