% DBAutoDoc — PVLDB Format
% Template: Proceedings of the VLDB Endowment (pvldb style)
%
% To compile:
%   pdflatex main
%   bibtex main
...
%% PVLDB-specific metadata (adapt for final version)
% paper-specific
...
\newcommand\vldbavailabilityurl{https://github.com/MemberJunction/MJ}
% use 'plain' for review versions, 'empty' for camera ready
...
% Suppress acmart boilerplate
\settopmatter{printfolios=true, printccs=false, printacmref=false}
\renewcommand\footnotetextcopyrightpermission[1]{}
% Disable column balancing — it creates empty trailing pages
\makeatletter
\@ACM@balancefalse
\makeatother

% Prevent float queue backup — allow LaTeX more flexibility
...
\affiliation{%
  \institution{Blue Cypress}
}

\author{Thomas Altman}
\email{thomas@tasiolabs.com}
\affiliation{%
  \institution{Tasio Labs (a Blue Cypress company)}
}
...
Section~\ref{sec:related} reviews related work. Section~\ref{sec:architecture} provides a technical overview of the architecture. Section~\ref{sec:keydiscovery} describes the key discovery pipeline. Section~\ref{sec:iterative} presents the iterative refinement algorithm. Section~\ref{sec:implementation} covers implementation details. Section~\ref{sec:evaluation} reports experimental results including ablation analysis. Section~\ref{sec:discussion} discusses limitations, threats to validity, and ethical considerations. Section~\ref{sec:future} outlines future work. Section~\ref{sec:conclusion} concludes.
...
\State \textsc{Discover}: Run PK/FK detection (Sec.~\ref{sec:keydiscovery}) $\to$ augment $R$
...
\textbf{OrgA} and \textbf{OrgB} are production enterprise databases (details in Section~\ref{sec:enterprise}).
...
\textbf{Interpretation.} These results address a natural concern about training data contamination (see Section~\ref{sec:threats}): if the LLM simply ``knew'' AdventureWorks relationships from pre-training, the deterministic pipeline would add no value. The 23-point F1 improvement from gates and pruning demonstrates that the pipeline's filtering and scoring mechanisms contribute substantial precision gains that the LLM cannot achieve alone. Furthermore, on private databases unseen by any LLM (OrgA, OrgB) and a purpose-built synthetic database (LousyDB), the system achieves comparable quality (Section~\ref{sec:enterprise}), ruling out memorization as the primary explanation.
...
#    (backups and stripping scripts in research/v1/)

# 3. Create config.json pointing to DB and LLM
