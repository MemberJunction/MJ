 ...
 * ai-configurations) so model-selection / failover / execution tests exercise
 * believable catalogs: real driver-class strings (AnthropicLLM / OpenAILLM /
 * GeminiLLM / GroqLLM…), real API names, real PowerRanks, and the multi-vendor /
 * multi-priority layout (a model offered by its developer + several inference
 * providers at different priorities) that selection logic actually has to
 * navigate.
 *
 ...
 * stable test UUIDs.
 *
 * Pure data only — no vitest/mock coupling. Tests wire these into whatever
 * AIEngine mock their package uses (see the AIPromptRunner suites in
 * packages/AI/Prompts for the reference wiring). Originally proven in
 * packages/AI/Prompts/src/__tests__/__fixtures__/ai-metadata.fixtures.ts and
 * promoted here so every AI package can build a models+vendors+prompts catalog
 * without copying it.
 */
...
// ---------------------------------------------------------------------------
// Lightweight fixture types (only the fields the AI runners read)
// ---------------------------------------------------------------------------
