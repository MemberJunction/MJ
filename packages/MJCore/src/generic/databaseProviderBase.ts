...
export abstract class DatabaseProviderBase extends ProviderBase {
     ...
     * providers. Used by a non-atomic `mj sync push` (`--no-atomic`) so parallel
     * graphs do not interleave `EntityTransactionScope`s on one provider.
     *
     ...
     */
    private static readonly _allPlatformUUIDPattern: RegExp =
     ...
     * Used by the static convenience method for code that doesn't have a provider instance.
     */
    private static readonly _allPlatformDefaultPattern: RegExp =
     ...
     */
    public IsUUIDGenerationFunction(value: string): boolean {
     ...
     */
    public IsNonUUIDDatabaseFunction(value: string): boolean {
     ...
     */
    public static IsUUIDGenerationFunctionAllPlatforms(value: string): boolean {
     ...
     * Prefer the instance method when you have a provider reference.
     */
    public static IsNonUUIDDatabaseFunctionAllPlatforms(value: string): boolean {
     ...
     */
    protected ShouldTrackRecordChanges(entityInfo: EntityInfo): boolean {
    ...
    protected BuildRecordChangePayload(
        ...
        const dataForFullJSON = newData ?? oldData;
        const fullRecordJSON = JSON.stringify(this.EscapeQuotesInProperties(dataForFullJSON, quoteToEscape));
     ...
     *
     * Implementations should delegate the dialect-agnostic assembly work to
     * {@link BuildRecordChangePayload} and only handle SQL string rendering
     * locally — that's how the duplication between SQL Server and PostgreSQL
     * stays minimal.
     *
     * @param restoreContext When non-null, the resulting RecordChange row is
