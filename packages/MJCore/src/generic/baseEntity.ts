...
export abstract class BaseEntity<T = unknown> {
    ...
    public GetFieldByCodeName(codeName: string): EntityField | null {
        ...
        if (this._codeNameCache === null) {
            ...
            for (const f of this.Fields) {
                // If there are duplicate code names, this will keep the last one,
                // matching the behavior of the previous array.find() when iterating backward,
                // actually array.find returns the FIRST match, so we should only set if it doesn't exist
                if (!this._codeNameCache.has(f.CodeName.trim().toLowerCase())) {
                    this._codeNameCache.set(f.CodeName.trim().toLowerCase(), f);
                }
            }
        }
