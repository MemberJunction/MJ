...
export type RunViewParams = {
  ...
  /**
   * optional - if provided, this will be used to offset the rows returned by the view. This is useful for paging through large data sets.
   */
  OffsetRows?: number;
};
...
export class RunView {
  ...
  public static get Provider(): IRunViewProvider {
    ...
    if (g) return g[RunView._globalProviderKey];
    else throw new Error('No global object store, so we cant get the static provider');
  }
  public static set Provider(value: IRunViewProvider) {
    const g = MJGlobal.Instance.GetGlobalObjectStore();
    if (g) g[RunView._globalProviderKey] = value;
    else throw new Error('No global object store, so we cant set the static provider');
  }
  ...
  public static async GetEntityNameFromRunViewParams(params: RunViewParams): Promise<string> {
    if (params.EntityName) return params.EntityName;
