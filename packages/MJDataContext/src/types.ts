...
export class DataContextItem {
  ...
  public set Data(value: any[]) {
    ...
    if (this.DataLoaded) this.DataLoadingError = null;
  }
  ...
  get Description(): string {
    ...
    if (this.AdditionalDescription && this.AdditionalDescription.length > 0) ret += ` (More Info: ${this.AdditionalDescription})`;
    return ret;
  }
  ...
  public async LoadData(
    ...
    try {
      if (this.Data && this.Data.length > 0 && !forceRefresh)
        // if we already have data and we aren't forcing a refresh, then we are done
        return true;
      else {
        switch (this.Type) {
          case 'full_entity':
  ...
  public async LoadMetadataFromEntityRecord(dataContextItem: DataContextItemEntity, provider: IMetadataProvider, contextUser: UserInfo) {
    ...
    if (this.EntityID) {
      ...
      if (this.Type === 'full_entity') this.RecordName = this.EntityName;
    }
   ...
   * @returns
   */
  protected async LoadFromSingleRecord(
    contextUser: UserInfo,
  ...
  public ValidateDataExists(ignoreFailedLoad: boolean = false): boolean {
    if (ignoreFailedLoad && !this.DataLoaded) return true;
    else return this.Data ? this.Data.length >= 0 : false; // can have 0 to many rows, just need to make sure we have a Data object to work with
  }
...
export class DataContext {
  ...
  public async LoadMetadata(DataContextID: string, contextUser?: UserInfo, provider?: IMetadataProvider): Promise<boolean> {
    try {
      if (!DataContextID || DataContextID.length === 0) throw new Error(`Data Context ID not set or invalid`);
      ...
      if (!dciEntityInfo) throw new Error(`Data Context Items entity not found`);
      ...
      if (!this.ID) throw new Error(`Data Context ID: ${DataContextID} not found`);

      const result = await rv.RunView<DataContextItemEntity>(
        { EntityName: 'Data Context Items', IgnoreMaxRows: true, ExtraFilter: `DataContextID = '${DataContextID}'` },
        contextUser
      );
  ...
  public async SaveItems(contextUser?: UserInfo, persistItemData: boolean = false): Promise<boolean> {
    try {
      if (!this.ID || this.ID.length === 0) throw new Error(`Data Context ID not set or invalid`);
      ...
      for (const itemEntry of itemsArray) {
        ...
        if (item.DataContextItemID && item.DataContextItemID.length > 0) await dciEntity.Load(item.DataContextItemID);
        else dciEntity.NewRecord();
        ...
        switch (item.Type) {
          ...
          case 'single_record':
            ...
            if (item.Type === 'single_record') dciEntity.RecordID = item.RecordID;
            break;
          case 'view':
        ...
        if (persistItemData && item.Data && item.Data.length > 0) dciEntity.DataJSON = JSON.stringify(item.Data);
        else dciEntity.DataJSON = null; //JSON.stringify(item.Data);
  ...
  public async Load(
    ...
    return (
      (await this.LoadMetadata(DataContextID, contextUser)) &&
    ...
    if (rawData) {
      ...
      if (rawData.Items && rawData.Items.length > 0) {
        for (const rawItem of rawData.Items) {
          ...
          if (item) newContext.Items.push(item);
        }
      }
    }
    return newContext;
    ...
    try {
      ...
      if (await newContext.Save()) {
        ...
        for (let item of context.Items) {
          ...
          if (!includeData) newItem.DataJSON = null; // if we aren't including the data, we need to clear it out

          newItem.TransactionGroup = tg;
          await newItem.Save();
        }
        const result = await tg.Submit();
        if (!result) throw new Error(`Error saving new data context items`);
