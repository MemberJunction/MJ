...
import { ScheduledJobResult, NotificationContent, AgentJobConfiguration } from '@memberjunction/scheduling-base-types';
...
export class AgentScheduledJobDriver extends BaseScheduledJob {
  public async Execute(context: ScheduledJobExecutionContext): Promise<ScheduledJobResult> {
    ...
    const conversationMessages = config.InitialMessage ? [{ role: 'user' as const, content: config.InitialMessage }] : [];
  ...
  public ValidateConfiguration(schedule: any): ValidationResult {
    ...
    try {
      ...
      if (!config.AgentID) {
        result.Errors.push(
          new ValidationErrorInfo('Configuration.AgentID', 'AgentID is required', config.AgentID, ValidationErrorType.Failure)
        );
      }
