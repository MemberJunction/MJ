...
import { UserInfo, Metadata, LogError, LogStatusEx, IsVerboseLoggingEnabled } from '@memberjunction/global';
...
export class SchedulingEngine extends SchedulingEngineBase {
   ...
   */
  public async ExecuteScheduledJobs(contextUser: UserInfo, evalTime: Date = new Date()): Promise<ScheduledJobRunEntity[]> {
   ...
   */
  public async ExecuteScheduledJob(jobId: string, contextUser: UserInfo): Promise<ScheduledJobRunEntity> {
    await this.Config(false, contextUser);
   ...
   * @private
   */
  private async executeJob(job: ScheduledJobEntity, contextUser: UserInfo): Promise<ScheduledJobRunEntity> {
    ...
    try {
      ...
      const plugin = MJGlobal.Instance.ClassFactory.CreateInstance<BaseScheduledJob>(BaseScheduledJob, jobType.DriverClass);
   ...
   * @private
   */
  private async createJobRun(job: ScheduledJobEntity, contextUser: UserInfo): Promise<ScheduledJobRunEntity> {
    const md = new Metadata();
    const run = await md.GetEntityObject<ScheduledJobRunEntity>('MJ: Scheduled Job Runs', contextUser);
   ...
   * @private
   */
  private async updateJobStatistics(job: ScheduledJobEntity, success: boolean, runId: string): Promise<void> {
    job.RunCount++;
    if (success) {
      job.SuccessCount++;
    } else {
  ...
  private async sendNotificationsIfNeeded(
    ...
    const shouldNotify = (result.Success && job.NotifyOnSuccess) || (!result.Success && job.NotifyOnFailure);
    ...
    await NotificationManager.SendScheduledJobNotification(recipientUserId, content, channels);
  }
  ...
  private async tryAcquireLock(job: ScheduledJobEntity): Promise<boolean> {
    ...
    console.log(
      `    🔒 tryAcquireLock: job.LockToken=${job.LockToken?.substring(0, 8) || 'NULL'}, ExpectedCompletionAt=${job.ExpectedCompletionAt?.toISOString() || 'NULL'}`
    );

    if (job.LockToken != null) {
      const now = new Date();
      console.log(
        `      Lock exists! Checking if stale: ExpectedCompletionAt=${job.ExpectedCompletionAt?.toISOString()}, now=${now.toISOString()}`
      );
   ...
   * @private
   */
  private async createQueuedJobRun(job: ScheduledJobEntity, contextUser: UserInfo): Promise<ScheduledJobRunEntity> {
    const md = new Metadata();
    const run = await md.GetEntityObject<ScheduledJobRunEntity>('MJ: Scheduled Job Runs', contextUser);
  ...
  private async cleanupStaleLocks(contextUser: UserInfo): Promise<void> {
    ...
    for (const job of this.ScheduledJobs) {
      if (job.LockToken) {
        console.log(
          `    Job "${job.Name}": LockToken=${job.LockToken?.substring(0, 8)}..., ExpectedCompletionAt=${job.ExpectedCompletionAt?.toISOString() || 'NULL'}`
        );

        if (job.ExpectedCompletionAt) {
          const isStale = job.ExpectedCompletionAt < now;
          console.log(
            `      → Is stale? ${isStale} (${job.ExpectedCompletionAt.getTime()} < ${now.getTime()} = ${job.ExpectedCompletionAt.getTime() < now.getTime()})`
          );
