...
export class CronExpressionHelper {
   ...
   */
  public static IsExpressionDue(cronExpression: string, timezone: string, evalTime: Date): boolean {
    try {
      const options = {
   ...
   */
  public static GetNextRunTime(cronExpression: string, timezone: string, fromDate?: Date): Date {
    try {
      const options = {
  ...
  public static ValidateExpression(cronExpression: string): ValidationResult {
    ...
    if (!cronExpression || cronExpression.trim().length === 0) {
      result.Errors.push(
        new ValidationErrorInfo('CronExpression', 'Cron expression is required', cronExpression, ValidationErrorType.Failure)
      );
      return result;
    }
    ...
    try {
      ...
      result.Errors.push(new ValidationErrorInfo('CronExpression', errorMessage, cronExpression, ValidationErrorType.Failure));
    }
