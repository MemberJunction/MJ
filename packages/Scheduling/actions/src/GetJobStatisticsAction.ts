...
export class GetScheduledJobStatisticsAction extends BaseJobAction {
  ...
  protected async InternalRunAction(params: RunActionParams): Promise<ActionResultSimple> {
    try {
      ...
      const filters: string[] = [`StartedAt >= '${dateThreshold.toISOString()}'`];
      ...
      for (const stats of jobStatsMap.values()) {
        ...
        const completedRuns = jobRuns.filter(
          (r) => r.ScheduledJobID === stats.JobID && r.Status === 'Completed' && r.StartedAt && r.CompletedAt
        );
