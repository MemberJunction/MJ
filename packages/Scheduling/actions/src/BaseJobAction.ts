...
import { Metadata, UserInfo, RunView } from '@memberjunction/global';
...
export abstract class BaseJobAction extends BaseAction {
   ...
   */
  protected async loadJob(jobId: string, contextUser: UserInfo): Promise<{ job?: ScheduledJobEntity; error?: ActionResultSimple }> {
    if (!jobId) {
      return {
        error: {
          Success: false,
