...
describe('ResolveEffectiveCloneOptions', () => {
    ...
    it('falls back to built-in defaults for an entity with no configuration', () => {
        const { Options } = ResolveEffectiveCloneOptions(null, undefined, false);
    ...
    it("lets a request lower the caps but not raise them when UserEditable is 'scope'", () => {
        const lowered = ResolveEffectiveCloneOptions({ MaxDepth: 3, MaxRecords: 500, UserEditable: 'scope' }, { MaxDepth: 1, MaxRecords: 10 }, false);
        expect(lowered.Options).toMatchObject({ MaxDepth: 1, MaxRecords: 10 });
        expect(lowered.Warnings).toEqual([]);

        const raised = ResolveEffectiveCloneOptions({ MaxDepth: 3, MaxRecords: 500, UserEditable: 'scope' }, { MaxDepth: 8, MaxRecords: 5000 }, false);
        expect(raised.Options).toMatchObject({ MaxDepth: 3, MaxRecords: 500 });
        expect(raised.Warnings.map((w) => [w.Code, w.Field])).toEqual([
            ['OPTION_OVERRIDE_IGNORED', 'MaxDepth'],
            ['OPTION_OVERRIDE_IGNORED', 'MaxRecords'],
        ]);
    });

    it("lets a request raise the caps when UserEditable is 'all' (the default)", () => {
        const { Options, Warnings } = ResolveEffectiveCloneOptions({ MaxDepth: 2 }, { MaxDepth: 6 }, false);
        expect(Options.MaxDepth).toBe(6);
        expect(Warnings).toEqual([]);
    });

    it("keeps the configured scope when UserEditable is 'none' or 'fields'", () => {
        for (const editable of ['none', 'fields'] as const) {
            const { Options, Warnings } = ResolveEffectiveCloneOptions(
                { MaxDepth: 2, SoftLinks: 'skip', UserEditable: editable },
                { MaxDepth: 1, SoftLinks: 'include' },
                false
            );
            expect(Options).toMatchObject({ MaxDepth: 2, SoftLinks: 'skip' });
            expect(Warnings.map((w) => w.Field)).toEqual(['MaxDepth', 'SoftLinks']);
        }
    });

    it('keeps hooks suppressed without the Fire Hooks authorization', () => {
        const denied = ResolveEffectiveCloneOptions(null, { EntityActions: 'fire', AIActions: 'fire' }, false);
        ...
        const granted = ResolveEffectiveCloneOptions(null, { EntityActions: 'fire' }, true);
    ...
    it('applies the Fire Hooks rule to a configured default of fire as well', () => {
        const { Options, Warnings } = ResolveEffectiveCloneOptions({ Hooks: { EntityActions: 'fire' } }, {}, false);
