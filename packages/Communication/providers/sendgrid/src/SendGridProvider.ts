...
export class SendGridProvider extends BaseCommunicationProvider {
  ...
  public async SendSingleMessage(message: ProcessedMessage): Promise<MessageResult> {
    ...
    try {
      ...
      return sgMail
        .send(msg)
        .then((result: [sgMail.ClientResponse, {}]) => {
  ...
  public async CreateDraft(params: CreateDraftParams): Promise<CreateDraftResult> {
    return {
      Success: false,
      ErrorMessage:
        'SendGrid does not support creating draft messages. Drafts are only supported by email providers with mailbox access (Gmail, MS Graph).',
    };
  }
}
