...
export class ProcessedMessageServer extends ProcessedMessage {
  public async Process(forceTemplateRefresh?: boolean, contextUser?: UserInfo): Promise<{ Success: boolean; Message?: string }> {
    ...
    if (this.BodyTemplate) {
      ...
      if (!this.HTMLBodyTemplate && !this.HTMLBody) {
        // if we have an HTMLBodyTemplate, we will process it separately below
    ...
    if (this.SubjectTemplate) {
      ...
      if (subjectContent) {
        ...
        return {
          Success: false,
          Message:
            'SubjectTemplate does not have an associated HTML Template Content option and this is required for processing the subject of the message.',
        };
      }
    } else {
