...
export class CommunicationEngine extends CommunicationEngineBase {
  ...
  public async SendMessages(
    ...
    if (!run) throw new Error(`Failed to start communication run.`);
    ...
    if (!(await this.EndRun(run))) throw new Error(`Failed to end communication run.`);
  ...
  public async SendSingleMessage(
    ...
    if (!message.MessageType) {
      ...
      const providerMessageType = providerEntity.MessageTypes.find(
        (pmt) => pmt.Name.trim().toLowerCase() === providerMessageTypeName.trim().toLowerCase()
      );
   ...
   */
  public async CreateDraft(message: Message, providerName: string, contextUser?: UserInfo): Promise<CreateDraftResult> {
    try {
      if (!this.Loaded) {
        return {
          Success: false,
