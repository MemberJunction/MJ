...
import { EntityInfo, LogStatus, Metadata, RunView, RunViewParams, RunViewResult, UserInfo } from '@memberjunction/global';
...
export class EntityCommunicationsEngine extends EntityCommunicationsEngineBase {
  ...
  public async RunEntityCommunication(params: EntityCommunicationParams): Promise<EntityCommunicationResult> {
    try {
      ...
      if (!entityInfo) throw new Error(`Entity ${params.EntityID} not found`);
      ...
      const provider: CommunicationProviderEntityExtended = CommunicationEngine.Instance.Providers.find(
        (p) => p.Name.trim().toLowerCase() === params.ProviderName.trim().toLowerCase()
      );
      if (!provider) {
      ...
      const providerMessageType = provider.MessageTypes.find(
        (mt) => mt.Name.trim().toLowerCase() === params.ProviderMessageTypeName.trim().toLowerCase()
      );
      ...
      if (result && result.Success) {
        ...
        const sendResult = await CommunicationEngine.Instance.SendMessages(
          params.ProviderName,
          params.ProviderMessageTypeName,
          params.Message,
          recipients,
          params.PreviewOnly
        );
  ...
  protected async PopulateRecipientContextData(
    entityInfo: EntityInfo,
    entityMessageType: EntityCommunicationMessageTypeExtended,
    message: Message,
    records: any[]
    ...
    const relatedData = await this.GetRelatedData(
      message,
      records,
      entityInfo.FirstPrimaryKey.Name,
      entityInfo.FirstPrimaryKey.NeedsQuotes
    );
  ...
  protected async PopulateSingleRecipientContextData(
    record: any,
    relatedData: { paramName: string; data: any[] }[],
    pkey: any,
    params: TemplateParamEntity[]
   ...
   * @param records
   */
  protected async GetRelatedData(
    message: Message,
    recipients: any[],
    recipientPrimaryKeyFieldName: string,
    recipientPrimaryKeyNeedsQuotes: boolean
  ): Promise<{ paramName: string; data: any[] }[]> {
