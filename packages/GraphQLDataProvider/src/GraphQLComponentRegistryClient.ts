...
export class GraphQLComponentRegistryClient {
   ...
   * ```
   */
  public async ResolveComponentDependencies(registryId: string, componentId: string): Promise<ComponentDependencyTree | null> {
    try {
      // Build the query
      const query = gql`
   ...
   * ```
   */
  public async GetLatestVersion(registryName: string, namespace: string, name: string): Promise<string | null> {
    try {
