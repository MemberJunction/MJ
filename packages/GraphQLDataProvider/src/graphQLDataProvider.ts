...
export class GraphQLProviderConfigData extends ProviderConfigDataBase {
   ...
   */
  get MJAPIKey(): string {
    return this.Data.MJAPIKey;
  }
  set MJAPIKey(key: string) {
    this.Data.MJAPIKey = key;
  }
  ...
  implements IEntityDataProvider, IMetadataProvider, IRunViewProvider, IRunReportProvider, IRunQueryProvider
{
  ...
  constructor() {
    super();
    if (!GraphQLDataProvider._instance) GraphQLDataProvider._instance = this;
  }
  ...
  protected getGraphQLTypeNameBase(entity: EntityInfo): string {
    ...
    const schemaPrefix =
      entity.SchemaName.trim().toLowerCase() === GraphQLDataProvider.MJ_CORE_SCHEMA.trim().toLowerCase()
        ? 'MJ'
   ...
   */
  public async Config(
    configData: GraphQLProviderConfigData,
    providerToUse?: IMetadataProvider,
    separateConnection?: boolean,
    forceRefreshSessionId?: boolean
  ): Promise<boolean> {
    try {
      if (separateConnection) {
        ...
        if (!GraphQLDataProvider.Instance._client)
          GraphQLDataProvider.Instance._client = this.CreateNewGraphQLClient(
            configData.URL,
            configData.Token,
            GraphQLDataProvider.Instance._sessionId,
            configData.MJAPIKey
          );
  ...
  public async RunView<T = any>(params: RunViewParams, contextUser?: UserInfo): Promise<RunViewResult<T>> {
    ...
    try {
      ...
      if (params) {
        ...
        if (params.MaxRows !== undefined) innerParams.MaxRows = params.MaxRows;
        if (params.StartRow !== undefined) innerParams.StartRow = params.StartRow; // Add StartRow parameter
        ...
        if (!dynamicView) {
          ...
          innerParams.ExcludeDataFromAllPriorViewRuns = params.ExcludeDataFromAllPriorViewRuns
            ? params.ExcludeDataFromAllPriorViewRuns
            : false;
  ...
  public async RunViews<T = any>(params: RunViewParams[], contextUser?: UserInfo): Promise<RunViewResult<T>[]> {
    ...
    try {
      ...
      for (const param of params) {
        ...
        if (param.MaxRows !== undefined) innerParam.MaxRows = param.MaxRows;
        if (param.StartRow !== undefined) innerParam.StartRow = param.StartRow; // Add StartRow parameter
  ...
  protected async getEntityNameAndUserView(
    params: RunViewParams,
    contextUser?: UserInfo
  ): Promise<{ entityName: string; v: UserViewEntityExtended }> {
    ...
    if (!params.EntityName) {
      if (params.ViewID) {
        ...
        entityName = v.Entity;
      } else throw new Error(`No EntityName, ViewID or ViewName passed to RunView`);
    } else entityName = params.EntityName;
  ...
  protected getViewRunTimeFieldList(e: EntityInfo, v: UserViewEntityExtended, params: RunViewParams, dynamicView: boolean): string[] {
    ...
    if (params.Fields) {
      for (const kv of e.PrimaryKeys) {
        if (params.Fields.find((f) => f.trim().toLowerCase() === kv.Name.toLowerCase()) === undefined) fieldList.push(kv.Name); // always include the primary key fields in view run time field list
      }
      ...
      if (dynamicView) {
        ...
        for (const kv of e.PrimaryKeys) {
          if (fieldList.find((f) => f.trim().toLowerCase() === kv.Name.toLowerCase()) === undefined) fieldList.push(kv.Name); // always include the primary key fields in view run time field list
        }

        // Now: include the fields that are part of the view definition
        v.Columns.forEach((c) => {
          if (c.hidden === false && !fieldList.find((item) => item.trim().toLowerCase() === c.EntityField?.Name.trim().toLowerCase())) {
            // don't include hidden fields and don't include the pkey field again
            if (!c.EntityField) {
            ...
            } else fieldList.push(mapper.MapFieldName(c.EntityField.CodeName));
          }
        });
      }
    }
    ...
    try {
      ...
      const newRequest = {
        ...
        SurvivingRecordCompositeKey: {
          KeyValuePairs: this.ensureKeyValuePairValueIsString(request.SurvivingRecordCompositeKey.KeyValuePairs),
        },
  ...
  public async Save(entity: BaseEntity, user: UserInfo, options: EntitySaveOptions): Promise<{}> {
    ...
    try {
      ...
      result.OriginalValues = entity.Fields.map((f) => {
        return { FieldName: f.CodeName, Value: f.Value };
      });
      ...
      for (let i = 0; i < filteredFields.length; i++) {
        ...
        if (val === null && f.EntityFieldInfo.AllowsNull === false) {
          ...
          if (f.EntityFieldInfo.DefaultValue !== null) {
            ...
            if (f.FieldType === EntityFieldTSType.Number || f.FieldType === EntityFieldTSType.Boolean) val = 0;
            else val = '';
          }
        }
      ...
      if (type.trim().toLowerCase() === 'update' && options.SkipOldValuesCheck === false) {
        const ov = [];
        entity.Fields.forEach((f) => {
          let val = null;
          if (f.OldValue !== null && f.OldValue !== undefined) {
            if (f.EntityFieldInfo.TSType === EntityFieldTSType.Date) val = f.OldValue.getTime().toString();
            else if (f.EntityFieldInfo.TSType === EntityFieldTSType.Boolean) val = f.OldValue === true ? '1' : '0';
            else if (typeof f.OldValue !== 'string') val = f.OldValue.toString();
            else val = f.OldValue;
          }
      ...
      if (entity.TransactionGroup) {
        ...
        } else throw new Error(`Save failed for ${entity.EntityInfo.ClassName}`);
      }
    } catch (e) {
      result.Success = false;
      result.EndedAt = new Date();
  ...
  public async Load(entity: BaseEntity, primaryKey: CompositeKey, EntityRelationshipsToLoad: string[] = null, user: UserInfo): Promise<{}> {
    try {
      ...
      for (let i = 0; i < primaryKey.KeyValuePairs.length; i++) {
        const field: EntityFieldInfo = entity.Fields.find(
          (f) => f.Name.trim().toLowerCase() === primaryKey.KeyValuePairs[i].FieldName.trim().toLowerCase()
        ).EntityFieldInfo;
        ...
        if (pkeyOuterParamString.length > 0) pkeyOuterParamString += ', ';
        pkeyOuterParamString += `$${field.CodeName}: ${pkeyGraphQLType}!`;

        // build up the param string for the inner query call
        if (pkeyInnerParamString.length > 0) pkeyInnerParamString += ', ';
        ...
        if (field.TSType === EntityFieldTSType.Number) {
          if (isNaN(primaryKey.GetValueByIndex(i))) throw new Error(`Primary Key value ${val} (${field.Name}) is not a valid number`);
          vars[field.CodeName] = parseInt(val); // converting to number here for graphql type to work properly
        } else vars[field.CodeName] = val;
      }

      const rel =
        EntityRelationshipsToLoad && EntityRelationshipsToLoad.length > 0
          ? this.getRelatedEntityString(entity.EntityInfo, EntityRelationshipsToLoad)
          : '';
      ...
      if (entity.TransactionGroup) {
        ...
        entity.TransactionGroup.AddTransaction(
          new TransactionItem(
            ...
            inner,
            vars,
            { mutationName: queryName, mutationInputTypes: mutationInputTypes },
            (results: any, success: boolean) => {
              // we get here whenever the transaction group does gets around to committing
              // our query.
        ...
        if (d && d[queryName]) {
          ...
          for (let key of entity.PrimaryKey.KeyValuePairs) {
            ...
            if (typeof returnedVal === 'number') returnedVal = returnedVal.toString();
            // now compare the two values
            if (originalVal !== returnedVal) {
              throw new Error(
                `Primary key value mismatch in server Delete response. Field: ${key.FieldName}, Original: ${originalVal}, Returned: ${returnedVal}`
              );
            }
          }
          result.Success = true;
        ...
        } else throw new Error(`Delete failed for ${entity.EntityInfo.Name}: ${entity.PrimaryKey.ToString()} `);
      }
    } catch (e) {
      result.EndedAt = new Date(); // done processing
      result.Success = false;
  ...
  public async GetRecordFavoriteStatus(userId: string, entityName: string, primaryKey: CompositeKey): Promise<boolean> {
    ...
    if (!valResult.IsValid) return false;

    const e = this.Entities.find((e) => e.Name === entityName);
    if (!e) throw new Error(`Entity ${entityName} not found in metadata`);
    ...
    if (data && data.GetRecordFavoriteStatus && data.GetRecordFavoriteStatus.Success) return data.GetRecordFavoriteStatus.IsFavorite;
  }
  ...
  public async SetRecordFavoriteStatus(
    ...
    if (data && data.SetRecordFavoriteStatus !== null) return data.SetRecordFavoriteStatus.Success;
  }
  ...
  public async GetEntityRecordName(entityName: string, primaryKey: CompositeKey): Promise<string> {
    ...
    if (data && data.GetEntityRecordName && data.GetEntityRecordName.Success) return data.GetEntityRecordName.RecordName;
  }
  ...
  public async GetEntityRecordNames(info: EntityRecordNameInput[]): Promise<EntityRecordNameResult[]> {
    ...
    if (data && data.GetEntityRecordNames) return data.GetEntityRecordNames;
  }
  ...
  public async ExecuteGQL(query: string, variables: any, refreshTokenIfNeeded: boolean = true): Promise<any> {
    try {
      ...
      if (e && e.response && e.response.errors?.length > 0) {
        //e.code === 'JWT_EXPIRED') {
  ...
  protected CreateNewGraphQLClient(url: string, token: string, sessionId: string, mjAPIKey: string): GraphQLClient {
    ...
    if (mjAPIKey) headers['x-mj-api-key'] = mjAPIKey;
  ...
  get LocalStorageProvider(): ILocalStorageProvider {
    if (!this._localStorageProvider) this._localStorageProvider = new BrowserIndexedDBStorageProvider();
  ...
  public PushStatusUpdates(sessionId: string = null): Observable<string> {
    ...
    if (existingRequest) return existingRequest.observable;
...
class BrowserLocalStorageProvider extends BrowserStorageProviderBase {
  public async getItem(key: string): Promise<string | null> {
    ...
    else return await super.GetItem(key);
  }
