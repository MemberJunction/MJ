...
import { CompositeKey, LogError } from '@memberjunction/global';
...
export class GraphQLActionClient {
   ...
   * ```
   */
  public async RunAction(actionID: string, params?: ActionParam[], skipActionLog: boolean = false): Promise<ActionResult> {
    try {
   ...
   * @private
   */
  private createActionVariables(actionID: string, params?: any[], skipActionLog: boolean = false): any {
    return {
      input: {
        ActionID: actionID,
        Params: params,
  ...
  private convertKeyValuePair(kvp: any): any {
    return {
      ...
      Value:
        kvp.Value !== null && kvp.Value !== undefined
          ? typeof kvp.Value === 'object'
            ? JSON.stringify(kvp.Value)
            : kvp.Value.toString()
          : null,
    };
  }
