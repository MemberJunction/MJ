...
export class GraphQLAIClient {
  ...
  public async RunAIAgent(
    ...
    try {
      ...
      if (params.onProgress) {
        subscription = this._dataProvider.PushStatusUpdates(this._dataProvider.sessionId).subscribe((message: string) => {
          try {
  ...
  public async EmbedText(params: EmbedTextParams): Promise<EmbedTextResult> {
    try {
      ...
      const textArray = Array.isArray(params.textToEmbed) ? params.textToEmbed : [params.textToEmbed];
      ...
      const returnEmbeddings = Array.isArray(params.textToEmbed) ? embedResult.embeddings : embedResult.embeddings[0];
