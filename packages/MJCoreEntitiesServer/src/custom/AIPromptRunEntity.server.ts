...
export class AIPromptRunEntityServer extends AIPromptRunEntityExtended {
  ...
  protected ShouldCalculateCost(): boolean {
    ...
    if (!this.TokensPrompt || !this.TokensCompletion || (this.TokensPrompt <= 0 && this.TokensCompletion <= 0)) {
      return false;
    }
  ...
  protected async CalculateAndSetCost(): Promise<void> {
    try {
      ...
      const priceUnitType = AIEngineBase.Instance.ModelPriceUnitTypes.find((put) => put.ID === activeCost.UnitTypeID);
      ...
      const normalizedCost = priceCalculator.CalculateNormalizedCost(activeCost, this.TokensPrompt, this.TokensCompletion);
  ...
  protected async TriggerParentCostRollup(): Promise<void> {
    try {
      ...
      const parent = await md.GetEntityObject<AIPromptRunEntityServer>('MJ: AI Prompt Runs', this.ContextCurrentUser);
