...
import { BaseEntity, EntityInfo, LogError, Metadata, IMetadataProvider } from '@memberjunction/global';
...
export class UserViewEntity_Server extends UserViewEntityExtended {
  ...
  protected async GetAIModel(): Promise<AIModelEntityExtended> {
    ...
    const models = AIEngine.Instance.Models.filter(
      (m) => m.AIModelType.trim().toLowerCase() === 'llm' && m.Vendor.trim().toLowerCase() === this.AIVendorName.trim().toLowerCase()
    );
      ...
      if (result && result.data) {
        ...
        if (llmResponse) {
          ...
          try {
            ...
            if (!cleansed) throw new Error('Invalid JSON response from AI: ' + llmResponse);
            ...
            if (parsed.whereClause && parsed.whereClause.length > 0) {
              ...
              let ret: string = '';
              if (trimmed.toLowerCase().startsWith('where ')) ret = trimmed.substring(6);
              else ret = parsed.whereClause;
        ...
        } else throw new Error('Null response from AI');
      } else throw new Error('No result returned from AI');
    } catch (e) {
      LogError(e);
      throw e;
    }
...
${
  ...
  fkeyBaseViewsDistinct
    .map((v) => {
      ...
      } else return ''; // already did this at some point
    })
    .join('\n')
}
${
...
`;
    ...
    return (
      gptSysPrompt +
      (processedViews.length > 1 /*we always have 1 from the entity base view*/ ? relationships : '') +
      `
