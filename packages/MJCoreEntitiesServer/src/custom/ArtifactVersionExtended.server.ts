import { BaseEntity, EntitySaveOptions, Metadata, RunView, LogError, IMetadataProvider } from '@memberjunction/global';
import {
...
export class ArtifactVersionExtended extends ArtifactVersionEntity {
  ...
  protected async SaveAttributeRecords(attributes: any[]): Promise<ArtifactVersionAttributeEntity[]> {
    ...
    for (const attrData of serialized) {
      ...
      if (existingAttr) {
        ...
        existingAttr.StandardProperty =
          (attrData.standardProperty as 'name' | 'description' | 'displayMarkdown' | 'displayHtml' | null) || null;
  ...
  protected async LoadArtifactTypeHierarchy(typeId: string): Promise<ArtifactTypeEntity[]> {
    ...
    while (currentTypeId && maxDepth > 0) {
      const artifactType = await md.GetEntityObject<ArtifactTypeEntity>('MJ: Artifact Types', this.ContextCurrentUser);
