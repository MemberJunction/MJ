import { BaseEntity, Metadata, RunView, IMetadataProvider } from '@memberjunction/global';
    ...
    try {
      ...
      if ((config && oldConfig) || (config && !this.IsSaved)) {
        ...
        if (this.IsSaved) {
          ...
          const dataContextChanged =
            Object.keys(castedConfig.dataContext || {})?.length !== Object.keys(castedOldConfig.dataContext || {})?.length;
          ...
          const executionResultsChanged =
            JSON.stringify(castedConfig.executionResults) !== JSON.stringify(castedOldConfig.executionResults);
