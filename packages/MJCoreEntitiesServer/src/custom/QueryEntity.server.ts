...
export class QueryEntityExtended extends QueryEntity {
  ...
  private async cleanupEmptyQueryAsync(): Promise<void> {
    try {
      await Promise.all([this.removeAllQueryParameters(), this.removeAllQueryFields(), this.removeAllQueryEntities()]);
  ...
  private async extractAndSyncData(): Promise<void> {
    try {
      ...
      const aiPrompt = AIEngine.Instance.Prompts.find((p) => p.Name === 'SQL Query Parameter Extraction' && p.Category === 'MJ: System');
      ...
      const syncPromises: Promise<void>[] = [this.syncQueryParameters(result.result.parameters)];
  ...
  private async syncQueryParameters(extractedParams: ExtractedParameter[]): Promise<void> {
    ...
    try {
      ...
      const paramsToAdd = extractedParams.filter((p) => !existingParams.some((ep) => ep.Name.toLowerCase() === p.name.toLowerCase()));

      const paramsToUpdate = existingParams.filter((ep) => extractedParams.some((p) => p.name.toLowerCase() === ep.Name.toLowerCase()));

      const paramsToRemove = existingParams.filter((ep) => !extractedParamNames.includes(ep.Name.toLowerCase()));
  ...
  private async syncQueryFields(extractedFields: ExtractedField[]): Promise<void> {
    ...
    try {
      ...
      const fieldsToAdd = extractedFields.filter((f) => !existingFields.some((ef) => ef.Name.toLowerCase() === f.name.toLowerCase()));

      const fieldsToUpdate = existingFields.filter((ef) => extractedFields.some((f) => f.name.toLowerCase() === ef.Name.toLowerCase()));

      const fieldsToRemove = existingFields.filter((ef) => !extractedFieldNames.includes(ef.Name.toLowerCase()));
  ...
  private async syncQueryEntities(extractedEntities: ExtractedEntity[]): Promise<void> {
    ...
    try {
      ...
      const entityMappings = extractedEntities
        .map((extracted) => {
          // Find matching entity in metadata
          const matchingEntity = md.Entities.find(
            (e) =>
      ...
      const entitiesToAdd = entityMappings.filter((mapping) => !existingEntities.some((ee) => ee.EntityID === mapping!.entityID));

      const entitiesToRemove = existingEntities.filter((ee) => !entityMappings.some((mapping) => mapping!.entityID === ee.EntityID));
