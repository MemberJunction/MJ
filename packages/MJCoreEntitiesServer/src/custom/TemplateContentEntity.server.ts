import { BaseEntity, EntitySaveOptions, LogError, Metadata, RunView, IMetadataProvider } from '@memberjunction/global';
...
export class TemplateContentEntityExtended extends TemplateContentEntity {
  ...
  private async extractAndSyncParameters(): Promise<void> {
    try {
      ...
      const aiPrompt = AIEngine.Instance.Prompts.find((p) => p.Name === 'Template Parameter Extraction' && p.Category === 'MJ: System');
  ...
  private async syncTemplateParameters(extractedParams: ExtractedParameter[]): Promise<void> {
    ...
    try {
      ...
      const isMultipleContents =
        templateContentsResult.Success && templateContentsResult.Results && templateContentsResult.Results.length > 1;
      ...
      const paramsToAdd = extractedParams.filter(
        (p) => !relevantExistingParams.some((ep) => ep.Name.toLowerCase() === p.name.toLowerCase())
      );
      ...
      const paramsToRemove = relevantExistingParams.filter((ep) => !extractedParamNames.includes(ep.Name.toLowerCase()));
