...
import { loadEntityConfig, loadSyncConfig, resolveSqlLoggingSchemaPlaceholders } from '../config';
      ...
      if (syncConfig?.sqlLogging?.enabled) {
        ...
        if (provider && typeof provider.CreateSqlLogger === 'function') {
          ...
          this.sqlLoggingSession = await provider.CreateSqlLogger(filepath, {
            ...
            // See MJ#3618 — an Open App binds ${flyway:defaultSchema} to its own schema, so the
            // core schema needs its own placeholder rather than the single-schema default.
            schemaPlaceholders: resolveSqlLoggingSchemaPlaceholders(syncConfig),
          });
