/**
 * 5.x LTS port of MJ `next`'s guard (#4199 / PR #4201, plus the pull wording added alongside
 * the #3415 pull fix).
 *
...
export function describeMissingEntitySubclass(entityName: string, options: MissingEntitySubclassOptions = {}): string | null {
  ...
  return (
    ...
    `is enabled and scoped to this process (Processes / ExcludeProcesses), and that this run was not started ` +
    `with MJ_DYNAMIC_PACKAGES=none. Run with --verbose to see which packages were loaded.`
  );
}
