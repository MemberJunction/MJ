...
class RSUConfig {
  get MigrationsPath(): string {
    return process.env.RSU_MIGRATIONS_PATH || 'migrations/rsu';
  }
  get AdditionalSchemaInfoPath(): string {
    return process.env.RSU_ADDITIONAL_SCHEMA_INFO_PATH || 'additionalSchemaInfo.json';
  }
  get DefaultSchema(): string {
    return process.env.RSU_DEFAULT_SCHEMA || '__mj';
  }
  get WorkDir(): string {
    return process.env.RSU_WORK_DIR || process.cwd();
  }
  /** Directory where CodeGen runs (needs node_modules). Defaults to WorkDir. */
  get CodeGenDir(): string {
    return process.env.RSU_CODEGEN_DIR || this.WorkDir;
  }
  get CodeGenTimeoutMs(): number {
    return parseInt(process.env.RSU_CODEGEN_TIMEOUT_MS || '600000', 10);
  }
  get CodeGenCommand(): string | undefined {
    return process.env.RSU_CODEGEN_COMMAND;
  }
  get CompileTimeoutMs(): number {
    return parseInt(process.env.RSU_COMPILE_TIMEOUT_MS || '300000', 10);
  }
  ...
  get GitTargetBranch(): string {
    return process.env.RSU_GIT_TARGET_BRANCH || 'next';
  }
  get GitRemote(): string {
    return process.env.RSU_GIT_REMOTE || 'origin';
  }
  get GitUserName(): string | undefined {
    return process.env.RSU_GIT_USER_NAME;
  }
  get GitUserEmail(): string | undefined {
    return process.env.RSU_GIT_USER_EMAIL;
  }
  /** GitHub repo in "owner/repo" format for Octokit-only mode (no local .git). */
  get GitHubRepo(): string | undefined {
    return process.env.RSU_GITHUB_REPO;
  }
...
export class RuntimeSchemaManager extends BaseSingleton<RuntimeSchemaManager> {
      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        if (!execStep) {
          itemResults.push({
            ...
            FilePath: filePath,
            Success: false,
            Error: itemSteps.find((s) => s.Status === 'failed')?.Message,
            Steps: itemSteps,
          });
          continue;
        }
  ...
  private async runPostMigrationPipeline(
    ...
    if (!inputs.every((i) => i.SkipGitCommit)) {
      ...
      const description = successfulItems.length === 1 ? successfulItems[0].Input.Description : `Batch of ${successfulItems.length} schema changes`;
  ...
  private recordMetrics(item: MigrationItemResult, allSteps: RSUPipelineStep[], result: RSUPipelineResult): void {
    try {
      ...
      LogError(`[RSU] Metrics recording failed: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  private async validateEnvironment(): Promise<boolean> {
    if (!this.IsEnabled) {
      throw new RSUError('DISABLED', 'Runtime Schema Update is disabled. Set ALLOW_RUNTIME_SCHEMA_UPDATE=1 to enable.');
    }
    await this.validateFilesystemWritable();
    return true;
  }
    ...
    for (const dir of uniqueDirs) {
      if (!existsSync(dir)) {
        try {
          ...
          throw new RSUError('FILESYSTEM', `Cannot create directory ${dir}: ${(e as Error).message}. Ensure the filesystem is writable.`);
        }
      }
      const probe = nodePath.join(dir, `.rsu_write_probe_${Date.now()}`);
      try {
        writeFileSync(probe, '');
        unlinkSync(probe);
  ...
  private async acquireLock(): Promise<boolean> {
    ...
    if (this.IsDBLockEnabled) {
      ...
      if (!acquired) {
        ...
        throw new RSUError('CONCURRENT', 'Another RSU pipeline is running on a different instance. Wait for it to complete.');
      }
    }
  ...
  private async writeMigrationFile(input: RSUPipelineInput): Promise<string> {
    ...
    const tableSlug = input.AffectedTables.slice(0, 3)
  ...
  private async writeAdditionalSchemaInfo(inputs: RSUPipelineInput[]): Promise<boolean> {
    ...
    if (existsSync(configFilePath)) {
      try {
        existing = JSON.parse(readFileSync(configFilePath, 'utf-8'));
      } catch {
        /* start fresh */
      }
    }
  ...
  private async waitForMJAPI(): Promise<boolean> {
    ...
    throw new RSUError('RESTART_TIMEOUT', `MJAPI did not become healthy within ${maxWaitMs / 1000}s on port ${port}`);
  }
   ...
   */
  private async gitCommitAndPRForCycle(migrationFilePaths: string[], affectedTables: string[], description: string): Promise<string> {
  ...
  private async apiCreateCommit(octokit: Octokit, owner: string, repo: string, message: string, treeSha: string, parentSha: string): Promise<string> {
  ...
  private async apiUpdateBranchRef(octokit: Octokit, owner: string, repo: string, branch: string, sha: string): Promise<void> {
  ...
  private async collectAllRSUFiles(migrationFilePaths: string[]): Promise<Array<{ repoPath: string; absolutePath: string }>> {
