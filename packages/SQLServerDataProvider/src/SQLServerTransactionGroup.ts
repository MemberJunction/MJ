...
export class SQLServerTransactionGroup extends TransactionGroupBase {
    ...
    private async executeBatchedNoVars(
        ...
        returnResults: TransactionResult[]
                    ...
                    //
                    // The sequential path below is ATOMIC but not BATCHED: each item's generated
                    // CRUD procedure call is its own round trip inside the transaction, so a
                    // 100-item group costs 100 wire hops whose wall time is dominated by
                    // per-statement overhead, not SQL. Measured on a live sync: ~0.3ms of server
                    // execution inside a wall cost two orders of magnitude larger per statement.
                    //
                    // Here the items travel together as one multi-statement batch. Everything
                    // else is unchanged: the same statements, the same order, the same single
                    // transaction, per-item results still mapped back to their items.
                    //
                    // Result mapping cannot assume one recordset per item — a statement that
                    // returns no rows produces NO recordset, so a positional zip drifts. Each
                    // item is therefore preceded by a sentinel SELECT of its index; recordsets
                    // between sentinel k and sentinel k+1 belong to item k.
                    //
                    // Parameters: one request carries ONE parameter namespace, so per-item `?`
                    // placeholders are renumbered into a single global @p sequence.
                    ...
                    try {
                        ...
                        const sets: any[] = (queryResult as any).recordsets ?? [queryResult.recordset ?? []];
                        const perItem: any[] = new Array(items.length).fill(undefined);
                        let current = -1;
                        ...
                        // One statement to the server, so a failure fails all of it — the same
                        // outcome the sequential path produces, which also rolls back on the
                        // first error. Per-item attribution for the failing row is the caller's
                        // degradation path (re-apply individually), exactly as before.
                        try {
                            await transaction.rollback();
                        } catch (rollbackError) {
