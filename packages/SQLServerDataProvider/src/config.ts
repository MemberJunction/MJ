import { BaseEntity, Metadata, RunView, LogError, LogStatus, RunReport, RunQuery, SetProvider } from '@memberjunction/global';
...
export async function setupSQLServerClient(config: SQLServerProviderConfigData): Promise<SQLServerDataProvider> {
  try {
    ...
    if (pool.connected) {
      ...
      else throw new Error('Failed to connect to database'); // don't do recursive call here as it would go on forever
    }
  } catch (e) {
