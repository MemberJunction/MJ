...
import { EntityPermissionType, UserInfo, Metadata } from '@memberjunction/global';
...
export class ComponentSpecRuntime extends ComponentSpec {
  ...
  private convertToEntityPermissionType(permission: ComponentEntitySimplePermission): EntityPermissionType {
    switch (permission) {
      ...
      default:
        return EntityPermissionType.Read; // Safe default
    }
  }
   ...
   */
  public async ValidateUserPermissions(user: UserInfo): Promise<{
    canRender: boolean;
  ...
  public GetDataAccessSummary(): {
    ...
    const entitySummary =
      this.dataRequirements?.entities.map((e) => ({
        name: e.name,
        permissions: e.permissionLevelNeeded,
      })) || [];

    const querySummary =
      this.dataRequirements?.queries.map((q) => ({
        name: q.name,
  ...
  public DependsOnLibrary(libraryNameOrGlobal: string): boolean {
    return this.libraries?.some((lib) => lib.name === libraryNameOrGlobal || lib.globalVariable === libraryNameOrGlobal) || false;
  }
