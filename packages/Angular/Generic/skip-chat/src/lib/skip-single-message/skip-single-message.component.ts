...
import { IMetadataProvider, LogError, Metadata, RunView, RunViewParams, UserInfo } from '@memberjunction/global';
...
export class SkipSingleMessageComponent extends BaseAngularComponent implements AfterViewInit, OnDestroy {
  ...
  private GetHtmlFromCache(detail: ConversationDetailEntity): string | null {
    if (
      ...
      detail.ID !== undefined &&
      detail.ID.length > 0 &&
      SkipSingleMessageComponent._detailHtml[detail.ID] !== undefined &&
      SkipSingleMessageComponent._detailHtml[detail.ID] !== null
    ) {
  ...
  private CacheHtml(detail: ConversationDetailEntity, html: string) {
    ...
    if (detail.ID !== null && detail.ID !== undefined && detail.ID.length > 0) SkipSingleMessageComponent._detailHtml[detail.ID] = html;
  }
  ...
  public get Message(): string {
    if (this._cachedMessage === null) {
      if (
        this.ConversationDetailRecord.ID &&
        this.ConversationDetailRecord.ID.length > 0 &&
        this.ConversationDetailRecord.Role &&
  ...
  public GetMessageRowCssClass(): string {
    if (this.ConversationDetailRecord.Role.trim().toLowerCase() === 'ai') {
      if (this.ConversationDetailRecord.ID.length > 0) return 'ai-message';
      else return 'ai-message-in-progress';
  ...
  public get IsTemporaryMessage(): boolean {
    if (this.ConversationDetailRecord?.Role?.trim().toLowerCase() === 'ai') {
      if (this.ConversationDetailRecord.ID?.length > 0) return false;
      else return true;
    } else return false;
  }
  ...
  public get ShouldShowCompletionTime(): boolean {
    return this.IsAIMessage && !this.IsTemporaryMessage && !!this.ConversationDetailRecord.CompletionTime;
  }
