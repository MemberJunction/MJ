...
export class SkipArtifactViewerComponent extends BaseAngularComponent implements OnInit, OnChanges {
  ...
  private async loadArtifact(): Promise<void> {
    ...
    try {
      ...
      // Load the artifact
      const artifactEntity = await provider.GetEntityObject('ConversationArtifact', provider.CurrentUser);
      ...
      const artifactTypeEntity = await provider.GetEntityObject('ArtifactType', provider.CurrentUser);
      ...
      const versions = await provider.ExecuteQuery({
      ...
      this.artifactVersion = Array.isArray(versions) ? versions[0] : versions;
      
      // Process content based on content type
      this.processContent();
    } catch (err) {
      LogError('Error loading artifact', err);
  ...
  private processContent(): void {
    ...
    try {
      ...
      LogError('Error processing artifact content', err);
