...
import { CompositeKey, KeyValuePair, LogError, Metadata } from '@memberjunction/global';
...
export class SkipDynamicUIComponentComponent implements AfterViewInit, OnDestroy {
  ...
  public get selectedReportOption(): ComponentOption | null {
    return this.componentOptions.length > this.selectedReportOptionIndex ? this.componentOptions[this.selectedReportOptionIndex] : null;
  }
  ...
  public getTabTitle(index: number): string {
    ...
    if (option.AIRankExplanation && option.name) return option.name + ' (' + option.AIRankExplanation + ')';
  ...
  private updateCurrentReport(): void {
    ...
    try {
      ...
      this.currentError = {
        ...
        message:
          'Failed to assemble the component code. This usually happens when sub-components are missing or placeholders cannot be resolved.',
  ...
  private handleOpenEntityRecord(entityName: string, key: CompositeKey): void {
    if (entityName) {
      ...
      if (!entityMatch) {
        ...
        const altMatch = md.Entities.filter(
          (e) =>
            e.BaseTable.toLowerCase() === entityName.toLowerCase() ||
