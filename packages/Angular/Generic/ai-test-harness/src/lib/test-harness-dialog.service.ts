...
import { AITestHarnessDialogComponent, AITestHarnessDialogData } from './ai-test-harness-dialog.component';
...
export class TestHarnessDialogService {
    ...
    openAgentTestHarness(options: {
        ...
        if (dialogRef.content && dialogRef.content.instance) {
            ...
            console.error('Dialog content or instance is null!');
        }
     ...
     * Uses the same test harness component but configures it for prompt mode.
     * 
     * @param options - Configuration options for the dialog
     ...
     * @param options.width - Dialog width (CSS units or viewport percentage, defaults to '90vw')
     * @param options.height - Dialog height (CSS units or viewport percentage, defaults to '90vh')
     * @param options.initialTemplateVariables - Initial template variables for prompt execution
     * @param options.selectedModelId - Pre-selected AI model ID
     * @param options.preventClose - Whether to prevent dialog closure (currently unused)
     * @returns Reference to the opened dialog for further control
     */
    openPromptTestHarness(options: {
        ...
        const data: AITestHarnessDialogData = {
            ...
            mode: 'prompt' // Explicitly set mode to prompt
        };
