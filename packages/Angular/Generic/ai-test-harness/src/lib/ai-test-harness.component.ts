...
export class AITestHarnessComponent implements OnInit, OnDestroy, OnChanges, AfterViewChecked {
  ...
  public activeTab: 'agentVariables' | 'executionMonitor' | 'agentSettings' | 'templateVariables' | 'modelSettings' | 'savedConversations' =
    'agentVariables';
  ...
  private async loadAvailableModels() {
    ...
    if (this.entity && 'AIModelTypeID' in this.entity) {
      ...
      if (prompt.AIModelTypeID) {
        filteredModels = AIEngineBase.Instance.Models.filter((model) => model.AIModelTypeID === prompt.AIModelTypeID && model.IsActive);
      } else {
    ...
    try {
      ...
      if (!defaultModel) {
        const candidates = AIEngineBase.Instance.Models.filter(
          (m) => m.IsActive && (!prompt.AIModelTypeID || m.AIModelTypeID === prompt.AIModelTypeID)
        );
  ...
  private async loadDefaultVendor() {
    try {
      ...
      const modelVendors = AIEngineBase.Instance.ModelVendors.filter(
        (mv) => mv.ModelID === this.defaultModel.ID && mv.Status === 'Active' && mv.Type?.trim().toLowerCase() === 'inference provider'
      );
  ...
  private loadVendorsForModel() {
    ...
    const modelVendors = AIEngineBase.Instance.ModelVendors.filter(
      (mv) => mv.ModelID === this.selectedModelId && mv.Status === 'Active' && mv.Type?.trim().toLowerCase() === 'inference provider'
    );
  ...
  private loadPromptDefaults() {
    if (this.mode === 'prompt' && this.entity && this.isPromptEntity(this.entity)) {
      ...
      if (prompt.StopSequences) {
        ...
        this.advancedParams.stopSequences = prompt.StopSequences.split(',')
          .map((s: string) => s.trim())
   ...
   */
  public selectTab(
    tab: 'agentVariables' | 'executionMonitor' | 'agentSettings' | 'templateVariables' | 'modelSettings' | 'savedConversations'
  ) {
    ...
    if (tab === 'executionMonitor') {
      ...
      if (this.conversationMessages.length > 0) {
        const lastAssistantMessage = this.conversationMessages.filter((m) => m.role === 'assistant' && m.agentRunId).pop();
  ...
  private async executeAgent(userMessage: string) {
    ...
    try {
      ...
      MJNotificationService.Instance.CreateSimpleNotification('Failed to execute agent: ' + (error as Error).message, 'error', 6000);
    } finally {
      this.isExecuting = false;
      if (this.elapsedTimeInterval) {
        clearInterval(this.elapsedTimeInterval);
  ...
  private async executePrompt(userMessage: string) {
    ...
    try {
      ...
      MJNotificationService.Instance.CreateSimpleNotification('Failed to execute prompt: ' + (error as Error).message, 'error', 6000);
    } finally {
      this.isExecuting = false;
      if (this.elapsedTimeInterval) {
        clearInterval(this.elapsedTimeInterval);
  ...
  public saveConversation() {
    if (this.conversationMessages.length === 0) {
      MJNotificationService.Instance.CreateSimpleNotification('No messages to save', 'warning', 3000);
      return;
    }
  ...
  public confirmSaveConversation() {
    ...
    MJNotificationService.Instance.CreateSimpleNotification('Conversation saved successfully', 'success', 3000);
  ...
  public async deleteConversation(conversation: SavedConversation, event: Event) {
    ...
    if (confirm(`Delete conversation "${conversation.name}"?`)) {
      ...
      if (index >= 0) {
        ...
        MJNotificationService.Instance.CreateSimpleNotification('Conversation deleted', 'info', 3000);
      }
    }
  }
  ...
  public exportConversation() {
    if (this.conversationMessages.length === 0) {
      MJNotificationService.Instance.CreateSimpleNotification('No messages to export', 'warning', 3000);
      return;
    }
      ...
      reader.onload = (e) => {
        try {
          ...
          if (data.messages && Array.isArray(data.messages)) {
            ...
            MJNotificationService.Instance.CreateSimpleNotification('Conversation imported successfully', 'success', 3000);
          } else {
  ...
  public async copyMessage(message: ConversationMessage) {
    try {
      ...
      MJNotificationService.Instance.CreateSimpleNotification('Message copied to clipboard', 'success', 2000);
    } catch (error) {
      MJNotificationService.Instance.CreateSimpleNotification('Failed to copy message', 'error', 3000);
    }
  }
  ...
  public copyJsonContent() {
    if (this.currentJsonContent) {
      navigator.clipboard
        .writeText(this.currentJsonContent)
        .then(() => {
          // Success - JSON copied
        })
        .catch((err) => {
  ...
  private extractHumanReadableContent(jsonStr: string): string | null {
    try {
      ...
      if (
        parsed.nextStep &&
        parsed.nextStep.userMessage &&
        typeof parsed.nextStep.userMessage === 'string' &&
        parsed.nextStep.userMessage.trim()
      ) {
        return parsed.nextStep.userMessage;
      }

      // Priority 3: Other common human-readable fields
      const contentFields = ['message', 'content', 'response', 'text', 'output', 'result', 'answer', 'reply', 'description', 'summary'];
  ...
  private createJsonDisplayHtml(obj: any, depth: number = 0): string {
    ...
    if (Array.isArray(obj)) {
      ...
      const items = obj
        .map((item) => `<div class="json-array-item">${nextIndent}${this.createJsonDisplayHtml(item, depth + 1)}</div>`)
        .join(',\n');
