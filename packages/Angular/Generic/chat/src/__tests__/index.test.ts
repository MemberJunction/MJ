...
import { ChatModule, ChatComponent, ChatWelcomeQuestion } from '../public-api';
...
describe('@memberjunction/ng-chat', () => {
  ...
  it('exports its load-bearing classes as constructors', () => {
    expect(ChatModule).toBeTypeOf('function');
    expect(ChatComponent).toBeTypeOf('function');
    expect(ChatWelcomeQuestion).toBeTypeOf('function');
  });
});
