...
import { SharedGenericModule, LoadingComponent, RecentAccessService } from '../public-api';
...
describe('@memberjunction/ng-shared-generic', () => {
  ...
  it('exports its load-bearing classes as constructors', () => {
    expect(SharedGenericModule).toBeTypeOf('function');
    expect(LoadingComponent).toBeTypeOf('function');
    expect(RecentAccessService).toBeTypeOf('function');
  });
});
