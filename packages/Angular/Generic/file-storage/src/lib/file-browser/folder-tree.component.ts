 ...
 * Loads actual folder structure from storage providers via GraphQL.
 */
@Component({
...
export class FolderTreeComponent {
   ...
   */
  private _provider: FileStorageProviderEntity | null = null;

  @Input()
  set provider(value: FileStorageProviderEntity | null) {
    const previousProvider = this._provider;
    this._provider = value;

    if (value && value.ID !== previousProvider?.ID) {
      this.resetNavigation();
      this.loadFolders();
    }
  }
  get provider(): FileStorageProviderEntity | null {
  ...
  /**
   * Loads folders from the storage provider for the current path
   */
