...
export class JoinGridComponent implements AfterViewInit {
  ...
  public async CancelEdit() {
    ...
    this._GridData.forEach((row) => {
      row.ColumnData.forEach((cell) => {
        ...
        if (cell.data) {
          const match = this._pendingInserts.find(
            (obj) =>
              obj.Get(this.JoinEntityColumnForeignKey) === cell.ColumnForeignKeyValue &&
              obj.Get(this.JoinEntityRowForeignKey) === row.RowForeignKeyValue
          );
          if (match) {
          ...
          const match = this._pendingDeletes.find(
            (obj) =>
              obj.Get(this.JoinEntityColumnForeignKey) === cell.ColumnForeignKeyValue &&
              obj.Get(this.JoinEntityRowForeignKey) === row.RowForeignKeyValue
          );
  ...
  public IsCellReallyDirty(cell: JoinGridCell): boolean {
    if (cell.data) {
      ...
      if (cell.data.Dirty) return true;
    } else {
      ...
      const record = this._pendingDeletes.find(
        (obj) =>
          obj.Get(this.JoinEntityColumnForeignKey) === cell.ColumnForeignKeyValue &&
          obj.Get(this.JoinEntityRowForeignKey) === cell.RowForeignKeyValue
      );
      if (record) return true; // found a pending delete, we're dirty
    }
    return false;
  }
  ...
  public async Refresh() {
    ...
    if (this.ColumnsMode === 'Entity') {
      ...
      if (!this._columnsEntityInfo) throw new Error('Invalid entity name provided for columns entity.');
    }
    this._rowsEntityInfo = md.EntityByName(this.RowsEntityName);
    if (!this._rowsEntityInfo) throw new Error('Invalid entity name provided for rows entity.');
  ...
  protected async PopulateRowsAndColsData() {
    ...
    if (this.ColumnsMode === 'Entity') {
      ...
      if (this.ColumnsEntityDataSource === 'Array') {
        ...
        this._columnsEntityData = await this.RunColumnsOrRowsView(
          this.ColumnsEntityDataSource,
          this.ColumnsEntityName,
          this.ColumnsEntityViewName,
          this.ColumnsExtraFilter,
          this.ColumnsOrderBy
        );
      }
    }
    if (this.RowsEntityDataSource === 'Array') {
      ...
      this._rowsEntityData = await this.RunColumnsOrRowsView(
        this.RowsEntityDataSource,
        this.RowsEntityName,
        this.RowsEntityViewName,
        this.RowsExtraFilter,
        this.RowsOrderBy
      );
    }
  }

  protected async RunColumnsOrRowsView(
    ...
    if (extraFilter) params.ExtraFilter = extraFilter;
  ...
  protected async PopulateGridData() {
    ...
    if (!this._joinEntityData) throw new Error('_joinEntityData must be populated before calling PopulateGridData()');
    ...
    this._rowsEntityData!.forEach((row, rowIndex) => {
      ...
      if (this.ColumnsMode === 'Entity') {
        for (let i = 0; i < this._columnsEntityData!.length; i++) {
          ...
          const join = this._joinEntityData!.find(
            (j) =>
              j.Get(this.JoinEntityRowForeignKey) === row.Get(this._rowsEntityInfo!.FirstPrimaryKey.Name) &&
        ...
        if (!this.JoinEntityDisplayColumns) throw new Error('JoinEntityDisplayColumns is required when ColumnsMode is set to Fields');
  ...
  public async _FlipRecord(event: MouseEvent, row: JoinGridRow, cell: JoinGridCell, stopPropagation: boolean = false) {
    ...
    if (!cell) throw new Error('cell is a required parameter');
    ...
    if (cell.data) {
      if (cell.data.IsSaved) {
        ...
        if (index >= 0) this._pendingInserts.splice(index, 1);
      }
      ...
      let record = this._pendingDeletes.find(
        (obj) =>
          obj.Get(this.JoinEntityColumnForeignKey) === cell.ColumnForeignKeyValue &&
          obj.Get(this.JoinEntityRowForeignKey) === row.RowForeignKeyValue
      );
  ...
  public async UpdateCellValueDirect(row: JoinGridRow, colIndex: number, newValue: string) {
    ...
    if (this.ColumnsMode !== 'Fields') throw new Error('This method should only be called when ColumnsMode=Entity');
    ...
    if (!joinData) LogError('Could not find join data for rowForeignKey ' + row.RowForeignKeyValue);
    else {
  ...
  public GetJoinEntityField(colName: string): EntityFieldInfo {
    ...
    if (!entity) throw new Error('Invalid entity name provided for JoinEntity');
    const field = entity.Fields.find((f) => f.Name === colName);
    if (!field) throw new Error('Invalid field name provided for JoinEntity');
    return field;
  }
