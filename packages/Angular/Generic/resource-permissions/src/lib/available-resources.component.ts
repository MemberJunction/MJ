...
import { EntityFieldInfo, RunView, UserInfo } from '@memberjunction/global';
...
export class AvailableResourcesComponent extends BaseAngularComponent implements AfterViewInit {
  ...
  public async Refresh() {
    ...
    if (this.resourcePermissions.length === 0) {
      ...
      if (!rt || !rt.EntityID) throw new Error(`Resource Type ${this.ResourceTypeID} not found`);

      const p = this.ProviderToUse;
      const entity = p.Entities.find((e) => e.ID === rt.EntityID);
      if (!entity || !entity.NameField) throw new Error(`Entity ${rt.EntityID} not found, or no Name field defined`);
      ...
      if (this.ExtraColumns && this.ExtraColumns.length > 0) {
        ...
        extraColumns.forEach((ec) => {
          ...
          if (field) this.gridExtraColumns.push(field);
        });
      }
      ...
      if (!result || !result.Success) throw new Error(`Error running view for entity ${entity.Name}`);
