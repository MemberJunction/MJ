...
export class RequestResourceAccessComponent extends BaseAngularComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    const rt = ResourcePermissionEngine.Instance.ResourceTypes.find(
      (x) => x.Name.trim().toLowerCase() === this.ResourceType.trim().toLowerCase()
    );
    if (rt) this.ResourceTypeObject = rt;
    else throw new Error(`Resource Type ${this.ResourceType} not found`);
  }
  ...
  public async requestAccess() {
    ...
    if (await permission.Save()) {
      ...
      MJNotificationService.Instance.CreateSimpleNotification(
        `Failed to request access to ${this.ResourceType} record ${this.ResourceName} (${this.ResourceRecordID})`,
        'error',
        5000
      );
    }
