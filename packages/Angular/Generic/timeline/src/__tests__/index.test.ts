...
import { TimelineModule, TimelineComponent, TimelineGroup } from '../public-api';
...
describe('@memberjunction/ng-timeline', () => {
  ...
  it('exports its load-bearing classes as constructors', () => {
    expect(TimelineModule).toBeTypeOf('function');
    expect(TimelineComponent).toBeTypeOf('function');
    expect(TimelineGroup).toBeTypeOf('function');
  });
});
