...
export class DashboardViewerComponent extends BaseAngularComponent implements OnDestroy {
   ...
   */
  public async addPanel(partTypeId: string, panelConfig: PanelConfig, title: string, icon?: string, location?: LayoutLocation): Promise<void> {
   ...
   */
  private updatePanelInLayout(layout: ResolvedLayoutConfig, panelId: string, newConfig: PanelConfig, title?: string, icon?: string): void {
  ...
  private subscribeToLayoutEvents(): void {
    ...
    this._glService.onLayoutChanged.pipe(takeUntil(this._destroy$)).subscribe((event: LayoutChangedEvent) => {
      this.onLayoutChanged(event);
    });

    this._glService.onPanelClosed.pipe(takeUntil(this._destroy$)).subscribe((panelId: string) => {
      this.onPanelClosed(panelId);
    });

    this._glService.onPanelSelected.pipe(takeUntil(this._destroy$)).subscribe((panelId: string) => {
  ...
  private onPanelSelected(_panelId: string): void {
  ...
  private async createDynamicPartComponent(
    ...
    try {
      ...
      const partInstance = await MJGlobal.Instance.ClassFactory.CreateInstanceAsync<BaseDashboardPart>(BaseDashboardPart, partType.DriverClass);
