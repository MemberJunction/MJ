...
describe('WorkflowPlanCardComponent (DOM)', () => {
    const spec: TaskGraphSpec = {
        ...
        tasks: [
            { tempId: 'a', name: 'Gather', description: 'g', kind: 'Agent', configuration: { agentName: 'Sage' }, dependsOn: [] },
            { tempId: 'b', name: 'Summarize', description: 's', kind: 'Agent', configuration: { agentName: 'Sage' }, dependsOn: ['a'] },
        ],
    };
    ...
    describe('④ name it inline, then offer the editor', () => {
        const spec: TaskGraphSpec = {
            workflowName: 'Quarterly review',
            tasks: [{ tempId: 'a', name: 'Pull numbers', description: 'd', kind: 'Agent', configuration: { agentName: 'Sage' }, dependsOn: [] }],
        };
