...
export class CollectionPermissionService {
   ...
   */
  async checkPermission(collectionId: string, userId: string, currentUser: UserInfo): Promise<CollectionPermission | null> {
    const rv = new RunView();
    const result = await rv.RunView<CollectionPermissionEntity>(
      {
        EntityName: 'MJ: Collection Permissions',
   ...
   */
  async checkBulkPermissions(collectionIds: string[], userId: string, currentUser: UserInfo): Promise<Map<string, CollectionPermission>> {
  ...
  async grantPermission(
    ...
    const permission = await md.GetEntityObject<CollectionPermissionEntity>('MJ: Collection Permissions', currentUser);
   ...
   */
  async updatePermission(permissionId: string, permissions: PermissionSet, currentUser: UserInfo): Promise<boolean> {
    const md = new Metadata();
    const permission = await md.GetEntityObject<CollectionPermissionEntity>('MJ: Collection Permissions', currentUser);
   ...
   */
  async updatePermissionCascade(collectionId: string, userId: string, permissions: PermissionSet, currentUser: UserInfo): Promise<void> {
   ...
   */
  async revokePermissionCascade(collectionId: string, userId: string, currentUser: UserInfo): Promise<void> {
   ...
   */
  private async revokeChildPermissions(parentCollectionId: string, userId: string, currentUser: UserInfo): Promise<void> {
    const rv = new RunView();
    const childrenResult = await rv.RunView(
      {
        EntityName: 'MJ: Collections',
   ...
   */
  async copyParentPermissions(parentCollectionId: string, childCollectionId: string, currentUser: UserInfo): Promise<void> {
   ...
   */
  async createOwnerPermission(collectionId: string, ownerId: string, currentUser: UserInfo): Promise<void> {
    await this.grantPermission(
      collectionId,
      ownerId,
      {
