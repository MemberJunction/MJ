...
export class ArtifactPermissionService {
  constructor(private collectionPermissionService: CollectionPermissionService) {}
   ...
   */
  async getExplicitPermission(artifactId: string, userId: string, currentUser: UserInfo): Promise<ArtifactPermission | null> {
    const rv = new RunView();
    const result = await rv.RunView<ArtifactPermissionEntity>(
      {
        EntityName: 'MJ: Artifact Permissions',
  ...
  async grantPermission(
    ...
    const permission = await md.GetEntityObject<ArtifactPermissionEntity>('MJ: Artifact Permissions', currentUser);
   ...
   */
  async updatePermission(permissionId: string, permissions: ArtifactPermissionSet, currentUser: UserInfo): Promise<boolean> {
    const md = new Metadata();
    const permission = await md.GetEntityObject<ArtifactPermissionEntity>('MJ: Artifact Permissions', currentUser);
   ...
   */
  private async getArtifactCollections(artifactId: string, currentUser: UserInfo): Promise<CollectionArtifactEntity[]> {
    const rv = new RunView();
