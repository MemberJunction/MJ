...
import { EntityInfo, RunView, RunViewParams } from '@memberjunction/global';
...
export class EntityCommunicationsPreviewComponent implements OnInit {
  ...
  async ngOnInit() {
    if (!this.entityInfo || !this.runViewParams) throw new Error('EntityInfo and RunViewParams are required');
  ...
  protected async loadMessagePreviews() {
    ...
    if (!sendGrid) throw new Error('SendGrid provider not found');

    const email = sendGrid.MessageTypes.find((mt) => mt.Name === 'Email');
    if (!email) throw new Error('Email message type not found');

    if (!this.selectedTemplate) throw new Error('No template selected');
