...
export class UserRolesGridComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes && (changes.RoleRecord || changes.UserRecord)) this.Refresh();
  }
  async Refresh() {
    if (this.Mode === 'Roles')
    ...
    if (this.Mode === 'Users')
      if (!this.UserID || !this.UserRecord) throw new Error("UserID and UserRecord are required when Mode is 'Users'");
  ...
  public async saveUserRoles() {
    ...
    for (const ur of this.userRoles) {
      if (this.IsReallyDirty(ur)) {
        ...
        if (ur.Selected) await ur.Save();
        else await ur.Delete();
      }
    }
  ...
  public flipState($event: MouseEvent | undefined, ur: UserRoleEntity_Ext, flipState: boolean) {
    if (flipState) ur.Selected = !ur.Selected;
