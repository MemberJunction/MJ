...
} from './application-dialog/application-dialog.component';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, SharedSettingsModule, ApplicationDialogComponent],
...
export class ApplicationManagementComponent implements OnInit, OnDestroy {
  ...
  public async loadInitialData(): Promise<void> {
    try {
      ...
      const [apps, appEntities] = await Promise.all([this.loadApplications(), this.loadApplicationEntities()]);
  ...
  private applyFilters(): void {
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter(
        (app) => app.Name?.toLowerCase().includes(searchLower) || app.Description?.toLowerCase().includes(searchLower)
      );
    }
