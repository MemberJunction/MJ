...
import { EntityPermissionEntity, EntityEntity, RoleEntity, UserEntity } from '@memberjunction/core-entities';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, SharedSettingsModule, PermissionDialogComponent],
  ...
  private applyFilters(): void {
    ...
    if (filters.entitySearch) {
      ...
      filtered = filtered.filter(
        (ea) => ea.entity.Name?.toLowerCase().includes(searchLower) || ea.entity.Description?.toLowerCase().includes(searchLower)
      );
    }
