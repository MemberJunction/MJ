...
export class ApplicationViewComponent extends BaseBrowserComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.route.paramMap.subscribe(async (params) => {
      ...
      if (appName && appName !== this.app?.Name) {
        ...
        const matches = this.app.ApplicationEntities.map((ae) => md.Entities.find((e) => e.ID === ae.EntityID))
        ...
        if (userAppEntities && userAppEntities.Success) {
          this.SelectedAppEntities = <EntityEntityExtended[]>(
            userAppEntities.Results.map((uae) => this.AllAppEntities.find((ae) => uae.EntityID === ae.ID)).filter((val) => val)
          ); // now we have our selected app entities and they're sorted properly
          ...
          if (this.SelectedAppEntities.length === 0 && defaultEntities.length > 0) {
            ...
            this.SelectedAppEntities = <EntityEntityExtended[]>(
              defaultEntities.map((de) => this.AllAppEntities.find((aae) => de.EntityID === aae.ID)).filter((val) => val)
            );
      ...
      if (this.app && this.SelectedAppEntities.length) {
        if (entityName) {
          ...
          } else await this.loadEntityAndFolders(this.SelectedAppEntities[0]);
        } else {
  ...
  public IsEntitySelected(entity: EntityEntityExtended) {
    if (this.currentlySelectedAppEntity?.ID === entity.ID) return true;
    else return false;
  }
    ...
    if (save) {
      ...
      if (!(await tg.Submit())) {
        ...
        this.sharedService.CreateSimpleNotification(
          'There was an error saving your entity selections. Please try again later or notify a system administrator.',
          'error',
          3500
        );
      }
  ...
  public onAppEntityButtonClicked(e: EntityEntityExtended): void {
    if (e.ID === this.currentlySelectedAppEntity?.ID) return;
  ...
  protected async loadEntityAndFolders(entity: EntityEntityExtended | undefined): Promise<void> {
    ...
    const categoryFilter: string =
      `UserID='${md.CurrentUser.ID}' AND EntityID='${this.currentlySelectedAppEntity.ID}' AND ` + parentFolderIDFilter;

    const categoryIDFilter: string = this.selectedFolderID ? `CategoryID='${this.selectedFolderID}'` : 'CategoryID IS NULL';
    const userViewFilter: string =
      `UserID = '${md.CurrentUser.ID}' AND EntityID='${this.currentlySelectedAppEntity.ID}' AND ` + categoryIDFilter;
  ...
  public async HandleLinkToSharedView(okClicked: Boolean) {
    ...
    if (okClicked) {
      ...
      if (!success) {
        this.sharedService.CreateSimpleNotification(
          'There was an error linking to the shared view(s). Please try again later or notify a system administrator.',
          'error',
          3500
        );
      }
  ...
  public async OnViewPropertiesDialogClose(args: {
    Saved?: boolean;
  ...
  public createNewRecord(): void {
    ...
    if (!permissions.CanCreate) {
      LogError(
        `Unable to create new record: Current User ${md.CurrentUser.Name} does not have permission to create records for ${entityInfo.Name}`
      );
      return;
    }
    ...
    this.router.navigate(['resource', 'record', '' /*add this 3rd param that's blank so the route validates*/], {
      queryParams: {
