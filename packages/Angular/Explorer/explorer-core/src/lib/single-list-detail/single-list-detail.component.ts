...
export class SingleListDetailComponent implements OnInit {
  ...
  constructor(
    ...
    this.filterItemsSubject.pipe(debounceTime(this.filterDebounceTime)).subscribe(() => this.filterItems(this.filter));

    this.filterListrecordsSubject.pipe(debounceTime(this.filterDebounceTime)).subscribe(() => this.loadListRecords());
  }
  ...
  private async loadList(listID: string): Promise<void> {
    ...
    this.viewColumns = this.sourceEntityInfo.Fields.filter((field: EntityFieldInfo) => field.DefaultInView).map(
    ...
    this.visibleColumns = this.viewColumns
      .filter((x) => x.hidden === false && x.EntityField)
      .sort((a, b) => {
  ...
  public async addTolist(): Promise<void> {
    ...
    await Promise.all(
      this.userViewsToAdd.map(async (userView: UserViewEntityExtended) => {
    ...
    for (let i = 0; i < recordIDs.length; i += chunkSize) {
      ...
      await Promise.all(
        chunk.map(async (recordID: string) => {
  ...
  private async loadListRecords(): Promise<void> {
    ...
    this.listRecords = rvResult.Results.filter((record: Record<string, any>) => {
      const alreadyExits: boolean = this.sourceGridData.some(
        (selectedRecord: Record<'ID' | 'Name', string>) => selectedRecord.ID === record[primaryKeyName]
      );
      return !alreadyExits;
