...
export class NavigationComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public async handleContextMenuOption(option: number): Promise<void> {
    ...
    if (this.activeTabIndex > this.tabs.length)
      // DO NOT add 1 here because in this case, the array boundary is the max for the tab control
    ...
    else this.activeTabIndex = this.activeTabIndex; // this is a hack to force the tab control to update the selected tab
  ...
  protected async NavigateFromUrl() {
    ...
    if (url === '/') {
      ...
      const item = this.drawerItems.find((i) =>
        url
          .toLowerCase()
          .trim()
          .startsWith((<any>i).path?.toLowerCase().trim())
      );
  ...
  selectDrawerItem(index: number) {
    ...
    if (ulElement) {
      ...
      for (let i = 0; i < ulElement.children.length; ++i) {
        if (i !== index) this.renderer.removeClass(ulElement.children[i], 'k-selected');
      }
    }
  ...
  public innerSelectTab(tab: any) {
    ...
    if (index === 0) {
      ...
      let url = this.selectedDrawerItem ? (<any>this.selectedDrawerItem).path : '/home';
      if (this.selectedDrawerItem !== null && this.selectedDrawerItem !== undefined) url = (<any>this.selectedDrawerItem).path;
      if (!this._mostRecentURL.startsWith(url)) {
        ...
        if (
          this._mostRecentHomeURL.startsWith(url) ||
  ...
  protected async loadWorkspace() {
    ...
    if (workspaces.Success) {
      ...
      if (!this.workSpace) throw new Error('Error loading workspace');
    ...
    } else throw new Error('Error loading workspace');
  }
  ...
  protected async LoadWorkspaceItems(): Promise<void> {
    ...
    for (let item of this.workSpaceItems) {
      ...
      setTimeout(async () => {
        ...
        if (newTab === this.tabs[this.activeTabIndex - 1])
          // subtract one since the activeTabIndex is relative to the full set of tabs and the this.tabs array doesn't include the HOME tab
  ...
  protected setAppTitle(title: string = '') {
    if (title === '') this.titleService.setTitle(this.applicationName);
    else this.titleService.setTitle(title + ' (' + this.applicationName + ')');
  }
  ...
  protected findExistingTab(data: ResourceData): Tab | null {
    ...
    if (data.ResourceType.trim().toLowerCase() === 'search results') {
      ...
      existingTab = this.tabs.find(
        (t) =>
          t.data.ResourceTypeID === data.ResourceTypeID &&
      ...
      existingTab = this.tabs.find((t) => {
        if (t.data.ResourceTypeID === data.ResourceTypeID && t.data.ResourceRecordID === data.ResourceRecordID) {
  ...
  protected async AddOrSelectTab(data: ResourceData) {
    ...
    if (existingTab) {
      ...
      existingTab.data.Configuration.___rawQueryParams = {
        ...existingTab.data.Configuration.___rawQueryParams,
        ...data.Configuration.___rawQueryParams,
      };
      ...
      if (tqp) this.tabQueryParams['tab_' + (index + 1)] = { ...tqp, ...existingTab.data.Configuration.___rawQueryParams };
      else this.tabQueryParams['tab_' + (index + 1)] = existingTab.data.Configuration.___rawQueryParams;

      // add one because the HOME tab is not in the tabs array but it IS part of our tab structure
      this.activeTabIndex = index + 1;

      this.scrollIntoView();
      if (existingTab.label) this.setAppTitle(existingTab.label);
      else this.setAppTitle();
      this.loader = false;
    } else {
      const newTab: Tab = {
  ...
  waitForDomUpdate(): Promise<void> {
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        resolve();
      }, 0); // Resolve on the next tick to ensure changes are reflected in the DOM
    });
  }
  ...
  private updateBrowserURL(tab: Tab, data: ResourceData) {
    ...
    switch (rt?.Name.toLowerCase().trim()) {
      ...
      case 'queries':
        ...
        break;
      case 'records':
        const recIDAsString: string =
          data.ResourceRecordID !== null && data.ResourceRecordID !== undefined
            ? typeof data.ResourceRecordID === 'string'
              ? data.ResourceRecordID
              : data.ResourceRecordID.toString()
            : '';
    ...
    if (!cachedQueryParams) {
      ...
      if (cachedQueryParams) {
        ...
        else this.tabQueryParams['tab_' + tabIndex] = { ...cachedQueryParams }; // stuff it into the correct key
      }
    }
    if (cachedQueryParams) {
  ...
  scrollIntoView() {
    if (this.mjTabStrip) this.mjTabStrip.scrollIntoView(this.activeTabIndex);
  }
  ...
  public async SaveSingleWorkspaceItem(tab: Tab): Promise<boolean> {
    try {
      if (!this.workSpace) throw new Error('No workspace loaded');

      let index = this.tabs.indexOf(tab);
      if (index < 0) index = this.tabs.length; // this situation occurs when the tab hasn't yet been added to the tabs collection so the index will be = the length of the tabs collection

      const md = new Metadata();
      let wsItem: WorkspaceItemEntity;
      if (!tab.workspaceItem) {
        wsItem = await md.GetEntityObject<WorkspaceItemEntity>('Workspace Items');
        if (tab.data.ID && tab.data.ID.length > 0) await wsItem.Load(tab.data.ID);
        else {
          wsItem.NewRecord();
      ...
      if (!result) {
        ...
        LogError(
          `Error saving workspace item ${wsItem.Name} to the database. ${wsItem.LatestResult.Message || wsItem.LatestResult.Error || wsItem.LatestResult.Errors?.join('\n')}`
        );
        MJNotificationService.Instance.CreateSimpleNotification(
          `Error saving workspace item ${wsItem.Name} to the database. ${wsItem.LatestResult.Message}`,
          'error',
          5000
        );
        return false;
  ...
  public handleTabSelected(event: TabEvent) {
    if (event.index !== null && event.index >= 0 && event.index <= this.tabs.length) {
      ...
      } else this.innerSelectTab(null); // home
    }
  }
      ...
      if (!transGroup) {
        if (!(await entity.Delete())) {
          ...
          this.sharedService.CreateSimpleNotification(
            'Error deleting workspace item ' + tab.workspaceItem.Name + ' from the database. Please contact your system administrator.',
            'error',
            5000
          );
        }
  ...
  public getActiveTabId(): any | null {
    if (this.activeTabIndex === 0) {
      return null;
    } // subtract 1 from the activeTabIndex if it is not the first tab since our data structure is for tabs 1 to n
    else return this.tabs[this.activeTabIndex - 1]?.id;
  }
  ...
  protected get activeTabIndex(): number {
    if (this.mjTabStrip) return this.mjTabStrip.SelectedTabIndex;
    else return -1;
  }

  protected set activeTabIndex(index: number) {
    if (this.mjTabStrip) this.mjTabStrip.SelectedTabIndex = index;
  }
  ...
  public getAppItemFromViewItem(viewItem: DrawerItem): DrawerItem | null {
    let entityItem = this.getEntityItemFromViewItem(viewItem),
      appItem = null;
