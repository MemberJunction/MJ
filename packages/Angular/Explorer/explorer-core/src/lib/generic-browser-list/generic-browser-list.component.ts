...
import { BaseEntity, Metadata, KeyValuePair, RunView, CompositeKey, EntityInfo } from '@memberjunction/global';
import {
  AfterAddFolderEvent,
  AfterAddItemEvent,
...
export class GenericBrowserListComponent implements OnInit {
   ...
   */
  public dropdownOptions = [{ text: 'Folder' }];

  constructor(public sharedService: SharedService) {
    this.filterItemsSubject.pipe(debounceTime(this._resizeDebounceTime)).subscribe(() => this.filterItems(this.filter));
  }
  ...
  private async unlinkResource(item: Item): Promise<boolean> {
    ...
    if (item.ResourceLinkID) {
      ...
      if (await link.Load(item.ResourceLinkID)) return await link.Delete();
      else return false;
    } else return false;
  }
