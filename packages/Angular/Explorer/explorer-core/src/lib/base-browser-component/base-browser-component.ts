import { LogStatus, Metadata, RunView } from '@memberjunction/global';
...
export class BaseBrowserComponent extends BaseNavigationComponent {
  ...
  public get displayAsGrid(): boolean {
  ...
  protected async LoadData(params?: LoadDataParams): Promise<void> {
    ...
    if (!params?.skipLoadResourceLinks) {
      ...
      if (!rt) throw new Error(`Resource Type for entity ${this.itemEntityName} not found`);
