...
import { ApplicationEntityInfo, Metadata, RunView, EntityRecordNameInput, ApplicationInfo } from '@memberjunction/global';
...
export class SingleApplicationComponent implements OnInit {
  ...
  private findApplication(md: Metadata, appName: string): void {
    ...
    if (this.app) {
      ...
      console.warn(
        `Application '${appName}' not found in metadata. Available applications:`,
        md.Applications.map((a) => a.Name)
      );
    }
  }
