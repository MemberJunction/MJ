...
import { Metadata, RunView, EntityRecordNameInput, CompositeKey, EntityRecordNameResult, LogError } from '@memberjunction/global';
  ...
  favoriteItemClick(fav: UserFavoriteEntity) {
    if (fav) {
      if (fav.Entity === 'User Views') {
        ...
        } else throw new Error(`Entity ${fav.Entity} not found in metadata`);
      }
    }
  }
