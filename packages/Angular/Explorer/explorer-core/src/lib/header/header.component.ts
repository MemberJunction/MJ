...
import { EntityInfo, Metadata, RunView } from '@memberjunction/global';
...
export class HeaderComponent implements OnInit {
  ...
  private async syncAvatarFromAuthProvider(user: any): Promise<boolean> {
    try {
      ...
      // Check if Microsoft
      if (claims && claims.authority && (claims.authority.includes('microsoftonline.com') || claims.authority.includes('microsoft.com'))) {
  ...
  private async loadSearchableEntities() {
    ...
    if (this.searchableEntities.length > 0) this.selectedEntity = this.searchableEntities[0];
  }

  private isMicrosoft(claims: any): boolean {
    return claims && claims.authority && (claims.authority.includes('microsoftonline.com') || claims.authority.includes('microsoft.com'));
  }
