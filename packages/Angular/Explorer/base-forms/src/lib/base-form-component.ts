...
export abstract class BaseFormComponent extends BaseRecordComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  public CalcTopAreaHeight(): void {
    ...
    if (this.topArea && this.topArea.nativeElement) {
      ...
      setTimeout(() => {
        ...
        if (height > 0) this.TopAreaHeight = `${height}px`;
      }, 100);
    }
  }
    ...
    if (this.record) {
      ...
      this.sharedService.CreateSimpleNotification(
        'Changes for: ' +
          ...
          ' ' +
          this.record.PrimaryKey.ToString() +
          '\n\n' +
  ...
  public BuildRelationshipViewParamsByEntityName(relatedEntityName: string, relatedEntityJoinField?: string): RunViewParams {
    ...
    if (eri) return this.BuildRelationshipViewParams(eri);
    else return {};
  }
   ...
   * @returns
   */
  public GetEntityRelationshipByRelatedEntityName(
    relatedEntityName: string,
    relatedEntityJoinField?: string
  ): EntityRelationshipInfo | undefined {
    if (this.record) {
      const r = <BaseEntity>this.record;
      const ret = r.EntityInfo.RelatedEntities.filter(
      ...
      if (this.record) {
        ...
        if (valResults.Success) {
          ...
          if (result) {
            ...
            if (StopEditModeAfterSave) this.EndEditMode();
      ...
      if (r.Dirty || this.PendingRecordsDirty()) {
        ...
        if (pendingRecords && pendingRecords.length > 0) {
          ...
          pendingRecords.forEach((p) => {
            if (p.action === 'save') p.entityObject.Revert(); // only do this if we are saving the record, ignore deletes
          });
        }
  ...
  protected PendingRecordsDirty(): boolean {
    ...
    if (pendingRecords && pendingRecords.length > 0) {
      for (const p of pendingRecords) {
        ...
        else if (p.entityObject.Dirty) return true;
      }
    }
    return false;
  }
  ...
  protected PopulatePendingRecords() {
    ...
    if (result && result.pendingChanges) {
      for (const p of result.pendingChanges) this.PendingRecords.push(p);
    }
  }
  ...
  protected async InternalSaveRecord(): Promise<boolean> {
    if (this.record) {
      ...
      } else return await this.record.Save();
    } else return false;
  }
