...
import { BaseEntity, CompositeKey, LogError, Metadata, RecordDependency, RunView } from '@memberjunction/global';
  ...
  public async saveExistingRecord(event: MouseEvent) {
    ...
    const timer = setInterval(() => {
      ...
      statusMessage.innerHTML =
        `<div>Saving...<span class="form-toolbar-elapsed-time">(${Math.floor(elapsedTime)} sec${elapsedTime === 0 || elapsedTime > 1 ? 's' : ''})</span></div>` +
        (serverUpdateMessage ? `<div class="form-toolbar-server-update-message">${serverUpdateMessage}</div>` : '');
    }, 100);
    ...
    if (!hasCascadeDeletes) {
      ...
      if (dependencies.length > 0) {
        SharedService.Instance.CreateSimpleNotification(
          `This record cannot be deleted because it is being used by ${dependencies.length} other records.`,
          'error',
          2000
        );
        return;
