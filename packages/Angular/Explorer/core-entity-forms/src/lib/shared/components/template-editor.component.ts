...
export class TemplateEditorComponent implements OnInit, OnChanges, OnDestroy, AfterViewInit {
      ...
      try {
        ...
        if (result) {
          ...
          MJNotificationService.Instance.CreateSimpleNotification(
            `Failed to delete template content. ${contentToDelete.LatestResult?.Message}`,
            'error'
          );
        }
      } catch (error) {
  ...
  private updateUnsavedChangesFlag() {
    this.hasUnsavedChanges = this.templateContents.some((content) => content.Dirty) || this.isAddingNewContent;
  }
  ...
  async onRunTemplate() {
    if (!this.template?.IsSaved || !this.currentTemplateContent) {
      MJNotificationService.Instance.CreateSimpleNotification('Please save the template before running it.', 'warning');
      return;
    }
    ...
    if (this.hasUnsavedChanges) {
      ...
      if (!saveResult) {
        MJNotificationService.Instance.CreateSimpleNotification('Failed to save template changes.', 'error');
        return;
      }
    }
