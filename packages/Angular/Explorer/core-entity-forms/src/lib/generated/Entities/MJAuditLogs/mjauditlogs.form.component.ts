import { Component } from '@angular/core';
import { MJAuditLogsEntity } from '@memberjunction/core-entities';
import { RegisterClass } from '@memberjunction/global';
import { BaseFormComponent } from '@memberjunction/ng-base-forms';

@RegisterClass(BaseFormComponent, 'MJ: Audit Logs') // Tell MemberJunction about this class
@Component({
    standalone: false,
    selector: 'gen-mjauditlogs-form',
    templateUrl: './mjauditlogs.form.component.html'
})
export class MJAuditLogsFormComponent extends BaseFormComponent {
    public record!: MJAuditLogsEntity;

    override async ngOnInit() {
        await super.ngOnInit();
        this.initSections([
            { sectionKey: 'auditEntry', sectionName: 'Audit Entry', isExpanded: true },
            { sectionKey: 'targetEntityReference', sectionName: 'Target Entity Reference', isExpanded: true },
            { sectionKey: 'systemMetadata', sectionName: 'System Metadata', isExpanded: false }
        ]);
    }
}

