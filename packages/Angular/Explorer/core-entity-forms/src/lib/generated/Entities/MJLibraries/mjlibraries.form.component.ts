import { Component } from '@angular/core';
import { MJLibrariesEntity } from '@memberjunction/core-entities';
import { RegisterClass } from '@memberjunction/global';
import { BaseFormComponent } from '@memberjunction/ng-base-forms';
import {  } from "@memberjunction/ng-entity-viewer"

@RegisterClass(BaseFormComponent, 'MJ: Libraries') // Tell MemberJunction about this class
@Component({
    standalone: false,
    selector: 'gen-mjlibraries-form',
    templateUrl: './mjlibraries.form.component.html'
})
export class MJLibrariesFormComponent extends BaseFormComponent {
    public record!: MJLibrariesEntity;

    override async ngOnInit() {
        await super.ngOnInit();
        this.initSections([
            { sectionKey: 'libraryIdentification', sectionName: 'Library Identification', isExpanded: true },
            { sectionKey: 'contentAvailability', sectionName: 'Content & Availability', isExpanded: true },
            { sectionKey: 'systemMetadata', sectionName: 'System Metadata', isExpanded: false },
            { sectionKey: 'actions', sectionName: 'Actions', isExpanded: false },
            { sectionKey: 'items', sectionName: 'Items', isExpanded: false }
        ]);
    }
}

