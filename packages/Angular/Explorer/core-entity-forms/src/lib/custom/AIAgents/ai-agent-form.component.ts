...
export class AIAgentFormComponentExtended extends AIAgentFormComponent implements OnDestroy {
  ...
  public openTestHarness() {
    if (!this.record?.ID) {
      MJNotificationService.Instance.CreateSimpleNotification('Please save the AI agent before testing', 'warning', 4000);
      return;
    }
  ...
  public async addPrompt() {
    ...
    const pendingAddIds = this.PendingRecords.filter(
      (p) => p.entityObject.EntityInfo.Name === 'MJ: AI Agent Prompts' && p.action === 'save'
  ...
  public async configureActions() {
    ...
    const pendingAddIds = this.PendingRecords.filter(
      (p) => p.entityObject.EntityInfo.Name === 'AI Agent Actions' && p.action === 'save'
  ...
  public getExecutionResultPreview(execution: AIAgentRunEntityExtended, trimLongMessages: boolean): string {
    try {
      ...
      if (parsed.returnValue?.nextStep?.userMessage) {
        ...
        if (trimLongMessages) return message.length > 120 ? message.substring(0, 120) + '...' : message;
        else return message;
      }

      // Otherwise return the stringified result
      const stringified = JSON.stringify(parsed, null, 2);
      if (trimLongMessages) return stringified.length > 120 ? stringified.substring(0, 120) + '...' : stringified;
      else return stringified;
    } catch {
      // If not JSON, just return the string
      const result = execution.Result || '';
      if (trimLongMessages) return result.length > 120 ? result.substring(0, 120) + '...' : result;
      else return result;
    }
  }
   ...
   */
  public addNote() {
    MJNotificationService.Instance.CreateSimpleNotification('Opening new note form...', 'info', 2000);
      ...
      if (result && (result as any).text === 'Remove') {
        try {
          ...
          const pendingAddIndex = this.PendingRecords.findIndex(
            (p) =>
              p.entityObject.EntityInfo.Name === 'MJ: AI Agent Prompts' &&
              p.action === 'save' &&
  ...
  public async addSubAgents() {
    try {
      ...
      const pendingSubAgentIds = this.PendingRecords.filter(
        (p) => p.entityObject.EntityInfo.Name === 'AI Agents' && p.action === 'save' && p.entityObject.Get('ParentID') === this.record.ID
    ...
    try {
      ...
      if (result && (result as any).text === 'Remove') {
        try {
          ...
          const pendingAddIndex = this.PendingRecords.findIndex(
            (p) =>
              p.entityObject.EntityInfo.Name === 'AI Agent Actions' && p.action === 'save' && p.entityObject.Get('ActionID') === action.ID
          );
  ...
  protected async InternalSaveRecord(): Promise<boolean> {
    ...
    try {
      ...
      const subAgentRecords = this.PendingRecords.filter(
        (p) =>
          p.entityObject.EntityInfo.Name === 'AI Agents' && p.action === 'save' && p.entityObject.Get('_tempParentId') === this.record.ID
      );
