...
export class PromptSelectorDialogComponent implements OnInit, OnDestroy {
  ...
  private async loadPrompts() {
    ...
    try {
      ...
      MJNotificationService.Instance.CreateSimpleNotification('Error loading prompts. Please try again.', 'error', 3000);
      this.prompts$.next([]);
  ...
  togglePromptSelection(prompt: AIPromptEntityExtended) {
    ...
    if (this.isPromptLinked(prompt)) {
      MJNotificationService.Instance.CreateSimpleNotification(`"${prompt.Name}" is already linked to this agent`, 'info', 2000);
      return;
    }
  ...
  selectPrompts() {
    ...
    if (selectedPromptObjects.length === 0) {
      MJNotificationService.Instance.CreateSimpleNotification('Please select at least one prompt', 'warning', 2000);
      return;
    }
