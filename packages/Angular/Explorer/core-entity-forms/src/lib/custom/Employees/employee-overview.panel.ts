...
@Component({
    selector: 'mj-employee-overview-panel',
    standalone: true,
    imports: [CommonModule],
    changeDetection: ChangeDetectionStrategy.OnPush,
    template: `
        <div class="mj-emp-overview-grid">
            <!-- Card 1: Org Hierarchy & Reporting -->
                    ...
                    @if (Reports.length === 0) {
                        ...
                        @for (rep of Reports; track rep.ID) {
                            <div class="mj-metric-row">
                                <span class="mj-metric-label">{{ rep.FirstName }} {{ rep.LastName }}</span>
                                <span class="mj-pill mj-pill-green">{{ rep.Title || 'Member' }}</span>
                            </div>
                        }
                    }
                </div>
            ...
            <div class="mj-overview-card">
                <div class="mj-card-header">
                    <div class="mj-card-title"><i class="fa-solid fa-briefcase" style="color: #10b981;"></i> Position Details</div>
                    <span class="mj-card-badge">Profile</span>
                </div>
                <div class="mj-card-body">
                    <div class="mj-metric-row">
                        <span class="mj-metric-label">Job Title</span>
                        <span class="mj-metric-val">{{ Employee?.Title || 'Staff Member' }}</span>
                    </div>
                    <div class="mj-metric-row">
                        <span class="mj-metric-label">Work Email</span>
                        <span class="mj-metric-val">{{ Employee?.Email || 'None' }}</span>
                    </div>
                    <div class="mj-metric-row">
                        <span class="mj-metric-label">Employment Status</span>
                        <span class="mj-pill mj-pill-green">Active</span>
                    </div>
                </div>
            </div>
        </div>
