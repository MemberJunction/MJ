...
export class TemplateParamsGridComponent implements OnInit, OnChanges {
  ...
  async loadTemplateParams() {
    ...
    try {
      ...
      if (results.Success) {
        ...
        MJNotificationService.Instance.CreateSimpleNotification('Failed to load template parameters', 'error');
      }
    } catch (error) {
  ...
  public getTypeDescription(type: string): string {
    switch (type) {
      case 'Scalar':
        return 'Single value (text, number, etc.)';
      case 'Array':
        return 'List of values';
      case 'Object':
        return 'JSON object';
      case 'Record':
        return 'Single record from an entity';
      case 'Entity':
        return 'Multiple records from an entity';
      default:
        return '';
    }
  }
