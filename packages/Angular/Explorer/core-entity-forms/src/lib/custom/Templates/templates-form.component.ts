...
export class TemplatesFormExtendedComponent extends TemplateFormComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    if (value && !this.categoryOptions.find((opt) => opt.value === value)) {
      ...
      if (this.isDuplicateCategory(value)) {
        const existingCategory = this.categoryOptions.find(
          (option) => option.text && option.text.trim().toLowerCase() === value.trim().toLowerCase()
        );
        if (existingCategory) {
      ...
      try {
        ...
        if (saved) {
          ...
          MJNotificationService.Instance.CreateSimpleNotification(`New category "${newCategory.Name}" created successfully.`, 'success');
        } else {
          MJNotificationService.Instance.CreateSimpleNotification(
        ...
        MJNotificationService.Instance.CreateSimpleNotification('Error creating new category. Please try again.', 'error');
      }
    }
  }
  ...
  private updateUnsavedChangesFlag() {
    this.hasUnsavedChanges =
      this.templateContents.some((content) => content.Dirty) || this.isAddingNewContent || this.record?.Dirty || false;
  }
  ...
  async SaveRecord(StopEditModeAfterSave: boolean = true): Promise<boolean> {
    ...
    if (this.record.CategoryID && !this.categoryOptions.find((opt) => opt.value === this.record.CategoryID)) {
      ...
      if (this.isDuplicateCategory(this.record.CategoryID)) {
        const existingCategory = this.categoryOptions.find(
          (option) => option.text && option.text.trim().toLowerCase() === this.record.CategoryID?.trim().toLowerCase()
        );
        if (existingCategory) {
          // Use the existing category instead
          this.record.CategoryID = existingCategory.value;
