...
export class TemplateParamDialogComponent implements OnInit {
  ...
  async loadTemplateParams() {
    ...
    try {
      ...
      MJNotificationService.Instance.CreateSimpleNotification('Error loading template parameters', 'error');
    } finally {
  ...
  onParameterChange() {
    ...
    this.hasUnsavedParameters = this.parameters.some(
      (param) => param.key && !param.isFromTemplate && !this.originalTemplateParams.find((tp) => tp.Name === param.key)
    );
  }
  ...
  async updateTemplateParams() {
    ...
    const newParams = this.parameters.filter(
      (param) => param.key && !param.isFromTemplate && !this.originalTemplateParams.find((tp) => tp.Name === param.key)
    );
    ...
    try {
      ...
      MJNotificationService.Instance.CreateSimpleNotification(`Added ${newParams.length} new parameter(s) to template`, 'success');
      ...
      MJNotificationService.Instance.CreateSimpleNotification('Error saving template parameters', 'error');
    }
  }
  ...
  saveResults() {
    ...
    const content = this.testResult.success ? this.testResult.output || 'No output' : this.testResult.error || 'No error details';
    ...
    MJNotificationService.Instance.CreateSimpleNotification(`Results saved as ${filename}`, 'success');
  }
