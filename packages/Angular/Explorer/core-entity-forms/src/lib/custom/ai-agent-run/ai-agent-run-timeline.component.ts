...
export class AIAgentRunTimelineComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.timelineItems$ = combineLatest([this.steps$, this.subRuns$, this.actionLogs$, this.promptRuns$]).pipe(
      map(([steps, subRuns, actionLogs, promptRuns]) => this.buildTimelineItems(steps, subRuns, actionLogs, promptRuns)),
      shareReplay(1)
    );
  ...
  private buildHierarchicalItems(steps: AIAgentRunStepEntity[], baseLevel: number, promptRuns?: AIPromptRunEntity[]): TimelineItem[] {
