...
export class AIPromptFormComponentExtended extends AIPromptFormComponent implements OnInit {
  ...
  public async loadTemplate() {
    ...
    const pendingTemplate = this.PendingRecords.find(
      (p) => p.entityObject.EntityInfo.Name === 'Templates' && p.entityObject.Get('ID') === this.record.TemplateID
    );
    ...
    try {
      ...
      MJNotificationService.Instance.CreateSimpleNotification('Error loading associated template', 'error', 5000);
    } finally {
      this.isLoadingTemplate = false;
    }
  }
  ...
  public executeAIPrompt() {
    if (!this.record?.ID) {
      MJNotificationService.Instance.CreateSimpleNotification('Please save the AI prompt before executing', 'warning', 4000);
      return;
    }

    if (this.record.Status !== 'Active') {
      MJNotificationService.Instance.CreateSimpleNotification('AI prompt must be Active to execute', 'warning', 4000);
      return;
    }
  ...
  public openTestHarness() {
    if (!this.record?.ID) {
      MJNotificationService.Instance.CreateSimpleNotification('Please save the AI prompt before testing', 'warning', 4000);
      return;
    }
  ...
  public handleTemplateContentDelete(templateContent: TemplateContentEntity) {
    if (templateContent.IsSaved) {
      ...
      for (let i = currentPendingRecords.length - 1; i >= 0; i--) {
        const record = currentPendingRecords[i];
        if (
          record.entityObject === templateContent ||
  ...
  public onTemplateRun(template: TemplateEntity) {
    ...
    MJNotificationService.Instance.CreateSimpleNotification('Template run functionality coming soon', 'info', 3000);
  }
  ...
  public async loadVendorsForModel(modelId: string): Promise<{ vendors: AIVendorEntity[]; modelVendors: AIModelVendorEntity[] }> {
    ...
    try {
      ...
      const modelVendors = engine.ModelVendors.filter(
        (mv) => mv.ModelID === modelId && mv.TypeID === this.__InferenceProvider_VendorTypeDefinitionID
      );
  ...
  public async addNewModel() {
    ...
    try {
      ...
      MJNotificationService.Instance.CreateSimpleNotification('Error creating new model', 'error', 5000);
    }
  }
  ...
  public async removePromptModel(index: number) {
    ...
    try {
      ...
      MJNotificationService.Instance.CreateSimpleNotification('Error removing model', 'error', 5000);
    }
  }
