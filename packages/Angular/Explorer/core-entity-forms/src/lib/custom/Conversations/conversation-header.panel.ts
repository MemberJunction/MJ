...
import { MJConversationEntity, UserInfoEngine } from '@memberjunction/core-entities';
...
@Component({
    selector: 'mj-conversation-header-panel',
    standalone: true,
            ...
            <div class="mj-entity-hero-header">
                <div class="mj-hero-top">
                    <div class="mj-hero-identity">
                        <div class="mj-hero-avatar" style="background: linear-gradient(135deg, #38bdf8, #0284c7);">
                            <i class="fa-solid fa-comments"></i>
                        </div>
                        <div>
                            <h1 class="mj-hero-title">{{ Conversation.Name || 'AI Conversation' }}</h1>
                            <div class="mj-hero-subline">
                                <span>User: {{ Conversation.User || 'User Session' }}</span>
                                <span>•</span>
                                <span>Created: {{ Conversation.CreatedAt | date:'medium' }}</span>
                            </div>
                        </div>
                    </div>
                ...
                @if (!IsCollapsed) {
                    <div class="mj-hero-metrics-row">
                        <div class="mj-hero-metric">
                            <span class="mj-hero-metric-label">Conversation ID</span>
                            <span class="mj-hero-metric-val">{{ Conversation.ID?.substring(0, 8) }}</span>
                        </div>
                        <div class="mj-hero-metric">
                            <span class="mj-hero-metric-label">Status</span>
                            <span class="mj-hero-metric-val" style="color: #38bdf8;">Active Session</span>
                        </div>
                    </div>
                }
            </div>
...
export class ConversationHeaderPanel extends BaseFormPanel<MJConversationEntity> {
