...
import { Metadata, RunView, RUN_QUERY_SQL_FILTERS } from '@memberjunction/global';
...
export class QueryFormExtendedComponent extends QueryFormComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  // Category data
  public categoryOptions: Array<{ text: string; value: string }> = [{ text: 'Select Category...', value: '' }];
  ...
  async ngOnInit() {
    ...
    await Promise.all([this.loadQueryParameters(), this.loadQueryFields(), this.loadQueryEntities(), this.loadQueryPermissions()]);
  ...
  private updateUnsavedChangesFlag() {
    this.hasUnsavedChanges = this.queryParameters.some((param) => param.Dirty) || this.record?.Dirty || false;
  }
  ...
  async SaveRecord(StopEditModeAfterSave: boolean = true): Promise<boolean> {
    ...
    if (result) {
      ...
      if (this.record && this.record.ID) {
        await Promise.all([this.loadQueryParameters(), this.loadQueryFields(), this.loadQueryEntities()]);
      }
    }
