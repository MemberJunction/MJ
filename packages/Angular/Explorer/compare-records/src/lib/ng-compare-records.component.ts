...
export class CompareRecordsComponent {
  ...
  */
  ...
  protected async preprocessRecordsToCompare() {
    ...
    if (!entity) throw new Error('Entity not found: ' + this.entityName);
    ...
    if (loadFromDatabase.length > 0) {
      ...
      for (const r of loadFromDatabase) {
        ...
        for (const pkey of entity.PrimaryKeys) {
          if (innerFilter.length > 0) innerFilter += ' AND ';
  ...
  protected async prepareViewData() {
    try {
      ...
      if (!this.entityInfo) throw new Error('Entity not found: ' + this.entityName);
      ...
      this.entityInfo.Fields.forEach((f) =>
        this.visibleColumns.push(<ViewColumnInfo>{ hidden: false, Name: f.Name, EntityField: f, ID: f.ID, DisplayName: f.DisplayName })
      );
          ...
          if (!column.hidden) {
            ...
            this.recordsToCompare.forEach((record, index: number) => {
              ...
              this.columns[index + 1] = {
                field: '_' + record.PrimaryKey.Values(),
                CompositeKey: record.PrimaryKey,
                title: this.GetColumnHeaderTextFromCompositeKey(record.PrimaryKey),
                width: 200,
                locked: true,
                lockable: false,
                filterable: false,
            ...
            if ((this.suppressBlankFields || this.showDifferences) && !['ID', 'Name'].includes(obj.Fields)) {
              ...
              let addValue = false;
              const hasBlankFields = values.every(
                (item: any) => item['Value'] === '' || item['Value'] === null || item['Value'] === undefined
              );
  ...
  public FormatColumnValue(dataItem: any, column: any, maxLength: number) {
    //column: ViewColumnInfo, value: string, maxLength: number) {
    try {
  ...
  public IsCellSelected(dataItem: any, column: any) {
    ...
    } else return false; // selection mode is off, always return false
  }
  ...
  public GetCellStyle(dataItem: any, column: any) {
    ...
    if (this.IsCellSelected(dataItem, column)) {
      if (this.IsItemFieldMapped(dataItem)) return 'cell cell-selected-override';
      else return 'cell cell-selected' + readOnlyClass;
    } else {
      if (this.selectionMode) return 'cell cell-not-selected' + readOnlyClass;
      else return 'cell'; // selection mode is off, just regular cell format
    }
  }
      ...
      if (!event.dataItem[currentRecordCompositeKeyString].metaData.EntityField.ReadOnly) {
        ...
        if (!this.selectedRecordCompositeKey.Equals(currentRecordCompositeKey)) {
          ...
          if (fieldMapIndex >= 0) {
            ...
            this.fieldMap.push({
              fieldName: fieldName,
              CompositeKey: currentRecordCompositeKey,
              value: event.dataItem[currentRecordCompositeKeyString].Value,
            });
          }
        } else {
    ...
    if (event.target instanceof HTMLElement) {
      ...
      if (element.classList.contains('k-header') || element.closest('.k-header')) {
        ...
        if (columnIndex > 0) {
          ...
          this.selectedRecordCompositeKey =
            this.recordsToCompare[columnIndex - 1].PrimaryKey; /*always remove 1 because the first col is the "Fields" column*/
        }
      }
    }
  }
