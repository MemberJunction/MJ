 ...
 * the component's current NON-SENSITIVE state (active tab, aggregate counts, loading
 * flag) into the key-value context object that flows to the async chat agent and the
 * realtime co-agent via `NavigationService.SetAgentContext`. They are intentionally
 * free of Angular / component dependencies so the resulting context shape can be
 * unit-tested in isolation — including an explicit assertion that no secret-like
 * key (secret / password / token / key value / credential value) can ever appear.
 *
 * What flows out: ActiveTab, TabLabel, CredentialCount, TypeCount, IsLoading.
 * What NEVER flows out: a credential's secret value, an API key, a password, a
 * token, a connection string, or any other reveal-able credential material. Those
 ...
 * construction this contains ONLY non-sensitive navigation/metadata — there is no
 * field here for a credential secret, password, token, or key value, so none can
 * be published.
 */
 ...
 *
 * Returns a flat key-value object of NON-SENSITIVE metadata only: active tab,
 * its label, the two aggregate counts, and the loading flag. Keeping this a pure
 * function (no `this`) makes the context shape unit-testable and lets the test
 * assert that no secret-bearing key can ever appear.
 *
