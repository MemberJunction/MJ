...
import { Metadata, CompositeKey, LogError, LogStatus } from '@memberjunction/global';
...
export class PromptVersionControlComponent implements OnInit, OnDestroy {
  ...
  private applyFilters(): void {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (v) => v.changesDescription.toLowerCase().includes(searchTerm) || v.changedBy.toLowerCase().includes(searchTerm)
      );
    }
  ...
  public async restoreVersion(version: PromptVersion): Promise<void> {
    ...
    const confirm = window.confirm(
      `Are you sure you want to restore to version ${version.version} from ${version.changedAt.toLocaleString()}? This will overwrite the current prompt.`
    );
    if (!confirm) return;
  ...
  private filterAvailablePrompts(searchTerm: string): void {
    if (!searchTerm || searchTerm.trim() === '') {
      ...
      this.filteredAvailablePrompts = this.availablePrompts.filter(
        (prompt) => prompt.Name.toLowerCase().includes(term) || (prompt.Description && prompt.Description.toLowerCase().includes(term))
      );
    }
  }
