 ...
 *
 * @see EntityDesignerEngine — for read-only data access (no action invocations)
 */
...
// Must stay in sync with @RegisterClass registrations in entity-designer-actions package.
...
export class EntityDesignerService {
    ...
    private async fetchActionId(actionName: string): Promise<string> {
        ...
        if (!result.Success || result.Results.length === 0) {
            throw new Error(
                `Entity Designer action '${actionName}' not found in MJ. ` +
                `Ensure entity-designer-actions metadata has been pushed via mj sync push.`
            );
        }
