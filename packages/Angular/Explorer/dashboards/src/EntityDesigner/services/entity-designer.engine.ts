/**
 * @module entity-designer.engine
 * @description Read-only data access engine for the Entity Designer Angular UI.
 *
 * `EntityDesignerEngine` is a `BaseSingleton` (NOT an Angular service) that:
 ...
 * ### Usage
 * ```typescript
 * const engine = EntityDesignerEngine.Instance;
 * const entities = await engine.loadAccessibleEntities();
 * ```
 *
 * @see EntityDesignerService — for action invocation (create, modify, validate)
 */
...
// ─── Constants (mirror entity-designer-core/src/interfaces.ts) ───────────────
...
/**
 * Singleton data access engine for Entity Designer.
 * All methods read data; none modifies state (mutations go through `EntityDesignerService`).
 */
export class EntityDesignerEngine extends BaseSingleton<EntityDesignerEngine> {

    public static get Instance(): EntityDesignerEngine {
        return super.getInstance<EntityDesignerEngine>();
    }
     ...
     *   Entity Designer or Agent Manager, with ownership annotations.
    ...
    /** Admin path: load all Entity-Designer-managed entities + ownership annotations. */
