...
export class StepRelationshipsComponent {
    ...
    public get AvailableSchemas(): string[] {
        return [...new Set(new Metadata().Entities.map(e => e.SchemaName))]  // global-provider-ok: TODO multi-provider migration — DatabaseDesigner UI doesn't yet thread Provider input
    ...
    public EntitiesForSchema(schemaName: string): EntityInfo[] {
        ...
        return new Metadata().Entities  // global-provider-ok: TODO multi-provider migration — DatabaseDesigner UI doesn't yet thread Provider input
    ...
    public ColumnsForEntity(entityId: string): EntityColumnInfo[] {
        if (!entityId) return [];
        const entity = new Metadata().Entities.find(e => UUIDsEqual(e.ID, entityId));  // global-provider-ok: TODO multi-provider migration — DatabaseDesigner UI doesn't yet thread Provider input
    ...
    private buildRowState(fk: ForeignKeySpec, index: number): FkRowState {
        const entity = new Metadata().Entities.find(  // global-provider-ok: TODO multi-provider migration — DatabaseDesigner UI doesn't yet thread Provider input
