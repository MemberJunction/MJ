...
export class ActionEngineServer extends ActionEngineBase {
  ...
  protected async InternalRunAction(params: RunActionParams): Promise<ActionResult> {
    ...
    try {
      ...
      const resultCodeEntity: ActionResultCodeEntity | undefined = this.ActionResultCodes.find(
        (r) => r.ActionID === params.Action.ID && r.ResultCode.trim().toLowerCase() === simpleResult.ResultCode.trim().toLowerCase()
      );
      const result: ActionResult = {
        RunParams: params,
