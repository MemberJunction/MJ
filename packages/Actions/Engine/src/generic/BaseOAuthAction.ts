...
import { Metadata, RunView, LogError, LogStatus } from '@memberjunction/global';
...
export abstract class BaseOAuthAction extends BaseAction {
   ...
   */
  protected async makeAuthenticatedRequest<T>(requestFn: (token: string) => Promise<T>, retryOnAuthFailure: boolean = true): Promise<T> {
  ...
  protected isAuthError(error: any): boolean {
    ...
    const authErrorPatterns = ['unauthorized', 'invalid_token', 'expired_token', 'invalid_grant', '401', 'forbidden', 'authentication'];
    ...
    return (
      statusCode === 401 ||
      statusCode === 403 ||
      authErrorPatterns.some((pattern) => errorMessage.includes(pattern) || errorCode.includes(pattern))
    );
  }
   ...
   */
  protected async updateStoredTokens(accessToken: string, refreshToken?: string, expiresIn?: number): Promise<void> {
   ...
   */
  protected async setCustomAttribute(attributeNumber: 1 | 2 | 3 | 4 | 5, value: string): Promise<void> {
    if (!this._companyIntegration) {
      throw new Error('Company integration not initialized');
    }
