...
export abstract class EntityActionInvocationBase {
   ...
   * @param valueType
   */
  public FindActionParam(
    allParams: ActionParamEntity[],
   ...
   * @returns
   */
  public async MapParams(
    params: ActionParamEntity[],
    entityActionParams: EntityActionParamEntity[],
...
export class EntityActionInvocationMultipleRecords extends EntityActionInvocationBase {
  ...
  public async InvokeAction(params: EntityActionInvocationParams): Promise<EntityActionResult> {
    ...
    if (this.ValidateParams(params)) {
      ...
      const invocationInstance = MJGlobal.Instance.ClassFactory.CreateInstance<EntityActionInvocationBase>(
        EntityActionInvocationBase,
        'SingleRecord'
      ); // get the single record class
      if (!invocationInstance) throw new Error('Error creating instance of invocation type');
