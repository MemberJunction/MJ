...
export abstract class BaseAccountingAction extends BaseAction {
  ...
  protected async getOAuthTokens(integration: CompanyIntegrationEntity): Promise<{ accessToken: string; refreshToken?: string }> {
    ...
    if (!integration.AccessToken) {
      throw new Error(
        `No access token found for ${this.integrationName} integration. Please set environment variable BIZAPPS_${this.accountingProvider.toUpperCase().replace(/\s+/g, '_')}_${companyId}_ACCESS_TOKEN or configure in database.`
      );
    }
