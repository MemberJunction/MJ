...
export class GetQuickBooksAccountBalancesAction extends QuickBooksBaseAction {
  ...
  private async getAccounts(params: ActionParam[], contextUser: UserInfo): Promise<any[]> {
    ...
    if (this.getParamValue(params, 'AccountTypes')) {
      const types = this.getParamValue(params, 'AccountTypes')
        .split(',')
        .map((t: string) => `'${t.trim()}'`);
    ...
    // Execute the query
    const response = await this.queryQBO<{ QueryResponse: { Account: any[] } }>(query, contextUser);
   ...
   * Calculate trial balance summary
   */
  private calculateTrialBalanceSummary(balances: AccountBalance[], asOfDate: Date): TrialBalanceSummary {
    let totalDebits = 0;
    let totalCredits = 0;
