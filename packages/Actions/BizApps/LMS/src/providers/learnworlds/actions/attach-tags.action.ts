...
/**
 * Action to attach tags to a LearnWorlds user
 */
@RegisterClass(BaseAction, 'AttachTagsAction')
export class AttachTagsAction extends LearnWorldsBaseAction {
  /**
   * Attaches one or more tags to a LearnWorlds user.
   */
  ...
  protected async InternalRunAction(params: RunActionParams): Promise<ActionResultSimple> {
    ...
    try {
      ...
      const result = await this.AttachTags(tagParams, ContextUser);

      this.setOutputParam(Params, 'Success', result.Success);
      this.setOutputParam(Params, 'ResultTags', result.Tags);

      return this.buildSuccessResult(`Successfully attached ${result.Tags.length} tag(s) to user ${result.UserID}`, Params);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      return this.buildErrorResult('ERROR', `Error attaching tags: ${errorMessage}`, Params);
    }
  }
  ...
  public get Description(): string {
    return 'Attaches one or more tags to a LearnWorlds user';
  }
}
