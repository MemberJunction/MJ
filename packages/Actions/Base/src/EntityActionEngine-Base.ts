import { BaseEngine, BaseEnginePropertyConfig, BaseEntity, IMetadataProvider, UserInfo } from '@memberjunction/global';
import {
...
export class EntityActionEngineBase extends BaseEngine<EntityActionEngineBase> {
  ...
  public GetActionsByEntityName(entityName: string, status?: 'Active' | 'Pending' | 'Disabled'): EntityActionEntityExtended[] {
    return this._EntityActions.filter(
      (e) => (!status || e.Status === status) && e.Entity.trim().toLowerCase() === entityName.trim().toLowerCase()
    );
  }
   ...
   * @returns
   */
  public GetActionsByEntityNameAndInvocationType(
    entityName: string,
    ...
    return entityActions.filter((e) => {
      const invocations = e.Invocations.find(
        (i) => (!status || i.Status === status) && i.InvocationType.trim().toLowerCase() === invocationType.trim().toLowerCase()
      );
