...
export class WorkerPool {
  ...
  private handleWorkerCrash(worker: Worker, code: number | null, signal: string | null): void {
    ...
    LogError(
      new Error(
        `Worker ${worker.id} crashed (code: ${code}, signal: ${signal}). ` + `Crash count: ${worker.crashCount}/${this.maxCrashesPerWorker}`
      )
    );
    ...
    if (worker.currentRequest) {
      ...
      worker.currentRequest.reject(
        new Error('Worker process crashed during code execution. This may indicate a severe error in the code.')
      );
      worker.currentRequest = null;
    }
