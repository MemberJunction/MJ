...
export class ListStorageProvidersAction extends BaseFileStorageAction {
  protected async InternalRunAction(params: RunActionParams): Promise<ActionResultSimple> {
    ...
    try {
      ...
      if (!result.Success) {
        return this.createErrorResult(`Failed to retrieve storage providers: ${result.ErrorMessage}`, 'QUERY_FAILED');
      }
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      return this.createErrorResult(`Failed to list storage providers: ${errorMessage}`, 'LIST_FAILED');
    }
  }
}
