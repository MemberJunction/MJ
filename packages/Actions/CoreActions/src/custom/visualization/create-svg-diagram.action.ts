...
export class CreateSVGDiagramAction extends BaseAction {
    ...
    protected async InternalRunAction(params: RunActionParams): Promise<SVGActionResult> {
        try {
            const diagramType = this.getParamValue(params, 'DiagramType')?.toLowerCase();
            ...
            const seed = parseInt(this.getParamValue(params, 'Seed') || '0');
    ...
    private async renderFlowDiagram(
        ...
        const nodes: FlowNode[] = JSON.parse(nodesParam);
        const edges: FlowEdge[] = edgesParam ? JSON.parse(edgesParam) : [];
        const direction = (this.getParamValue(params, 'Direction') || 'TB') as FlowLayout['direction'];
    ...
    private async renderOrgChart(params: RunActionParams, viewBox: ViewBox, branding: Branding, title: string): Promise<string> {
        ...
        const rootNode: OrgNode = JSON.parse(nodesParam);
    ...
    private async renderERDiagram(
        ...
        const tables: ERTable[] = JSON.parse(nodesParam);
        const relations: ERRelation[] = edgesParam ? JSON.parse(edgesParam) : [];
