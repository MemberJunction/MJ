...
import { LogError, RunView } from '@memberjunction/global';
...
export class SummarizeContentAction extends BaseAction {
  protected async InternalRunAction(params: RunActionParams): Promise<ActionResultSimple> {
    try {
      ...
      if (!url && !content) {
        return this.createErrorResult("Must provide either 'url' or 'content' parameter", 'MISSING_INPUT');
      }

      // Validation: If content provided, must have sourceUrl
      if (content && !sourceUrl) {
        return this.createErrorResult("When using 'content' parameter, 'sourceUrl' is required for citations", 'MISSING_SOURCE_URL');
      }
      ...
      if (url) {
        ...
        if (!fetchResult.success) {
          return this.createErrorResult(`Failed to fetch web page: ${fetchResult.error}`, 'FETCH_FAILED');
        }
      ...
      if (!summaryResult.success) {
        return this.createErrorResult(`Failed to generate summary: ${summaryResult.error}`, 'SUMMARIZATION_FAILED');
      }
   ...
   */
  private async fetchWebPageContent(url: string, params: RunActionParams): Promise<{ success: boolean; content?: string; error?: string }> {
    try {
  ...
  private getPromptByNameAndCategory(name: string, category: string): AIPromptEntityExtended | undefined {
    return AIEngine.Instance.Prompts.find(
      (p) =>
        p.Name.trim().toLowerCase() === name.trim().toLowerCase() && p.Category?.trim().toLowerCase() === category?.trim().toLowerCase()
    );
  }
