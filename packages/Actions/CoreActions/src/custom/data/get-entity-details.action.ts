  ...
  protected async InternalRunAction(params: RunActionParams): Promise<ActionResultSimple> {
    try {
      ...
      const fields = entity.Fields.map((f) => ({
        ...
        ValueListValues: f.EntityFieldValues && f.EntityFieldValues.length > 0 ? f.EntityFieldValues.map((v) => v.Value).join(', ') : null,
      }));
      ...
      if (includeRelatedEntityInfo) {
        const relatedEntityNames = new Set(fields.filter((f) => f.RelatedEntity).map((f) => f.RelatedEntity!));
        relatedEntities = Array.from(relatedEntityNames).map((name) => ({
          Name: name,
          FieldsReferencingThis: fields.filter((f) => f.RelatedEntity === name).map((f) => f.Name),
        }));
      }
      ...
      const message = this.buildDetailedMessage(entity, fields, sampleData, totalRowCount, primaryKeyFields, relatedEntities);
