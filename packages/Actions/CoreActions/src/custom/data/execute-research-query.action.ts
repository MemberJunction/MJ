...
export class ExecuteResearchQueryAction extends BaseAction {
  ...
  protected async InternalRunAction(params: RunActionParams): Promise<ActionResultSimple> {
    ...
    try {
      ...
      const returnType = this.getStringParam(params, 'returntype') || (analysisRequest ? 'data and analysis' : 'data only');
      ...
      try {
        ...
        const results = (await Promise.race([
          ...
          new Promise((_, reject) => setTimeout(() => reject(new Error('Query timeout exceeded')), timeout * 1000)),
        ])) as any;
        ...
        if (dataFormat === 'csv') {
          ...
          const trimmedResults = columnMaxLength > 0 ? this.trimResultColumns(results, columnMaxLength) : results;
        ...
        if (analysisRequest && (returnType === 'analysis only' || returnType === 'data and analysis')) {
          if (results.length === 0) {
            ...
            const analysisResult = await this.analyzeQueryData(results, columns, analysisRequest, params, columnMaxLength);
        ...
        if (
          queryError.message &&
          (queryError.message.toLowerCase().includes('permission') || queryError.message.toLowerCase().includes('denied'))
        ) {
          return {
            Success: false,
