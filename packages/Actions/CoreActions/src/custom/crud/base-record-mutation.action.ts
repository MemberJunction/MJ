...
import { BaseEntity, Metadata, LogError, CompositeKey } from '@memberjunction/global';
...
export abstract class BaseRecordMutationAction extends BaseAction {
   ...
   */
  protected async getEntityObject(
    entityName: string,
    contextUser: any
  ): Promise<{
    entity?: BaseEntity;
  ...
  protected async loadRecord(
    ...
    if (!loadResult) {
      ...
      if (latestResult?.Message?.toLowerCase().includes('permission') || latestResult?.Message?.toLowerCase().includes('denied')) {
   ...
   */
  protected analyzeEntityError(entity: BaseEntity, operation: 'create' | 'update' | 'delete', entityName: string): ActionResultSimple {
    const latestResult = entity.LatestResult;
    ...
    if (latestResult) {
      ...
      if (latestResult.Message?.toLowerCase().includes('permission') || latestResult.Message?.toLowerCase().includes('denied')) {
        resultCode = 'PERMISSION_DENIED';
      } else if (latestResult.Message?.toLowerCase().includes('validation') || latestResult.Message?.toLowerCase().includes('required')) {
        resultCode = 'VALIDATION_ERROR';
      } else if (
        operation === 'update' &&
        (latestResult.Message?.toLowerCase().includes('concurrent') || latestResult.Message?.toLowerCase().includes('modified'))
      ) {
        resultCode = 'CONCURRENT_UPDATE';
      } else if (operation === 'delete') {
        if (
          latestResult.Message?.toLowerCase().includes('reference') ||
