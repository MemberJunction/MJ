...
async function withBoundedRetry(label: string, fn: (attempt: number) => Promise<void>): Promise<void> {
...
export const AgentMemoryGuardsChecks: NamedCheck[] = [
  {
    ...
    Fn: async (ctx): Promise<void> => {
      await withBoundedRetry('MG1', async (attempt) => {
        const fx = requireFixture(ctx);
        const tag = `${fx.Marker}-MG1-a${attempt}`;
  ...
  },
  {
    ...
    RequiresLiveModel: true,
    Fn: async (ctx): Promise<void> => {
      await withBoundedRetry('MG3', async (attempt) => {
        const fx = requireFixture(ctx);
        const tag = `${fx.Marker}-MG3-a${attempt}`;
        const agent = await resolveWriter(ctx);
  ...
  },
  {
    ...
    RequiresLiveModel: true,
    Fn: async (ctx): Promise<void> => {
      await withBoundedRetry('MG4', async (attempt) => {
        const fx = requireFixture(ctx);
        const tag = `${fx.Marker}-MG4-a${attempt}`;
        const agent = await resolveWriter(ctx);
  ...
  },
  {
    ...
    RequiresLiveModel: true,
    Fn: async (ctx): Promise<void> => {
      await withBoundedRetry('MG5', async (attempt) => {
        const fx = requireFixture(ctx);
        const tag = `${fx.Marker}-MG5-a${attempt}`;
        const agent = await resolveWriter(ctx);
