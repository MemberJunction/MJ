...
| `predictive-studio` | 8 (PS1–PS8) | IT14 - Predictive Studio Stack Seams | server | deterministic; PS5 live-sidecar leg gated `PS_INTEGRATION=1` |
...
### 7.1 `predictive-studio` (PS1–PS8) — IT14

**Machinery under test.** Predictive Studio's **stack seams** over the real provider: ML entity CRUD with typed fields, the `'ML Model'` Record Set Processing work-type registration + resolution through the substrate's pluggable `RecordProcessorRegistry` (the exact call `RecordProcessExecutor.buildProcessor()` makes for non-built-in work types), the four Predictive Studio Actions in real metadata invoked through the real Action-execution path, and (PS6–PS8) the typed-component model — component-type CRUD *including the hierarchy columns*, profile resolution + tree lint over the SHIPPED seed tree, and the Model → Component → Binding → Entity Field meaning chain. PS1–PS5 were graduated verbatim from `predictive-studio-tests.ts`. The PS engine is imported for its `@RegisterClass` side effects (the same registration path a server bootstrap uses — the PS engine is NOT in `server-bootstrap-lite`), anchored by `LoadMLModelInferenceProcessor()`/`LoadPredictiveStudioActions()`.
...
**Fixtures/lifecycle.** `ctx.PredictiveStudioFixture` — Setup resolves a target entity + a seeded ML algorithm (hard assert naming `mj sync push --include=ml-algorithms`), then creates the Pipeline → Model → Scoring Binding lineage; handle published up-front, populated per record. Setup then adds the PS6–PS8 component leg: a `MJ: ML Components` instance of a seeded CONCRETE Model leaf, `Model.RootComponentID` pointed at it, and one `MJ: ML Component Bindings` row onto a real field of the target entity. That leg is **skipped without failing Setup** when the component tree isn't seeded, so a DB missing `mj sync push` for the `ml-component-type*` dirs produces a readable PS6/PS7 assertion instead of an FK error during setup. Teardown deletes child → parent, and **cuts the model ↔ component reference cycle first** (`Model.RootComponentID = null`, save, then delete the component) — component FKs the model while the model's `RootComponentID` FKs the component, so neither can be deleted first. All best-effort. The registry registration is deliberately left in place (process-wide, idempotent, last-wins).
...
| `predictive-studio.PS6` | ML Component Type CRUD round-trips **with the hierarchy columns present** | a seeded concrete Model leaf reads back with typed `Kind='Model'`/`Status='Published'`, `IsAbstract=false`, a non-null `DriverClass`, and non-null `ParentIDPath`/numeric `ParentIDDepth > 0` | the hierarchy **opt-in trap**: a self-referencing FK without `Configuration.Hierarchy.IsHierarchy` registers virtual Root/Depth/Path fields the base view never emits, so every read fails "column does not exist" — and a grid renders "no data" rather than an error, making it invisible until someone opens the tree |
| `predictive-studio.PS7` | `MLComponentEngine` resolves inherited profiles over the SHIPPED seed tree, which lints clean | `Lint()` returns zero Error/Warning findings on the seeded tree; XGBoost's resolved profile inherits `impute` (Tree Ensemble), `n_estimators`/`max_depth`/`learning_rate` (Tree Ensemble + Boosting), `Explainability='global-importance'`, and the Model root's `single-feature-dominance` gate; `PreprocessingBank` provenance names a node ABOVE the leaf; MLP's `Replace`d `min-rows-per-feature` gate no longer resolves to the root | an unprincipled partition shipping in metadata (a property on a node it isn't true of), and silent breakage of the union / override-nearest / Replace merge semantics the whole guidance layer reads through |
| `predictive-studio.PS8` | the Model → Component → Binding → Entity Field meaning chain reads back; abstract types are refused | `Model.RootComponentID` → the component; component's type + model FKs, `PromotionState='Draft'`, non-null `ParentComponentIDPath`; the binding's `Role='Input'`, `DataType='Number'`, `EntityFieldID` matching a real field, and a **non-empty denormalized `EntityField`** proving the FK resolves; saving a component of an ABSTRACT type returns `false` | bindings that point at nothing (a model whose inputs have column names but no business meaning), and the server-side abstract-instantiation guard silently not being registered |
