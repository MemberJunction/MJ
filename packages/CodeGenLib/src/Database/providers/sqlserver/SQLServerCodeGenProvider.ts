...
`;
    ...
    /**
     * Returns true when codegen should emit a `@<Param>_Clear bit = 0` companion
     * parameter for the given field. The companion lets callers disambiguate
     * "leave unchanged / apply DB default" (omit the parameter) from "explicitly
     * set this column to NULL" (`@<Param>_Clear = 1`). Required only for nullable
     * columns whose database default is itself non-NULL — without the companion,
     * a caller could not insert/preserve a literal NULL because `ISNULL(@Param, <default>)`
     * would always substitute the default.
     */
    protected needsClearCompanion(ef: EntityFieldInfo): boolean {
        ...
        return formattedDefault.toUpperCase() !== 'NULL';
    }

    /**
     * Builds the SQL Server parameter declaration list for a CRUD stored procedure
     * with tolerant signatures: every non-required parameter defaults to NULL so
     * historical EXEC calls stay valid as the schema evolves. Primary keys remain
     * required on update, and required NOT-NULL columns with no database default
     * remain required on create. Nullable columns with non-NULL database defaults
     * receive a `@<Param>_Clear bit = 0` companion parameter (emitted immediately
     * before the main parameter) so callers can explicitly request NULL.
     */
        ...
        for (const ef of entityFields) {
            ...
            if (
                ...
                if (ef.IsPrimaryKey) {
                    ...
                    if (!isUpdate && !ef.AutoIncrement) {
                        defaultParamValue = ' = NULL';
                    }
                } else if (isUpdate) {
                    // Update: every non-PK parameter is optional (merge semantics)
                    defaultParamValue = ' = NULL';
                } else if (ef.AllowsNull || ef.HasDefaultValue) {
                    // Create: any non-required column (nullable OR has default) is optional
                    defaultParamValue = ' = NULL';
                }
     ...
     * - **`'@'` prefix**: Produces parameter references with tolerant default handling:
     ...
     *   - Nullable fields with non-NULL defaults use a `_Clear` companion: when
     *     `@<Param>_Clear = 1` the column is explicitly set to NULL; otherwise
     *     `ISNULL(@<Param>, <default>)` applies the database default when omitted.
     *   - Plain nullable fields with no default emit the parameter reference unchanged
     *     (NULL passes through directly).
     *
            ...
            if (prefix !== '' && ef.IsSpecialDateField) {
                ...
                if (!prefix || prefix.length === 0) {
                    ...
                    if (ef.HasDefaultValue && !ef.AllowsNull) {
                        ...
                        const formattedDefault = this.formatDefaultValue(ef.DefaultValue, ef.NeedsQuotes);
                        sVal = `CASE WHEN @${ef.CodeName}_Clear = 1 THEN NULL ELSE ISNULL(${sVal}, ${formattedDefault}) END`;
                    }
                }
                sOutput += sVal;
            }
    ...
    /**
     * Generates the SET clause body for `spUpdate*` with tolerant merge semantics:
     * each column wraps the parameter with `ISNULL(@Param, [Column])` so omitting
     * a parameter preserves the existing row value. Nullable columns whose database
     * default is non-NULL also emit a `_Clear` companion branch:
     * `[Col] = CASE WHEN @<Param>_Clear = 1 THEN NULL ELSE ISNULL(@<Param>, [Col]) END`,
     * letting callers distinguish "leave unchanged" from "explicitly set NULL."
     */
        ...
        for (const ef of entityFields) {
            if (
                ...
                if (this.needsClearCompanion(ef)) {
                    sOutput += `[${ef.Name}] = CASE WHEN @${ef.CodeName}_Clear = 1 THEN NULL ELSE ISNULL(@${ef.CodeName}, [${ef.Name}]) END`;
                } else {
                    sOutput += `[${ef.Name}] = ISNULL(@${ef.CodeName}, [${ef.Name}])`;
                }
            }
        }
        return sOutput;
