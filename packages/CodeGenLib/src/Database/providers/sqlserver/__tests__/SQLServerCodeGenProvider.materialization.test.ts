...
describe('SQLServerCodeGenProvider — materialization DDL', () => {
    ...
    describe('surrogate column types', () => {
        it('synthetic surrogate is an IDENTITY int; keyed surrogate is varchar(64) to match the SHA2_256 hex hash', () => {
            expect(provider.getMaterializedSurrogateColumnType()).toBe('int IDENTITY(1,1)');
            expect(provider.getMaterializedHashSurrogateColumnType()).toBe('varchar(64)');
        });
    });
