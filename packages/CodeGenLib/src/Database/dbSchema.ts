...
export class DBSchemaGeneratorBase {
  ...
  public generateDBSchemaJSONOutput(entities: EntityInfo[], outputDir: string): boolean {
    try {
      if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true }); // create the directory if it doesn't exist
  ...
  protected generateFieldJSON(field: EntityFieldInfo, simpleVersion: boolean): string {
    const relEntity =
      field.RelatedEntity && field.RelatedEntity.length > 0
        ? `\n                "RelatedEntity": "${this.escapeJsonString(field.RelatedEntity)}",`
        : '';
    const relField =
      relEntity && field.RelatedEntityFieldName && field.RelatedEntityFieldName.length > 0
