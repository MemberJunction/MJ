...
${updatedAtTrigger}
    ...
    protected createEntityFieldsInsertString(entity: EntityInfo, entityFields: EntityFieldInfo[], prefix: string, excludePrimaryKey: boolean = false): string {
        ...
        for (let i: number = 0; i < entityFields.length; ++i) {
            ...
            // IMPORTANT: Skip IsSpecialDateField (__mj_CreatedAt, __mj_UpdatedAt, __mj_DeletedAt) because triggers handle these automatically
            if ( (excludePrimaryKey && ef.IsPrimaryKey) || (ef.IsPrimaryKey && autoGeneratedPrimaryKey) || ef.IsVirtual || !ef.AllowUpdateAPI || ef.AutoIncrement || ef.IsSpecialDateField) {
