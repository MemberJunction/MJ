import { EntityInfo, EntityFieldInfo, EntityPermissionInfo, Metadata, UserInfo } from '@memberjunction/global';
...
export class SQLCodeGenBase {
  ...
  public async manageSQLScriptsAndExecution(
    ...
    try {
      ...
      if (configInfo.forceRegeneration?.enabled && configInfo.forceRegeneration?.entityWhereClause) {
        ...
        try {
          ...
          logStatus(
            `Force regeneration filter enabled: ${this.entitiesQualifiedForForcedRegeneration.length} entities qualified based on entityWhereClause: ${whereClause}`
          );
        } catch (error) {
      ...
      const includedEntities = baselineEntities.filter(
        (e) => configInfo.excludeSchemas.find((s) => s.toLowerCase() === e.SchemaName.toLowerCase()) === undefined
      ); //only include entities that are NOT in the excludeSchemas list
      const excludedEntities = baselineEntities.filter(
        (e) => configInfo.excludeSchemas.find((s) => s.toLowerCase() === e.SchemaName.toLowerCase()) !== undefined
      ); //only include entities that ARE in the excludeSchemas list in this array
  ...
  public async generateAndExecuteEntitySQLToSeparateFiles(options: {
    ...
    if (!options.batchSize) options.batchSize = 5; // default to 5 if not specified
    ...
    try {
      ...
      for (let i = 0; i < totalEntities; i += options.batchSize) {
        ...
        const promises = batch.map(async (e) => {
          ...
          if (!pkeyField) {
            logError(
              `SKIPPING ENTITY: Entity ${e.Name}, because it does not have a primary key field defined. A table must have a primary key defined to quality to be a MemberJunction entity`
            );
        ...
        const results = await Promise.all(promises);
        results.forEach((r) => {
          if (!r.Success) bFail = true; // keep going, but will return false at the end
  ...
  public async generateAndExecuteSingleEntitySQLToSeparateFiles(options: {
    ...
    try {
      ...
      } else return { Success: true, Files: files };
    } catch (err) {
      logError(err as string);
      return { Success: false, Files: [] };
    }
  ...
  protected logSQLForNewOrModifiedEntity(entity: EntityInfo, sql: string, description: string, logSql: boolean = false) {
    ...
    if (logSql) {
      ...
      const isNewOrModified =
        !!ManageMetadataBase.newEntityList.find((e) => e === entity.Name) ||
      ...
      const isCascadeDependencyRegeneration =
        description.toLowerCase().includes('spdelete') && this.entitiesNeedingDeleteSPRegeneration.has(entity.ID);

      // Check if force regeneration is enabled for relevant SQL types
      const isForceRegeneration =
        configInfo.forceRegeneration?.enabled &&
  ...
  public async generateSingleEntitySQLToSeparateFiles(options: {
    ...
    try {
      ...
      if (options.writeFiles && !fs.existsSync(options.directory)) fs.mkdirSync(options.directory, { recursive: true });
      ...
      if (options.writeFiles && !fs.existsSync(schemaDirectory)) fs.mkdirSync(schemaDirectory, { recursive: true }); // create the directory if it doesn't exist
      ...
      if (!options.onlyPermissions) {
        const shouldGenerateIndexes =
          autoIndexForeignKeys() || (configInfo.forceRegeneration?.enabled && configInfo.forceRegeneration?.indexes);
        ...
        if (options.writeFiles) {
          const filePath = path.join(
            options.directory,
            this.SQLUtilityObject.getDBObjectFileName('index', options.entity.SchemaName, options.entity.BaseTable, false, true)
          );
          this.logSQLForNewOrModifiedEntity(
            options.entity,
            s,
            'Index for Foreign Keys for ' + options.entity.BaseTable,
            options.enableSQLLoggingForNewOrModifiedEntities
          );
      ...
      if (!options.onlyPermissions && options.entity.BaseViewGenerated && !options.entity.VirtualEntity) {
        ...
        if (recursiveFKs.length > 0) {
          for (const field of recursiveFKs) {
            ...
            const s =
              this.generateSingleEntitySQLFileHeader(options.entity, functionName) + this.generateRootIDFunction(options.entity, field);
            const filePath = path.join(
              options.directory,
              this.SQLUtilityObject.getDBObjectFileName('function', options.entity.SchemaName, functionName, false, true)
            );
            if (options.writeFiles) {
              this.logSQLForNewOrModifiedEntity(
                options.entity,
                s,
                `Root ID Function SQL for ${options.entity.Name}.${field.Name}`,
                options.enableSQLLoggingForNewOrModifiedEntities
              );
              fs.writeFileSync(filePath, s);
              files.push(filePath);
        ...
        const s =
          this.generateSingleEntitySQLFileHeader(options.entity, options.entity.BaseView) +
          (await this.generateBaseView(options.pool, options.entity));
        const filePath = path.join(
          options.directory,
          this.SQLUtilityObject.getDBObjectFileName('view', options.entity.SchemaName, options.entity.BaseView, false, true)
        );
        if (options.writeFiles) {
          this.logSQLForNewOrModifiedEntity(
            options.entity,
            s,
            `Base View SQL for ${options.entity.Name}`,
            options.enableSQLLoggingForNewOrModifiedEntities
          );
          fs.writeFileSync(filePath, s);
          files.push(filePath);
      ...
      const s =
        this.generateSingleEntitySQLFileHeader(options.entity, 'Permissions for ' + options.entity.BaseView) +
        this.generateViewPermissions(options.entity);
      if (s.length > 0) permissionsSQL += s + '\nGO\n';
      if (options.writeFiles) {
        const filePath = path.join(
          options.directory,
          this.SQLUtilityObject.getDBObjectFileName('view', options.entity.SchemaName, options.entity.BaseView, true, true)
        );
        fs.writeFileSync(filePath, s);
        this.logSQLForNewOrModifiedEntity(
          options.entity,
          s,
          `Base View Permissions SQL for ${options.entity.Name}`,
          options.enableSQLLoggingForNewOrModifiedEntities
        );
      ...
      if (!options.entity.BaseViewGenerated) sRet += s + '\nGO\n';
      ...
      if (options.entity.AllowCreateAPI && !options.entity.VirtualEntity) {
        ...
        if (!options.onlyPermissions && options.entity.spCreateGenerated) {
          ...
          if (options.writeFiles) {
            const filePath = path.join(
              options.directory,
              this.SQLUtilityObject.getDBObjectFileName('sp', options.entity.SchemaName, spName, false, true)
            );

            this.logSQLForNewOrModifiedEntity(
              options.entity,
              s,
              `spCreate SQL for ${options.entity.Name}`,
              options.enableSQLLoggingForNewOrModifiedEntities
            );
        ...
        if (options.writeFiles) {
          const filePath = path.join(
            options.directory,
            this.SQLUtilityObject.getDBObjectFileName('sp', options.entity.SchemaName, spName, true, true)
          );

          this.logSQLForNewOrModifiedEntity(
            options.entity,
            s,
            `spCreate Permissions for ${options.entity.Name}`,
            options.enableSQLLoggingForNewOrModifiedEntities
          );
        ...
        // means we already generated the permissions for it above and it is part of sRet already, but we always save it to a file, (per above line)
        if (!options.entity.spCreateGenerated) sRet += s + '\nGO\n';
      }
      ...
      if (options.entity.AllowUpdateAPI && !options.entity.VirtualEntity) {
        ...
        if (!options.onlyPermissions && options.entity.spUpdateGenerated) {
          ...
          if (options.writeFiles) {
            ...
            this.logSQLForNewOrModifiedEntity(
              options.entity,
              s,
              `spUpdate SQL for ${options.entity.Name}`,
              options.enableSQLLoggingForNewOrModifiedEntities
            );
        ...
        if (options.writeFiles) {
          ...
          this.logSQLForNewOrModifiedEntity(
            options.entity,
            s,
            `spUpdate Permissions for ${options.entity.Name}`,
            options.enableSQLLoggingForNewOrModifiedEntities
          );
        ...
        // now, append the permissions to the return string IF we did NOT generate the proc - because if we generated the proc, that
        // means we already generated the permissions for it above and it is part of sRet already, but we always save it to a file, (per above line)
        if (!options.entity.spUpdateGenerated) sRet += s + '\nGO\n';
      }
      ...
      if (options.entity.AllowDeleteAPI && !options.entity.VirtualEntity) {
        ...
        if (
          ...
          if (options.writeFiles) {
            ...
            this.logSQLForNewOrModifiedEntity(
              options.entity,
              s,
              `spDelete SQL for ${options.entity.Name}`,
              options.enableSQLLoggingForNewOrModifiedEntities
            );
        ...
        if (options.writeFiles) {
          ...
          this.logSQLForNewOrModifiedEntity(
            options.entity,
            s,
            `spDelete Permissions for ${options.entity.Name}`,
            options.enableSQLLoggingForNewOrModifiedEntities
          );
        ...
        // now, append the permissions to the return string IF we did NOT generate the proc - because if we generated the proc, that
        // means we already generated the permissions for it above and it is part of sRet already, but we always save it to a file, (per above line)
        if (!options.entity.spDeleteGenerated) sRet += s + '\nGO\n';
      }
      ...
      if (options.entity.FullTextSearchEnabled || (configInfo.forceRegeneration?.enabled && configInfo.forceRegeneration?.fullTextSearch)) {
        ...
        if (!options.onlyPermissions) {
          ...
          const filePath = path.join(
            options.directory,
            this.SQLUtilityObject.getDBObjectFileName(
              'full_text_search_function',
              options.entity.SchemaName,
              options.entity.BaseTable,
              false,
              true
            )
          );
          if (options.writeFiles) {
            this.logSQLForNewOrModifiedEntity(
              options.entity,
              ft.sql,
              `Full Text Search SQL for ${options.entity.Name}`,
              options.enableSQLLoggingForNewOrModifiedEntities
            );
        ...
        if (sP.length > 0) permissionsSQL += sP + '\nGO\n';
        ...
        if (options.writeFiles) {
          this.logSQLForNewOrModifiedEntity(
            options.entity,
            sP,
            `Full Text Search Permissions for ${options.entity.Name}`,
            options.enableSQLLoggingForNewOrModifiedEntities
          );
        ...
        // means we already generated the permissions for it above and it is part of sRet already, but we always save it to a file, (per above line)
        if (!options.entity.FullTextSearchFunctionGenerated) sRet += sP + '\nGO\n';
      }
  ...
  public async generateEntitySQL(pool: sql.ConnectionPool, entity: EntityInfo): Promise<string> {
    ...
    else sOutput += this.generateViewPermissions(entity) + '\n\n';
    ...
    if (entity.AllowCreateAPI && !entity.VirtualEntity) {
      ...
      else sOutput += this.generateSPPermissions(entity, entity.spCreate, SPType.Create) + '\n\n';
    }
    ...
    if (entity.AllowUpdateAPI && !entity.VirtualEntity) {
      ...
      // custom SP, still generate the permissions
      else sOutput += this.generateSPPermissions(entity, entity.spUpdate, SPType.Update) + '\n\n';
    }
    ...
    if (entity.AllowDeleteAPI && !entity.VirtualEntity) {
      ...
      // custom SP, still generate the permissions
      else sOutput += this.generateSPPermissions(entity, entity.spDelete, SPType.Delete) + '\n\n';
    }
  ...
  async generateEntityFullTextSearchSQL(pool: sql.ConnectionPool, entity: EntityInfo): Promise<{ sql: string; functionName: string }> {
    ...
    const catalogName =
      entity.FullTextCatalog && entity.FullTextCatalog.length > 0 ? entity.FullTextCatalog : dbDatabase + '_FullTextCatalog';
    ...
    if (entity.FullTextSearchFunctionGenerated) {
      ...
      if (fullTextFieldsSimple.length === 0)
        throw new Error(
          `FullTextSearchFunctionGenerated is true for entity ${entity.Name}, but no fields are marked as FullTextSearchEnabled`
        );
      if (!entity.FullTextSearchFunction || entity.FullTextSearchFunction.length === 0) {
        ...
        if (!u) throw new Error('Could not find the first user in the cache, cant generate the full text search function without a user');

        const e = <EntityEntity>await md.GetEntityObject('Entities', u);
        await e.Load(entity.ID);
        e.FullTextSearchFunction = functionName;
        if (!(await e.Save())) throw new Error(`Could not update the FullTextSearchFunction for entity ${entity.Name}`);
      }
  ...
  async getEntityPrimaryKeyIndexName(pool: sql.ConnectionPool, entity: EntityInfo): Promise<string> {
    ...
    if (result && result.length > 0) return result[0].IndexName;
    else throw new Error(`Could not find primary key index for entity ${entity.Name}`);
  }
  ...
  generateIndexesForForeignKeys(pool: sql.ConnectionPool, entity: EntityInfo): string {
    ...
    for (const f of entity.Fields) {
      if (f.RelatedEntity && f.RelatedEntity.length > 0) {
        ...
        if (indexName.length > MAX_INDEX_NAME_LENGTH) indexName = indexName.substring(0, MAX_INDEX_NAME_LENGTH); // truncate to max length if necessary
...
${whereClause}GO${permissions}
  ...
  async generateBaseViewRelatedFieldsString(pool: sql.ConnectionPool, entityFields: EntityFieldInfo[]): Promise<string> {
    ...
    for (const ef of qualifyingFields) {
      ...
      if (nameField !== '') {
        ...
        const bFound =
          entityFields.find((f) => f.IsVirtual === false && f.Name.trim().toLowerCase() === candidateName.trim().toLowerCase()) !==
          undefined;
        if (bFound) ef._RelatedEntityNameFieldMap = candidateName + '_Virtual';
        else ef._RelatedEntityNameFieldMap = candidateName;
        ...
        if (
          ef.RelatedEntityNameFieldMap === null ||
  ...
  protected getIsNameFieldForSingleEntity(entityName: string): { nameField: string; nameFieldIsVirtual: boolean } {
    ...
    if (e) {
      ...
      if (e.NameField) return { nameField: ef.Name, nameFieldIsVirtual: ef.IsVirtual };
    } else logStatus(`ERROR: Could not find entity with name ${entityName}`);
  ...
  protected generateSPPermissions(entity: EntityInfo, spName: string, type: SPType): string {
    ...
    for (let i: number = 0; i < entity.Permissions.length; i++) {
      const ep: EntityPermissionInfo = entity.Permissions[i];
      if ((type == SPType.Create && ep.CanCreate) || (type == SPType.Update && ep.CanUpdate) || (type == SPType.Delete && ep.CanDelete)) {
        if (ep.RoleSQLName && ep.RoleSQLName.length > 0) {
...
BEGIN
    SET NOCOUNT ON;
    ${preInsertCode}${
      preInsertCode.includes('INSERT INTO')
        ? ''
        : `
    INSERT INTO
...
GO`;
  ...
  protected generateSPUpdate(entity: EntityInfo): string {
    ...
    const hasUpdatedAtField: boolean =
      entity.Fields.find((f) => f.Name.toLowerCase().trim() === EntityInfo.UpdatedAtFieldName.trim().toLowerCase()) !== undefined;
...
${updatedAtTrigger}
  ...
  protected createEntityFieldsInsertString(
    entity: EntityInfo,
    entityFields: EntityFieldInfo[],
    prefix: string,
    excludePrimaryKey: boolean = false
  ): string {
    ...
    for (let i: number = 0; i < entityFields.length; ++i) {
      ...
      if (prefix !== '' && ef.IsSpecialDateField) {
        ...
        else sOutput += `NULL`; // we don't set the deleted at field on an insert, only on a delete
...
GO${permissions}
  ...
  protected generateCascadeCursorOperation(
  ...
  protected buildPrimaryKeyComponents(
    entity: EntityInfo,
    prefix: string = ''
  ): {
  ...
  protected buildUpdateCursorParameters(
    entity: EntityInfo,
    _fkField: EntityFieldInfo,
    prefix: string = ''
  ): {
    declarations: string;
    selectFields: string;
  ...
  protected async getModifiedEntitiesWithUpdateAPI(entities: EntityInfo[]): Promise<Map<string, string>> {
    ...
    for (const entityName of modifiedEntityNames) {
      ...
      if (entity) {
        ...
        if (nonUpdateEntity) {
          logStatus(
            `  - ${entityName} found but AllowUpdateAPI=${nonUpdateEntity.AllowUpdateAPI}, spUpdateGenerated=${nonUpdateEntity.spUpdateGenerated}`
          );
        } else {
   ...
   */
  protected async markEntitiesForCascadeDeleteRegeneration(pool: sql.ConnectionPool, entities: EntityInfo[]): Promise<void> {
    try {
  ...
  protected orderEntitiesByDependencies(entities: EntityInfo[], entityIdsToOrder: Set<string>): string[] {
    ...
    if (circularDeps.size > 0) {
      logStatus(
        `Note: ${circularDeps.size} entities have circular cascade delete dependencies and will be regenerated in arbitrary order.`
      );
    }
