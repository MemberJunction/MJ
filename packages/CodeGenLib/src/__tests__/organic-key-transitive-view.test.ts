...
describe('generateCreateOrReplaceViewSQL', () => {
  describe('PostgreSQL', () => {
    // lts/5's PostgreSQL dialect leaves an all-lowercase schema unquoted: hr."vw…" is equivalent to "hr"."vw…".
...
describe('migration log — a view unit is alone in its batch', () => {
  ...
  beforeEach(() => {
    ...
    // lts/5's SQLLogging has no test hooks; point the logger at the temp file through its public getter.
    vi.spyOn(SQLLogging, 'SQLLoggingFilePath', 'get').mockReturnValue(logFile);
  });
