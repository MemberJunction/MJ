...
GO${permissions}
    ...
    protected generateUpdatedAtTrigger(entity: EntityInfo): string {
        const updatedAtField = entity.Fields.find(f => f.Name.toLowerCase().trim() === ManageMetadataBase.UpdatedAtFieldName);
        if (!updatedAtField)
            return '';

        const triggerStatement = `
------------------------------------------------------------
----- TRIGGER FOR ${ManageMetadataBase.UpdatedAtFieldName} field for the ${entity.BaseTable} table
------------------------------------------------------------
...
BEGIN
    ...
    UPDATE 
        [${entity.SchemaName}].[${entity.BaseTable}]
    SET 
        ${ManageMetadataBase.UpdatedAtFieldName} = GETUTCDATE()
    FROM 
...
GO`;
    ...
    protected generateSPUpdate(entity: EntityInfo): string {
        ...
        const hasUpdatedAtField: boolean = entity.Fields.find(f => f.Name.toLowerCase().trim() === ManageMetadataBase.UpdatedAtFieldName.trim().toLowerCase()) !== undefined;
    ...
    protected createEntityFieldsParamString(entityFields: EntityFieldInfo[], isUpdate: boolean): string {
        ...
        for (let i: number = 0; i < entityFields.length; ++i) {
            ...
            if (
              ...
              ef.Name.toLowerCase().trim() !== ManageMetadataBase.UpdatedAtFieldName.trim().toLowerCase() &&
              ef.Name.toLowerCase().trim() !== ManageMetadataBase.CreatedAtFieldName.trim().toLowerCase() 
            ) {
              if (!isFirst) sOutput += ',\n    ';
              else isFirst = false;
    ...
    protected createEntityFieldsInsertString(entity: EntityInfo, entityFields: EntityFieldInfo[], prefix: string): string {
        ...
        for (let i: number = 0; i < entityFields.length; ++i) {
            ...
            if ((!ef.IsPrimaryKey || !autoGeneratedPrimaryKey) && 
                ...
                if (prefix !== '' && (ef.Name.toLowerCase().trim() === ManageMetadataBase.UpdatedAtFieldName.trim().toLowerCase() || 
                                      ef.Name.toLowerCase().trim() === ManageMetadataBase.CreatedAtFieldName.trim().toLowerCase())
                   )
                    sOutput += `GETUTCDATE()`;
                else {
    ...
    protected createEntityFieldsUpdateString(entityFields: EntityFieldInfo[]): string {
        ...
        for (let i: number = 0; i < entityFields.length; ++i) {
            ...
            if (!ef.IsPrimaryKey && 
                ...
                ef.AllowUpdateAPI && 
                ef.AutoIncrement === false && 
                ef.Name.toLowerCase().trim() !== ManageMetadataBase.CreatedAtFieldName.trim().toLowerCase() && // do not update the CreatedAt field
                ef.Name.toLowerCase().trim() !== ManageMetadataBase.UpdatedAtFieldName.trim().toLowerCase() && // do not update the UpdatedAt field - done via trigger
