...
BEGIN
    ...
    [${entity.SchemaName}].[${entity.BaseTable}]
        (
            ${createEntityFieldsInsertString(entity, entity.Fields, '')}
        )
    VALUES
        (
            ${createEntityFieldsInsertString(entity, entity.Fields, '@')}
...
function createEntityFieldsParamString(entityFields: EntityFieldInfo[], includePrimaryKey: boolean): string {
    let sOutput: string = '', isFirst: boolean = true;
    for (let i: number = 0; i < entityFields.length; ++i) {
        const ef: EntityFieldInfo = entityFields[i];
        if ( (ef.AllowUpdateAPI || (ef.IsPrimaryKey && includePrimaryKey)) && 
            !ef.IsVirtual && 
            (!ef.IsPrimaryKey || includePrimaryKey) && 
