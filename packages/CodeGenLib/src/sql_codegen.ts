...
function generateSPCreate(entity: EntityInfo): string {
    ...
    const efString: string = createEntityFieldsParamString(entity.Fields, false);
...
BEGIN
    ...
    SELECT * FROM [${entity.SchemaName}].[${entity.BaseView}] WHERE ID = SCOPE_IDENTITY()
END
GO${permissions}
`
}
...
function generateSPUpdate(entity: EntityInfo): string {
    const spName: string = entity.spDelete ? entity.spDelete : `spUpdate${entity.ClassName}`;
...
BEGIN
    ...
    SELECT * FROM [${entity.SchemaName}].${entity.BaseView} WHERE ID = @ID
END
GO${permissions}
    `
}

function createEntityFieldsParamString(entityFields: EntityFieldInfo[], includeID: boolean): string {
...
function createEntityFieldsInsertString(entityFields: EntityFieldInfo[], prefix: string): string {
    let sOutput: string = '', isFirst: boolean = true;
    for (let i: number = 0; i < entityFields.length; ++i) {
        const ef: EntityFieldInfo = entityFields[i];
        if (ef.Name !== 'ID' && 
