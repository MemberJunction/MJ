...
export async function runCommands(commands: CommandInfo[]): Promise<CommandExecutionResult[]>{
  try {
...
export async function runCommand(command: CommandInfo ): Promise<CommandExecutionResult> {
  let cp: ChildProcess;
  try {
    let output = '';
