import { EntityFieldInfo, EntityFieldValueListType, EntityInfo, Metadata, TypeScriptTypeFromSQLType } from '@memberjunction/global';
import fs from 'fs';
...
export class EntitySubClassGeneratorBase {
  ...
  public generateEntitySubClassFileHeader(): string {
    return `import { BaseEntity, EntitySaveOptions, EntityDeleteOptions, CompositeKey, ValidationResult, ValidationErrorInfo, ValidationErrorType, Metadata, ProviderType, DatabaseProviderBase } from "@memberjunction/global";
...
export const loadModule = () => {
   ...
   */
  public async generateEntitySubClass(
    pool: sql.ConnectionPool,
    entity: EntityInfo,
    includeFileHeader: boolean = false,
    ...
    if (entity.PrimaryKeys.length === 0) {
      ...
      const fields: string = sortedFields
        .map((e) => {
          let values: string = '';
...
`;
  ...
  public GenerateValidateFunction(entity: EntityInfo): null | { code: string; validators: ValidatorResult[] } {
    ...
    const unsortedValidators = ManageMetadataBase.generatedValidators.filter(
      (f) => f.entityName.trim().toLowerCase() === entity.Name.trim().toLowerCase()
    );
    ...
    if (validators.length === 0) {
      ...
      const validationFunctions = validators
        .map((f) => {
          ...
          const formattedText = cleansedText
            .split('\n')
            .map((l) => `    ${l}`)
            .join('\n');
