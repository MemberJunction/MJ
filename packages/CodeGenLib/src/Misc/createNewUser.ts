import { ApplicationInfo, LogError, LogStatus, Metadata, RunView, RunViewResult, UserInfo } from '@memberjunction/global';
...
export class CreateNewUserBase {
  ...
  ): Promise<{ Success: boolean; Message: string; Severity: 'warning' | 'error' | undefined }> {
    try {
            ...
            if (await user.Save()) {
              ...
              if (newUserSetup.CreateUserApplicationRecords) {
                ...
                for (const appName of newUserSetup.UserApplications) {
                  ...
                  const application: ApplicationInfo | undefined = md.Applications.find(
                    (a) => a.Name.trim().toLocaleLowerCase() === toLowerCase
                  );
                  if (!application) {
                  ...
                  const userApplication: UserApplicationEntity = await md.GetEntityObject<UserApplicationEntity>(
                    'User Applications',
                    currentUser
                  );
                  ...
                  if (userApplicationSaveResult) {
                    ...
                    for (const [index, appEntity] of rvResult.Results.entries()) {
                      ...
                      if (userAppEntitySaveResult) {
                        ...
                        LogError(
                          `Failed to create User Application Entity for new user ${user.Name}:`,
                          undefined,
                          userAppEntity.LatestResult
                        );
                      }
                    }
                  } else {
                    logError(
                      `Failed to create User Application ${appName} for new user ${user.Name}:`,
                      undefined,
                      userApplication.LatestResult
                    );
                  }
                }
              }
