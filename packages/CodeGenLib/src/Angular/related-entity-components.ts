import { EntityFieldInfo, EntityInfo, EntityRelationshipInfo, Metadata, UserInfo } from '@memberjunction/global';
...
export abstract class RelatedEntityDisplayComponentGeneratorBase {
  ...
  protected GetForeignKey(entityName: string, relatedEntityName: string): EntityFieldInfo {
    ...
    if (!e) throw new Error('Could not find entity ' + entityName);

    // now find the field in e that matches the related entity
    const field = e.Fields.find((f) => f.RelatedEntity === relatedEntityName);
    if (!field) throw new Error('Could not find a foreign key field in entity ' + entityName + ' that links to ' + relatedEntityName);
  ...
  public static async GetComponent(
    ...
    ...params: any[]
  ): Promise<RelatedEntityDisplayComponentGeneratorBase> {
    ...
    if (relationshipInfo.DisplayComponentID && relationshipInfo.DisplayComponentID.length > 0) {
      ...
      const component = TypeTablesCache.Instance.EntityRelationshipDisplayComponents.find(
        (x) => x.ID === relationshipInfo.DisplayComponentID
      );
      if (component) {
        key = component.Name;
      } else throw new Error(`Could not find the DisplayComponent with ID ${relationshipInfo.DisplayComponentID}`);
    }
    ...
    if (instance) {
      ...
      const component = MJGlobal.Instance.ClassFactory.CreateInstance<RelatedEntityDisplayComponentGeneratorBase>(
   ...
   */
  private static _componentInstanceMap: Map<string, RelatedEntityDisplayComponentGeneratorBase> = new Map<
    string,
    RelatedEntityDisplayComponentGeneratorBase
  >();
