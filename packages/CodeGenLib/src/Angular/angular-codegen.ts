...
export class AngularClientGeneratorBase {
   ...
   */
  public async generateAngularCode(
    entities: EntityInfo[],
    directory: string,
    modulePrefix: string,
    ...
    try {
      ...
      if (!fs.existsSync(entityPath)) fs.mkdirSync(entityPath, { recursive: true }); // create the directory if it doesn't exist
      ...
      for (let i: number = 0; i < entities.length; ++i) {
        ...
        if (entity.PrimaryKeys && entity.PrimaryKeys.length > 0 && entity.IncludeInAPI) {
          ...
          if (!fs.existsSync(thisEntityPath)) fs.mkdirSync(thisEntityPath, { recursive: true }); // create the directory if it doesn't exist

          const { htmlCode, additionalSections, relatedEntitySections } = await this.generateSingleEntityHTMLForAngular(
            entity,
            contextUser
          );
          ...
          if (additionalSections.length > 0) {
            ...
            if (!fs.existsSync(sectionPath)) fs.mkdirSync(sectionPath, { recursive: true }); // create the directory if it doesn't exist
          ...
          componentImports.push(
            `import { ${componentName}, Load${componentName} } from "./Entities/${entity.ClassName}/${entity.ClassName.toLowerCase()}.form.component";`
          );
          ...
          relatedEntitySections.forEach((s) => {
            ...
            s.GeneratedOutput!.Component!.ImportItems.forEach((i) => {
              if (!match.modules.includes(i.ModuleName)) match.modules.push(i.ModuleName);
            });
          });
          ...
          logStatus(
            `   Entity ${entity.Name} does not have a primary key or is not included in the API, skipping code generation for this entity`
          );
        }
      }
   ...
   */
  protected generateAngularModule(
...
${sections
  .filter((s) => !s.IsRelatedEntity && s.ClassName && s.EntityClassName && s.FileNameWithoutExtension)
  .map((s) => `import { ${s.ClassName}, Load${s.ClassName} } from "./Entities/${s.EntityClassName}/sections/${s.FileNameWithoutExtension}"`)
  .join('\n')}
${relatedEntityModuleImports
  .filter((remi) => remi.library.trim().toLowerCase() !== '@memberjunction/ng-user-view-grid')
...
export function Load${modulePrefix}GeneratedForms() {
    ...
    ${sections
      .filter((s) => !s.IsRelatedEntity && s.ClassName)
      .map((s) => `Load${s.ClassName}();`)
      .join('\n    ')}
}
    `;
  }
   ...
   */
  protected generateAngularModuleCode(
    componentNames: { componentName: string; relatedEntityItemsRequired: { itemClassName: string; moduleClassName: string }[] }[],
    sections: AngularFormSectionInfo[],
  ...
  protected generateSingleEntityTypeScriptForAngular(
    ...
    const sectionImports: string =
      additionalSections.length > 0
        ? additionalSections.map((s) => `import { Load${s.ClassName} } from "./sections/${s.FileNameWithoutExtension}"`).join('\n')
        : '';

    // next, build a list of distinct imports at the library level and for components within the library
    const libs: { lib: string; items: string[] }[] =
      relatedEntitySections.length > 0
        ? relatedEntitySections
            .filter((s) => s.GeneratedOutput && s.GeneratedOutput.Component && s.GeneratedOutput.Component.ImportPath)
            .map((s) => {
              return {
    ...
    libs.forEach((l) => {
      if (!distinctLibs.find((ll) => ll.lib === l.lib)) distinctLibs.push(l);
    });
    ...
    relatedEntitySections
      .filter((s) => s.GeneratedOutput && s.GeneratedOutput.Component && s.GeneratedOutput.Component.ImportItems)
      .forEach((s) => {
        const lib = distinctLibs.find((l) => l.lib === s.GeneratedOutput!.Component!.ImportPath);
        if (lib) {
          s.GeneratedOutput!.Component!.ImportItems.forEach((i) => {
            if (!lib.items.includes(i.ClassName)) lib.items.push(i.ClassName);
          });
        }
      });
    ...
    const generationInjectedCode: string =
      relatedEntitySections.length > 0
        ? relatedEntitySections
            .filter((s) => s.GeneratedOutput && s.GeneratedOutput!.CodeOutput!.length > 0)
            .map((s) =>
              s
                .GeneratedOutput!.CodeOutput!.split('\n')
                .map((l) => `    ${l}`)
                .join('\n')
            )
            .join('\n')
...
`;
  ...
  protected generateTopAreaHTMLForAngular(entity: EntityInfo): string {
    if (!this.entityHasTopArea(entity)) return '';
    else
  ...
  protected generateAngularAdditionalSections(entity: EntityInfo, startIndex: number): AngularFormSectionInfo[] {
    ...
    for (const field of sortedFields) {
      if (field.IncludeInGeneratedForm) {
        if (
          field.GeneratedFormSectionType === GeneratedFormSectionType.Category &&
          field.Category &&
          field.Category !== '' &&
          field.IncludeInGeneratedForm
        )
    ...
    for (const section of sections) {
      ...
      if (section.Type === GeneratedFormSectionType.Top) {
        ...
        if (section.Type === GeneratedFormSectionType.Category) sectionName = this.stripWhiteSpace(section.Name.toLowerCase());
        else if (section.Type === GeneratedFormSectionType.Details) sectionName = 'details';
...
export function Load${entity.ClassName}${this.stripWhiteSpace(section.Name)}Component() {
    // does nothing, but called in order to prevent tree-shaking from eliminating this component from the build
      ...
      if (section.Type !== GeneratedFormSectionType.Top) index++; // don't increment the tab index for TOP AREA, becuse it won't be rendered as a tab
    }
    ...
    for (const field of sortedFields) {
      if (field.IncludeInGeneratedForm) {
        ...
        if (field.GeneratedFormSectionType === GeneratedFormSectionType.Top && section.Type === GeneratedFormSectionType.Top) {
          ...
          field.Category &&
          section.Name &&
          field.Category.trim().toLowerCase() === section.Name.trim().toLowerCase()
        ) {
          // match, include the field in the output
          bMatch = true;
        } else if (
          field.GeneratedFormSectionType === GeneratedFormSectionType.Details &&
    ...
    for (const field of section.Fields) {
      ...
      if (!field.ReadOnly) {
        ...
        if (field.ValueListTypeEnum !== EntityFieldValueListType.None) {
          ...
          if (field.TSType === EntityFieldTSType.Boolean) editControl = `checkbox`;
          else if (field.TSType === EntityFieldTSType.Date) editControl = `datepicker`;
          else if (field.TSType === EntityFieldTSType.Number) editControl = `numerictextbox`;
          else if (field.TSType === EntityFieldTSType.String) {
            if (field.Length < 0 || field.MaxLength > 100)
              // length < 0 means nvarchar(max) or similar, so use textarea
              editControl = `textarea`;
            else editControl = `textbox`;
          }
        }
      }
...
`;
    ...
    if (relatedEntity.DisplayName && relatedEntity.DisplayName.length > 0) {
      ...
      if (relationships.length > 1) {
        ...
        if (fkeyField) {
          ...
          if (f) tabName += ` (${f.DisplayNameOrName})`;
        }
      }
      return tabName;
    }
   ...
   */
  protected async generateRelatedEntityTabs(
    entity: EntityInfo,
    startIndex: number,
    contextUser: UserInfo
  ): Promise<AngularFormSectionInfo[]> {
    const md = new Metadata();
    ...
    for (const relatedEntity of sortedRelatedEntities) {
      ...
      switch (relatedEntity.DisplayIconType) {
        ...
        case 'Related Entity Icon':
          ...
          if (re && re.Icon && re.Icon.length > 0) icon = `<span class="${re.Icon} tab-header-icon"></span>`;
          break;
        default:
          // none
          break;
...
${componentCodeWithTabs}                    
  ...
  protected async generateSingleEntityHTMLForAngular(
    entity: EntityInfo,
    contextUser: UserInfo
  ): Promise<{ htmlCode: string; additionalSections: AngularFormSectionInfo[]; relatedEntitySections: AngularFormSectionInfo[] }> {
    ...
    const htmlCode =
      topArea.length > 0
        ? this.generateSingleEntityHTMLWithSplitterForAngular(topArea, additionalSections, relatedEntitySections)
   ...
   */
  protected generateSingleEntityHTMLWithSplitterForAngular(
    topArea: string,
    additionalSections: AngularFormSectionInfo[],
    relatedEntitySections: AngularFormSectionInfo[]
  ): string {
...
${this.innerTabStripHTML(additionalSections, relatedEntitySections)}
  ...
  protected innerTopAreaHTML(topArea: string): string {
    if (topArea.trim().length === 0) return '';
    else
  ...
  protected innerTabStripHTML(additionalSections: AngularFormSectionInfo[], relatedEntitySections: AngularFormSectionInfo[]): string {
    ...
    return `                <mj-tabstrip (TabSelected)="onTabSelect($event)"  (ResizeContainer)="InvokeManualResize()">
                    ...
                    ${
                      additionalSections
                        ? additionalSections
                            .filter((s) => s.Type !== GeneratedFormSectionType.Top)
                            .map((s) => s.TabCode)
                            .join('\n               ')
                        : ''
                    }
   ...
   */
  protected generateSingleEntityHTMLWithOUTSplitterForAngular(
    topArea: string,
    additionalSections: AngularFormSectionInfo[],
    relatedEntitySections: AngularFormSectionInfo[]
  ): string {
