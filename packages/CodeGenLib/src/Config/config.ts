...
/**
 * `excludeTables` accepts the historical `{ schema, table }` object **or** a
 * `'schema.table'` / table-only string (`'%.%History'`, `'%Audit%'`). Strings
 * are normalized to the object form at parse time — see `parseExcludeTableEntry`.
 */
...
const fileEmitSchema = z.object({
  ...
  perSchema: z.boolean().default(true),
  /** Skip the disk write when generated bytes are identical. Default true. */
  writeIfChanged: z.boolean().default(true),
   ...
   */
  dirtySchemaOnly: z.boolean().default(true),
  /**
   * SQL Server per-entity SQL generation width. PostgreSQL stays serial (1)
   * regardless — catalog deadlocks under parallel phased DDL. Default 8.
   */
  sqlEntityBatchSize: z.coerce.number().int().positive().default(8),
}).default({
  perSchema: true,
  writeIfChanged: true,
  parallel: true,
...
const schemaOutputOverrideSchema = z.object({
  /** Exact schema name or `%` wildcard (`bsd_%`). */
  schema: z.string().min(1),
  EntitySubClasses: z.string().optional(),
  GraphQLServer: z.string().optional(),
  Angular: z.string().optional(),
  skip: z.array(z.enum(['EntitySubClasses', 'GraphQLServer', 'Angular', 'SQL'])).optional(),
});

export type FileEmitConfig = z.infer<typeof fileEmitSchema>;
export type SchemaOutputOverrideConfig = z.infer<typeof schemaOutputOverrideSchema>;
...
const configInfoSchema = z.object({
  ...
  excludeTables: excludeTableEntrySchema.array().default([
    { schema: '%', table: 'sys%' },
   ...
   */
  fileEmit: fileEmitSchema,
  /**
   * Route (or skip) generated artifacts for matching schemas to a directory
   * other than the default `output.*` entry. First match wins.
   */
  schemaOutput: schemaOutputOverrideSchema.array().optional(),
