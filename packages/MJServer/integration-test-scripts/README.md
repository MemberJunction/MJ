...
> **Transport doctrine — client-first (target state).** Integration checks should exercise the stack **through the client layer** — `GraphQLDataProvider` and the other non-visual client-side objects in `@memberjunction/graphql-dataprovider` (NOT the Angular UI) — against a running MJAPI. Driving BaseEntity CRUD, `RunView`/`RunViews`/`RunQuery`, and Remote Operations over the real GraphQL wire exercises serialization, the resolver auth/scope layer, field mapping, and transport framing, catching a class of bugs pure in-process server calls never touch. **Server-side in-process invocation (`bootstrapIntegrationServer`) is the exception**, used only where there is no client surface — server-cache instrumentation (counting the *server's* `LocalCacheManager`), internal provider-SQL-shape probes, or raw `sys.*`/`information_schema` audits. When an otherwise server-only capability needs coverage, **prefer adding a typed Remote Operation** (per [REMOTE_OPERATIONS_GUIDE](../../../guides/REMOTE_OPERATIONS_GUIDE.md) / [TRANSPORT_LAYER_ARCHITECTURE_GUIDE](../../../guides/TRANSPORT_LAYER_ARCHITECTURE_GUIDE.md)) and invoking it client-side over the wire — that makes it wire-testable *and* agent/workflow-invocable. Today most bundles are `bootstrapIntegrationServer` (in-process); migrating them to client transport where a client path exists is tracked in [plans/integration-test-expansion](../../../plans/integration-test-expansion/README.md). Client transport requires MJAPI running (`bootstrapIntegrationClient` fails fast if it isn't).
...
| `view-execution-tests.ts` | **Deterministic · CLIENT-FIRST (needs a live MJAPI; skips cleanly if unreachable)** — the Viewing System (Domain 11) DET tier over the real GraphQL wire, fully **read-only** (discovers its universe from existing `MJ: Entity Fields` rows; creates/updates/deletes nothing): V1 dynamic `{EntityName, ExtraFilter}` returns exactly the matching rows (PK **set equality**, not counts); V2 Filter-JSON → WHERE compile (nested groups parenthesized, operators, quoting by field type) through the real `MJUserViewEntityExtended` path on a never-saved view; V3 ExtraFilter injection guard (`;`/`--`/`/* */`/UNION rejected, forbidden words **inside a string literal** accepted); V4 `Fields` projection = requested subset **plus PK**, no extra columns; V9 OFFSET pagination union == full set with no dup/gap; V10 keyset (`AfterKey`) walk returns every row once and ends on a short page; V11 keyset on a **composite-PK** entity is refused (never silently degrades to OFFSET); V12 MaxRows / IgnoreMaxRows / entity `UserViewMaxRows` fallback; V13 aggregates ignore pagination but honor the WHERE. MUT (V5–V8, V16, V17) and LIVE (V18) are a later wave; V14/V15 need a second authenticated identity and stay with `rls-isolation` (9 tests) |
| `app-wiring-tests.ts` | **Deterministic · CLIENT-FIRST (needs a live MJAPI; skips cleanly if unreachable)** — the "every shipped app is wired correctly" contract (Domain 12), **parameterized over ALL applications** so new apps inherit it automatically, fully **read-only**. AW1: every Application row loads and the provider metadata cache matches the entity table exactly (no missing, no stale). AW2: `DefaultNavItems` is valid JSON and every item declares Label + ResourceType (+ DriverClass when `ResourceType='Custom'`). AW3: each Active app with nav items declares **exactly one** `isDefault` tab. AW4: DriverClass values are non-empty and **globally unique** across all apps — the catalog's latent risk #1 (two apps resolving the same component). AW5: non-null `Path` values are globally unique (slug uniqueness). AW6: every `MJ: Application Entities` link resolves to a real app **and** a real entity. AW7: every `MJ: Application Roles` grant resolves and **CanAdmin ⇒ CanAccess**. AW8: every app-scoped `MJ: Application Settings` row points at a real application (null ApplicationID = global, valid). AW9: every app's `AgentSettings` `DefaultAgentID`/`RelevantAgents` resolve to real `MJ: AI Agents`. AW10: non-Active (Deprecated/Disabled) apps are excluded from the Active set and never flagged `DefaultForNewUser`. Every collection-iterating check first asserts the collection is non-empty, so a failed load cannot pass vacuously. **G5 is NOT here** — DriverClass→Angular `@RegisterClass` resolution is invisible to the server, so it ships as the static gate [`.github/scripts/check-driverclass-registrations.sh`](../../../.github/scripts/check-driverclass-registrations.sh) (10 tests) |
| `permission-engine-tests.ts` | **Deterministic · CLIENT-FIRST (needs a live MJAPI; skips cleanly if unreachable)** — the unified permission model (Domain 3), read-only except the gated PE11/PE12. Every check is a **DENY**, a **DIFFERENCE** between two identities/paths, or a **SHAPE** assertion — never an allow-only result a high-privilege context user satisfies vacuously. PE1: every ACTIVE `MJ: Permission Domains` row ClassFactory-resolves a provider whose `DomainName` matches (catches a tree-shaken or key-drifted provider silently vanishing). PE2: every provider conforms to the normalized `PermissionAction`/`GranteeType` vocabulary. PE3: catalog row actions/grantees match the provider class metadata — drift detector. **PE3b**: the `SupportsDeny` leg, asserted **asymmetrically** — over-advertising Deny is a hard failure (an operator's Deny would be silently ignored), under-advertising is a warning; currently warns on `Entity Permissions` (provider honors Deny, catalog row declares `SupportsDeny: false`). PE4: an unknown domain fails **CLOSED** on all 7 actions with a Reason that names it; rows `[]`. PE5: the unified agent provider refuses a null (domain-wide) resource and denies a stranger id on all 4 actions. **PE6/PE7**: the **two-access-path contract** for Agents and Skills — the cached runtime helper is **OPEN** by default (View+Run for a non-owner) while the unified provider over the **same table** is **CLOSED** (`[]`, Execute denied); the asymmetry is BY DESIGN (guide §3) and the *divergence itself* is the assertion. PE8: any grant row switches the open default **off** for non-matching users, and `Delete⇒Edit⇒Run⇒View` collapses **downward only** (a Run grant never escalates to Edit/Delete) — exercised through the exported pure core with unsaved synthetic rows, zero mutation. **PE9/PE10** (two REAL identities): the seeded role-less `it-nogrant@integration.test` has **no** entity CRUD permission where the context user does, and can execute **zero** Authorizations. PE11/PE12 (mutation, `RUN_MUTATION_TESTS=1`): an unregistered provider class is contained (real domains intact, nothing granted), and an **async**-throwing provider does not crash the `GetAllUserPermissions` fan-out. **PE13 (mutation) is KNOWN-RED by design** — it pins a real defect this bundle found: `ClassFactory.CreateInstance` returns the *abstract base* (not null) for an unknown key, so `PermissionEngine.instantiateProviders()`'s "skip unresolvable domain" branch is dead code and installs a method-less stub; `GetAllUserPermissions` then throws **synchronously inside `.map()`**, before `Promise.allSettled` can isolate it, so one bad catalog row rejects the whole aggregate for every user. Left failing deliberately (a fix is a human decision); mutation-tier, so the default gate is unaffected. Checks that need a discoverable shape (zero-grant non-owned agent/skill, the seeded user, a non-empty Authorization catalog) **skip-as-pass with a loud warning** rather than assert vacuously. Each mutation check creates + deletes its own catalog row in its own try/finally, with a lifecycle Teardown sweep as backstop (14 tests) |
| `entity-writes-tests.ts` | **Deterministic · CLIENT-FIRST (needs a live MJAPI; skips cleanly if unreachable)** — the core data **write-side** contract (Domain 2) over the real GraphQL wire. Six of the eight checks mutate and are gated behind `RUN_MUTATION_TESTS=1`; they create + tear down their own throwaway `MJ: Action Categories` / `MJ: Lists` rows (per-run name prefix + "(mj-integration-test — safe to delete)") and never touch a pre-existing record. **EW1** (CD1): create + 2 *targeted* updates produce exactly 3 Record Changes — 1 Create, 2 Update — identified by CONTENT not position, with exact before/after `ChangesJSON` (`Description` null→value, `Status` Active→Disabled), and `MJ: Record Changes` writes **no** change rows about itself. (Note: `RecordChange.RecordID` stores the CompositeKey TEXT `"ID|<uuid>"`, not a bare UUID — a naive `RecordID='<uuid>'` filter silently matches nothing.) **EW2** (CD2): the row a save returns carries correct VIEW-only virtual values (`Parent`, `RootParentID`) on both INSERT and UPDATE, alongside correct base columns — the `@ResultTable` skew detector. **EW3** (CD3): an `AfterKey` walk over a 12-row fixture set visits every row **exactly once** (union == the fixture set, no duplicates) across 3 pages and ends on a short page. **EW4** (CD4, read-only): a valid `AfterKey` call succeeds (positive control) while `StartRow` / non-PK `OrderBy` / a wrong-shape key are each refused — one illegal ingredient at a time. **EW5** (CD5): a save inside the dedup **linger** window drops the lingered entry, so the next identical view sees the new row; the check asserts the re-run landed inside `ProviderBase.DedupLingerMs` or reports itself inconclusive. **EW6** (CD11): a FK written in the OPPOSITE case to the platform's is found by **both** lowercase and uppercase filters and resolves to the right row. **EW7** (CD12): a non-UTC-offset `datetimeoffset` survives save → in-memory → `Load()` → `RunView` to the millisecond. **EW8** (CD10, read-only): a `MJ: Tag Scopes` save with `SkipAsyncValidation` — so the LOCAL object provably never evaluates the invariant — is still refused by `MJTagScopeEntityServer.ValidateAsync`, attributing the refusal to the entity object the **resolver** instantiated (8 tests) |
| `cache-gauntlet-tests.ts` | **Deterministic · MUTATION tier (`RUN_MUTATION_TESTS=1`)** — LIVE coverage of the **subset-slot × mutation** cell that shipped TWO production cache bugs (#3195 `totalRowCount` collapse, #3199 rows maintained in place). An audit of the 61 pre-existing cache checks found this cell had **no live coverage**: S16 tests that `MaxRows` *fingerprints* separately (slot identity), S17 tests that a *filtered* slot invalidates on save, S23 tests that an *unfiltered* slot upserts in place — nothing ever saved into a **subset** slot. **CG1**: three successive saves never inflate a `MaxRows:1` slot past its limit (the #3199 save half). **CG2**: deleting the cached row never leaves a `MaxRows:1` slot serving **zero** rows while the DB still has rows for a `TOP 1` (the #3199 delete half — the worse one). **CG3**: a *filtered* slot still removes a deleted row in place, pinning the **legitimate** half of the asymmetry (a deleted row matches no predicate, so removal is always safe) so a future over-correction doesn't needlessly invalidate it. **CG4**: a `StartRow` offset window never exceeds `MaxRows` across a save. **CG5**: a subset slot keeps a DB-accurate `TotalRowCount`, proving the #3195 and #3199 fixes compose. Uses `ExecutionTime === 0` as the cache-hit oracle. **Proven to fail**: with `isSubsetFingerprint` neutered, CG1/CG2/CG4/CG5 go red while CG3 correctly stays green. Self-cleaning `MJ: User Settings` fixtures (5 tests). **Known gap**: schema-drift staleness (`schemaHash`) still has no live check — see the bundle header. |
...
| `ps-live-recordprocess-scoring.ts` | **Predictive Studio Phase-2 tier (gated, GraphQL wire)** — a saved `WorkType='ML Model'` Record Process scores + writes back over the wire via `RecordProcess.RunNow` (dry-run writes nothing; apply writes). Self-cleaning |
| `ps-live-modelaction-generation.ts` | **Predictive Studio Phase-2 tier (gated, GraphQL wire)** — promoting a model to Published auto-generates the per-model "Score with `<model>`" Action with the modelId baked into its `ModelID` `DefaultValue`. Self-cleaning |
| `ps-live-renewal-lifecycle.ts` · `ps-live-multimodel-lifecycle.ts` · `ps-live-writeback-demo.ts` | **Predictive Studio Phase-2 tier (gated, GraphQL wire)** — full train→holdout→score/promote lifecycles + the entity-column write-back demo over GraphQL. Self-cleaning |
...
| `agent-memory-tests.ts` | **Live model tier (gated, GraphQL wire)** — the **agent MEMORY lifecycle** end-to-end over the wire (`GraphQLAIClient`). The specific memories are nondeterministic (LLM), so every assertion is at the PROCESS level, isolated by a per-run marker string: **Phase A — Formation** (memory-triggering convos → `MJ: AI Agent Notes` rows with `Status='Provisional'`/`AuthorType='Agent'`, agent-scoped), **Phase B — Hardening** (run the **Memory Manager** → Provisional flips to Active with `ExpiresAt` cleared), **Phase C — Injection** (re-run Sage → the formed note's unique ID appears in the follow-up run's step `memoryAttribution`, the reliable injection observable — an AccessCount bump is checked as a fallback but does not fire in practice). Self-cleaning: every marker note is deleted in `finally` (8 checks) |
