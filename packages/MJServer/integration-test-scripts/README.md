...
| `geo-address-cache-tests.ts` | **Deterministic** — the address-level geocode cache (`MJ: Geo Address Caches`, V202607091200) behind GeoCodeSyncService's memo→cache→provider lookup: normalization/SHA-256 hashing stability, success + negative (`not_geocodable` with `ExpiresAt` TTL) entry round-trips through the generated entity/procs, point lookup by `AddressHash`, the unique-index write-through race backstop, and Status CHECK rejection. Self-cleaning via tagged `NormalizedAddress` sweep (6 tests) |
