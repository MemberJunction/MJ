...
| `startup-mode-tests.ts` | **Deterministic** — configurable startup mode ('full' \| 'task') against the process's REAL `@RegisterForStartup` registration set: a task-mode `Startup()` executes zero engines while still succeeding, an engine then lazy-loads on demand via its own `Config()` (first-touch-pays-the-load), and a full-mode run executes every sync registration. No fixtures — mutates only StartupManager's process-level load state, which the bundle's Teardown restores (3 tests) |
