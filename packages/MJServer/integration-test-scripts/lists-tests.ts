 ...
 * like MJAPI) and runs the 'lists' bundle (LS1–LS3) from the shared IntegrationCheckRegistry, wrapping
 * it in the bundle's registered lifecycle (Setup creates a throwaway MJ: Lists row + its members;
 * Teardown deletes them). The check bodies + fixture live ONCE in @memberjunction/testing-integration
 * and are consumed identically by this script and by the IntegrationTestDriver (IT20).
 *
 * Deterministic (no model calls). Covers the v5.48 ListSource keyset-pagination change.
 *
