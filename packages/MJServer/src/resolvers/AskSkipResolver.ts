...
import { AskSkipHTMLReportQAAgent } from './AskSkip.HTMLQAAgent.temp.js';
...
export class AskSkipResolver {
  ...
  protected async FinishConversationAndNotifyUser(
    ...
    if (apiResponse.resultType === 'html') {
      this.TestHTMLReport(apiResponse, dataContext); // do not await this, we are just firing off the test here as a TEST harness, this is all going to move to Skip API later here temporarily
    }
  ...
  protected async TestHTMLReport(
    apiResponse: SkipAPIAnalysisCompleteResponse,
    dataContext: DataContext
  ): Promise<void> {
    try {
      // test the HTML report that was generated by Skip
      const htmlReport = apiResponse.htmlReport;
      const htmlReportObjectName =  apiResponse.htmlReportObjectName;
      const qaAgent = new AskSkipHTMLReportQAAgent();
      ...
      const loadedItems = dataContext.Items.filter(i => i.DataLoaded && i.Data?.length > 0);
      for (let i = 0; i < loadedItems.length; i++) {
          flattenedDataContext["data_item_" + i] = loadedItems[i].Data;
      }

      const result = await qaAgent.testComponent(htmlReport, htmlReportObjectName, flattenedDataContext);
      console.log('QA Agent result:', result);
    }
    catch (e) {
      console.error('Error testing HTML report:', e);
      //eat the error
    }
  }
}
