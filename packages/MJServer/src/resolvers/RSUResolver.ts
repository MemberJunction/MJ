...
export class RSUResolver {
    ...
    async RunRuntimeSchemaUpdateBatch(
        ...
        // MJ-APPLY-6 — deliberately the BARE call, not RunPipelineBatchWithRetry. Everywhere else
        // the MigrationSQL was generated by SchemaBuilder, so the manager knows what a replay would
        // re-execute. Here it is arbitrary SQL handed in by the caller, and `executeMigration` is
        // NOT transactional: it splits on GO/statement boundaries and issues one ExecuteSQL per
        // chunk, so a migration that fails on chunk 3 leaves chunks 1-2 committed while the step is
        // recorded `failed` — which is exactly the state `canSafelyRetry` reads as "nothing was
        // applied, safe to replay". The replay then re-runs the committed chunks and reports THEIR
        // "already exists" error in place of the real one. For generated DDL that risk is accepted
        // upstream (SchemaBuilder.RunSchemaPipeline); for caller-authored DDL it is not ours to
        // take. This is also an attended operator mutation with no in-repo caller — the human on
        // the other end sees the failure and decides. Keep it in step with the singular
        // RunRuntimeSchemaUpdate above, which uses bare RunPipeline for the same reason.
