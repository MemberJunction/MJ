 ...
 * cache's own objects.
 *
 * Now that `LocalCacheManager` deep-freezes cached rows, that in-place rename does not corrupt
 * the cache — it throws. Reproduced live against a running MJAPI before the fix:
 *
 *     UserByEmail → "Cannot add property _mj__CreatedAt, object is not extensible"
 *
 * deterministically, on every call, because `MJ: Users` has caching enabled. `UserByID` and
...
describe('ArrayMapFieldNamesToCodeNames on frozen cache rows', () => {
    it('maps a frozen ARRAY of frozen rows without mutating either', async () => {
        // The array itself is frozen too — LocalCacheManager freezes the array as well as the
        // rows, because `results.sort()`/`.push()` on a live cache array is its own hazard.
