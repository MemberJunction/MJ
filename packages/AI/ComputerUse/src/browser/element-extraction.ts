 ...
 * click/type with Playwright's actionability auto-wait. Extracted here so both
 * `PlaywrightBrowserAdapter` and `SharedContextBrowserAdapter` share ONE
...
export async function clickInteractiveElement(
    ...
    await locatorFor(page, element).click({
...
export async function typeIntoInteractiveElement(
    ...
    const locator = locatorFor(page, element);
    await locator.fill(text, { timeout: actionTimeoutMs });
    if (pressEnter) {
        await locator.press('Enter', { timeout: actionTimeoutMs });
    }
}
