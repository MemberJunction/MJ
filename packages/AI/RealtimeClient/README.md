...
| `SetOutputMuted(muted)` / `IsOutputMuted` | **Speaker mute — local and invisible to the provider.** Silences what the listener hears of the agent and nothing else: the session, the model's turn, playback scheduling, `IsAudioPlaying` and the output meter continue as if the speaker were on, and no frame is ever sent (muting must never interrupt the agent). Concrete on the base; drivers implement `applyOutputMute` against the plane they own (`RealtimePcmPlayback.SetMuted` on client-owned audio, the `<audio>` sink's `muted` on WebRTC) and re-apply the state whenever they (re)create it, so a mute set before `Connect` sticks. |
