...
/**
 * @deprecated Archival logic has been replaced by the Ebbinghaus decay function in MemoryManagerAgent
 * (see memory-manager-agent.ts, decayBasedArchival method). The Memory Manager now handles all memory
 * lifecycle operations in a single sequential pipeline: extraction, importance scoring, consolidation,
 * verification, contradiction detection, stale reference pruning, and decay-based archival.
 *
 * This class is retained for reference and rollback purposes. Its scheduled job should be disabled
 * (set Status='Disabled' in .memory-cleanup-job.json metadata).
 *
 * Original behavior:
 ...
 *
 * Replaced by: MemoryManagerAgent.decayBasedArchival() which uses continuous Ebbinghaus-inspired
 * decay with importance-weighted retention, protection tiers, and per-agent configuration.
 */
