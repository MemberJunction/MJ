...
export class FlowAgentType extends BaseAgentType {
    ...
    try {
      ...
      if (currentStep?.StepType === 'Prompt' && promptResult) {
        ...
        if (promptResponse) {
          ...
          if (promptResponse.nextStep?.type === 'Chat' || (promptResponse.taskComplete && promptResponse.message)) {
   ...
   * @private
   */
  private async getValidPaths(stepId: string, payload: unknown, flowState: FlowExecutionState): Promise<AIAgentStepPathEntity[]> {
    // Load all paths from this step
    const allPaths = AIEngine.Instance.GetPathsFromStep(stepId).sort((a, b) => b.Priority - a.Priority);
        ...
        if (outputParam === '*') {
          ...
          for (const part of parts) {
            if (value && typeof value === 'object' && !Array.isArray(value)) {
              ...
              const actualKey = Object.keys(value as Record<string, unknown>).find((key) => key.toLowerCase() === part.toLowerCase());
  ...
  public AfterLoopIteration<P>(iterationResult: {
    ...
    const payloadChange = this.applyActionOutputMapping(outputParams, iterationResult.currentPayload, resolvedOutputMapping);

    if (payloadChange?.updateElements) {
      // Deep merge to preserve existing payload structure
      return this._payloadManager.deepMerge(iterationResult.currentPayload, payloadChange.updateElements);
    }
