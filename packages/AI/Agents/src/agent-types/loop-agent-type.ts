...
import { LogError, LogStatusEx } from '@memberjunction/global';
...
export class LoopAgentType extends BaseAgentType {
  ...
  protected isValidLoopResponse(simpleResponse: unknown): { success: boolean; message?: string } {
    ...
    if (!response.taskComplete && !response.nextStep) {
      if (response.message?.trim().length > 0 || (typeof response.reasoning === 'string' && response.reasoning?.trim().length > 0)) {
        ...
        response.payloadChangeRequest?.newElements ||
  ...
  public async InjectPayload<T = any, ATS = any>(
    ...
    if (!prompt) throw new Error('Prompt parameters are required for payload injection');
    if (!prompt.data) prompt.data = {};
   ...
   */
  private resolveTemplates(obj: Record<string, unknown>, context: Record<string, any>, itemVariable: string): Record<string, unknown> {
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (typeof value === 'string') {
        ...
        result[key] = value.map((v) => (typeof v === 'string' ? this.resolveValueFromContext(v, context, itemVariable) : v));
