...
import { PayloadOperation, parsePathWithOperations, parsePathsWithOperations, isOperationAllowed } from './types/payload-operations';
...
export class PayloadManager {
   ...
   * ```
   */
  public extractDownstreamPayload<P = any>(
    subAgentName: string,
    fullPayload: P | null | undefined,
   ...
   * @private
   */
  private extractPath(source: any, pathParts: string[], destination: any, currentPath: string[]): void {
   ...
   * @private
   */
  private extractArrayPath(source: any, arrayPart: string, remainingParts: string[], destination: any, currentPath: string[]): void {
   ...
   * @private
   */
  private groupUnauthorizedChanges(
    changes: Array<{ path: string; from: any; to: any; operation?: string; reason?: string }>
  ): Record<string, Array<{ path: string; from: any; to: any; operation?: string; reason?: string }>> {
  ...
  private isPathAllowed(actualPath: string, allowedPatterns: string[]): boolean {
    ...
    for (const pattern of allowedPatterns) {
      ...
      if (pathPattern.includes('**')) {
        // Handle deep wildcards
        const regex = pathPattern.replace(/\./g, '\\.').replace(/\*\*/g, '.*').replace(/\*/g, '[^.]+');
   ...
   */
  private isOperationAllowedForPath(actualPath: string, operation: PayloadOperation, allowedPatterns: string[]): boolean {
    const normalizedPath = actualPath.replace(/\[(\d+)\]/g, '.$1');
  ...
  private processArrayChanges(
    ...
    for (let i = 0; i < target.length; i++) {
      ...
      if (updateArray && this.isSignificantValue(updateArray[i])) {
        ...
        if (
          typeof updateArray[i] === 'object' &&
          typeof elementToAdd === 'object' &&
          !Array.isArray(updateArray[i]) &&
          !Array.isArray(elementToAdd) &&
          updateArray[i] !== null &&
          elementToAdd !== null
        ) {
  ...
  private processKeyChange<P = any>(
    ...
    const effectiveUpdateValue = updateValue !== undefined ? updateValue : newValue !== undefined && key in target ? newValue : undefined;
    ...
    if (effectiveUpdateValue !== undefined && key in target) {
      ...
      if (
        typeof effectiveUpdateValue === 'object' &&
        effectiveUpdateValue !== null &&
    ...
    } else if (removeValue !== undefined && removeValue !== '__DELETE__' && typeof target[key] === 'object' && target[key] !== null) {
  ...
  public applyPayloadScope<P = any>(payload: P, scopePath: string): any | null {
    ...
    const pathParts = scopePath.startsWith('/') ? scopePath.slice(1).split('/') : scopePath.split('/');
  ...
  public transformChangeRequestPaths<P = any>(
    ...
    const pathPrefix = scopePath.startsWith('/') ? scopePath.slice(1).replace(/\//g, '.') : scopePath.replace(/\//g, '.');
