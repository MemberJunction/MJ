...
import { UserInfo, Metadata, RunView, LogStatus, LogStatusEx, LogError, LogErrorEx, IsVerboseLoggingEnabled, IMetadataProvider, DatabaseProviderBase, EntitySaveOptions } from '@memberjunction/core';
...
export class BaseAgent {
     ...
     */
    private _stepInsertPromises: WeakMap<MJAIAgentRunStepEntityExtended, Promise<boolean>> = new WeakMap();
...
The context is now within limits. Please retry your request with the recovered context.`,
     ...
     */
    private async saveStepRecord(stepEntity: MJAIAgentRunStepEntityExtended, phase: 'insert' | 'update'): Promise<boolean> {
        try {
            ...
            LogError(`Error on ${phase} of agent run step record ${stepEntity.ID || '(unsaved)'}: ${(e as Error)?.message ?? e}`);
            return false;
        }
    }
