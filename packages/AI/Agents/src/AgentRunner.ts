...
export class AgentRunner {
    ...
    try {
      ...
      if (options.conversationDetailId) {
        ...
        if (options.conversationId) {
          ...
          const tempDetail = await md.GetEntityObject<MJConversationDetailEntity>('MJ: Conversation Details', contextUser);
        ...
        agentResponseDetail = await md.GetEntityObject<MJConversationDetailEntity>('MJ: Conversation Details', contextUser);
        ...
        const userMessageDetail = await md.GetEntityObject<MJConversationDetailEntity>('MJ: Conversation Details', contextUser);
      ...
      const updateDetailPromise = (async () => {
        if (agentResponseDetail && agentResponseDetailId) {
          ...
          agentResponseDetail.Message =
            agentResult.agentRun?.Message || (agentResult.success ? '✅ Completed' : agentResult.agentRun?.ErrorMessage || '❌ Failed');
      ...
      const processArtifactsPromise = (async () => {
        ...
        if (shouldCreateArtifacts && agentResult.success && agentResult.payload) {
          return this.ProcessAgentArtifacts(agentResult, agentResponseDetailId!, options.sourceArtifactId, contextUser, md);
        }
        return undefined;
      })();
      ...
      const processFileArtifactsPromise = (async () => {
        if (agentResult.success && agentResponseDetailId && agentResult.fileOutputs?.length) {
          await this.ProcessFileArtifacts(agentResult.fileOutputs, agentResponseDetailId, contextUser, agentResult.resolvedStorageAccountId, md);
        }
      })();
      ...
      const saveMediaPromise = (async () => {
        if (agentResult.mediaOutputs && agentResult.mediaOutputs.length > 0) {
          ...
          const ids = await this.SaveAgentRunMedia(agentResult.agentRun.ID, mediaToSave, contextUser, md);

          // Create ConversationDetailAttachment records for UI display
          if (agentResponseDetailId && ids.length > 0) {
            await this.CreateConversationMediaAttachments(agentResponseDetailId, mediaToSave, ids, contextUser, md);
          }
          return ids;
        }
        return [];
      ...
      const [, artifactInfo] = await Promise.all([updateDetailPromise, processArtifactsPromise, processFileArtifactsPromise, saveMediaPromise]);
  ...
  protected async LinkArtifactToConversationDetail(
    ...
    const junction = await provider.GetEntityObject<MJConversationDetailArtifactEntity>('MJ: Conversation Detail Artifacts', contextUser);
  ...
  public async ProcessAgentArtifacts<R>(
    ...
    try {
      ...
      // Priority 2: Try to find previous artifact for this message
        ...
        const previousArtifact = await this.FindPreviousArtifactForMessage(conversationDetailId, contextUser, md);
      ...
      if (!isNewArtifact && newVersionNumber > 1) {
        const existingVersionId = await this.CheckForDuplicateVersion(artifactId, serializedContent, newVersionNumber - 1, contextUser, provider);
      ...
      if (isNewArtifact && newVersionNumber === 1) {
        const nameAttr = (version as any).Attributes?.find((attr: any) => attr.StandardProperty === 'name' || attr.Name?.toLowerCase() === 'name');
      ...
      return this.LinkArtifactToConversationDetail(version.ID, conversationDetailId, artifactId, newVersionNumber, contextUser, md);
    } catch (error) {
  ...
  public async SaveAgentRunMedia(
    ...
    try {
      ...
      for (let i = 0; i < mediaToSave.length; i++) {
        ...
        try {
          const mediaEntity = await md.GetEntityObject<MJAIAgentRunMediaEntity>('MJ: AI Agent Run Medias', contextUser);
  ...
  public async CreateConversationMediaAttachments(
    ...
    try {
      ...
      for (let i = 0; i < mediaOutputs.length; i++) {
        ...
        try {
          const attachment = await md.GetEntityObject<MJConversationDetailAttachmentEntity>('MJ: Conversation Detail Attachments', contextUser);
  ...
  public async ProcessFileArtifacts(
    ...
    await Promise.all([ArtifactMetadataEngine.Instance.Config(false, contextUser), FileStorageEngine.Instance.Config(false, contextUser)]);

    await Promise.all(fileOutputs.map((fo) => this.processFileOutput(fo, conversationDetailId, contextUser, resolvedStorageAccountId, md)));
  }
   ...
   * @param contextUser - User context for DB operations
   */
  public async ReprocessRunFileArtifacts(agentRunId: string, conversationDetailId: string, contextUser: UserInfo, provider?: IMetadataProvider): Promise<void> {
    const md = provider || this._provider;
  ...
  private async processFileOutput(
    ...
    try {
      ...
      try {
        const fileId = await this.uploadBase64ToStorage(fo.fileData!, fo.fileName, fo.mimeType, contextUser, resolvedStorageAccountId, provider);
   ...
   * vs. inline content).
   */
  private async createArtifactWithVersion(params: {
    mimeType: string;
    fileName: string;
