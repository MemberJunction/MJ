 ...
 */
export interface ContentTruncationWarning
  extends PayloadWarning<{
 ...
 */
export interface KeyRemovalWarning
  extends PayloadWarning<{
    removedKeys: string[];
    hadContent: boolean;
    contentSize: number;
...
export class PayloadChangeAnalyzer {
  ...
  constructor(config?: Partial<PayloadAnalyzerConfig>) {
    this.config = {
      ...
      maxAnalysisDepth: 10,
      truncationPatterns: [/\.\.\.$/, /\[truncated\]/i, /\[shortened\]/i, /etc\.?$/i, /and more/i],
      ...config,
    };
  }
  ...
  public analyzeChangeRequest<P = any>(
    ...
    this.analyzeChanges(originalPayload, resultPayload, changeRequest, [], warnings);

    // Analyze for pattern anomalies
    this.detectPatternAnomalies(originalPayload, resultPayload, warnings);
   ...
   */
  private analyzeStringChange(original: string, result: string, path: string[], warnings: PayloadWarning[]): void {
    ...
    const hasTruncationPattern = this.config.truncationPatterns.some((pattern) => pattern.test(result) && !pattern.test(original));
   ...
   */
  private analyzeArrayChange(original: any[], result: any[], path: string[], warnings: PayloadWarning[]): void {
    const originalLength = original.length;
    const resultLength = result.length;
    ...
    if (this.config.analyzeNestedChanges) {
      ...
      for (let i = 0; i < minLength; i++) {
        this.analyzeChanges(original[i], result[i], {} as AgentPayloadChangeRequest<any>, [...path, `[${i}]`], warnings);
      }
    }
  }
  ...
  private analyzeObjectChange(
    ...
    if (this.config.analyzeNestedChanges) {
      for (const key of resultKeys) {
        if (key in original) {
          this.analyzeChanges(original[key], result[key], changeRequest, [...path, key], warnings);
        }
      }
    }
  }
   ...
   */
  private analyzeRemoval(original: any, path: string[], warnings: PayloadWarning[]): void {
    // Skip if the original was empty/null
    if (original === null || original === undefined || original === '') return;

    const contentSize = typeof original === 'string' ? original.length : JSON.stringify(original).length;

    if (contentSize > 50) {
      // Only warn for substantial content
      warnings.push({
        type: PayloadWarningType.KeyRemoval,
   ...
   */
  private analyzeTypeChange(original: any, result: any, path: string[], warnings: PayloadWarning[]): void {
   ...
   */
  private detectPatternAnomalies(original: any, result: any, warnings: PayloadWarning[]): void {
    ...
    const placeholderPatterns = [/^TODO:/i, /^TBD$/i, /^\[.*\]$/, /^<.*>$/];
   ...
   */
  private walkPayload(obj: any, path: string[], callback: (value: any, path: string[]) => void): void {
