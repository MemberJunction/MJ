...
import { LogError, LogStatusEx, IsVerboseLoggingEnabled, RunView, RunQuery, UserInfo } from '@memberjunction/global';
import { RunViewParams, RunQueryParams } from '@memberjunction/global';
...
export class AgentDataPreloader {
   ...
   * ```
   */
  public async PreloadAgentData(agentId: string, contextUser: UserInfo, runId?: string): Promise<PreloadedDataResult> {
    try {
      LogStatusEx({
   ...
   * @private
   */
  private async loadDataSourcesForAgent(agentId: string, contextUser: UserInfo): Promise<AIAgentDataSourceEntity[]> {
   ...
   * @private
   */
  private async executeDataSource(source: AIAgentDataSourceEntity, contextUser: UserInfo, runId?: string): Promise<unknown> {
    // Check cache first
   ...
   * @private
   */
  private async executeRunView(source: AIAgentDataSourceEntity, contextUser: UserInfo): Promise<unknown> {
   ...
   * @private
   */
  private async executeRunQuery(source: AIAgentDataSourceEntity, contextUser: UserInfo): Promise<unknown> {
    if (!source.QueryName) {
   ...
   */
  private getCachedData(source: AIAgentDataSourceEntity, runId?: string): unknown | null {
   ...
   * @private
   */
  private cacheData(source: AIAgentDataSourceEntity, data: unknown, runId?: string): void {
    if (source.CachePolicy === 'None') {
      return;
    }
