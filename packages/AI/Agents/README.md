...
- **`RealtimeTurnModerator`** (`src/realtime/realtime-turn-moderator.ts`, exported as `RealtimeTurnModeratorDecision`) — the **multi-agent turn moderator** behind the bridge engine's `SetTurnModerator` seam. In a multi-agent voice room it decides, once per turn, which agent(s) (0+, ordered) should speak next — routing direct address *and* relevance, letting a productive agent↔agent exchange run, and going quiet on unproductive ping-pong. It runs as a fast **`AIPromptRun`** (not an agent run — a stateless who-speaks classification) via `AIPromptRunner`, tied to the co-agent's `AIAgentRun` for full observability, resolving its room-wide config from the **Realtime agent type's `DefaultConfiguration`** (`turnTaking.moderator` in `realtime-coagent-config.ts`) and the per-agent `mode` from the target agent's layer. The seeded `"Realtime: Turn Moderator"` prompt is bound to a small/fast model (GPT-OSS-120B on Cerebras, swappable in metadata) because it sits on the latency-critical path. Degrades to a name-contains safety net only when no prompt is configured; the engine itself falls back to the per-agent matcher broadcast when no moderator is wired. The engine-side mechanics (lookback, serialized floor queue, pre-staging, barge-in invalidation) and the config cascade are documented in the [Real-Time Co-Agents Guide §4](../../../guides/REALTIME_CO_AGENTS_GUIDE.md#turn-moderator-multi-agent-turn-taking) and the [Realtime Bridges Guide §9](../../../guides/REALTIME_BRIDGES_GUIDE.md#turn-moderator--room-level-who-speaks-next-engine-seam).
...
- **`RealtimeChannelServerHost`** (`src/realtime/realtime-channel-server-host.ts`) — the **server half** of the interactive-channel plugin registry (`MJ: AI Agent Channels.ServerPluginClass`; base class `BaseRealtimeChannelServer` in `@memberjunction/ai`, mirroring the browser's `BaseRealtimeChannelClient`). A `BaseSingleton` host that resolves the ACTIVE registry rows at session mint into one fresh plugin instance per session (unregistered keys skip with a log, never fatal) and routes the durable lifecycle into them: session started (`SessionManager.CreateSession`), channel-state saves pre-persistence (a plugin may validate/normalize the persisted state of record — it can transform a save, never lose or block one), and session close from every provenance (explicit / janitor / shutdown / error), with a brief post-close linger so the client's final state flush still routes through the plugin before disposal. Every hook is failure-isolated — a throwing plugin can never break a live call.
- **`WhiteboardChannelServer`** (`src/realtime/whiteboard-channel-server.ts`, key `'WhiteboardChannelServer'`) — the reference server channel plugin: guards the Whiteboard's persisted state of record by validating each landed save parses as a JSON object (flagging corrupt payloads at save time instead of at the next resume) and canonicalizing valid payloads to compact JSON. `LoadWhiteboardChannelServer()` is invoked from MJServer's `agentSessions` module.
- **`MediaChannelServer`** (`src/realtime/media-channel-server.ts`, key `'MediaChannelServer'`) — the server channel plugin for the **Media** surface. Like the Whiteboard it canonicalizes the channel's persisted media state, and additionally resolves the agent's **media kit** at session start: it implements `IRealtimeChannelServerDataAware` to receive the session `contextUser`+`provider`, reads a per-session `mediaCollectionID` override out of `RealtimeChannelServerContext.AgentSessionConfig` (the persisted `AIAgentSession.Config_`), and feeds the model a manifest via `SendContextNote`. The kit-resolution logic is the pure, unit-tested `agent-media-library.ts` (`buildAgentMediaContextNote` / `resolveAgentMediaManifest` / `formatAgentMediaManifest`); precedence is `runtime override > AIAgent.DefaultMediaCollectionID > none`. The agent then surfaces items with the existing `Media_ShowMedia` client tool — no new client tool. `LoadMediaChannelServer()` is invoked from MJServer's `agentSessions` module. See the [Real-Time Co-Agents Guide](../../../guides/REALTIME_CO_AGENTS_GUIDE.md).
...
**Retrieval tools** (`ConversationToolManager`, the `conversationToolCalls` inline response field — zero turn cost, mirroring `artifactToolCalls`): the summary is deliberately lossy; the full history stays addressable by the persisted per-conversation `Sequence`. `getMessageBySequence`, `getMessagesByRange` (max 50 messages / 32k chars), `searchConversation` (keyword/regex + role/range filters, snippets), and `summarizeRange` — a recursive sub-call that re-summarizes any ≤500-message slice through a task-specific lens on a cheap model (`Summarize Conversation Range` seeded prompt), its `AIPromptRun` linked via the Tool step's `TargetLogID`. Results arrive as one-shot conversation messages with the standard compact-after-3-turns lifecycle. Message edits after summarization are flagged via `OriginalMessageChanged` (server-side detection in `MJConversationDetailEntityServer`); summaries are never regenerated — agents page in exact rows instead.

**Prior-turn tool-result carry-forward.** Because each turn rebuilds its messages from the conversation window, a tool result paged in on turn N would be gone on turn N+1. `injectPriorTurnToolResults` re-injects the immediately previous completed root run's successful read-tool results (eligibility decided by the `toolFamily` stamped in each Tool step's `OutputData`) as one transient, compactable message — one-turn memory by construction. The per-turn prior-run lookup is served by `PriorTurnToolResultCache` (a `BaseSingleton` wrapping `MJLruCache`): the completing root run publishes its own Tool-step projections from memory at finalize — an empty array for tool-free runs, so the common case costs **zero DB queries per turn** — and the loader falls back to the original RunView pair only on a cache miss. Both loaders share one entity-typed predicate (`BaseAgent.carryForwardPredicate`) so the SQL filter and the in-memory projection cannot drift; freshness semantics and the multi-node staleness bound are documented on the cache class.
