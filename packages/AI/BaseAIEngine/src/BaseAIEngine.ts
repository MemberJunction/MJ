import { BaseEngine, IMetadataProvider, LogError, Metadata, RunView, UserInfo } from '@memberjunction/global';
import {
  AIActionEntity,
  AIAgentActionEntity,
  AIAgentModelEntity,
...
export class AIEngineBase extends BaseEngine<AIEngineBase> {
  ...
  protected override async AdditionalLoading(contextUser?: UserInfo): Promise<void> {
    ...
    for (const PromptCategory of this._promptCategories) {
      this._prompts
        .filter((prompt: AIPromptEntityExtended) => {
    ...
    for (const agent of this._agents) {
      this._agentActions
        .filter((action: AIAgentActionEntity) => {
  ...
  public async GetHighestPowerModel(vendorName: string, modelType: string, contextUser?: UserInfo): Promise<AIModelEntityExtended> {
    try {
      ...
      const models = AIEngineBase.Instance.Models.filter(
        (m) =>
          m.AIModelType?.trim().toLowerCase() === modelType.trim().toLowerCase() &&
          (vendorName && vendorName.length > 0 ? m.Vendor?.trim().toLowerCase() === vendorName.trim().toLowerCase() : true)
      ); // if vendorname is not provided, then we get all models of the specified type
  ...
  ): AIModelCostEntity | null {
    const now = new Date();
    const activeCosts = this._modelCosts.filter(
      (cost) =>
  ...
  public GetSubAgents(
    ...
    const childAgents = this._agents.filter((a) => a.ParentID === agentID && (!status || a.Status === status));

    // Get related agents (AgentRelationships)
    const relStatus = relationshipStatus ?? 'Active'; // Default to Active for relationships
    const activeRelationships = this._agentRelationships.filter((ar) => ar.AgentID === agentID && ar.Status === relStatus);
  ...
  public GetConfigurationParam(configurationId: string, paramName: string): AIConfigurationParamEntity | null {
    return (
      this._configurationParams.find((p) => p.ConfigurationID === configurationId && p.Name.toLowerCase() === paramName.toLowerCase()) ||
      null
    );
  }
  ...
  public GetAgentSteps(agentId: string, status?: string): AIAgentStepEntity[] {
    return this._agentSteps.filter((step) => step.AgentID === agentId && (!status || step.Status === status));
  }
  ...
  protected static checkMetadataLoaded(): void {
    if (!AIEngineBase.Instance.Loaded) throw new Error('AI Metadata not loaded, call AIEngineBase.Config() first.');
  }
