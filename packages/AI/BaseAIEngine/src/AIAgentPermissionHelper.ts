...
export class AIAgentPermissionHelper {
   ...
   */
  public static async HasPermission(agentId: string, user: UserInfo, permission: 'view' | 'run' | 'edit' | 'delete'): Promise<boolean> {
   ...
   */
  public static async GetEffectivePermissions(agentId: string, user: UserInfo): Promise<EffectiveAgentPermissions> {
    try {
      ...
      const agentPermissions = AIEngineBase.Instance.AgentPermissions.filter((p) => p.AgentID === agentId);
      ...
      const matchingPermissions = agentPermissions.filter((p) => p.UserID === user.ID || (p.RoleID && userRoleIds.includes(p.RoleID)));
   ...
   */
  public static async GetAccessibleAgents(user: UserInfo, permission: 'view' | 'run' | 'edit' | 'delete'): Promise<AIAgentEntity[]> {
    try {
      // Ensure AIEngineBase is loaded with cached metadata
      await AIEngineBase.Instance.Config(false, user);
