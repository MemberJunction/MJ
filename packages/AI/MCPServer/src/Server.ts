        ...
        async execute(props: any) {
          try {
            ...
            const messages: ChatMessage[] =
              props.conversationHistory?.map((msg: any) => ({
                role: msg.role,
                content: msg.content,
              })) || [];
...
function addEntityUpdateTool(entity: EntityInfo, contextUser: UserInfo) {
  ...
  const toolConfig = {
    ...
    async execute(props: any) {
      const md = new Metadata();
      const record = await md.GetEntityObject(entity.Name, contextUser);
      const loaded = await record.InnerLoad(
        new CompositeKey(
      ...
      if (loaded) {
        ...
        return JSON.stringify({
          success,
          record: await convertEntityObjectToJSON(record),
          errorMessage: !success ? record.LatestResult.Message : undefined,
        });
      } else {
...
function getMatchingEntitiesForTool(
