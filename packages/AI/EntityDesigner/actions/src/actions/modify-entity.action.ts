 ...
 *  3. Load existing table info via EntityDesignerPipelineExecutor.LoadExistingTableInfo
 *  4. Delegate to EntityDesignerPipelineExecutor.ModifyEntity
 */
...
export class ModifyEntityAction extends BaseEntityDesignerAction {
    protected async InternalRunAction(params: RunActionParams): Promise<ActionResultSimple> {
        try {
            ...
            const existingInfo = await EntityDesignerPipelineExecutor.LoadExistingTableInfo(
                td!.SchemaName,
                td!.TableName,
                params.ContextUser
            );
            ...
            const execResult = await EntityDesignerPipelineExecutor.ModifyEntity(
                td!,
                existingInfo,
                params.ContextUser,
                options
