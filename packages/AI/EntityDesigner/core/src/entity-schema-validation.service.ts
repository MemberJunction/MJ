/**
 * @module entity-schema-validation.service
 * @description Pure TypeScript service that runs all deterministic Entity Designer
 ...
 * rules change:
 *  - Agent path: `EntityDesignerSchemaValidator` delegates here via `runValidationChecks()`
 ...
 *     so Entity Designer and the DDL pipeline share a single source of truth
 ...
 */
export class EntitySchemaValidationService {
    ...
    private checkBlockedSchema(schemaName: string, errors: string[]): void {
        ...
        if (blocked.has(schemaName.toLowerCase())) {
            errors.push(
                `Schema '${schemaName}' is protected and cannot be used with the Entity Designer. ` +
