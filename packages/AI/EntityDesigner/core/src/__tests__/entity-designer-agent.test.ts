...
vi.mock('@memberjunction/ai-agents', () => ({
    BaseAgent: class BaseAgent {
        // Default implementation — intercepted by EntityDesignerAgent overrides
        ...
        // logStatus is called by EntityDesignerAgent; no-op in tests
...
import { EntityDesignerAgent } from '../agents/entity-designer-agent.js';
...
function makeAgent(): TestableAgent {
    return new EntityDesignerAgent() as unknown as TestableAgent;
}
...
describe('EntityDesignerAgent.determineNextStep', () => {
    ...
    describe('Intercept 1 — subagent fast path (bypass Requirements Analyst)', () => {
        it('routes to Schema Designer when mode=subagent, tableSpec present, no SchemaDesign', async () => {
            ...
            const payload: EntityDesignerPayload = {
                mode: 'subagent',
                callerContext: {
            ...
            expect(result.subAgent.name).toBe('Entity Schema Designer');
        });
        ...
        it('does NOT fire when callerContext.tableSpec is absent (lenient fallback)', async () => {
            const agent = makeAgent();
            const payload: EntityDesignerPayload = { mode: 'subagent' }; // no callerContext
        ...
        it('does NOT fire in standalone mode', async () => {
            const agent = makeAgent();
            const payload: EntityDesignerPayload = { mode: 'standalone' };
    ...
    describe('Intercept 2 — create_now approval forces Schema Validator', () => {
        it('routes to Schema Validator when last user message contains create_now', async () => {
            ...
            expect(result.step).toBe('Sub-Agent');
            expect(result.subAgent.name).toBe('Entity Schema Validator');
        });
    ...
    describe('Intercept 3 — validation passed forces Schema Builder', () => {
        it('routes to Schema Builder when ValidationResult.Valid=true and no EntityDesignerResult', async () => {
            const agent = makeAgent();
            const payload: EntityDesignerPayload = {
                ValidationResult: { Valid: true, Errors: [], Warnings: [] },
                // EntityDesignerResult absent — build not yet started
            };

            const result = await agent.determineNextStep(makeParams(), BLANK_AGENT_TYPE, BLANK_PROMPT_RESULT, payload) as SubAgentStep;

            expect(result.step).toBe('Sub-Agent');
            expect(result.subAgent.name).toBe('Entity Schema Builder');
        });
        ...
        it('does NOT fire when EntityDesignerResult is already present (build already ran)', async () => {
            const agent = makeAgent();
            const payload: EntityDesignerPayload = {
                ValidationResult: { Valid: true, Errors: [], Warnings: [] },
                EntityDesignerResult: { Success: true, EntityName: 'Customer Orders' },
            };
        ...
        it('does NOT fire on the first agent turn when payload is null', async () => {
            ...
            const result = await agent.determineNextStep(makeParams(), BLANK_AGENT_TYPE, BLANK_PROMPT_RESULT, null as unknown as EntityDesignerPayload);
            expect(result.step).toBe('Chat');
        });
    });
