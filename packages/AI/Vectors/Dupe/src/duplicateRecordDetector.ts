...
export class DuplicateRecordDetector extends VectorBase {
  ...
  public async getDuplicateRecords(params: PotentialDuplicateRequest, contextUser?: UserInfo): Promise<PotentialDuplicateResponse> {
    ...
    let duplicateRun: DuplicateRunEntity = params.Options?.DuplicateRunID
      ? await this.getDuplicateRunEntity(params.Options?.DuplicateRunID)
      : await this.getDuplicateRunEntityByListID(list.ID);
    ...
    for (const [index, vector] of embedTextsResult.vectors.entries()) {
      ...
      let filterResult: BaseResponse = await this._vectorDB.queryIndex({
        vector: vector,
        topK: topK,
      ...
      let dupeRunDetail: DuplicateRunDetailEntity = duplicateRunDetails.find(
        (detail: DuplicateRunDetailEntity) => detail.RecordID === compositeKey.Values()
      );
      if (dupeRunDetail) {
  ...
  private async createDuplicateRunDetailMatchesForRecord(
    DuplicateRunDetailID: string,
    duplicateResult: PotentialDuplicateResult
  ): Promise<DuplicateRunDetailMatchEntity[]> {
    const md: Metadata = new Metadata();
    ...
    for (const dupeResult of dupeResponse.PotentialDuplicateResult) {
      for (const [index, dupe] of dupeResult.Duplicates.entries()) {
        ...
        if (dupe.ProbabilityScore >= entityDocument.AbsoluteMatchThreshold) {
          ...
          if (result.Success) {
            let dupeRunMatchRecord: DuplicateRunDetailMatchEntity = await md.GetEntityObject<DuplicateRunDetailMatchEntity>(
