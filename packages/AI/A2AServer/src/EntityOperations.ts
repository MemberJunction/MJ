import { BaseEntity, EntityInfo, LogError, Metadata, RunView, RunQuery, UserInfo, CompositeKey } from '@memberjunction/global';
...
export class EntityOperations {
  ...
  constructor() {
    ...
    if (a2aServerSettings?.userEmail) {
      const user = UserCache.Instance.Users.find(
        (u: UserInfo) => u.Email && u.Email.toLowerCase() === a2aServerSettings?.userEmail?.toLowerCase()
      );
      ...
      if (user) {
        ...
        LogError(
          'EntityOperations',
          'Constructor',
          `User with email ${a2aServerSettings?.userEmail} not found. Using first user in cache.`
        );
  ...
  public findEntity(entityName: string): EntityInfo | null {
    ...
    return (
      this.metadata.Entities.find(
        (e) => e.Name.toLowerCase() === entityName.toLowerCase() || e.ClassName.toLowerCase() === entityName.toLowerCase()
      ) || null
    );
  }
  ...
  public parseCommandFromText(textContent: string): { operation: string; entityName: string; parameters: OperationParameters } {
    ...
    const entityMatches = textContent.match(/from\s+(\w+)/i) || textContent.match(/(get|update|delete|query)\s+(\w+)/i);
    if (entityMatches) {
    ...
    if (operation === 'get' || operation === 'update' || operation === 'delete') {
      const idMatch = textContent.match(/id\s*[=:]\s*(\w+)/i) || textContent.match(/where\s+id\s*[=:]\s*(\w+)/i);
