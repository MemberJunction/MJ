"""Predictive Studio FORECAST sidecar launcher.
...
assigned port, print it as ``PREDICTIVE_STUDIO_FORECAST_PORT=<n>`` on stdout so the
...
def main() -> None:
    ...
    port). The resolved port is printed as ``PREDICTIVE_STUDIO_FORECAST_PORT=<n>``
    ...
    print(f"PREDICTIVE_STUDIO_FORECAST_PORT={actual_port}", flush=True)
