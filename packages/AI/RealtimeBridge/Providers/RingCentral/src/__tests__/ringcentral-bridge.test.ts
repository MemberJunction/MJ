...
// ──────────────────────────────────────────────────────────────────────────────
// FakeRingCentralCallSdk — an in-memory ITelephonyCallSdk with drive helpers and
// capture sinks. No network, no real RingCentral client.
// ──────────────────────────────────────────────────────────────────────────────

class FakeRingCentralCallSdk implements ITelephonyCallSdk {
...
/** The RingCentral telephony seed-row capability shape: telephony only — NO video/screen/diarization. */
const RINGCENTRAL_FEATURES: MJAIBridgeProviderEntity_IBridgeProviderFeatures = {
    OutboundDial: true,
    InboundRouting: true,
    AudioIn: true,
    AudioOut: true,
...
function ctx(
    features: MJAIBridgeProviderEntity_IBridgeProviderFeatures = RINGCENTRAL_FEATURES,
...
/** Builds a RingCentralBridge wired to a FakeRingCentralCallSdk via the creation seam. */
function makeBridge(sdk: FakeRingCentralCallSdk): RingCentralBridge {
    const bridge = new RingCentralBridge();
...
let sdk: FakeRingCentralCallSdk;
beforeEach(() => {
    sdk = new FakeRingCentralCallSdk();
});
...
describe('RingCentralBridge — outbound dial', () => {
    it('dials the destination from the agent number and returns the call handles', async () => {
        ...
        expect(result.ExternalConnectionId).toBe('RC-session-outbound-1');
        expect(result.BotParticipantId).toBe('agent');
        expect(bridge.CallId).toBe('RC-session-outbound-1');
...
describe('RingCentralBridge — inbound answer', () => {
    it('answers a routed inbound call and returns its id as the connection', async () => {
        const bridge = makeBridge(sdk);
        const result = await bridge.Connect(
            ctx(RINGCENTRAL_FEATURES, {
                [DIRECTION_CONFIG_KEY]: 'Inbound',
                [INBOUND_CALL_ID_CONFIG_KEY]: 'RC-session-inbound-9',
            }),
        );
        expect(sdk.Answered).toBe('RC-session-inbound-9');
        expect(sdk.Dialed).toBeUndefined();
        expect(result.ExternalConnectionId).toBe('RC-session-inbound-9');
    ...
    it('requires InboundRouting for an inbound call', async () => {
        ...
        await expect(
            bridge.Connect(
                ctx({ AudioIn: true, AudioOut: true }, { [DIRECTION_CONFIG_KEY]: 'Inbound', [INBOUND_CALL_ID_CONFIG_KEY]: 'RC-1' }),
            ),
        ).rejects.toBeInstanceOf(BridgeCapabilityNotSupportedError);
    });
    ...
    it('throws when an inbound connect omits the InboundCallId', async () => {
        const bridge = makeBridge(sdk);
        await expect(
            bridge.Connect(ctx(RINGCENTRAL_FEATURES, { [DIRECTION_CONFIG_KEY]: 'Inbound' })),
...
describe('RingCentralBridge — DTMF', () => {
...
describe('RingCentralBridge — transfer', () => {
    it('transfers the live call through the SDK', async () => {
        const bridge = makeBridge(sdk);
        await bridge.Connect(ctx());
        await bridge.TransferCall('+15550001111');
        expect(sdk.Transferred).toEqual({ callId: 'RC-session-outbound-1', to: '+15550001111' });
    });
...
describe('RingCentralBridge — participants', () => {
...
describe('RingCentralBridge — no video / screen / Meeting Controls', () => {
    ...
    it('StartRecording stays capability-gated (RingCentral telephony seed row does not enable Recording)', async () => {
        const bridge = makeBridge(sdk);
        await bridge.Connect(ctx());
...
// ──────────────────────────────────────────────────────────────────────────────
// The default RingCentralCallSdk (no real client bound) throws the explicit bind-me error.
// ──────────────────────────────────────────────────────────────────────────────

describe('RingCentralBridge — real RingCentral client binding TODO', () => {
    it('a RingCentralBridge with the default (unbound) RingCentral SDK throws bind-me on Connect', async () => {
        const bridge = new RingCentralBridge(); // default factory → unbound RingCentralCallSdk
        await expect(bridge.Connect(ctx())).rejects.toThrow(/no real RingCentral client bound/i);
    });

    it('the unbound RingCentralCallSdk throws bind-me from dial directly', async () => {
        const unbound = new RingCentralCallSdk();
        await expect(unbound.dial('+1555', '+1444')).rejects.toThrow(/no real RingCentral client bound/i);
    });
});
