/**
 * Adapts the {@link IZoomMeetingSdk} participant / hand-raise / speaking stream into an
 * {@link IBridgeMeetingControlsEventSource} so the bridge engine can wire the **Meeting Controls**
 * facilitator channel (roster · hand-raise queue · who's-speaking · mute) for a Zoom session. This is
 * the §4b "the bridge contributes a channel" pattern realized for Zoom — no channel logic lives here,
...
import { IZoomMeetingSdk, ZoomParticipant, ZoomParticipantRole } from './zoom-sdk';

/**
 * Maps a Zoom participant role to the bridge's meeting-participant role. Zoom has no distinct "Agent"
 * role; the bot is flagged via `IsSelf` and surfaced as `'Agent'`.
 */
function mapRole(role: ZoomParticipantRole, isSelf: boolean | undefined): BridgeMeetingParticipantRole {
    if (isSelf) {
        return 'Agent';
    }
    switch (role) {
...
/**
 * Zoom's adapter to the Meeting Controls channel's event source. It maintains a live roster and
 ...
 * there is a roster to facilitate) and returns it from `ZoomBridge.GetMeetingControlsEventSource`.
 */
export class ZoomMeetingControlsEventSource implements IBridgeMeetingControlsEventSource {
    ...
    /** Zoom supports host-driven mute, so the facilitator `MuteParticipant` tool is offered. */
    public readonly Capabilities: ReadonlyArray<BridgeMeetingControlsCapability> = ['Mute'];

    /**
     * @param sdk The Zoom SDK seam used only to actuate {@link MuteParticipant} — all *perception* is
     *   fed in by the driver via the `Ingest*` methods so the SDK keeps a single subscriber per event.
     */
    constructor(private readonly sdk: IZoomMeetingSdk) {}
     ...
     *
     * @param participants The full current Zoom roster.
     */
