...
export class AgentArchitectAgent extends BaseAgent {
   ...
   */
  protected async validateAgentSpec(spec: AgentSpec, params: ExecuteAgentParams): Promise<{ errors: string[]; correctedSpec?: AgentSpec }> {
    const errors: string[] = [];
    let corrected = false;
    ...
    if (!spec) {
      errors.push(
        '❌ AgentSpec is null or undefined. Ensure you place the AgentSpec in payload.agentSpec (inside a payloadChangeRequest with newElements or replaceElements)'
      );
      return { errors };
    }
    ...
    if (!correctedSpec.StartingPayloadValidationMode) {
      ...
      errors.push(
        `❌ StartingPayloadValidationMode must be either "Fail" or "Warn", got: "${correctedSpec.StartingPayloadValidationMode}"`
      );
    }
    ...
    if (isFlowAgent) {
      ...
      if (correctedSpec.Prompts && correctedSpec.Prompts.length > 0) {
        errors.push(
          '❌ Flow agents MUST have empty Prompts array. If LLM is needed, use a Prompt step OR a Loop sub-agent instead of agent-level prompts.'
        );
      }
          ...
          if (step.StepType === 'Prompt') {
            ...
            if (!step.PromptID || step.PromptID === '') {
              if (!step.PromptText || step.PromptText.trim() === '') {
                errors.push(
                  `❌ Prompt step "${step.Name}" (index ${i}) has empty PromptID but missing PromptText. For inline prompt creation, PromptText is required.`
                );
              }
              // PromptName and PromptDescription are recommended but not required
              if (!step.PromptName) {
                console.log(
                  `⚠️ Warning: Prompt step "${step.Name}" (index ${i}) is missing PromptName (will default to "[Step Name] Prompt")`
                );
              }
            }
    ...
    if (correctedSpec.FinalPayloadValidationMode && !['Fail', 'Retry', 'Warn'].includes(correctedSpec.FinalPayloadValidationMode)) {
      errors.push(`❌ FinalPayloadValidationMode must be "Fail", "Retry", or "Warn", got: "${correctedSpec.FinalPayloadValidationMode}"`);
    }

    if (correctedSpec.InvocationMode && !['Any', 'Agent', 'User', 'Never'].includes(correctedSpec.InvocationMode)) {
   ...
   */
  protected async validateActions(actions: AgentSpec['Actions'], params: ExecuteAgentParams): Promise<{ errors: string[] }> {
    const errors: string[] = [];
    ...
    try {
      ...
      const actionIds = actions.map((a) => a.ActionID).filter((id) => id && id.trim().length > 0);
      ...
      for (const action of actions) {
        ...
        if (!foundIds.has(action.ActionID)) {
          errors.push(
            `❌ Action with ID "${action.ActionID}" not found in database. Please use "Find Candidate Actions" or "List Actions" to get valid action IDs.`
          );
        }
      }
    } catch (error: any) {
   ...
   */
  protected validateSubAgents(subAgents: AgentSpec['SubAgents']): { errors: string[]; corrected: boolean } {
    const errors: string[] = [];
    let corrected = false;
    ...
    for (let i = 0; i < subAgents.length; i++) {
      ...
      if (subAgent.SubAgent.ID === null || subAgent.SubAgent.ID === undefined) {
        errors.push(
          `❌ SubAgent at index ${i} has null/undefined SubAgent.ID (use empty string "" for new sub-agents, or provide existing agent GUID)`
        );
      }
      ...
      if (!subAgent.SubAgent.TypeID) {
        ...
        if (isFlowSubAgent) {
          ...
          if (subAgent.SubAgent.Prompts && subAgent.SubAgent.Prompts.length > 0) {
            errors.push(
              `❌ Flow SubAgent[${i}] "${subAgent.SubAgent.Name}" MUST have empty Prompts array. Use Prompt steps or Loop sub-agents for LLM functionality.`
            );
          }
      ...
      if (subAgent.Type === 'related') {
        ...
        if (!subAgent.SubAgent.ID || subAgent.SubAgent.ID === '') {
          errors.push(
            `❌ Related SubAgent[${i}] "${subAgent.SubAgent.Name}" must have existing agent ID. Use Find Candidate Agents to get the ID.`
          );
        }
        ...
        if (!subAgent.SubAgentInputMapping && !subAgent.SubAgentOutputMapping) {
          errors.push(
            `❌ Related SubAgent[${i}] "${subAgent.SubAgent.Name}" must have at least SubAgentInputMapping or SubAgentOutputMapping`
          );
        }
      }
      ...
      if (subAgent.SubAgent.SubAgents && subAgent.SubAgent.SubAgents.length > 0) {
        ...
        if (nestedValidation.errors.length > 0) {
          ...
          const prefixedErrors = nestedValidation.errors.map((err) => `SubAgent[${i}] "${subAgent.SubAgent.Name}" -> ${err}`);
