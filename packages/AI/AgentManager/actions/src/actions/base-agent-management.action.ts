...
import { BaseEntity, Metadata, LogError, RunView, UserInfo } from '@memberjunction/global';
...
export abstract class BaseAgentManagementAction extends BaseAction {
  ...
  protected getUuidParam(params: RunActionParams, paramName: string, required: boolean = true): ParameterResult<string> {
    ...
    if (!value || value.trim().toUpperCase() === 'NULL' || value.trim() === '' || !this.isValidUUID(value.trim())) {
      return {
        error: {
          Success: false,
          ResultCode: 'VALIDATION_ERROR',
