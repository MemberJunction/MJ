/**
 * Shared BaseLLM streaming/ChatResult conformance suite applied to MiniMaxLLM.
 *
 * MiniMaxLLM is OpenAILLM with a different base URL, so its entire streaming/ChatResult behavior
 * is inherited from @memberjunction/ai-openai. Unlike minimax.test.ts (which mocks both
...
RunLLMConformanceSuite({
    ...
    CreateLLM: () => {
        const llm = new MiniMaxLLM('conformance-test-key');
