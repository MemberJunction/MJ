...
import { EntityInfo, LogError, LogStatus, Metadata, RunView, RunViewResult, UserInfo } from '@memberjunction/global';
import {
  EntityRecordDocumentEntityType,
  ListDetailEntity,
  ListEntity,
...
export class RexRecommendationsProvider extends RecommendationProviderBase {
  ...
  public async Recommend(request: RecommendationRequest<RecommendContextData>): Promise<RecommendationResult> {
    ...
    for (let i = 0; i < recommendationList.length; i += Config.REX_BATCH_SIZE) {
      ...
      const recordDocuments: EntityRecordDocumentEntityType[] | null = await this.GetEntityRecordDocuments(
        batch,
        entityDocumentID,
        request.CurrentUser
      );
      ...
      await Promise.all(
        batch.map(async (recommendation: RecommendationEntity, index: number) => {
          const recordDocument: EntityRecordDocumentEntityType | undefined = recordDocuments.find(
            (rd) => rd.RecordID == recommendation.SourceEntityRecordID
          );
          if (!recordDocument) {
            LogError(
              `No record document found for recommendation. Source Entity: ${recommendation.SourceEntityID}, Source Entity Record ID: ${recommendation.SourceEntityRecordID}`
            );
          ...
          if (!vectorID) {
            LogError(
              `No vector ID found for recommendation. Source Entity: ${recommendation.SourceEntityID}, Source Entity Record ID: ${recommendation.SourceEntityRecordID}`
            );
            result.AppendWarning(
              `No vector ID found for recommendation. Source Entity ID: ${recommendation.SourceEntityID}, Source Entity Record ID: ${recommendation.SourceEntityRecordID}`
            );
            return;
          }
          ...
          if (!recommendations) {
            LogError(
              `Error getting recommendations for recommendation: Source Entity: ${recommendation.SourceEntityID}, Source Entity Record ID: ${recommendation.SourceEntityRecordID}`
            );
            result.AppendWarning(
              `Error getting recommendations for recommendation: Source Entity: ${recommendation.SourceEntityID}, Source Entity Record ID: ${recommendation.SourceEntityRecordID}`
            );
          ...
          LogStatus(
            `Creating ${recommendations.length} recommendation items for recommendation ${index + 1}/${batch.length} of batch ${batchCount + 1}`
          );
          const recommendationItemEntities: RecommendationItemEntity[] = await this.ConvertRecommendationsToItemEntities(
            recommendation,
            recommendations,
            request.CurrentUser,
            typeMap
  ...
  private async GetEntityRecordDocuments(
    recommendations: RecommendationEntity[],
    entityDocumentID: string,
    currentUser?: UserInfo
  ): Promise<EntityRecordDocumentEntityType[] | null> {
  ...
  private async GetAccessToken(): Promise<string | null> {
    try {
      ...
      const response: AxiosResponse<RasaResponse<RasaTokenResponse>> = await axios.post<RasaResponse<RasaTokenResponse>>(
        `${Config.REX_API_HOST}/tokens`,
        body,
        config
      );
  ...
  protected async GetRecommendations(params: GetRecommendationParams): Promise<RecommendationResponse[] | null> {
    try {
      ...
      const response: AxiosResponse<RasaResponse<RecommendationResponse>> = await axios.post<RasaResponse<RecommendationResponse>>(
        `${Config.REX_RECOMMEND_HOST}/suggest?entity=0&id_response=0`,
        body,
        config
      );
      if (!response) {
  ...
  protected async ConvertRecommendationsToItemEntities(
    recommendationEntity: RecommendationEntity,
    recommendations: RecommendationResponse[],
    currentUser: UserInfo,
    typeMap: Record<string, string>
  ): Promise<RecommendationItemEntity[]> {
    const md = new Metadata();

    const entities: RecommendationItemEntity[] = await Promise.all(
      recommendations.map(async (recommendation: RecommendationResponse) => {
