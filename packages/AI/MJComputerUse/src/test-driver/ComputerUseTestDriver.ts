 ...
 ///tier telemetry. `tier` is the tier that produced this
    ...
    private async dispatchRun(
        ...
        if (decision.tier !== 'llm') {
            this.logToTestRun(context, 'info', `Tier: llm — '${decision.tier}' requested but this test has no script yet`);
        }
    ...
    private async maybeRecordScript(args: {
        ...
        try {
            ...
            this.logToTestRun(context, 'warn', `Script recording failed (non-fatal): ${e instanceof Error ? e.message : String(e)}`);
        }
    }
