...
```typescript
{
    ...
    // ── Perception and the replay tier ──
    elementGrounding?: boolean;              // Default: true
    recordReplayScript?: boolean;            // Default: true
    replayHeal?: 'llm' | 'deterministic' | 'off';   // Default: 'llm'
    forceTier?: 'llm' | 'replay' | 'replay-with-heal';

    // ── Diagnostics ──
    trace?: 'off' | 'on' | 'retain-on-failure';
    generation?: { temperature?: number; effortLevel?: number };
    appProfile?: { ... };                    // App-specific settle/readiness hints
}
```

#### Perception and replay options

| Option | Default | What it does |
|---|---|---|
| `elementGrounding` | `true` | Extracts the page's interactive elements each step so the model acts by index rather than by coordinate. **Required for recording**: a coordinate click records a target with no selector and no role/name to heal from, so a run without grounding cannot produce a replay script. |
| `recordReplayScript` | `true` | Whether a green, recordable LLM run writes its script back to the test row. Set `false` for a test still being authored, or one whose trajectory should not become the baseline others replay. The run itself is unaffected. |
| `replayHeal` | `'llm'` | How far a diverged replay step may be repaired — see below. |
| `forceTier` | unset | Pins the tier, bypassing the per-build decision. `'llm'` re-derives; `'replay'`/`'replay-with-heal'` force deterministic execution when a script exists. |

#### `replayHeal`

| Value | Behaviour |
|---|---|
| `'llm'` | Role + name re-resolution, escalating an ambiguous or failed match to a focused model call. |
| `'deterministic'` | Role + name re-resolution only; the model is never asked. |
| `'off'` | No repair at all. The first failed guard diverges. |

Use `'off'` once a script is trusted as the contract: any deviation is the finding, and a heal would hide the UI change the test exists to catch.

```jsonc
// A pinned test: generated by the agent, then frozen. No model in the run at all.
{
  "elementGrounding": true,
  "replayHeal": "off",
  "AllowLLMFallback": false   // framework-level, see below
}
```

`AllowLLMFallback` is framework-level rather than driver-level, so it sits beside `ReplayScript` in the same `Configuration` JSON but is typed by the entity's JSONType. It governs re-deriving the **whole goal** after replay has given up; `replayHeal` governs repairing a **single step** during replay. Setting both off means a replayed run passes by executing exactly what was recorded, or fails naming the diverged step.
