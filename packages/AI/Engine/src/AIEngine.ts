import {
  ...
  ChatMessage,
  ChatMessageRole,
  ParallelChatCompletionsCallbacks,
  GetAIAPIKey,
  EmbedTextResult,
  EmbedTextParams,
  BaseEmbeddings,
...
import { BaseEntity, BaseEntityEvent, LogError, Metadata, UserInfo, RunView } from '@memberjunction/global';
...
export class AIEngine extends AIEngineBase {
   ...
   */
  public async SimpleLLMCompletion(
    userPrompt: string,
    contextUser: UserInfo,
    systemPrompt?: string,
    model?: AIModelEntityExtended,
    ...
    try {
      ...
      const { modelInstance, modelToUse } = await this.PrepareLLMInstance(contextUser, model, apiKey);
      ...
      } else throw new Error(`Error executing LLM model ${modelToUse.Name} : ${result.errorMessage}`);
    } catch (e) {
      LogError(e);
      throw e;
    }
  ...
  private async loadAgentEmbeddings(): Promise<void> {
    ...
    try {
      ...
      const agentsNeedingEmbeddings = agents.filter((agent) => !this._agentEmbeddingsCache.has(agent.ID));
      ...
      console.log(
        `AIEngine: Generating embeddings for ${agentsNeedingEmbeddings.length} agents (${agents.length - agentsNeedingEmbeddings.length} already cached)...`
      );
  ...
  private async loadActionEmbeddings(): Promise<void> {
    ...
    try {
      ...
      const actionsNeedingEmbeddings = actions.filter((action) => !this._actionEmbeddingsCache.has(action.ID));
      ...
      console.log(
        `AIEngine: Generating embeddings for ${actionsNeedingEmbeddings.length} actions (${actions.length - actionsNeedingEmbeddings.length} already cached)...`
      );
  ...
  private async generateSingleAgentEmbedding(agent: AIAgentEntityExtended): Promise<void> {
    try {
      const entries = await AgentEmbeddingService.GenerateAgentEmbeddings([agent], (text) => this.EmbedTextLocal(text));
  ...
  private async generateSingleActionEmbedding(action: ActionEntity): Promise<void> {
    try {
      const entries = await ActionEmbeddingService.GenerateActionEmbeddings([action], (text) => this.EmbedTextLocal(text));
   ...
   */
  public async FindSimilarAgents(taskDescription: string, topK: number = 5, minSimilarity: number = 0.5): Promise<AgentMatchResult[]> {
    if (!this._agentVectorService) {
   ...
   */
  public async FindSimilarActions(taskDescription: string, topK: number = 10, minSimilarity: number = 0.5): Promise<ActionMatchResult[]> {
    if (!this._actionVectorService) {
  ...
  public async ParallelLLMCompletions(
    ...
    try {
      ...
      const paramsArray: ChatParams[] = Array(iterations)
        .fill(0)
        .map((_, i) => {
          const params = new ChatParams();
  ...
  public get LocalEmbeddingModels(): AIModelEntityExtended[] {
    ...
    const embeddingModels = AIEngine.Instance.Models.filter(
      (m) =>
        m.AIModelType?.trim().toLowerCase() === AIEngine.Instance.EmbeddingModelTypeName.toLowerCase() &&
        m.Vendor &&
        m.Vendor.trim().toLowerCase() === AIEngine.Instance.LocalEmbeddingModelVendorName.toLowerCase() // Only use local embedding models
    );
  ...
  public async EmbedText(model: AIModelEntityExtended, text: string, apiKey?: string): Promise<EmbedTextResult> {
    ...
    const embedding = MJGlobal.Instance.ClassFactory.CreateInstance<BaseEmbeddings>(BaseEmbeddings, model.DriverClass, apiKey);
  ...
  public async ExecuteEntityAIAction(params: EntityAIActionParams): Promise<BaseResult> {
    ...
    try {
      ...
      if (entityAction.SkipIfOutputFieldNotEmpty && entityAction.OutputType.trim().toLowerCase() === 'field') {
        ...
        if (val && val.length > 0) return null; // if the output field is already populated, then we skip the action
      }

      // first, pre-process the entity AI Action
      const entityPrompt = params.systemPrompt
        ? params.systemPrompt
        : entityAction.Prompt && entityAction.Prompt.length > 0
          ? entityAction.Prompt
      ...
      if (!(await params.entityRecord.BeforeEntityAIAction(entityParams))) return null; // if the entity record BeforeEntityAIAction() call returns false, then we don't execute the action
      ...
      if (results) {
        ...
        if (!(await params.entityRecord.AfterEntityAIAction(entityParams))) return results; // if the entity record AfterEntityAIAction() call returns false, then we don't FURTHER process the results
