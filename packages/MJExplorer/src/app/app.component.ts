...
import { LogError, LogStatus, Metadata, SetProductionStatus } from '@memberjunction/global';
    ...
    try {
      ...
      if (err.response && Array.isArray(err.response.errors)) {
        return err.response.errors.some(
          (e: any) => e && e.message && typeof e.message === 'string' && e.message.includes('does not have read permissions on User Roles')
        );
      }
      ...
      if (err.message && typeof err.message === 'string') {
        ...
        return (
          message.includes('does not have read permissions on User Roles') ||
