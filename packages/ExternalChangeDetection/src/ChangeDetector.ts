...
export class ExternalChangeDetectorEngine extends BaseEngine<ExternalChangeDetectorEngine> {
  ...
  public get EligibleEntities(): EntityInfo[] {
    const netEligible = this._EligibleEntities.filter(
      (e) => !this.IneligibleEntities.map((i) => i.toLowerCase().trim()).includes(e.Name.toLowerCase().trim())
    );
    return netEligible;
  }
  ...
  public async DetectChangesForEntity(entity: EntityInfo): Promise<ChangeDetectionResult> {
    try {
      ...
      if (!entity) {
        ...
        throw new Error(
          `Entity ${entity.Name} is not eligible for external change detection. Refer to the documentation on the EligibleEntities and IneligibleEntities properties for more information.`
        );
      }
      ...
      if (createResult && createResult.length > 0) {
        for (const row of createResult) {
          ...
          item.Entity = entity;
          item.PrimaryKey = new CompositeKey(
      ...
      if (deleteResult && deleteResult.length > 0) {
        deleteResult.forEach((row) => {
          ...
          } else ck.LoadFromConcatenatedString(row.RecordID); // this is a string like 'Field1Value|Field2Value' (no quotes)
   ...
   * @param change
   */
  public async DetermineRecordChanges(
    md: Metadata,
    change: ChangeDetectionItem
  ): Promise<{ changes: FieldChange[]; latestRecord: BaseEntity }> {
    try {
      ...
      if (record) {
        ...
        if (result && result.FullRecordJSON && result.FullRecordJSON.length > 0) {
          ...
          LogStatus(
            `      WARNING: No record found, or no FullRecordJSON found, in vwRecordChanges for ${change.Entity.Name}: ${change.PrimaryKey.ToConcatenatedString()}`
          );
          return { changes: [], latestRecord: record };
        }
      } else throw new Error(`Failed to load record: ${change.Entity.Name}: ${change.PrimaryKey}`);
    } catch (e) {
      LogError(e);
  ...
  protected getPrimaryKeyString(entity: EntityInfo, tablePrefix: string): string {
    return entity.PrimaryKeys.map(
      (pk) => `'${pk.Name}${CompositeKey.DefaultValueDelimiter}' + CAST(${tablePrefix}.[${pk.Name}] AS NVARCHAR(MAX))`
  ...
  public async DetectChangesForEntities(entities: EntityInfo[]): Promise<ChangeDetectionResult> {
    try {
      if (!entities) throw new Error('entities parameter is required');
      else if (entities.length === 0) throw new Error('entities parameter must have at least one entity in it');
      ...
      entities.forEach((e) => {
        ...
        p.then((entityResult) => {
          UpdateCurrentConsoleProgress(
            `   Finished change detection changes for ${e.Name}`,
            ++numFinished,
            entities.length,
            entityResult.Success ? ConsoleColor.cyan : ConsoleColor.crimson
          );
      ...
      UpdateCurrentConsoleProgress(
        `   Finished detecting changes for ${entities.length} entities`,
        entities.length,
        entities.length,
  ...
  public async ReplayChanges(changes: ChangeDetectionItem[], batchSize: number = 20): Promise<boolean> {
    ...
    try {
      ...
      } else return true; // no changes to process
    } catch (e) {
      LogError(e);
      if (run) {
  ...
  protected async StartRun(): Promise<RecordChangeReplayRunEntity> {
    ...
    if (existingRun && existingRun.Success) {
      if (existingRun.Results.length > 0)
        throw new Error(
          `Existing Record Change Replay Run ${existingRun.Results[0].ID} is not complete or marked as error, cannot start a new run.`
        );
      else {
        const md = new Metadata();
        ...
        if (await run.Save()) {
          return run;
        } else throw new Error('Failed to start run');
      }
    } else {
      ...
      if (rc) {
        ...
        if (!entityObject) {
          ...
          if (latestRCData && latestRCData.FullRecordJSON?.length > 0) {
            ...
            return this.FinishRecordChangeRecord(
              rc,
              'error',
              'No prior record data is available in the Record Changes entity, so it is not possible to replay the Delete'
            );
          }
        }
   ...
   * @param change
   */
  protected async CreateRecordChangeRecord(
    md: Metadata,
    run: RecordChangeReplayRunEntity,
    change: ChangeDetectionItem
  ): Promise<RecordChangeEntity> {
    try {
      ...
      else rc.RecordID = change.PrimaryKey.ToConcatenatedString();

      rc.Source = 'External';
      rc.Type = change.Type;
      rc.Status = 'Pending';
      if (change.ChangedAt) rc.ChangedAt = change.ChangedAt;
      else {
      ...
      if (change.Type === 'Update') rc.ChangesJSON = JSON.stringify(changesObject);
      else rc.ChangesJSON = ''; // not null

      if (change.Type !== 'Delete') {
        const obj = change.LatestRecord?.GetAll();
        if (obj) rc.FullRecordJSON = JSON.stringify(obj);
        else rc.FullRecordJSON = ''; // null not allowed
      } else rc.FullRecordJSON = ''; // null not allowed

      const provider = <SQLServerDataProvider>this.ProviderToUse;
      if (change.Type === 'Update') rc.ChangesDescription = provider.CreateUserDescriptionOfChanges(changesObject);
      else if (change.Type === 'Create') rc.ChangesDescription = 'New Record';
      else rc.ChangesDescription = 'Record Deleted';
