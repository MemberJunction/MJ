...
// Source inspection is delegated to the source-auditor subagent (Explore-like
// browse, WebFetch, Bash for curl fallback).  Four blind parallel facets keeps
// no single inspector dominant.
const facets = await parallel([
    () => agent(`Inspect freshness of ${sourceID} — last-updated metadata, version markers, deprecation notices. Return score 0..1 plus evidence.`, { agentType: 'source-auditor', schema: FACET_SCHEMA, phase: 'parallel-audit', label: 'audit:freshness' }),
    () => agent(`Inspect coverage of ${sourceID} — proportion of plausibly-extractable objects/fields visible from this single source. Return score 0..1 plus evidence.`, { agentType: 'source-auditor', schema: FACET_SCHEMA, phase: 'parallel-audit', label: 'audit:coverage' }),
    () => agent(`Inspect authority of ${sourceID} — vendor-published vs. third-party, official docs vs. community wiki. Return score 0..1 plus evidence.`, { agentType: 'source-auditor', schema: FACET_SCHEMA, phase: 'parallel-audit', label: 'audit:authority' }),
    () => agent(`Inspect format quality of ${sourceID} — machine-parseable (OpenAPI / JSON) > structured HTML > prose. Return score 0..1 plus evidence.`, { agentType: 'source-auditor', schema: FACET_SCHEMA, phase: 'parallel-audit', label: 'audit:format' }),
]);
