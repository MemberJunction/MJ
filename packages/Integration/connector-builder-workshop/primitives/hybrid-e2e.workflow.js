...
const ENV_BRINGUP =
    ...
    `      STEP A2 — SEED FRAMEWORK REMOTE-OPERATIONS METADATA (after migrate, BEFORE codegen): \`npx mj sync push --dir metadata/remote-operations\` (entity 'MJ: Remote Operations' — 14 rows incl the RecordComparison 'Compare Records' + PredictiveStudio 'Train ML Model'/'Score Record Set'/'Run Feature Pipeline' Remote Operations). 🚨 MANDATORY: fresh-DB codegen — BOTH this setup pass (STEP B) AND the later IN-PROCESS RSU codegen that ApplyAll triggers — regenerates \`packages/MJCoreEntities/src/generated/remote_operations.ts\` FROM the DB's MJ:Remote-Operations rows. Those rows are NOT baked into any migration, so WITHOUT this push codegen OMITS the RecordComparison + PredictiveStudio RO base classes → the HEAD @memberjunction/record-comparison + predictive-studio packages fail to build (turbo build exit 2) / MJAPI-boot. Seeding here makes EVERY codegen pass emit them; this is the durable fix (the per-file git-restore in STEP B only patched the setup pass, NOT the RSU pass). Unrelated to the connector under test. If the scoped push errors on a missing root sync config, push from an isolated temp dir mirroring metadata/ with only \`.mj-sync.json\` + \`remote-operations/\`, or \`npx mj sync push --dir metadata --include remote-operations\`. VERIFY before codegen: the MJ:Remote Operations table (\`${DB_NAME}.__mj.RemoteOperation\`) has >= 14 rows.\n` +
