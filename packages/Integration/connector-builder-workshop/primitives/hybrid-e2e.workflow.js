...
if (HAS_BROKER_CREDS) {
    ...
    primaryResult = await agent(
        ...
        ENV_BRINGUP +
        `\nThen run connector-e2e (${RUNBOOK} step 6, MOCK mode, E2E_PLATFORM=sqlserver). MOCK mode needs RICH fixtures at packages/Integration/connectors/test/fixtures/${VENDOR}/fixtures/fixtures.json for the SHIPPED connector shape (config-driven base-URL connectors → ORIGIN mode: Transport:http, ConfigUrlKey=<the config key>, Routes mirroring the connector's real endpoints, Objects, DeltaPasses). AUTO-GENERATE these with \`node packages/Integration/connector-builder-workshop/scripts/gen-fixtures.mjs\` (env: VENDOR, INTEGRATION_NAME, DB coords, OUT_DIR=<that fixtures dir>, optional HUB/OBJECTS/SHAPER) — it reads the DEPLOYED schema + FK graph and emits an FK-connected multi-object set, multi-page hub, custom/overflow fields, soft-DELETES, value-type variety, and a delta pass (NEVER a thin 2-object stub). It defaults to a REST APIPath route-shaper; SOQL/GraphQL connectors pass SHAPER (Salesforce SOQL reference: connectors/test/gen-sf-rich-fixtures.mjs). If fixtures are missing or a STALE different-shape scaffold, regenerate them first. Never reuse stale fixtures.\n\n` +
        ...
        `  • firstSyncComplete — per-table rowcounts after the full pull vs after the idempotent re-run; a table that fills only on a later pass = door-before-child ordering defect (FALSE). secondSyncGrew — TRUE if any table grew between the full pull and the idempotent re-run. captureEngaged — every created table has __mj_integration_CustomOverflow AND customs captured where the source carries them.\n` +
