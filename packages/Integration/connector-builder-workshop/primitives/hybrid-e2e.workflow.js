...
// The connector .ts is named by its CLASS symbol (e.g. HigherLogicVanillaConnector.ts), NOT the registry
// slug (higherlogic-vanilla). The pre-flight file check must use the ClassName; fall back to the slug for
// older callers that don't pass it.
const CLASS_NAME = A?.className ?? CONNECTOR;
...
const ENV_BRINGUP =
    `⚠ DO NOT GUESS ANY STEP. Read ${RUNBOOK} and execute its env bring-up (steps 0→5) VERBATIM, in order. It captures every gotcha. The ONLY assumption is the Docker daemon is up. [rev2 2026-07-09: DB_TRUST_SERVER_CERTIFICATE MUST be \`true\`/\`1\` NEVER \`Y\` (see the 🚨 note below) so the RSU CodeGen applies the entity SQL; the connector's fixtures now carry the connector's real base-path prefix + a health/TestConnection route so the sync's ConnectionTest gets a 2xx instead of a 404 and rows land.]\n` +
    ...
    `      STEP A2 — SEED FRAMEWORK REMOTE-OPERATION CATEGORIES + OPERATIONS METADATA (after migrate, BEFORE codegen): FIRST \`npx mj sync push --dir metadata/remote-operation-categories\` (entity 'MJ: Remote Operation Categories' — 6 rows incl 'Record Comparison'/'Predictive Studio'/'AI Skills'), THEN \`npx mj sync push --dir metadata/remote-operations\` (entity 'MJ: Remote Operations' — 16 rows incl the RecordComparison 'Compare Records' + PredictiveStudio 'Train ML Model'/'Score Record Set'/'Run Feature Pipeline' + AI Skills 'Export AI Skill Markdown'/'Import AI Skill Markdown' Remote Operations). 🚨 CATEGORY-FIRST ORDER MATTERS: each RO row's CategoryID is an @lookup on 'MJ: Remote Operation Categories.Name'; if the category (esp. 'AI Skills', created only by the v5.44.x Agent_Skills_And_Plan_Mode migration which may sit above the e2e baseline) isn't seeded first, mj-sync ROLLS BACK THE ENTIRE remote-operations push on 'Lookup failed' → 0 RO rows → codegen strips ALL RO base classes → HEAD @memberjunction/ng-core-entity-forms (imports AISkillExport/ImportMarkdownOperation), record-comparison, and predictive-studio fail to build — THIS is the ApplyAll in-process-RSU AFTER-command 'npm build exit code 1' failure. Pushing categories first is idempotent (no-op if the migration already seeded them). 🚨 MANDATORY: fresh-DB codegen — BOTH this setup pass (STEP B) AND the later IN-PROCESS RSU codegen that ApplyAll triggers — regenerates \`packages/MJCoreEntities/src/generated/remote_operations.ts\` FROM the DB's MJ:Remote-Operations rows. Those rows are NOT baked into any migration, so WITHOUT this push codegen OMITS the RecordComparison + PredictiveStudio + AI Skills RO base classes → the HEAD packages that import them fail to build (exit 1/2) / MJAPI-boot / ApplyAll AFTER-command failure. Seeding here makes EVERY codegen pass emit them; this is the durable fix (the per-file git-restore in STEP B only patched the setup pass, NOT the RSU pass). Unrelated to the connector under test. If a scoped push errors on a missing root sync config, push from an isolated temp dir mirroring metadata/ with only \`.mj-sync.json\` + the two dirs, or \`npx mj sync push --dir metadata --include remote-operation-categories --include remote-operations\`. VERIFY before codegen: \`${DB_NAME}.__mj.RemoteOperationCategory\` has >= 6 rows AND \`${DB_NAME}.__mj.RemoteOperation\` has >= 16 rows (if RemoteOperation < 16, the category @lookup rolled the push back — seed categories first).\n` +
    `      STEP B — FULL CODEGEN against THIS fresh DB (BEFORE build, BEFORE sync push): \`node packages/MJCLI/bin/run.js codegen\` with the ISOLATION-OVERRIDE DB_* env (host ${DB_HOST}, port ${DB_PORT}, DB ${DB_NAME}, user ${DB_USER}) + advancedGeneration OFF. WHY FIRST: the committed generated.ts is empty/polluted with broken in-flight connector entities (e.g. salesforceAccount_/propfuel* Field lacking a GraphQL type → "NoExplicitTypeError" → MJAPI never binds). Full codegen regenerates the WHOLE generated tree (entity_subclasses incl. the IntegrationObjectEntity class WITH its MetadataSource setter, generated.ts, bootstrap manifests) to MATCH the fresh base DB — clean, base-only, self-consistent. If codegen dies importing a stale entity (e.g. MJRSUAuditLogEntity) from a polluted committed manifest, \`git checkout HEAD -- <that manifest>\` and retry. 🔧 FRAMEWORK-BOOTSTRAP BYPASS (unrelated-package RemoteOperations): fresh-DB codegen regenerates \`packages/MJCoreEntities/src/generated/remote_operations.ts\` from the DB's MJ:Remote-Operations rows, but the fresh baseline LACKS the RecordComparison + PredictiveStudio RemoteOperation metadata (those rows live only in metadata/remote-operations/, not baked into any migration) → codegen WIPES those RO base classes → the HEAD @memberjunction/record-comparison + predictive-studio packages that import them crash MJAPI at boot. These packages are IRRELEVANT to a connector sync e2e. IMMEDIATELY after codegen (BEFORE STEP C build) restore that ONE committed generated file to HEAD: \`git checkout HEAD -- packages/MJCoreEntities/src/generated/remote_operations.ts\` — HEAD carries the full RO class set. This does NOT touch entity_subclasses (which stay matched to the fresh DB, correct for the connector); STEP C then compiles the restored file so the RO classes exist at boot. If MJAPI STILL fails to boot on another unrelated missing generated symbol from a package not under test, apply the SAME \`git checkout HEAD -- <that generated file>\` + rebuild, then relaunch. VERIFY: \`grep -ciE 'salesforce|propfuel|orcid|openwater|path_?lms|growthzone' packages/MJAPI/src/generated/generated.ts\` returns 0.\n` +
    ...
    `  • DB_* = the ISOLATION-OVERRIDE SQL Server coords above (host ${DB_HOST}, port ${DB_PORT}, DB ${DB_NAME}, user ${DB_USER}) + DB_PLATFORM=sqlserver + DB_TRUST_SERVER_CERTIFICATE=true + MJ_CODEGEN_NO_AFTER=1; generate MJ_BASE_ENCRYPTION_KEY; MJAPI on :${GQL_PORT}. A non-fatal EADDRINUSE for the secondary instance-B/WS port is OK if GraphQL on :${GQL_PORT} answers.\n` +
    `  • 🚨 MJ_CODEGEN_NO_AFTER=1 IS MANDATORY in the MJAPI launch env [rev4]. mj.config.cjs reads it → \`commands: []\` so the in-process RSU CodeGen that ApplyAll triggers SKIPS its post-generation AFTER commands (npm run build×N + npm start). Those are REDUNDANT here (MJAPI is already up; the entity view/sproc SQL is applied by codegen's own SQL-execution step which runs BEFORE the AFTER commands) and they FAIL FAST in the tsx MJAPI child env (~0.08s each → "exited with code 254") → the RSU reports RunCodeGen:failed EVEN THOUGH ExecuteMigration:success (SQL WAS applied, tables DO exist) → ApplyAll is falsely-failed → sync aborts with entityMapCount:0/0 rows. With =1 the RSU stops after SQL execution → RunCodeGen:success → ApplyAll succeeds → sync lands rows. (Verified on the HLV build 2026-07-09: tables were created but ApplyAll false-failed on the npm AFTER-command until MJ_CODEGEN_NO_AFTER=1 was set.)\n` +
    `  • 🚨 DB_TRUST_SERVER_CERTIFICATE MUST be the literal string \`true\` (or \`1\`) EVERYWHERE — migrate, STEP B codegen, AND the MJAPI launch — NEVER \`Y\`. mj.config.cjs parses it as \`=== '1' || === 'true'\`, so \`Y\` silently becomes trustServerCertificate=FALSE. MJAPI's own pool tolerates that (DB_ENCRYPT=false skips its TLS), but the IN-PROCESS RSU CodeGen ApplyAll triggers uses its OWN CodeGenLib connection that STILL rejects the self-signed cert with trust=false — it GENERATES the new connector entities' view/sproc SQL to disk but FAILS TO APPLY it (swallowed "self-signed certificate" error) → the sync dies "Cannot find object 'vw<Entity>'/'spCreate<Entity>'" and lands 0 rows with applyAllRan:false. DB_ENCRYPT=false does NOT fix codegen — only trust=true does. If the runbook you read shows \`=Y\`, use \`=true\` instead. (Root-caused on the HLV build 2026-07-09: \`mj codegen\` failed on the cert with Y, succeeded with true, creating vwRanks/spCreateAddon.)\n` +
    `  • AFTER the e2e run, if it still reports "Cannot find object 'vw<X>'/'spCreate<X>'" / applyAllRan:false, the RSU CodeGen generated-but-unapplied the entity SQL — run one explicit CLI codegen against ${DB_NAME} with trust=true to apply it, then re-run the sync: \`DB_PLATFORM=sqlserver DB_HOST=${DB_HOST} DB_PORT=${DB_PORT} DB_DATABASE=${DB_NAME} DB_USERNAME=${DB_USER} DB_PASSWORD=<pw> DB_TRUST_SERVER_CERTIFICATE=true DB_ENCRYPT=false MJ_CORE_SCHEMA=__mj CODEGEN_DB_USERNAME=${DB_USER} CODEGEN_DB_PASSWORD=<pw> node packages/MJCLI/bin/run.js codegen\` (the connector entities must already be registered by ApplyAll — verify \`SELECT COUNT(*) FROM sys.views WHERE name='vw<SomeConnectorEntity>'\` = 1 afterward).\n` +
...
const preflight = await agent(
    ...
    `  1. connectorFileExists: does packages/Integration/connectors/src/${CLASS_NAME}.ts exist? (else the e2e cannot run — SETUP).\n` +
    ...
    `Return { ok, classification ('setup' if any check failed for an env/fixture/metadata reason, else 'ok'), connectorFileExists, metadataObjectCount, fixturesPresent, failures[{rule,detail}] }. ok = connectorFileExists AND metadataObjectCount>0 AND fixturesPresent.${A?.rerunNote ? `\n\n${A.rerunNote}` : ''}`,
...
// The bring-up's full `mj codegen` regenerates the ENTIRE generated tree in place. The snapshot must
// therefore cover EVERY dir codegen touches — otherwise an unprotected dir is left mutated (and, on a
// feature branch with pre-existing generated drift, the caller's uncommitted work is silently clobbered).
// Widened to include the dirs the EnvPreflight gate also watches (GeneratedEntities, MJAPI, ServerBootstrap)
// so the working tree is byte-identical after this primitive regardless of outcome OR crash.
const GENERATED_DIRS = 'packages/MJCoreEntities/src/generated packages/MJServer/src/generated packages/Angular/Explorer/core-entity-forms/src/lib/generated packages/GeneratedEntities/src/generated packages/MJAPI/src/generated packages/ServerBootstrap/src/generated';
...
if (HAS_BROKER_CREDS) {
    ...
    primaryResult = await agent(
        ...
        ENV_BRINGUP +
        `\n⏱ [rev3 2026-07-09] CRITICAL — the connector-e2e run is SLOW and MUST NOT be killed by a short bash timeout. ApplyAll's in-process RSU CodeGen for a large catalog (this connector = 65 entities) alone takes ~2–3 MINUTES, and the full mock sync + delta + idempotent passes add several more. The DEFAULT 120s bash timeout WILL kill it mid-ApplyAll (exit 143 / "bash session timeout") — leaving 0 tables + a false failure (that is exactly what happened last run). So run the connector-e2e command EITHER (a) with the MAXIMUM bash timeout — timeout: 600000 (10 min) on the Bash call — OR PREFERABLY (b) in the BACKGROUND (run_in_background:true) and then POLL its output/log until it prints its final JSON result (up to ~20 min), re-reading every ~30–60s. NEVER run it with the default/short timeout. A timeout is NOT a connector failure — re-run with the long timeout/background.\n` +
        ...
        `  • READ COVERAGE (MOCK = FULL, NO subset — agent-arc rule) — the mock vendor is FREE + INSTANT, so EVERY active object MUST land >=1 row. Set coveredObjects = the IO.Name list that landed >=1 row (this MUST equal the FULL active IO set). Bound ROW DEPTH per object (don't generate 50k mock rows), NEVER the object SET. The fixtures MUST carry data for EVERY object so all can sync — if a fixture is missing for any object, ADD it. Cover ALL objects for READS, not a representative few.\n` +
        `  • WRITE COVERAGE (shape-representative, NOT per-object exhaustive) — round-tripping create/update/delete on all 14 writable objects is REDUNDANT when most share an IDENTICAL generic mechanism (same CreateBodyShape/CreateMethod/CreateIDLocation + UpdateBodyShape/UpdateMethod/UpdateIDLocation + DeleteMethod/DeleteIDLocation tuple) — re-proving the same code path 14 times proves nothing the 1st proof didn't. Only ONE representative per DISTINCT mechanism family needs a genuine round-trip.\n` +
        (A?.deterministicWriteProof
            ? `    🔒 THIS PROOF IS ALREADY DONE — DO NOT ATTEMPT FREEFORM WRITE CALLS AGAINST THE MOCK. ${A.deterministicWriteProof}\n` +
              `    Your job for write-coverage is ONLY to copy the two arrays below VERBATIM into your returned assertions — do not re-derive, re-test, or second-guess them:\n` +
              `      assertions.exercisedWrites = ${JSON.stringify(A?.exercisedWritesList ?? [])}\n` +
              `      assertions.skippedObjects = ${JSON.stringify(A?.skippedWritesList ?? [])}\n` +
              `    (Deterministic, non-agent-driven proof — vitest calls the connector's real CreateRecord/UpdateRecord/DeleteRecord methods with mocked HTTP responses shaped exactly from this connector's own persisted metadata, asserting the exact wire shape (URL/method/body) and parsed CRUDResult for each. This is at least as strong a proof of the write CODE PATH as a freeform mock-DB round-trip — the only thing it does not cover is the WriteBackProcessor/RecordProcessor plumbing that calls these methods, which is shared framework code already proven by every other connector's HybridE2E run.)\n`
            : `    Instead, round-trip exactly ONE object per DISTINCT mechanism family (group the writable IOs by that tuple; pick one representative per group) — for THIS connector's frozen contract that is 8 representatives: Answers (canonical flat POST/POST/DELETE-path — also covers BlogComments/Comments/Questions, byte-identical tuple), Blogs (update ID in PATH not body), ExternalActivity (update via PUT not POST — also covers DiscussionPosts), EventTypes (shared create+update endpoint, POST-verb delete), ResourceLibraryDocuments (literal create + flat update mix), DocumentAttachments (literal create, no update), DemographicChoices (create-only, no update/delete — also covers DemographicTypes/Ideas), Volunteers (literal create with synthesized composite identity, delete-only, no update). Set exercisedWrites to these 8 (once each GENUINELY round-tripped with observed DB effect — no exceptions on the 8). For the other 6 (BlogComments, Comments, Questions, DiscussionPosts, DemographicTypes, Ideas), add them to skippedObjects with a CONCRETE per-object reason naming their representative and the exact shared tuple, e.g. {object:"BlogComments", reason:"identical mechanism to Answers: CreateBodyShape=flat/CreateMethod=POST/CreateIDLocation=body, UpdateBodyShape=flat/UpdateMethod=POST/UpdateIDLocation=body, DeleteMethod=DELETE/DeleteIDLocation=path — proven via Answers, re-testing here is redundant"} — this is a legitimate, evidenced skip (not a blanket coverageScopeReason/Goldilocks-subset, which is still REJECTED), because it identifies the EXACT shared code path already proven, not a vague scope excuse.\n`) +
        `status='pass' ONLY if applyAllRan AND forwardCompleteness AND syncLandedRows AND idempotentZeroWork AND firstSyncComplete!==false AND secondSyncGrew!==true AND captureEngaged!==false AND coveredObjects equals the full active IO set (every object) AND the NEW harness's \`coverage.all-objects\` gate step reported ok=TRUE. If coverage.all-objects is not ok (any object synced 0 rows), status='fail' — fix the relational fixtures / DAG access-path so that object lands rows; do NOT scope it away. Return scrubbed steps VERBATIM in steps (INCLUDING the coverage.all-objects step), the harness result.lifecycle[] array VERBATIM in lifecycle (the birth→death per-stage matrix the deterministic lifecycle gate enforces — any stage status==='fail' fails the e2e), a plain-English nl summary, phaseId='E2E.Hybrid.Mock', and mjapiPort.${A?.rerunNote ? `\n\n${A.rerunNote}` : ''}`,
