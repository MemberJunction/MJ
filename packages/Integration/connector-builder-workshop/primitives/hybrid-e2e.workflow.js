...
// Live e2e broker task: generic single-token `connector-e2e-live` by default, but MULTI-SECRET
// connectors (e.g. SharePoint OAuth2 client-credentials — TenantId/ClientId/ClientSecret) need a
// vendor-specific plan that assembles the connector's CredentialValues from several broker secrets.
// The planner passes `liveE2ETask:'<vendor>-e2e-live'`; the broker plan governs which secrets inject.
const LIVE_E2E_TASK = (typeof A?.liveE2ETask === 'string' && A.liveE2ETask) ? A.liveE2ETask : 'connector-e2e-live';
