...
// AND volatile-field-sensitive (a per-request timestamp inside the first 200 chars made identical records
// look different → false advance, which would mask the GZ dead-pagination bug). We key on the record's
// stable identity field (id/uuid/...) when present; only id-less records fall back to a content hash.
const ID_FIELDS = ['id', 'Id', 'ID', '_id', 'uuid', 'Uuid', 'UUID', 'guid', 'Guid', 'key', 'Key', 'code', 'Code'];
function recordIdentity(r) {
    if (r && typeof r === 'object') {
        ...
        for (const k of Object.keys(r)) {
            if (/(^|_)(id|uuid|guid)$/i.test(k) && r[k] != null && typeof r[k] !== 'object') return `${k}=${r[k]}`;
        }
    }
    try { return 'h:' + createHash('sha256').update(JSON.stringify(r)).digest('hex').slice(0, 16); } catch { return ''; }
}
function pageIDs(recs) {
    return (recs || []).map(recordIdentity);
}
/** Template-var substitution (P10-general): --template-vars '{"iD":"0000-...","eventId":"123"}'
 *  supplies KNOWN parent/key values (e.g. a vendor's documented public test record) so
 *  parent-scoped paths become probeable. Vars without a supplied value stay NAMED-unverified. */
const TEMPLATE_VARS = (() => { try { return args['template-vars'] ? JSON.parse(args['template-vars']) : {}; } catch { return {}; } })();
function substituteTemplateVars(path) {
    let out = path;
    for (const [k, v] of Object.entries(TEMPLATE_VARS)) {
        out = out.replace(new RegExp(`\\{${k}\\}`, 'gi'), encodeURIComponent(String(v)));
    }
    return out;
}
...
for (const io of ios) {
    ...
    // 1. PATH claim. Template vars resolve from --template-vars (documented test/parent IDs);
    //    unresolved vars are recorded 'unverified' BY NAME (never silently skipped; P9 coverage rule).
    const resolvedPath = substituteTemplateVars(path);
    if (stripTemplateVars(resolvedPath)) {
        emitVerdict({ object: name, kind: 'path', claim: path, verdict: 'unverified', evidence: 'template-var path needs a live parent ID; supply via --template-vars or probe after a parent page is captured' });
        continue;
    }
    ...
    if (r.status === 0) pathVerdict = { verdict: 'unverified', evidence: `network: ${r.error}` };
        ...
        // CREDENTIALED: a 2xx means we actually READ the path → confirmed. Non-2xx falls to wrong/unverified.
        if (r.status >= 200 && r.status < 300) pathVerdict = { verdict: 'confirmed', evidence: `HTTP ${r.status}${recs ? `, ${recs.length} record(s)` : ''}` };
        ...
        // KEYLESS: we can prove EXISTENCE only, never content. A 401/403 means the path is REAL and
        // auth-gated — but that is NOT 'confirmed' (pre-fix it was, so a wall of green 'confirmed' on a
        // keyless run really only meant "the host auth-gates", and a WRONG-but-auth-gated path passed —
        // Facet-D #H3). Distinct 'gated-exists' verdict: existence proven, content UNVERIFIED.
        if (r.status === 200) pathVerdict = { verdict: 'confirmed', evidence: `HTTP 200 (public, unauthenticated read)` };
        else if (r.status === 401 || r.status === 403) pathVerdict = { verdict: 'gated-exists', evidence: `HTTP ${r.status} — path real + auth-gated; existence proven, content UNVERIFIED (no credential)` };
        else if ([404, 405, 400].includes(r.status)) pathVerdict = { verdict: 'wrong', evidence: `HTTP ${r.status} on declared path` };
        else pathVerdict = { verdict: 'unverified', evidence: `HTTP ${r.status}` };
    }
    ...
    if (recs && recs.length > 0) {
        for (const iof of io.iofs.filter((x) => x.IsPrimaryKey === true)) {
            ...
            // #H6: a DECLARED primary key must be populated on EVERY probed record — a PK null on even one
            // record has no stable identity (dedupe/ordering break). Pre-fix, partial population (0<present<all)
            // was 'unverified' and slipped the floor, so a LARGELY-null PK shipped green. Now ANY null is 'wrong'
            // (feeds ProbeAmend → demote to content-hash identity or fix the PK). Only all-populated is confirmed.
            emitVerdict({
                object: name, kind: 'pk', claim: `${k} is the populated PK`,
                verdict: present === recs.length ? 'confirmed' : 'wrong',
                evidence: `${present}/${recs.length} populated, ${absent}/${recs.length} absent on the probe page${present < recs.length ? ` — declared PK is null/empty on ${recs.length - present} record(s) (the GZ #5 null-PK class; demote to content-hash identity or correct the PK)` : ''}`,
            });
        }
    }
...
const summary = {
    ...
    gatedExists: verdicts.filter((v) => v.verdict === 'gated-exists').length, // keyless: path real + auth-gated, content UNVERIFIED
    ...
    // honest ceiling: a keyless run can only ever reach existence-proof, never content-verification, so a
    // run with ZERO 'confirmed' content reads (everything gated-exists/unverified) is format-verified-no-creds.
    achievedCeiling: (!TOKEN || verdicts.filter((v) => v.verdict === 'confirmed').length === 0)
