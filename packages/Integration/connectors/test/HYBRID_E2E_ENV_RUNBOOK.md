...
`HS_LIVE_DB_NAME` to it, not a reused workbench DB. The connector's metadata is synced into
the fresh DB by the baseline's Metadata-Sync migration (or run `mj sync push` if absent).
