...
async function makeMssqlClient(dbCfg, mjSchema) {
    ...
    const pool = new sql.ConnectionPool({
        ...
        // PRODUCTION-SCALE: the mssql default requestTimeout is 15s. A real sync (tens of thousands
        // of rows) makes the DB busy, so a completeness COUNT or a teardown DELETE issued mid/post-sync
        // routinely exceeds 15s and fails with "RequestError: Timeout: Request failed to complete in
        // 15000ms" (and rolls back the teardown transaction). Make it a generous, env-tunable budget.
        connectionTimeout: Number(dbCfg.connectionTimeoutMs ?? process.env.E2E_DB_CONNECTION_TIMEOUT_MS ?? 60000),
        requestTimeout: Number(dbCfg.requestTimeoutMs ?? process.env.E2E_DB_REQUEST_TIMEOUT_MS ?? 600000),
    ...
    return {
        ...
        // Pre-run cleanup: empty an entity's mirror table + its record maps so forward.completeness
        // (rows === recordMap.total) measures THIS run only. Teardown deliberately RETAINS data tables
        // (deleteConnection deleteData is a no-op on tables by design), so prior runs' synthetic rows
        // would otherwise inflate the row count above the fresh record-map count → false completeness fail.
        async clearEntityData(entityName) {
            const e = await entityMeta(entityName);
            await pool.request().input('eid', sql.UniqueIdentifier, e.ID)
                .query(`DELETE FROM [${ident(mjSchema)}].[CompanyIntegrationRecordMap] WHERE EntityID = @eid`);
            await pool.request().query(`DELETE FROM [${ident(e.SchemaName)}].[${ident(e.BaseTable)}]`);
        },
...
async function makePgClient(dbCfg, mjSchema) {
    ...
    const client = new pg.default.Client({
        ...
        user: dbCfg.user,
        password: dbCfg.password,
        // Symmetric with mssql: pg defaults to NO timeout (a stalled query hangs the run forever),
        // so give it the same generous, env-tunable budget. statement_timeout is server-side;
        // query_timeout is client-side — set both so neither a busy DB nor a dropped socket wedges a run.
        statement_timeout: Number(dbCfg.requestTimeoutMs ?? process.env.E2E_DB_REQUEST_TIMEOUT_MS ?? 600000),
        query_timeout: Number(dbCfg.requestTimeoutMs ?? process.env.E2E_DB_REQUEST_TIMEOUT_MS ?? 600000),
        connectionTimeoutMillis: Number(dbCfg.connectionTimeoutMs ?? process.env.E2E_DB_CONNECTION_TIMEOUT_MS ?? 60000),
    });
    ...
    return {
        ...
        async clearEntityData(entityName) {
            const e = await entityMeta(entityName);
            await client.query(`DELETE FROM "${ident(mjSchema)}"."CompanyIntegrationRecordMap" WHERE "EntityID" = $1`, [e.ID]);
            await client.query(`DELETE FROM "${ident(e.SchemaName)}"."${ident(e.BaseTable)}"`);
        },
        async resolveId(query, params) {
