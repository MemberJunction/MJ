...
export function makeVerify(db, platform, mjSchema = '__mj') {
    ...
    async function entityMeta(entityName) {
        ...
        return { schema: r.s ?? r.S ?? r.schemaname, table: r.t ?? r.T ?? r.basetable, entityID: r.id ?? r.ID };
    }
    ...
    return {
        ...
        async rowByExternalId(ciid, entityName, externalId) {
            ...
            // The dest table's PK column is `ID` by MJ convention; the record map stores it in RecordID.
            const sql = pg
                ? `SELECT d.* FROM ${dest} d JOIN ${rm} m ON m."RecordID" = d."ID"::text
                   WHERE m."CompanyIntegrationID" = ${lit(ciid)} AND m."EntityID" = ${lit(e.entityID)}
                     AND m."ExternalSystemRecordID" = ${lit(externalId)} LIMIT 1`
                : `SELECT TOP 1 d.* FROM ${dest} d JOIN ${rm} m ON m.RecordID = CAST(d.ID AS NVARCHAR(255))
                   WHERE m.CompanyIntegrationID = ${lit(ciid)} AND m.EntityID = ${lit(e.entityID)}
...
export async function phaseIdempotent({ gql, verify, ciid, maps, cfg }) {
    ...
    steps.push(step('idempotent.zero-processed', processed === 0, {
        runID: sync.runID, processed, counts: sync.run?.Counts ?? null,
        note: 'a 2nd sync over unchanged vendor data must process 0 records (watermark + content-hash skip)',
    }));
