...
say(){ echo "[matrixALL $(date +%H:%M:%S)] $*"; }
for dir in cvent fonteva hivebrite neon-crm openwater orcid path-lms netsuite imis growthzone nimble-ams propfuel salesforce; do
  say "matrix $dir"
  RUN_DIRS="$dir" node "$LOGD/retest-matrix.mjs" > "$LOGD/retest-matrix-$dir.log" 2>&1 || say "  $dir process nonzero"
  say "  $dir: $(tail -1 "$LOGD/retest-matrix-$dir.log" | head -c 260)"
done
say "matrix-ALL done."
