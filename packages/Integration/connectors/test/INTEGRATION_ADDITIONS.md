...
- **B4** **Partition / Merkle hash-diff** — batch version of B3: hash partitions into rollups, deep-sync only changed partitions; order-invariant rollups. **NEW/BUILT (module: partitionRecords/partitionRollupHash/diffPartitions) — not yet wired as a sync mode.**
