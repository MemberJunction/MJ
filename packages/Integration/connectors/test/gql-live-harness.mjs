...
export async function phaseForward({ gql, db, hubspotTotal, ciid, maps, cfg }) {
    ...
    const narrowed = (fullProcessed == null || fullProcessed === 0) ? true : incrProcessed < fullProcessed;
    steps.push(step('forward.incremental.narrowed', narrowed, {
        runID: incr.runID, fullProcessed, incrProcessed,
        incrSucceeded: incr.run?.Counts?.Succeeded ?? null, counts: incr.run?.Counts ?? null,
        note: 'incremental must process FEWER records than the full pull (watermark + content-hash skip)',
    }));
