...
export const GQL = {
    ...
    // SKIP-APPLY fast path: create entity + field maps against ALREADY-EXISTING entities (no CodeGen).
    createEntityMaps: `mutation($ciid: String!, $maps: [EntityMapInput!]!) {
      IntegrationCreateEntityMaps(companyIntegrationID: $ciid, entityMaps: $maps) {
...
export async function phaseSetup({ gql, cfg }) {
    ...
    // ApplyAll scope (E7). DEFAULT 'scoped': apply ONLY cfg.objects + their FK parents — running the
    // FULL discovered catalog is infeasible at scale (e.g. Salesforce 1695 objects ⇒ 1695 CodeGen
    // tables per run). 'full' restores the old behavior (apply the entire discovered catalog — the
    // P0.5 at-scale DDL test) for callers that explicitly want it. The DAG/at-scale assertion stays
    // meaningful on the scoped set because the FK PARENTS of the selected objects are pulled in, so a
    // selected child + its parent(s) still exercise dependency-ordering; cfg.objects alone never drops
    // an edge it depends on. (matrix P0.5 / section G — now scoped-by-default)
    const applyScope = cfg.applyScope === 'full' ? 'full' : 'scoped';
    const sourceList = expectGqlSuccess('ListSourceObjects', (await gql(GQL.listSourceObjects, { ciid })).IntegrationListSourceObjects);
    const allObjectNames = (sourceList.Objects ?? []).map(o => o.Name);
    const applyObjectNames = applyScope === 'full'
        ? (allObjectNames.length ? allObjectNames : cfg.objects)               // full catalog (explicit opt-in)
        : scopedApplySet(cfg.objects, allObjectNames, cfg.applyParents);       // scoped: objects + FK parents (default)

    // SKIP-APPLY FAST PATH (test.md "one db, all entities" — applied ONCE; per-connector re-runs reuse
    // the existing entities). When the caller precomputes cfg.entityMapInputs (entity name + identity
    // field maps, derived from the deployed schema), create the maps DIRECTLY via CreateEntityMaps —
    // NO ApplyAll, NO 7-min CodeGen regenerating the whole entity layer. Falls back to full ApplyAll
    // when entityMapInputs is absent (fresh connector whose entities don't exist yet).
    let applied, maps;
    if (Array.isArray(cfg.entityMapInputs) && cfg.entityMapInputs.length) {
        const cm = expectGqlSuccess('CreateEntityMaps', (await gql(GQL.createEntityMaps, { ciid, maps: cfg.entityMapInputs })).IntegrationCreateEntityMaps);
        const en = new Map(cfg.entityMapInputs.map(i => [i.ExternalObjectName, i.EntityName]));
        maps = (cm.Created ?? []).map(m => ({ entityMapID: m.EntityMapID, entityName: en.get(m.ExternalObjectName), sourceObjectName: m.ExternalObjectName, fieldMapCount: m.FieldMapCount }));
        applied = { Warnings: [], Steps: [{ Name: 'skip-apply', Status: 'OK', Message: `CreateEntityMaps (reuse existing entities, no CodeGen): ${maps.length} maps` }] };
    } else {
        applied = expectGqlSuccess('ApplyAll', (await gql(GQL.applyAll, {
            input: {
                CompanyIntegrationID: ciid,
                SourceObjects: applyObjectNames.map(name => ({ SourceObjectName: name })),
        ...
        maps = (applied.EntityMapsCreated ?? []).map(m => ({
    ...
    return {
        ciid, credentialID, maps, syncMaps,
        applyAll: { scope: applyScope, objectsApplied: applyObjectNames.length, appliedObjects: applyObjectNames, catalogSize: allObjectNames.length, mapsCreated: maps.length, warnings: applied.Warnings ?? [], steps: applied.Steps ?? [] },
        listedMaps: listed.EntityMaps ?? [], connectionTest, schemaRefresh, referenceMode,
    };
}

/**
 * E7 — compute the SCOPED ApplyAll object set: the requested objects PLUS their FK parents, so a
 * child object's dependency edge is never dropped (the DAG/at-scale assertion stays meaningful) while
 * we avoid applying a giant catalog. Parents are derived two ways, unioned:
 *   1. EXPLICIT — `applyParents` (the plan can pass FK parents resolved from IntegrationObjectField
 *      RelatedIntegrationObjectID; authoritative when available).
 *   2. NAMING — an association/child whose name embeds a catalog object's name (`assoc_<a>_<b>`,
 *      `<parent>_items`, `<parent>Lines`): that embedded object is pulled in as a parent. Heuristic,
 *      credential-free, and only ADDS parents (never drops a requested object).
 * Always returns at least the requested objects (intersected with the live catalog when known).
 *
 * @param {string[]} objects        requested objects (cfg.objects)
 * @param {string[]} catalog        all discovered object names (may be empty if list unsupported)
 * @param {string[]} [applyParents] explicit FK parents the plan resolved (optional)
 * @returns {string[]} deduped scoped set
 */
export function scopedApplySet(objects, catalog, applyParents) {
    const reqLc = new Set((objects ?? []).map(o => String(o).toLowerCase()));
    const out = new Map(); // lower → original-cased name
    const add = (name) => { if (name) out.set(String(name).toLowerCase(), name); };
    for (const o of (objects ?? [])) add(o);
    for (const p of (applyParents ?? [])) add(p);
    // Naming-derived FK parents: for each requested child, any DIFFERENT catalog object whose name is a
    // substring of the child's name is treated as a parent the child depends on.
    const cat = catalog ?? [];
    for (const child of (objects ?? [])) {
        const childLc = String(child).toLowerCase();
        for (const cand of cat) {
            const candLc = String(cand).toLowerCase();
            if (candLc === childLc) continue;
            if (!reqLc.has(childLc)) continue;
            // Embedded parent name (avoid trivially-short tokens that would over-match).
            if (candLc.length >= 3 && childLc.includes(candLc)) add(cand);
        }
    }
    return [...out.values()];
}
...
export async function phaseForward({ gql, db, hubspotTotal, ciid, maps, cfg }) {
    const steps = [];
    // Scope every sync to the Goldilocks subset's maps — ApplyAll created maps for the whole catalog,
    // but only these objects have mock routes / bounded data to round-trip.
    const mapIDs = maps.map(m => m.entityMapID);

    // P1: full pull (subset maps).
    const full = await runSyncObserved(gql, ciid, { fullSync: true, syncDirection: 'Pull', entityMapIDs: mapIDs, maxPolls: cfg.maxPolls });
    ...
    const incr = await runSyncObserved(gql, ciid, { fullSync: false, syncDirection: 'Pull', entityMapIDs: mapIDs, maxPolls: cfg.maxPolls });
