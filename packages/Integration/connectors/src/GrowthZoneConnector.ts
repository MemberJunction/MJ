...
export class GrowthZoneConnector extends BaseRESTIntegrationConnector {
    ...
    public override async DiscoverFields(
        ...
        try {
            ...
            const apiPath = objectName.replace(/([a-z])([A-Z])/g, '/$2').toLowerCase();
            const response = await this.MakeHTTPRequest(auth, `${auth.Config.BaseURL}/api/${apiPath}?take=1&skip=0`, 'GET', headers);
            if (response.Status === 200) {
                const records = this.NormalizeResponse(response.Body, null);
                if (records.length > 0) return this.InferFieldsWithOverlay(records[0], objectName, GZ_OBJECTS);
            }
        } catch { /* fall through */ }
