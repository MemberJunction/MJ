...
describe('AptifyConnector (smoke)', () => {
    const connector = new AptifyConnector();

    describe('Identity', () => {
        it('should instantiate without throwing', () => {
            expect(connector).toBeDefined();
            expect(connector instanceof AptifyConnector).toBe(true);
        });

        it('IntegrationName getter returns the canonical name', () => {
            expect(connector.IntegrationName).toBe('Aptify');
        });
    });
    ...
    describe('GetDefaultFieldMappings', () => {
        it('should return an array (possibly empty) for PERSON', () => {
            const mappings = connector.GetDefaultFieldMappings('PERSON', 'TestEntity');
