...
export class IntegrationOrchestrator {
    ...
    /**
     * Sets standard integration columns (SourceRecordID, SourceJSON, LastSyncedAt)
     * on target entities generated by the Schema Builder.
    ...
    private SetStandardIntegrationFields(
        ...
        if (hasField('SourceRecordID')) {
            entity.Set('SourceRecordID', record.ExternalRecord.ExternalID);
        }
        if (hasField('SourceJSON')) {
            entity.Set('SourceJSON', JSON.stringify(record.ExternalRecord.Fields));
        }
        if (hasField('LastSyncedAt')) {
            entity.Set('LastSyncedAt', new Date().toISOString());
        }
    }
