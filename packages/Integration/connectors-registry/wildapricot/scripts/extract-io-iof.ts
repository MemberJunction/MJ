#!/usr/bin/env tsx
// scripts/extract-io-iof.ts — Wild Apricot IO/IOF extractor (single-pass, code-first).
//
// This script's structured output IS the emission. It walks the RAW admin OpenAPI 3.1
// spec (sources/openapi.admin.9.14.0.json), independently enumerates the full record-type
// universe (components.schemas, resolving allOf/$ref), descends the type graph for nested
// record-collections (list-of-$ref => own IO + access-path + FK; scalar / 1:1 struct => column),
// classifies each schema into first-class IO / nested-record IO / skip-with-reason wrapper,
// types every field FROM THE SPEC (provable-only, never NVARCHAR(MAX), never fabricated PK/FK),
// and persists IO+IOF rows + provenance/code-evidence to
// metadata/integrations/wildapricot/.wildapricot.integration.json via MetadataFileStore.
//
// SET-COMPLETENESS: the IO set is RESET at start so the on-disk emission reflects ONLY this
// run (no stale orphans from prior runs). The integration root `fields` block is preserved.
//
// FLOOR PARITY: floor-check independently runs enumerate-catalog.mjs over this spec and
// requires (emitted ∪ skipped-with-reason) == the enumerator's recordTypes set. Every
// enumerated name is therefore either emitted or skipped-with-reason — no silent residual.
//
// Sources read: credential-free OpenAPI spec ONLY. Prior connector code + prior metadata are
// OUTPUT and are NEVER read (circular-source defect).
...
// enumerate-catalog.mjs's OpenAPI inclusion rule: any schema where
// `s.type==='object' || s.properties || s.allOf || (!s.type && !s.enum)` and not `s.enum`.
// The `(!s.type && !s.enum)` clause admits a few malformed 0-property definitions our
// isObjectSchema excludes; replicate the enumerator's rule so accounting stays EXACTLY in
// parity — every enumerated name is either emitted or skipped-with-reason.
...
// Descend a container wrapper to its underlying item record.
...
function canonicalEntityForPath(methods: Record<string, RawSchema>, resourceSeg: string): string | null {
    ...
    // Normalize a list-item projection (<X>ListItem) to its fuller base entity <X>.
...
// RPC / action segments matched as WHOLE path segments (not substrings) so collection names
// like `paymentAllocations` (contains "Allocate") aren't mis-classified as RPC.
...
for (const [p, methods] of Object.entries(paths)) {
    ...
    if (!isParamSeg && isRpcSegment(lastSeg)) continue;
...
const ENTITY_ALIAS: Record<string, string> = {
    contactExtendedMembershipInfo: 'Contact',
    ContactsMe: 'Contact',
};
...
// AttachmentData door: only action-shaped GET paths (/attachments/GetInfos) — wire explicitly.
if (!endpointBySchema['AttachmentData'] && schemas['AttachmentData']) {
    ...
    if (attachList) {
        endpointBySchema['AttachmentData'] = {
            schema: 'AttachmentData', collectionPath: attachList, itemPath: attachItem ?? null,
            verbs: new Set<string>(['GET']), pkParam: attachItem ? 'attachmentId' : null, tag: 'Attachments',
        };
    }
}
...
// ── Persistence (batched: build the file in memory, write ONCE at the end). ────
// The base MetadataFileStore backs up + rewrites on EVERY upsert (5000+ backups per run).
// We subclass to buffer writes and flush a single time via Flush(), avoiding the backup storm
// while keeping the exact same on-disk shape the base store produces.
type FileShape = ReturnType<MetadataFileStore['ReadIntegration']>;
class BatchedStore extends MetadataFileStore {
    private buffered = false;
    private cached: NonNullable<FileShape> | null = null;
    public override UpsertIO(connector: string, io: Record<string, unknown> & { Name: string }): void {
        const file = this.file(connector);
        const ios = file.relatedEntities!['MJ: Integration Objects']!;
        if (io.IntegrationID == null) io.IntegrationID = '@parent:ID';
        const idx = ios.findIndex((i) => (i.fields.Name as string).toLowerCase() === io.Name.toLowerCase());
        if (idx >= 0) ios[idx].fields = { ...ios[idx].fields, ...io };
        else ios.push({ fields: io });
    }
    public override UpsertIOF(connector: string, ioName: string, iof: Record<string, unknown> & { Name: string }): void {
        const file = this.file(connector);
        const ios = file.relatedEntities!['MJ: Integration Objects']!;
        let io = ios.find((i) => (i.fields.Name as string).toLowerCase() === ioName.toLowerCase());
        if (!io) { io = { fields: { Name: ioName, IntegrationID: '@parent:ID' } }; ios.push(io); }
        ...
        const idx = iofs.findIndex((f) => (f.fields.Name as string).toLowerCase() === iof.Name.toLowerCase());
        if (idx >= 0) iofs[idx].fields = { ...iofs[idx].fields, ...iof };
        else iofs.push({ fields: iof });
        related['MJ: Integration Object Fields'] = iofs;
        io.relatedEntities = related;
    }
    private file(connector: string): NonNullable<FileShape> {
        if (!this.buffered) {
            // Read existing file to PRESERVE the integration root `fields` block (MetadataWriter
            // populated CredentialTypeID/APIBaseURL/etc.), but RESET the IO set for set-completeness.
            const existing = this.ReadIntegration(connector);
            this.cached = existing ?? { fields: { Name: connector, ClassName: 'WildapricotConnector' }, relatedEntities: {} };
            this.cached.relatedEntities = { 'MJ: Integration Objects': [] }; // RESET — no orphans
            this.buffered = true;
        }
        return this.cached!;
    }
    public Flush(connector: string): void {
        if (this.cached) this.WriteIntegration_(connector, this.cached);
    }
    // Access the private path helper + atomic write via the same on-disk shape (one-element array).
    private WriteIntegration_(connector: string, file: NonNullable<FileShape>): void {
        const slug = connector.toLowerCase();
        const path = resolve(METADATA_ROOT, slug, `.${slug}.integration.json`);
        mkdirSync(dirname(path), { recursive: true });
        writeFileSync(path, JSON.stringify([file], null, 2) + '\n', 'utf-8');
    }
}
const store = new BatchedStore(REGISTRY_ROOT, METADATA_ROOT);
    ...
    for (const [fname, fprop] of Object.entries(props)) {
        ...
        if (isPK) {
            ...
            const pkEvid = ep?.pkParam
                ? `Addressing path ${ep.itemPath} requires {${ep.pkParam}} to fetch one ${name}; PK field is ${fname}.`
                : `PK convention: ${fname} is the identity field on this Wild Apricot entity (universal Id convention).`;
            emitProvenance(`iof.${name}.${fname}.IsPrimaryKey`, `${name} primary key`, pkEvid);
            evidence++;
        }
    }
    // Synthetic parent FK for a nested record lacking its own parent-id column.
    if (!isFirstClass) {
        const ref = findReferencer(name);
        if (ref) {
...
for (const plan of plans) {
    if (plan.kind === 'skip') { pushSkip(plan.name, plan.reason ?? 'wrapper/variant — derives from an emitted record'); continue; }
...
for (const name of FLOOR_UNIVERSE) {
    ...
    pushSkip(name, 'malformed 0-property schema definition (stray non-standard `schema` key, no `properties`/`allOf`) — empty descriptor, not a record type; counted by the deterministic enumerator but has no fields to emit');
    backfilled++;
}

// ── Flush persistence: single metadata write + provenance + code-evidence. ──────
store.Flush(CONNECTOR);
for (const pe of provenanceBuf) store.AppendProvenance(CONNECTOR, pe as never);
...
// EXTRACTION_REPORT_MATRIX.csv (one row per object, emitted + skipped).
const MATRIX_OUT = resolve(dirname(EMISSION_OUT), 'EXTRACTION_REPORT_MATRIX.csv');
...
const rows = emission.map((e) => {
    const m = e.matrixRow;
    return [m.IOName, m.ExistingConnectorTs, m.ExistingMetadataJson, m.OpenAPIxPK, m.OpenAPIPathOps, m.OpenAPILocationHeader, m.VendorDocsProseScan, m.SDKTypes, m.PostmanCommunity, m.NamingConvention, m.CrossIOMatch, m.PKVerdict, m.FKVerdict, m.EvidenceCount].join(',');
});
writeFileSync(MATRIX_OUT, [header, ...rows].join('\n') + '\n', 'utf-8');
