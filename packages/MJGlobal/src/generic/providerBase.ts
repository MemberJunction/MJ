...
export abstract class ProviderBase implements IMetadataProvider, IRunViewProvider {
   ...
   */
  public abstract GetRecordFavoriteStatus(
    userId: string,
    entityName: string,
   ...
   */
  public abstract SetRecordFavoriteStatus(
    userId: string,
    entityName: string,
    CompositeKey: CompositeKey,
    isFavorite: boolean,
    ...
    if (params && params.length > 0) {
      for (const param of params) {
        ...
        if (param.ResultType === 'entity_object') {
          // we need to get the entity definition and then get all the fields for it
          const entity: EntityInfo | undefined = this.Entities.find(
            (e) => e.Name.trim().toLowerCase() === param.EntityName.trim().toLowerCase()
          );
          if (!entity) {
  ...
  protected async GetAllMetadata(providerToUse?: IMetadataProvider): Promise<AllMetadata> {
    try {
      ...
      if (d && d.Success) {
        ...
        simpleMetadata.AllEntities = this.PostProcessEntityMetadata(
          simpleMetadata.Entities,
          simpleMetadata.EntityFields,
          simpleMetadata.EntityFieldValues,
          simpleMetadata.EntityPermissions,
          simpleMetadata.EntityRelationships,
          simpleMetadata.EntitySettings
        );
  ...
  public get VisibleExplorerNavigationItems(): ExplorerNavigationItem[] {
    ...
    if (!this._cachedVisibleExplorerNavigationItems)
      this._cachedVisibleExplorerNavigationItems = this._localMetadata.AllExplorerNavigationItems.filter((e) => e.IsActive).sort(
  ...
  public async RefreshIfNeeded(providerToUse?: IMetadataProvider): Promise<boolean> {
    if (await this.CheckToSeeIfRefreshNeeded(providerToUse)) return this.Refresh(providerToUse);
    else return true;
  }
   ...
   */
  public abstract GetRecordDependencies(
    entityName: string,
    CompositeKey: CompositeKey,
    contextUser?: UserInfo
   ...
   * @param itemFilters
   */
  public abstract GetDatasetByName(
    datasetName: string,
   ...
   * @param itemFilters
   */
  public abstract GetDatasetStatusByName(
    datasetName: string,
    itemFilters?: DatasetItemFilterType[],
    contextUser?: UserInfo,
    providerToUse?: IMetadataProvider
   ...
   * @param itemFilters
   */
  public async GetAndCacheDatasetByName(
    datasetName: string,
    itemFilters?: DatasetItemFilterType[],
    contextUser?: UserInfo,
    providerToUse?: IMetadataProvider
  ...
  public GetDatasetCacheKey(datasetName: string, itemFilters?: DatasetItemFilterType[]): string {
    return (
      ...
      ProviderBase.localStorageRootKey +
  ...
  protected async GetLatestMetadataUpdates(providerToUse?: IMetadataProvider): Promise<MetadataInfo[]> {
    ...
    const d = await this.GetDatasetStatusByName(
      ProviderBase._mjMetadataDatasetName,
      f.length > 0 ? f : null,
      this.CurrentUser,
      providerToUse
    );
  ...
  protected async LoadLocalMetadataFromStorage() {
    try {
      ...
      if (ls) {
        ...
        this._latestLocalMetadataTimestamps = JSON.parse(
          await ls.GetItem(this.LocalStoragePrefix + ProviderBase.localStorageTimestampsKey)
        );
  ...
  private static localStorageKeys = [ProviderBase.localStorageTimestampsKey, ProviderBase.localStorageAllMetadataKey];
  ...
  public async SaveLocalMetadataToStorage() {
    try {
      ...
      if (ls) {
        // execution environment supports local storage, use it
        await ls.SetItem(
          this.LocalStoragePrefix + ProviderBase.localStorageTimestampsKey,
          JSON.stringify(this._latestLocalMetadataTimestamps)
        );
