...
import { MJEvent, MJEventType } from '../interface';
...
export abstract class BaseEngine<T> extends BaseSingleton<T> {
  ...
  protected async DebounceIndividualBaseEntityEvent(event: BaseEntityEvent): Promise<boolean> {
    try {
      ...
      if (!this._entityEventSubjects.has(entityName)) {
        ...
        subject.pipe(debounceTime(this.EntityEventDebounceTime)).subscribe(async (e) => {
  ...
  protected UpgradeObjectToConfig(obj: any): BaseEnginePropertyConfig {
    ...
    if (obj instanceof BaseEnginePropertyConfig) return obj;
    else return new BaseEnginePropertyConfig(obj);
  }
  ...
  protected async LoadSingleConfig(config: BaseEnginePropertyConfig, contextUser: UserInfo): Promise<void> {
    if (config.Type === 'dataset') return await this.LoadSingleDatasetConfig(config, contextUser);
    else return await this.LoadSingleEntityConfig(config, contextUser);
  }
