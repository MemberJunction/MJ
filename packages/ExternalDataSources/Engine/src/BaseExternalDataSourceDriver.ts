...
export abstract class BaseExternalDataSourceDriver<TConnection = unknown> {
   ...
   * the object name is a LITERAL identifier (e.g. a MongoDB collection). NOTE: `||` (not `??`) is
   * intentional — an EMPTY-string `ExternalObjectName` must fall through to `BaseTable`/`Name`, which
   * `??` (nullish only) would not do. Do not "simplify" this to `??`. SQL drivers OVERRIDE this to
