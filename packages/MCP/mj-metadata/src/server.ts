...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: 'delete_integration_object',
            description: 'Remove an Integration Object row (and its nested Object Fields) from the connector metadata file. Match key: ioName (case-insensitive). Used to prune an IO this run proves is not an independently-syncable collection.',
            inputSchema: {
                type: 'object',
                properties: {
                    connector: { type: 'string' },
