...
export class MetadataFileStore {
    ...
    /**
     * Remove an Integration Object row (and its nested Object Fields) from the
     * connector metadata file. Match key is `ioName` (case-insensitive). Used to
     * honor the extractor's set-completeness rule: an IO that this run proves is
     * NOT a syncable collection (e.g. a get-by-id-only or purely-nested schema
     * type) must be pruned, not left as an orphan asserting an unprovable path.
     * Returns `true` if a row was removed, `false` if no matching IO existed.
     */
    public DeleteIO(connectorName: string, ioName: string): boolean {
        const file = this.ReadIntegration(connectorName);
        if (!file) return false;
        const ios = file.relatedEntities?.['MJ: Integration Objects'] ?? [];
        const idx = ios.findIndex((i) => i.fields.Name.toLowerCase() === ioName.toLowerCase());
