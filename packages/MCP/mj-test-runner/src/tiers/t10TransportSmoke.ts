...
export function runT10TransportSmoke(connector: string, identity: ConnectorIdentity): TierHandlerResult {
    ...
    if ((d.requestCount ?? 0) === 0) {
        ...
        const credGated = /credential|token|configuration json|api[- ]?key|account\s?id|unauthor|auth\b|client\s?(id|secret)|base\s?url|communit|instance\s?url|subdomain|host|refresh|oauth|scope|tenant|principal|secret/i;
