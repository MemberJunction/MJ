...
function evaluateT12(
    ...
    for (const o of objects) {
        if (o.error) {
            // Parent-keyed / derived objects resolve their parent IDs from the TARGET DB (RunView
            // over already-synced rows). The offline child has no MJ provider, so these are out of
            // replay scope BY DESIGN — a visible skip with reason, never a false fail. Their identity
            // stability is covered by the live hybrid-e2e tier, which has a real DB. Mirrors T5.
            if (/reading 'RunView'|Metadata\.Provider|No provider|RunView is not a function/i.test(String(o.error))) {
    ...
    const summary =
        ...
        `${stableCount}/${objects.length} stable` +
    ...
    return {
        ...
        Details: { connector, class: identity.ClassName, fixtureSource, objects, totalRecords, dbDependentSkips, volatileInjected: data.volatileInjected },
    };
}
