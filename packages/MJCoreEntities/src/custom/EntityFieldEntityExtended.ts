...
import { BaseEntity, ValidationResult, ValidationErrorInfo, ValidationErrorType } from '@memberjunction/global';
  ...
  public override Set(FieldName: string, Value: any): void {
    ...
    if (FieldName.toLowerCase() === 'description' && !this.NewRecord) {
      ...
      if (Value !== currentDescription && autoUpdateDescription !== true) {
        console.warn(
          `Setting AutoUpdateDescription to true for Entity Field "${this.Name}" because Description is being manually updated. This will prevent CodeGen from overwriting this description.`
        );
