...
export class UserViewEntityExtended extends UserViewEntity {
  ...
  public get ViewSortInfo(): ViewSortInfo[] {
    if (this.SortState) {
      ...
      if (sortTemp && sortTemp.length > 0) return sortTemp.map((s) => new ViewSortInfo(s));
    }
    ...
    if (this.ViewSortInfo && this.ViewSortInfo.length > 0) {
      return this.ViewSortInfo.map((s) => {
        ...
        if (typeof s.direction === 'string') dir = s.direction.trim().toLowerCase();
          ...
          // some legacy view metadata has 1/2 for asc/desc
          dir = 'asc';
        else if (s.direction === 2) dir = 'desc';
        else dir = '';
  ...
  private CalculateUserCanView(): boolean {
    ...
    if (bOwner) {
      ...
      if (!rt) throw new Error('Resource Type User Views not found');

      const permLevel = ResourcePermissionEngine.Instance.GetUserResourcePermissionLevel(rt.ID, this.ID, md.CurrentUser);
      if (permLevel)
        // any permission level allows view access
        return true;
      // perm level not found so return false
      else return false;
    }
  }
  ...
  public get ViewResourceTypeID(): string {
    if (!this._ViewResourceTypeID) {
      ...
      if (!rtUV) throw new Error('Unable to find Resource Type for User Views entity');
  ...
  override async Load(ID: string, EntityRelationshipsToLoad?: string[]): Promise<boolean> {
    ...
    if (result) {
      ...
      if (match) this._ViewEntityInfo = match;
      else throw new Error('Unable to find entity info for entity ID: ' + this.EntityID);
    }
  ...
  override NewRecord(): boolean {
    ...
    if (result) {
      if (this.ContextCurrentUser) {
        ...
        const md = new Metadata();
        if (md.CurrentUser) this.UserID = md.CurrentUser.ID;
        else throw new Error('Unable to determine current user for new view record');
      }
      this.Name = '';
      this.IsShared = false;
  ...
  public async UpdateWhereClause(ignoreDirtyState?: boolean) {
    ...
    if (
      this.SmartFilterEnabled &&
      (this.SmartFilterEnabled === true || this.SmartFilterEnabled === 1) &&
      this.SmartFilterPrompt &&
      this.SmartFilterPrompt.length > 0
    ) {
   ...
   */
  public async GenerateSmartFilterWhereClause(
    prompt: string,
    entityInfo: EntityInfo
  ): Promise<{ whereClause: string; userExplanation: string }> {
  ...
  public override Set(FieldName: string, Value: any): void {
    ...
    if (FieldName.toLowerCase() == 'entityid') {
      ...
      if (match) this._ViewEntityInfo = match;
      else throw new Error('Unable to find entity info for entity ID: ' + Value);
    }
  }
  ...
  private convertFilterToSQL(field: string, operator: string, value: string, entity: EntityInfo): string {
    let op: string = '';

    const f = entity.Fields.find((f) => f.Name.trim().toLowerCase() === field.trim().toLowerCase());
    if (!f) throw new Error('Unable to find field ' + field + ' in entity ' + entity.Name);
...
export class ViewInfo {
  ...
  static async GetViewsForUser(entityId?: string, contextUser?: UserInfo): Promise<UserViewEntityExtended[]> {
    ...
    if (result && result.Success && rd) return rd;
  }
  ...
  static async GetViewEntity(viewId: string, contextUser?: UserInfo): Promise<UserViewEntityExtended> {
    ...
    if (await view.Load(viewId)) return view;
    else throw new Error('Unable to load view with ID: ' + viewId);
  }
  ...
  static async GetViewEntityByName(viewName: string, contextUser?: UserInfo): Promise<UserViewEntityExtended> {
    ...
    if (viewId) return await ViewInfo.GetViewEntity(viewId, contextUser);
    else throw new Error('Unable to find view with name: ' + viewName);
  }
}
