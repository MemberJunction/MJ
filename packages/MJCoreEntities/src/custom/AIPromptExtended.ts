...
export class AIPromptEntityExtended extends AIPromptEntity {
  ...
  protected async LoadRelatedEntities(): Promise<void> {
    await Promise.all([this.LoadTemplateText(), this.LoadTemplateParams()]);
  }
    ...
    if (this.ResultSelectorPromptID === this.ID && this.ID /*make sure ID !== null*/) {
      result.Errors.push(
        new ValidationErrorInfo(
          ...
          'The ResultSelectorPromptID cannot be the same as the ID. A result selector prompt cannot reference itself.',
          this.ResultSelectorPromptID,
