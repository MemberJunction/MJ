import { BaseEngine, BaseEnginePropertyConfig, IMetadataProvider, Metadata, UserInfo } from '@memberjunction/global';
...
export class ResourcePermissionEngine extends BaseEngine<ResourcePermissionEngine> {
  ...
  public GetUserResourcePermissionLevel(
    ...
    const rolePermissions = allPermissions.filter(
      (p) => p.Type === 'Role' && user.UserRoles.find((ur) => ur.RoleID === p.RoleID) !== undefined
    );
  ...
  public GetUserAvailableResources(user: UserInfo, ResourceTypeID?: string): ResourcePermissionEntity[] {
    let rolePermissions = this.Permissions.filter(
      (r) => r.Type === 'Role' && user.UserRoles.find((ur) => ur.RoleID === r.RoleID) !== undefined
    );
   ...
   */
  public GetResourceTypeInfoFields(ResourceTypeID: string): {
    ...
    if (!rt) throw new Error(`Resource Type ${ResourceTypeID} not found`);
    const entity = md.EntityByID(rt.EntityID);
    if (!entity) throw new Error(`Entity ${rt.EntityID} not found`);
    const usersEntity = md.EntityByName('Users');
    if (!usersEntity) throw new Error(`Entity Users not found`);
