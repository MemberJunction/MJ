    ...
    public override Set(FieldName: string, Value: any): void {
        ...
        if (FieldName.toLowerCase() === 'description' && !this.NewRecord) {
            ...
            if (Value !== currentDescription && autoUpdateDescription !== false) {
                console.warn(`Setting AutoUpdateDescription to false for Entity Field "${this.Name}" because Description is being manually updated. This will prevent CodeGen from overwriting this description.`);
