...
import { UserInfo, Metadata, RunView, BaseEntity } from '@memberjunction/global';
...
export class AutotagEntity extends AutotagBase {
  ...
  public async getNewOrModifiedContentItems(contentSourceParams: ContentSourceParams, contextUser: UserInfo): Promise<ContentItemEntity[]> {
    ...
    if (results.Results && results.Results.length > 0) {
      const contentItems: ContentItemEntity[] = await this.setContentItemsFromEntityResults(
        entityResults,
        contentSourceParams,
        lastRunDate
      );
  ...
  public async setContentItemsFromEntityResults(
    results: BaseEntity[],
