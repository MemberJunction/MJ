...
export class AutotagBaseEngine extends AIEngine {
  ...
  public async processChunkWithLLM(
    llm: BaseLLM,
    systemPrompt: string,
    userPrompt: string,
  ...
  public async getContentSourceParams(contentSource: ContentSourceEntity, contextUser: UserInfo): Promise<any> {
    ...
    if (results.Success && results.Results.length) {
      ...
      for (const contentSourceParam of contentSourceParamResults) {
        const params: ContentSourceTypeParams = await this.getDefaultContentSourceTypeParams(
          contentSourceParam.ContentSourceTypeParamID,
          contextUser
        );
  ...
  public async getDefaultContentSourceTypeParams(
    contentSourceTypeParamID: string,
    contextUser: UserInfo
  ): Promise<ContentSourceTypeParams> {
    const rv = new RunView();
  ...
  public async getContentItemParams(
    contentTypeID: string,
    contextUser: UserInfo
  ): Promise<{ modelID: string; minTags: number; maxTags: number }> {
    const rv = new RunView();
