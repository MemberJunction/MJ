...
import { ContentSourceTypeParams } from './content.types';
import pdfParse from 'pdf-parse';
import pdf2pic from 'pdf2pic';
import * as officeparser from 'officeparser';
import * as XLSX from 'xlsx';
import * as fs from 'fs';
import { ProcessRunParams, ContentItemProcessParams, ContentItemProcessParamsExtended } from './process.types';
...
let AutotagBaseEngine = (() => {
    let _classDecorators = [RegisterClass(AIEngine, 'AutotagBaseEngine')];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    let _classSuper = AIEngine;
    var AutotagBaseEngine = class extends _classSuper {
        static { _classThis = this; }
        static {
            const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(_classSuper[Symbol.metadata] ?? null) : void 0;
            __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
            AutotagBaseEngine = _classThis = _classDescriptor.value;
         ...
         */
        async ExtractTextAndProcessWithLLMEnhanced(contentItems, contextUser, structuredDataMap) {
            if (!contentItems || contentItems.length === 0) {
                console.log('No content items to process');
                return;
            }
         ...
         */
        async ProcessContentItemTextEnhanced(params, contextUser) {
            let LLMResults;
        ...
        async saveContentItemTags(contentItemID, LLMResults, contextUser) {
            ...
            for (const keyword of LLMResults.keywords) {
                if (!existingTagTexts.has(keyword)) {
                    ...
                    const contentItemTags = await md.GetEntityObject('Content Item Tags', contextUser);
         ...
         */
        async getExistingContentItemTags(contentItemID, contextUser) {
            try {
            ...
            for (const key in LLMResults) {
                ...
                if (key !== 'keywords' && key !== 'processStartTime' && key !== 'processEndTime' && key !== 'contentItemID' && key !== 'isValidContent') {
                    ...
                    if (existingAttribute) {
                        ...
                        const contentItemAttribute = await md.GetEntityObject('Content Item Attributes', contextUser);
         ...
         */
        async getExistingContentItemAttribute(contentItemID, attributeName, contextUser) {
            try {
                const rv = new RunView();
                const result = await rv.RunView({
        ...
        */
        ...
        async saveProcessRun(processRunParams, contextUser) {
            const md = new Metadata();
            const processRun = await md.GetEntityObject('Content Process Runs', contextUser);
         ...
         */
        async parseContentItem(contentItem, contextUser) {
            try {
         ...
         */
        async parseXLSXWithContext(dataBuffer, sourceParams) {
         ...
         */
        async selectSheetWithLLM(workbook, customPrompt) {
            try {
         ...
         */
        async getContentSourceEntity(contentSourceID, contextUser) {
            const rv = new RunView();
            const result = await rv.RunView({
                EntityName: 'Content Sources',
         ...
         */
        async getContentFileTypeEntity(contentFileTypeID, contextUser) {
            const rv = new RunView();
            const result = await rv.RunView({
                EntityName: 'Content File Types',
                ExtraFilter: `ID='${contentFileTypeID}'`,
        ...
        */
         ...
         */
        async parsePDFWithStructure(dataBuffer, assumeTabularContent = false) {
            try {
        ...
        async convertPDFToImages(pdfBuffer) {
            ...
            throw new Error('PDF to image conversion failed with all approaches. ' +
         ...
         */
        async processWithVisionModel(pdfBuffer, params, contextUser) {
            try {
    ...
    return AutotagBaseEngine = _classThis;
})();
