...
export class AutotagAzureBlob extends CloudStorageBase {
    ...
    */
     ...
     */
    async getExistingContentItemIdForBlob(blobPath, contentSourceId, contextUser) {
        try {
    ...
    async setAddedContentItem(blobName, filePath, contentSourceParams, contextUser) {
        try {
            const md = new Metadata();
    ...
    async setModifiedContentItem(blobName, filePath, contentSourceParams, contextUser) {
        try {
            const md = new Metadata();
            const contentItem = await md.GetEntityObject('Content Items', contextUser);
     ...
     */
    async downloadBlobToBuffer(blobName) {
     ...
     */
    async createTempFile(blobName, buffer) {
     ...
     */
    async getContentFileTypeFromExtension(fileName, contentSourceParams, contextUser) {
