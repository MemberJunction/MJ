import { IMetadataProvider, LogError, UserInfo, ValidationErrorInfo } from '@memberjunction/global';
...
export class TemplateEngineServer extends TemplateEngineBase {
  ...
  public async RenderTemplate(
    templateEntity: TemplateEntityExtended,
  ...
  protected mergeDefaultValues(templateEntity: TemplateEntityExtended, contentId: string, data: any): any {
    ...
    params
      .filter((p) => !(p as any).TemplateContentID)
      .forEach((p) => {
    ...
    params
      .filter((p) => (p as any).TemplateContentID === contentId)
      .forEach((p) => {
        paramsByName.set(p.Name, p);
      });
