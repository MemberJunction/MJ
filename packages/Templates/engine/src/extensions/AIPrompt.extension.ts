...
export class AIPromptExtension extends TemplateExtensionBase {
  ...
  public run(context: Context, params: any, body: any, callBack: NunjucksCallback) {
    ...
    AIEngine.Instance.Config(false, this.ContextUser).then(async () => {
      try {
        ...
        if (config.AIModel) {
          model = AIEngine.Instance.Models.find(
            (m) => m.Name.toLowerCase() === config.AIModel.toLowerCase() || m.APIName?.toLowerCase() === config.AIModel.toLowerCase()
          );
          if (!model) {
  ...
  protected async GetAIModel(vendorName: string, contextUser: UserInfo): Promise<AIModelEntityExtended> {
    ...
    const models = AIEngine.Instance.Models.filter(
      (m) => m.AIModelType.trim().toLowerCase() === 'llm' && m.Vendor.trim().toLowerCase() === vendorName.trim().toLowerCase()
    );
