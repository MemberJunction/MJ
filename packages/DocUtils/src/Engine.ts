...
import { BaseEngine, BaseEnginePropertyConfig, LogError, UserInfo, BaseEntity, IMetadataProvider } from '@memberjunction/global';
 ...
 * @memberjunction/global and an item within that library might be the BaseEntity or BaseEngine class.
 */
...
export class DocumentationEngine extends BaseEngine<DocumentationEngine> {
  ...
  protected override async AdditionalLoading(contextUser?: UserInfo): Promise<void> {
    ...
    for (const library of this.Libraries) {
      const items: LibraryItemEntityExtended[] = this.LibraryItems.filter(
        (item: LibraryItemEntityExtended) => item.LibraryID === library.ID
      );
