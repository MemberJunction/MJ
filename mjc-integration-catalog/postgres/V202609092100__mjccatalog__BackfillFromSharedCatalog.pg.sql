...
-- than by ON CONFLICT. A guard states the intent in the same words in both dialects, and it
-- COMPLETES a partially applied run instead of refusing it, which is what you want after a
-- connection timed out halfway through.
--
-- NEVER INSERTS __mj_CreatedAt / __mj_UpdatedAt. Both have defaults and MemberJunction owns them.
-- ID is omitted for the same reason: the table default (gen_random_uuid()) mints it.
-- ==============================================================================================
...
-- ==============================================================================================
--
-- FANS OUT TO EVERY CompanyIntegration, INCLUDING IsActive = false. A paused connection must
-- survive the cutover. If it were skipped it would silently own no catalog at all, and the moment
-- someone resumed it the fail-loud check would refuse it — a connection that worked yesterday
-- breaking because it happened to be paused during a migration.
--
...
SELECT
    ...
    CAST(NULL AS text),                                             -- ProvenanceDetail: no merge log was ever recorded for a shared row
    CASE WHEN so.selected_at IS NOT NULL
         THEN true ELSE false END,                                  -- IsSelected: comes from the MAPS, never from Status
    ...
    CAST(NULL AS timestamp with time zone)                          -- LastSampledAt: the backfill sampled nothing
FROM ${mjSchema}."CompanyIntegration" ci
JOIN ${mjSchema}."IntegrationObject" io
  ON io."IntegrationID" = ci."IntegrationID"
LEFT JOIN selected_object so
  ON so.connection_id = ci."ID"
 -- CASE-INSENSITIVE on purpose: an entity map's ExternalObjectName is typed by a human or lifted
 -- from an API response, and it matches the catalog object by name, not by id. A case mismatch
 -- would silently deselect an object the customer is actively syncing.
...
WITH selected_field AS (
    ...
    JOIN ${mjSchema}."CompanyIntegrationEntityMap" em
...
SELECT
    ...
    CAST(NULL AS uuid),                                             -- RelatedCompanyIntegrationObjectID: resolved in step 3
    iof."ID",                                                       -- IntegrationObjectFieldID: the provenance link
    CAST(NULL AS integer),                                          -- ObservedMaxLength: nothing was sampled, so there is no observed width
    CASE WHEN iof."MetadataSource" = 'Discovered'
         THEN 'Sampled' ELSE 'Declared' END,                        -- Provenance (lossy — see step 1)
    CAST(NULL AS text),                                             -- ProvenanceDetail
    CASE WHEN sf.selected_at IS NOT NULL
         THEN true ELSE false END,                                  -- IsSelected: from the field MAPS, never from Status
    sf.selected_at,                                                 -- SelectedAt
    iof."__mj_CreatedAt",                                           -- FirstSeenAt
    iof."__mj_CreatedAt",                                           -- LastSeenAt
    CAST(NULL AS timestamp with time zone)                          -- LastSampledAt
FROM ${mjSchema}."CompanyIntegrationObject" cio
JOIN ${mjSchema}."IntegrationObjectField" iof
...
UPDATE ${mjSchema}."CompanyIntegrationObjectField" ciof
   SET "RelatedCompanyIntegrationObjectID" = related_cio."ID"
  FROM ${mjSchema}."IntegrationObjectField" iof,
       ${mjSchema}."CompanyIntegrationObject" owner_cio,
       ${mjSchema}."CompanyIntegrationObject" related_cio
 WHERE ciof."IntegrationObjectFieldID" = iof."ID"
   AND owner_cio."ID" = ciof."CompanyIntegrationObjectID"
   AND related_cio."CompanyIntegrationID" = owner_cio."CompanyIntegrationID"
   AND related_cio."IntegrationObjectID"  = iof."RelatedIntegrationObjectID"
   AND iof."RelatedIntegrationObjectID" IS NOT NULL
