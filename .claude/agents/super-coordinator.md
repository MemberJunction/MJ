...
5. Read the run's terminal status. The amendment loops have already exhausted their caps (extract 3, codebuild 3) before reaching you, so a non-`Complete` status is honest escalation:
...
```json
{
  ...
  "AchievedCeiling": "live-content-verified" | "format-verified-no-creds",
  "Status": "Complete" | "RejectedByFloor" | "EscalatedToHuman" | "BudgetExhausted",
  "NextActions": "..."
}
```

**`AchievedCeiling` is REQUIRED and copied verbatim from `FloorCheckVerdict.summary.achievedCeiling` — never inferred or upgraded.** A `Complete` that did NOT run a live, content-verified e2e is `Complete` **with `AchievedCeiling: "format-verified-no-creds"`** — say so in `REPORT.md`'s headline, not buried. A bare `Complete` that reads as "fully live-proven" when the build was credential-free is the exact over-claim the honest-ceiling discipline forbids (Facet-D #H9). `live-content-verified` may be reported ONLY when the floor emitted it (live hybrid-e2e ran AND the reality-probe content-verified ≥1 read).
