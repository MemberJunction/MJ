...
- **Exhaust every enumeration level.** At every enumeration level — vendor's spec catalog, API areas within the catalog, endpoints within an area, parametric variables in endpoints, fields within an object — exhaust the set. Don't stop at the first batch you find. For each level, verify completeness against the vendor's top-level API reference / catalog index / sitemap before moving on. Stop only when you have evidence the set is exhausted, not when you have "enough" results.
