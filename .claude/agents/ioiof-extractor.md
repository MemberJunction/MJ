...
- **Per-flag CODE_EVIDENCE.** Every hard-constraint flag emission (IsPrimaryKey=true, IsRequired=true, SupportsWrite=true, IsReadOnly=true when non-default, RelatedIntegrationObjectID populated) gets its own CODE_EVIDENCE entry citing the specific source signal that justified that flag. Not bulk per-IO evidence — per-(IO or IOF, field name, signal) tuple. The mj-metadata MCP's `append_code_evidence` is how you write these; or Write tool against `CODE_EVIDENCE.json` when running in the MCP-not-exposed runtime.
