...
- **EXTRACTION_REPORT.md must classify every COVERABLE taxonomy from SOURCE_STUDY.** Two structured sections, no exceptions:
  - **Taxonomies covered** — for each COVERABLE taxonomy named in SOURCE_STUDY where you emitted at least one IO: taxonomy label + count of IOs emitted under it + source citation copied from SOURCE_STUDY.
  - **Taxonomies excluded with reasoning** — for each COVERABLE taxonomy named in SOURCE_STUDY where you emitted zero IOs: specific reason (parametric-only / runtime-bound / no GET surface / deprecated per source / out-of-scope per Phase 1 / etc.) + source citation copied from SOURCE_STUDY.
  - **Mechanical check the coordinator will run**: union of (covered ∪ excluded) MUST equal the full COVERABLE-taxonomy list from SOURCE_STUDY. Any unclassified COVERABLE taxonomy = report rejected, re-dispatched. No silent omissions.
  - INFORMATIONAL taxonomies (vendor mechanics, schemas, vocabularies — e.g., OAuth flow components, webhook signature schemes, rate-limit categories) are NOT included in the coverage check. You CONSUME them (they inform your extraction logic) but you don't enumerate them into IO rows. Document HOW you used each informational taxonomy in a separate "Informational taxonomies applied" section.
  - L1 container taxonomies (e.g., HubSpot's 17 spec-collection Areas) are NOT in the coverage skeleton — they CONTAIN the L2 surfaces which are the coverable ones. Don't classify L1 containers; classify their L2 leaves.
