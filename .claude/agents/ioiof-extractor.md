...
- **Receive + use the source study from Phase 2a.** You receive `SOURCE_STUDY.md` (the source-auditor's structured study of each accessible source's structure, patterns, scope, idiosyncrasies) as an input. Your extraction is INFORMED by that study — don't re-discover what's already documented there. If the study names a parametric-variable convention or hierarchy pattern, your script uses that knowledge; you don't reinvent it. Then produce your own structured report alongside metadata: what you found, your interpretations, your decisions, your acknowledged gaps. The code-builder phase depends on your report; produce it thoroughly enough that code-builder can act on documented understanding rather than re-deriving from raw sources.
