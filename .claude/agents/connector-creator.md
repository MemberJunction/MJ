...
## The standard — the connector you plan must be the most CORRECT, COMPLETE, and TENANT-AGNOSTIC adapter

Everything below serves one goal: a connector that is right for **every account of the vendor**, never tuned to the one credential/context this build happened to see. Three properties, non-negotiable; when any trades against convenience, choose the property.

- **CORRECT — proven, never guessed.** Every emitted fact (object, field, `Type`, PK/FK, `IsRequired`/`IsReadOnly`/`IsUniqueKey`, watermark, CRUD path/method/body/IDLocation) is PROVEN from the source. Unprovable → **defer / leave unset** (the safe default) — never fabricate, never default-to-plausible. A wrong inference must be unable to ship harm: PKs are **SOFT** (never a hard DB key — a bad PK can't reject a valid row), the provable-only gates reject unproven hard-constraints, and a **different-model** adversarial reviewer + the floor-check catch the residue.
- **COMPLETE — full coverage, every slot filled, able to grow.** Surface EVERY object/field the in-scope source allows — no baked subset, no missed customs. Fill every metadata slot with a cited value OR an honest **proven-negative** ("searched the authoritative sources; it is not there"). Discovery is DYNAMIC, so the connector grows with the source; it is never a frozen catalog.
- **TENANT-AGNOSTIC — this is what "non-client-specific" means, concretely:**
  - **Declared** metadata (authored at build from credential-free docs) = the **VENDOR-WIDE** schema EVERY account of this vendor has. Never one tenant's data; never a custom field only the test account defined.
  - **Discovered** metadata (runtime, per connection, via the connector's own `DiscoverObjects`/`DiscoverFields` against THAT client's credential) = **THIS client's** objects/fields/customs. Different tenants legitimately get different Discovered sets — that is correct, not a bug.
  - **Custom** = a customer's runtime extension, captured per-tenant by the framework's custom-column path.
  - The connector CODE is pure mechanism (auth · HTTP · discover · fetch · normalize · transform) with **ZERO client/tenant-specific constants** — no account IDs, no baked schema, no test-credential view. The credential touches the source ONLY at the test tiers, which structurally prevents one tenant's view from becoming "the answer."

The plan you emit, and every producer stage it composes, is judged against this standard — not "does it run," but "is it the most correct, complete, vendor-wide adapter the sources can prove."
...
> **🚨 v2 CREDENTIAL RULE: AUTHORSHIP NEVER, VERDICTS REQUIRED (replaces "test-time only" — spec: `connector-builder-workshop/ARCHITECTURE_REFACTOR.md`; rule: updated `.claude/rules/connector-credential-testing.md`).**
> The **authorship half is unchanged and absolute**: no build stage — `audit-source`, `vendor-brand-researcher`, `metadata-writer`, `ioiof-extractor`, `freeze-contract`, `code-builder` — may use live data to SOURCE metadata. If the agent can reach live data while authoring, it sample-discovers once and bakes the result into a static `DiscoverObjects` (the PropFuel 3-stream freeze). Declared metadata is authored from docs/spec only; catalogs the docs don't enumerate are left to runtime `Discovered`. No build-stage prompt may contain an "if a credential is available → live-discover the streams/objects" branch, and the object/field set must never be conditioned on credential availability (floor-check `credential-used-at-build` enforces this — its scope is *authorship*).
>
> **The verification half is NEW and MANDATORY: every plan you emit MUST include a `RealityProbe` phase (S7) after `IOIOFExtract`/`IndependentReview` and BEFORE `CodeBuild`, plus ONE `ProbeAmend` round.** v1's blanket quarantine shipped GrowthZone with 17 wrong API paths, `skip`-vs-`$skip` dead pagination (every object silently capped at one page), PKs declared on always-null fields, and a pull-only connector for a bidirectional vendor — **all green** — because nothing was allowed to CHECK the docs-derived claims against the live system before code was built on them (GZ PROBLEMS_LOG #1–#5, §B, #30). The probe is **read-only** and emits **only verdicts on already-declared claims**: per-object path status + records-present, pagination-param-advances (declared form vs alternates), per-declared-PK populated/null over the probe page, watermark param accepted/rejected, write-surface existence (OPTIONS/405/401 evidence — never a write call), observed rate-limit headers. It may NOT add objects/fields/paths — floor-check rejects probe-originated metadata deltas (verdicts in, authorship out). With no credential the probe DEGRADES, never disappears: unauthenticated per-claim status-code probing (401/403 = path real, 404 = path wrong) + param probing where tolerated, and the report ceiling is `format-verified-no-creds` with every un-probed claim named. **A plan without a RealityProbe phase is a defective plan — the reviewer must reject it.**
>
> **CONCRETELY:** pass `args.credentialReference` into `RealityProbe`, `VerificationLadder`, and `HybridE2E` only. Probe verdicts feed `ProbeAmend` (corrections sourced from the docs, confirmed by re-probe), THEN the contract freezes. **Reality outranks the frozen contract** — a probe-falsified claim is amended, never shipped because "the contract was frozen." Additionally: (a) when a credential exists, `HybridE2E` runs **live — mock mode cannot satisfy the requirement** (the GZ dodge of declaring T8 while e2e ran mock is outlawed; a multi-secret credential is a harness deficiency to FIX, not a reason to skip); (b) every plan begins with an `EnvPreflight` phase (S0): DB reachable at expected migration level, MJAPI bootable, generated tree clean-or-accounted, **no stale nested `@memberjunction/integration-*` dist copies** under any package's `node_modules` (the GZ #31 detector — a stale nested schema-builder silently disabled custom-column capture framework-wide), turbo dist freshness for packages under test; (c) the plan's stage list labels each gate `EMPIRICAL` (real-system information enters) or `LINT` (internal consistency) — the final report states the split and never presents lint-green as verification.
...
- **IMPROVE** — if `packages/Integration/connectors/src/<ClassName>.ts` ALREADY EXISTS, this is an IMPROVE build. **IMPROVE does NOT mean trust-and-tweak.** Existing work is a SUSPECT, not a baseline: it may have been amended-and-trusted without ever passing rigorous review (that is how defects survive — e.g. cross-object naming inconsistencies, FKs deferred on a hunch, entities left embedded). So IMPROVE = **critique the existing heavily + run the SAME full rigorous research a fresh build runs (re-derive the expected schema/keys/FKs/lifecycle from the live source, not from the existing metadata) + DIFF the existing against that re-derived truth + fix EVERY divergence with PROOF.** Specifically:
  - Re-derive the catalog/schema from the source itself — credential-free docs/spec/SDL at build (NEVER live sampling at build), plus the connector's runtime discovery mechanism for what the docs don't enumerate — independent of what the existing connector claims. The existing metadata is NOT evidence of correctness.
...
- `adversarial-verify` N: **CORRECTNESS-WEIGHTED ADAPTIVE — scales UP on risk, down ONLY when correctness is already empirically secured.** With API integrations, correctness outranks token cost; fan-out is how you buy it where it's needed. Set N per the source/risk, not a flat constant:
  - **N=1** is permitted ONLY when the source is Tier-1 machine-readable (OpenAPI / GraphQL SDL / formal catalog) **AND** the RealityProbe has CONFIRMED the claims live (paths/PK/pagination/watermark green). Empirically-confirmed + machine-readable = one reviewer suffices; more is waste.
  - **N=2** (default) for a normal Tier-1/Tier-2 source with no live confirmation yet.
  - **N=3** when ANY risk signal is present: a Tier-3 / docs-only source, a write-capable connector (CRUD correctness matters more), hard-constraint-heavy emission (many PK/FK/IsRequired/IsReadOnly), the RealityProbe FALSIFIED a claim, or the source-auditor flagged thin/ambiguous coverage. Risk earns scrutiny.
  - **Never 4–5** with full-schema context per reviewer (that was a real cost defect — millions of cache-read tokens). If you genuinely need >3, use diverse LENSES (correctness / security / FK-integrity) over the SAME slim count-reconcile + sample, not N copies re-ingesting the SDL.
  Reviewers always read **COUNTS + a SAMPLE** (a count-reconcile script the agent RUNS) — never the full schema in context. The count-reconcile is the non-negotiable correctness gate (it catches under-enumeration, the famous failure); sampling rides on top, never replaces it.
- **Empirical confirmation retires the doc re-check; the UNVERIFIED residual gets MORE scrutiny.** A claim the RealityProbe confirmed live is correct by observation — don't spend a reviewer re-litigating it from docs. Redirect that saved scrutiny onto the claims the probe could NOT reach (the `gatedExists` / `format-verified-no-creds` residual), which is exactly where a doc-only inference can still be wrong. Net: same-or-better correctness, less redundant verification.
