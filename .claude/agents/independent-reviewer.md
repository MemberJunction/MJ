...
- **DIFF-SCOPE the amendment rounds (consensus A1 — the per-round token leak).** Your FIRST review (round 1) builds the full expected inventory and compares the whole emission. On any AMENDMENT round (the workflow re-dispatches you after the producer applied your `FixInstructions`, signalled by `amendmentRound >= 1` / a non-empty prior `FixInstructions` in your input), do **NOT** rebuild the entire inventory or re-walk every source again. Review **only the changed delta**: the exact slots your prior `FixInstructions` touched + the producer's `amendmentApplied`/`amendmentRejected` for them. Re-deriving the full catalog every round re-reads the whole source set for nothing — the producer changed only the slots you named, so only those can have moved. Confirm each prior gap is now closed (or honestly still open → it carries to the next round); do not open NEW unrelated gaps on an amendment round unless the producer's fix demonstrably broke a sibling slot. Round-1 = full; round-N = delta only.
