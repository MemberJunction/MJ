...
## Open App packaging + the two-repo split (v2 — REQUIRED)

**Two repos, two kinds of output — never conflate them:**

1. **The connector DELIVERABLE → the `MemberJunction/Integrations` repo** (a SEPARATE git repo), at `<Category>/<Connector>/`. A connector is no longer a class inside the shared `@memberjunction/integration-connectors` package — it is its OWN installable **Open App**. The deliverable is: the connector `.ts`; `package.json` (`name` = `<PackageName>`); `mj-app.json` (`schema.name` = `mj_connector_<slug>`, `packages.server[0]` = `{ name: <PackageName>, role: 'bootstrap', startupExport: 'registerConnector' }`, `migrations`/`metadata` dirs, `mjVersionRange`, `categories`); `src/index.ts` (`export * from './<ClassName>.js'` + a no-op `registerConnector()`); the metadata under `metadata/integration/`; the seed migration in `migrations/` + `migrations-pg/` (pure `EXEC [__mj].spCreate*`, generated by `mj sync push`); and the tests. The repo's `scripts/validate-invariants.mjs` is the gate.

2. **The build MACHINERY → stays in the MJ monorepo.** The `connector-builder-workshop` locked **primitives** (`verification-ladder`, `extract-iiof-pipeline`, `verify-claim`, `audit-source`, `floor-check`, …), the floor-check rules, the per-vendor `<vendor>.workflow.js` plans, and the agents all live in MJ and run from there. You **USE** primitives from MJ — you do NOT copy them into the Integrations repo. When a pattern recurs across **3+ connectors** (ADR-001), **AUTHOR a new primitive / strategy in MJ** (`@memberjunction/connector-extractor-strategies` or a new workshop primitive) — never bake reusable logic into a single connector, and never author a primitive in the Integrations repo. Rule of thumb: **connector-specific output → Integrations repo; reusable build logic → a primitive in MJ.**

> **How identity flows (v2).** You build the connector in the MJ SANDBOX under its TS class symbol `<ClassName>` (symbol = `@RegisterClass` key = sandbox metadata `ClassName`), which keeps the sandbox ladder/T1 green with NO machinery change. The final **`OpenAppPublish`** stage (`scripts/publish-open-app.mjs`) assembles the verified connector into the Integrations repo as the Open App above and applies the four-way package-name identity, with `validate-invariants.mjs` as the gate. So `code-builder` does NOT emit a package-name key and does NOT write into the Integrations repo — it builds the sandbox connector at `packages/Integration/connectors/src/<ClassName>.ts`; the publish step ships the Open App.
...
- `@RegisterClass(BaseIntegrationConnector, '<ClassName>')` in the MJ SANDBOX build — the key you EMIT is the TS class symbol `<Vendor>Connector` (== `identity.Identity.ClassName` == the sandbox metadata `ClassName`), so the sandbox verification ladder / T1 three-way check stay green UNCHANGED. **You do NOT write the package-name key — the `OpenAppPublish` step rewrites it.** On publish, that key (and `MJ: Integrations.ClassName` / `ImportPath` / `package.json` name) all become the npm package name `@memberjunction/connector-<slug>` — the four-way identity that `validate-invariants.mjs` gates and that resolves on a customer instance (PUB.0). The `IntegrationName` getter === `MJ: Integrations.Name` is a SEPARATE display-name invariant, unchanged by publish.
...
- `DiscoverObjects`/`DiscoverFields` MUST surface every object/field the source allows. WHERE the structure comes from follows the three-case rule in `connector-code-conventions.md` § "Discovery: capture every object/field the source allows": (1) **no-auth-reachable** → `Declared` metadata (never hardcoded in code); (2) **auth-gated describe/list/introspection** → the connector discovers at runtime, right after creation, filling `Discovered` IO/IOF; (3) **knowable only by syncing data** → the framework's runtime custom-column capture (`__mj_integration_CustomOverflow` → post-sync promotion, §2 BUILT) mints the columns at sync. **NEVER sample data at build, NEVER bake a static catalog** — a baked catalog where the source admits more is a DEFECT (customs dropped at `FieldMappingEngine.MapSingleRecord`).
- **🚨 STANDARD DISCOVERY MUST NOT DEPEND ON A CREDENTIAL when a credential-free schema-of-record exists (this is the T3-deadlock fix).** If the vendor publishes a **publicly-reachable** schema — an OpenAPI/Swagger doc, a downloadable SDL, a SpectaQL/GraphDoc schema-reference page — that is **case 1**, and `DiscoverObjects`/`DiscoverFields` MUST enumerate the **full standard universe from it WITHOUT any credential** (fetch + parse the public schema at discovery time). A *live introspection/describe endpoint being auth-gated does NOT make the connector case 2* if the same schema is published credential-free elsewhere — using the auth-gated endpoint as the ONLY discovery path is the bug that deadlocked Path LMS at T3: the runtime `DocStructureSelfCheck` runs credential-free, the connector's discovery returned nothing without a token, and every persisted object read as "structure drift." Correct shape: **standard objects come from the public schema (no token, always works); a live credential is ADDITIVE** — it only adds tenant-specific customs (the `Discovered` extension), never the baseline. So discovery re-yields the standard universe credential-free → T3 passes. Enumerate **record types, not entry points** (same logic as `floor/enumerate-catalog.mjs`).
...
- **🆕 Two-stage `DiscoverFields` + statistics-first SOFT PK (v5.39.x — the canonical data-driven path for cases 2 & 3).** When a field or PK is knowable only from the data (no describe endpoint, or describe omits the PK), `DiscoverFields` runs **stage-1** (the describe/endpoint discovery above) THEN **stage-2**: call the base helper **`this.DiscoverFieldsViaStream(records, opts)`** with a READ-ONLY record stream (whatever your fetch yields — no save, no ack). It streams the real data **time-bounded** (`opts.Discovery.TimeBudgetMs` — configurable per-connection via the `IntegrationSetSyncConfig` GraphQL mutation → `discoveryTimeBudgetMs`; stops on data-exhaustion OR the budget, whichever first), accumulates the full column corpus + per-column uniqueness/null **statistics** (bounded memory), and returns `ExternalFieldSchema[]`. PK is set as a **SOFT key** on the **best-available identity** — the statistics pick (`pickPrimaryKeyFromStats`) **combined with naming-convention signal** (`id`/`<Obj>Id`/`*_id`/`uuid`/`code`), above a **low** floor; ties go to the LLM tiebreaker. Because the PK is soft (it can't reject a row — see the firewall below), **don't gate it behind strict significance** and **never leave an object PK-less when any column is plausibly identifying** — a missing PK makes CodeGen skip the entity and stalls the loop. Only defer (honest 'none') when no candidate exists at all. `AllowsNull`/`IsUniqueKey` stay **provable-only** (NOT NULL only from an endpoint's "required" flag — clean sample data is NOT proof). More data = stronger claims; the budget guarantees discovery never runs away. Do NOT hand-roll a sampling loop — use the helper. *(The `pickPrimaryKeyFromStats` helper now implements this low-floor soft policy in code: a confident unique+non-null tier first, else a near-unique / convention-named soft pick via the built-in `defaultPkNameRank`, never leaving an object PK-less when a column is plausibly identifying. `MinRowsForSignificance` only governs the confident tier now, not whether a key can be picked.)*
- **🔗 FULL-RECORD PASS-THROUGH — the custom-column contract (don't break M1–M4).** Whatever the fetch returns, the **complete source record must reach `ExternalRecord.Fields`** — never a filtered/mapped subset. The framework's custom-column capture diffs `keys(ExternalRecord.Fields)` against the active field maps; any key a connector drops *before* building the record is invisible and unrecoverable, so the connector silently can't grow custom columns. Base-fetch connectors get this structurally; if you **override `FetchChanges` and hand-build records**, set `Fields` to the FULL record (`{ ...source }` / the raw item) — NOT a narrow `{ Id, Name, … }` literal (that was the YourMembership bug). Persistence is unaffected (`SetEntityFields` writes only `MappedFields`), so the full record never causes unknown-column writes. The floor's `FullRecordPassThrough` gate fails a narrow-literal `Fields:`.
- **🔒 SOFT PK ONLY — never a hard DB key (the firewall).** A discovered or declared PK is SOFT. It rides `additionalSchemaInfo` → `applySoftPKFKConfig` sets the entity METADATA `IsPrimaryKey` + `IsSoftPrimaryKey` (the flag that stops schema-sync from ever enforcing it) → CodeGen uses the metadata PK for sproc/view generation; **`DDLGenerator` emits NO `PRIMARY KEY` and NO `UNIQUE` constraint** for it. So a wrong inference can never reject a valid row — the engine dedupes by the record-map + content hash, not a DB key. The connector's only job is to mark PKs *honestly* (provable-only); it never emits DB constraints and never calls `SoftPKClassifier` (runtime D4's job). Composite PKs are first-class: the engine's content-hash idempotency now handles them (`MatchedMJRecordID` = the `|`-joined key), so association/junction objects skip cleanly on re-sync.
- **🆕 SAMPLE-UNION FIELD ENRICHMENT (REQUIRED — the connector standard; full spec in `connector-code-conventions.md` § "Sample-union field enrichment").** Wire an `IntrospectSchema` override that enriches the Declared field set with live-sampled fields so a tenant's customs reach the schema: `const info = await super.IntrospectSchema(ci, user);` then per object (parallel, best-effort) `const sampled = await this.DiscoverFieldsViaFetch(ci, obj.ExternalName, user); obj.Fields = mergeDeclaredWithSampledFields(obj.Fields, sampled);` — import `mergeDeclaredWithSampledFields` from **`@memberjunction/connector-schema-merge`** + add the dep. Override `IntrospectSchema`, **NEVER `DiscoverFields`** (that recurses into `DiscoverFieldsViaFetch`'s fallback). Pass the two engine shapes directly (`SourceFieldInfo[]` declared × `ExternalFieldSchema[]` sampled — the helper maps + never-shrinks; do NOT hand-map). Connector-agnostic (identical eight lines every connector). Skip ONLY when schema-less or already self-sampling.
...
- **🚨 Walk the per-IO access path for nested-graph / reporting APIs — do NOT assume one flat query per object.** When the source is a nested-graph / reporting API (the object universe is larger than its directly-queryable entry points — the *doors*), the extractor emits per IO an **access path** in `Configuration`: the entry-query / door name, the nesting field-path down to the object's records (chain of field names, list `[]` segments — e.g. `account → orders[] → items[]`), and any door-level args. `FetchChanges` MUST READ that access path and WALK it: build a nested selection set descending the path, execute the **door** query, then descend the response down the path to emit the **leaf** records. A directly-queryable object is the **depth-0** case (empty path) — the flat top-level query. Assuming a flat per-object query when the object is actually nested is the Path LMS defect that pulled 0 rows (every credential-free check passed; only the live round-trip caught it). If an IO is nested and carries no access path, escalate — never ship it returning nothing.
...
- **`IntegrationName` getter is verbatim from upstream.** No abbreviation, no variation, no shortening. It equals the vendor DISPLAY name: `connector.IntegrationName === metadata.fields.Name === Phase1Handoff.Identity.Name` — unchanged by publish. DISTINCT from the registration key: in the SANDBOX the `@RegisterClass` key === the TS class symbol `<Vendor>Connector` (== sandbox metadata `ClassName`); the `OpenAppPublish` step rewrites that key + `ClassName` + `ImportPath` + `package.json` name to `@memberjunction/connector-<slug>` for the shipped Open App.
