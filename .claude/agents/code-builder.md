...
> **NOTE — v2 in progress.** The per-vendor plan templates + the workshop floor-check still pin the OLD monorepo output path (`packages/Integration/connectors/src/<ClassName>.ts`) and the flat `metadata/integrations/<vendor>/` location. Relocating the connector output to the Integrations-repo Open App layout (and emitting the packaging files) is the executable follow-up to this contract. **The identity + repo-split rules in this doc are correct and authoritative now.**
...
- `@RegisterClass(BaseIntegrationConnector, '<PackageName>')` — grandparent registration. **In the Open App model the driver KEY is the connector's npm package name `@memberjunction/connector-<slug>` (= `identity.Identity.PackageName`), NOT the TS class symbol.** The `.ts` file and the exported class are still named `<ClassName>` (`<Vendor>Connector`); only the `@RegisterClass` *key string* is `<PackageName>`. The **four-way identity invariant** holds exactly: `@RegisterClass key === MJ: Integrations.ClassName === MJ: Integrations.ImportPath === package.json name === <PackageName>` (one and the same string). The `IntegrationName` getter === `MJ: Integrations.Name` pairing is a SEPARATE invariant — the vendor DISPLAY name, which is neither the package name nor the class symbol.
...
- **`IntegrationName` getter is verbatim from upstream.** No abbreviation, no variation, no shortening. It equals the vendor DISPLAY name: `connector.IntegrationName === metadata.fields.Name === Phase1Handoff.Identity.Name`. This is DISTINCT from the registration key: `@RegisterClass key === metadata.fields.ClassName === metadata.fields.ImportPath === package.json name === Phase1Handoff.Identity.PackageName` (= `@memberjunction/connector-<slug>`), which is neither the display name nor the TS class symbol.
