...
write round-trip + true rate behavior are credential-only). A build is not "done" until it PASSES.

It is also **independently invokable** — `/test-connector <vendor> --mode live --ad-hoc` — to re-prove an
already-built connector **when a credential arrives later** (upgrading the read-only ceiling to
live-verified, no rebuild) or as a regression check when the vendor's spec shifts. See
`.claude/skills/test-connector/SKILL.md`.
