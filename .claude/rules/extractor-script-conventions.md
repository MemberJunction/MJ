...
- **NO artificial object/field ceiling.** The extractor emits EVERY object and field the source's machine-readable model exposes — the only bound is the source's real catalog size. A connector for a 1,694-object CRM emits 1,694 IOs; a magic cap (the retired "1000 per run") is exactly the artificial ceiling that silently truncates a large catalog into a famous-only subset (the Salesforce-class under-enumeration). If the emitted count is SMALLER than the enumerated universe, that is the bug — not the count being large.
- If a single one-shot pass is too large to complete in one run, **chunk by category and continue** — never DROP the tail. Chunking partitions the same full universe across passes; it does not cap it. Each chunk's emission is additive (idempotent upsert), and the run is only complete when `emitted == enumerated`.
