...
1. **`@lookup:MJ: Integration Objects.Name=<sibling>&IntegrationID=@parent:IntegrationID` on `RelatedIntegrationObjectID`** — the push-time UUID pointer. Correct and preferred for MOST connectors. **The qualifier MUST be `@parent:IntegrationID`, NEVER `@parent:ID`.** This is the one `@parent` ref that is NOT `@parent:ID` (contrast the always-`@parent:ID` parent refs on the line above): an IOF's `@parent` is its containing **IO**, which carries both `ID` (the IO's own id) and `IntegrationID` (the FK to the Integration). The sibling-object lookup filters by the **Integration's** id, so it must read the IO's `IntegrationID` field → `@parent:IntegrationID`. Writing `@parent:ID` here resolves to the IO's OWN id, matches no sibling, and rolls the entire `mj sync push` back on a "Lookup failed" — the exact defect that blocked the iMIS and GrowthZone connectors from deploying. `@parent:ID` is a *valid keyword elsewhere*, so it isn't malformed and a generic mj-sync validator won't catch it; the deterministic floor gate **`floor/fk-lookup-qualifier.mjs`** (a hard floor-check rule + a `--all` CI scan over `metadata/integrations/**`) enforces it. — The `@lookup` form only breaks when the FK graph **forward-references a sibling not yet committed in the same push transaction** (mj-sync resolves `@lookup` via a DB RunView that can't see same-transaction uncommitted rows → rollback; `?allowDefer` does NOT help — the retry is in the same transaction). That rollback hits a **dense, forward-referencing** graph (Path LMS, 84 cross-linked objects). A **sparse / backward-ordered** graph pushes clean in one shot (ORCID, 11 FKs all pointing at earlier objects) — KEEP the `@lookup`; it's load-bearing and dropping it buys nothing.
