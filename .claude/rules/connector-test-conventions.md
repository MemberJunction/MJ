...
- **Bound huge streams** — don't fully drain a 50k-row table every run; prove advancement+termination on a mid-size multi-page "Goldilocks" stream and bound the rest (memory `feedback_bounded_pagination_goldilocks`). **Goldilocks bounds ROW DEPTH per object, NEVER the object SET, and applies to LIVE only.**
- **MOCK = FULL OBJECT COVERAGE (no subset).** In MOCK mode the vendor is free + instant, so **EVERY active object must land ≥1 row and EVERY writable object's write must round-trip** — the mock fixtures must carry data for every object. A blanket `coverageScopeReason` / "Goldilocks subset" is **REJECTED** in mock mode (floor rules `behavioral-coverage-mock-incomplete` / `write-coverage-mock-incomplete`); only a *concrete per-object* `skipReason` for a structurally-unmockable object (e.g. no read endpoint) is honored. The ONLY things exempt from mock proof are a real-credential **live** round-trip and **Postgres** — everything else is mocked, all objects, all cells. `coveredObjects` MUST equal the full active IO set.
