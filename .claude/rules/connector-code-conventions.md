...
- Decorated with `@RegisterClass(BaseIntegrationConnector, '<PackageName>')` — grandparent registration, NOT the intermediate base; the factory dispatches off `BaseIntegrationConnector`. **The driver KEY is the connector's npm package name `@memberjunction/connector-<slug>` — the SAME string as `MJ: Integrations.ClassName`, `MJ: Integrations.ImportPath`, and the `package.json` name (the four-way invariant). It is NOT the TS class symbol.** The exported class stays named `<Vendor>Connector`; only the `@RegisterClass` key string is the package name. (Connector code lives in the `MemberJunction/Integrations` repo as an Open App at `<Category>/<Connector>/`; the build machinery + locked primitives stay in the MJ monorepo workshop.)
