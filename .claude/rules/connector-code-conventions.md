...
- Decorated with `@RegisterClass(BaseIntegrationConnector, '<DriverClass>')` — grandparent registration, NOT the intermediate base; the factory dispatches off `BaseIntegrationConnector`. In the MJ sandbox/registry the `<DriverClass>` key is the TS class symbol `<Vendor>Connector` (== sandbox metadata `ClassName`), which keeps the verification ladder / T1 three-way check green. **The shipped Open App in `MemberJunction/Integrations` carries the package-name key `@memberjunction/connector-<slug>` (== `MJ: Integrations.ClassName` == `ImportPath` == `package.json` name — the four-way invariant); the `OpenAppPublish` step applies that transform and `validate-invariants.mjs` gates it.** The exported class stays named `<Vendor>Connector` either way. (Build machinery + locked primitives stay in the MJ workshop; the connector DELIVERABLE ships to the Integrations repo as an Open App at `<Category>/<Connector>/`.)
...
- **`IsForeignKey` is READ from the source's own type model, NEVER guessed from field shape/name.** A field is an FK *only* when the source machine-readable model marks it as a **scalar value that references another object's primary key** — a SQL `FOREIGN KEY` constraint, an OpenAPI property carrying a `$ref`/relationship to another schema's key, a GraphQL **scalar** field (e.g. `accountId: ID`) that points at another type's id. A field whose *type is itself another object, a list of objects, or a GraphQL connection* (`Account.courses`, `Account.events`, a REST embedded/nested object) is an **ACCESS-PATH, not an FK** — emit it into the IO's `Configuration` nesting path (see the access-path rule below) and leave `IsForeignKey` false. Marking a relationship/connection/nesting edge as `IsForeignKey=true` is the path-LMS defect (248 half-set FKs: `IsForeignKey=true` + null target, because an edge has no scalar PK to reference). The rule generalizes across protocols: derive FK from what the source's type system *says*, not from the fact that a field "points at" another entity.

### GOVERNING PRINCIPLE — READ from the source's model, never GUESS from a template (the anti-overfit rule)

The two famous failures (Salesforce declaring 11 of 1,694 objects; path-LMS marking 248 GraphQL connection edges as FKs) are ONE failure: an attribute was **guessed from a remembered template** instead of **read from the source's own machine-readable model**. Provable-only silently collapses to *famous-only* / *relational-default* whenever the value is generated rather than derived. This applies to **every** structural attribute the extractor emits — these are all guess-points:

| attribute | the wrong guess | read it from |
|---|---|---|
| object set (scope) | the canonical/famous list | the enumerated universe (SDL types / describe / catalog) — narrow ONLY with named need- or breadth-evidence |
| `IsPrimaryKey` | "`id` is the PK" | the source's declared PK (constraint / `@id` / describe `idLookup`) |
| `IsForeignKey`/target | "a field that points at another type is an FK" | a scalar that the source marks as referencing another object's PK (edges/connections/nested objects are access-paths, not FKs) |
| `Type`/`MaxLength`/`Precision` | inferred from a data slice | the source's declared column/field type |
| `IsRequired`/`AllowsNull` | inferred from samples | the source's stated constraint (else leave undefined) |
| `IsReadOnly`/`IsUniqueKey` | assumed | the source's flags |
| pagination params | a guessed name (`skip` vs `$skip`) | the spec's documented param + the RealityProbe's advance check |
| watermark/incremental field | assumed `updatedAt` | the documented incremental cursor |
| `SupportsWrite` + CRUD paths | "a famous vendor has writes" | actual mutations/POSTs in the spec (0 mutations ⇒ read-only, like path-LMS) |
| access-path / nesting | guessed | walked from the type graph |

The rule, uniformly: **derive each attribute from the source's machine-readable model (SDL / OpenAPI / catalog) wherever one exists. Where the source does not state it, leave it provable-only (undefined / false-by-honest-default) and let the credentialed RealityProbe (empirical net) or runtime discovery resolve it — NEVER fabricate it from a template.** Keyless, the unread residual is reported as **unverified** (ceiling `format-verified-no-creds`), never implied as covered. A thin or template-shaped emission (e.g. declared objects ≪ enumerated universe, or a field-set that is all `NVARCHAR` with no source type) is a **flag requiring named evidence**, not a silent pass.
