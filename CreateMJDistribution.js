...
/**
 * Handle package.json for server-side packages (MJAPI, GeneratedEntities, GeneratedActions).
 * Removes tsc-alias references from build scripts since these packages don't use path aliases
 * and tsc-alias is only available via the monorepo root devDependencies (not in distribution).
 */
function handleServerPackageJson(dir, normalizedDir, archive) {
  console.log(`   Processing ${normalizedDir} package.json to remove tsc-alias references...`);
  ...
  if (packageJson.scripts) {
    for (const [scriptName, scriptCommand] of Object.entries(packageJson.scripts)) {
      if (typeof scriptCommand === 'string' && scriptCommand.includes('tsc-alias')) {
        // Remove "&& tsc-alias -f" or "&& tsc-alias" (with optional flags) from build scripts
        packageJson.scripts[scriptName] = scriptCommand.replace(/\s*&&\s*tsc-alias\b[^&]*/g, '').trim();
        console.log(`      Removed tsc-alias from script: ${scriptName}`);
      }
    }
  }
...
async function createMJDistribution() {
  ...
  for (const dirConfig of directories) {
    ...
    if (dirName === 'SQL Scripts') {
      ...
      // Handle server-side packages - flatten their tsconfig.json and strip tsc-alias from package.json
      if (dirName === 'MJAPI' || dirName === 'GeneratedEntities' || dirName === 'GeneratedActions') {
        handleServerTsconfig(dir, zipPath, archive);
        handleServerPackageJson(dir, zipPath, archive);
      }
      ...
      if (dirName === 'GeneratedEntities' || dirName === 'GeneratedActions' || dirName === 'MJAPI') {
        ...
        ignorePatterns.push('tsconfig.json'); // Ignore the original tsconfig.json (we flatten it to remove extends)
        ignorePatterns.push('package.json'); // Ignore the original package.json (we handle it separately to remove tsc-alias)
      }
