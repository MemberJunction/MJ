...
ALTER TABLE [${flyway:defaultSchema}].[Task]
    ADD [InputPayload]    NVARCHAR(MAX)     NULL,
...
EXEC sp_addextendedproperty
    ...
    @value = N'Structured input for this task, stored as JSON. Replaces the legacy __TASK_METADATA__ marker that was embedded in Description.',
    @level0type = N'SCHEMA', @level0name = '${flyway:defaultSchema}',
    @level1type = N'TABLE',  @level1name = 'Task',
    @level2type = N'COLUMN', @level2name = 'InputPayload';
GO

EXEC sp_addextendedproperty
    @name = N'MS_Description',
    @value = N'Structured output produced by this task, stored as JSON. Downstream tasks read their dependencies'' outputs from here. Replaces the legacy __TASK_OUTPUT__ marker that was embedded in Description.',
    @level0type = N'SCHEMA', @level0name = '${flyway:defaultSchema}',
    @level1type = N'TABLE',  @level1name = 'Task',
    @level2type = N'COLUMN', @level2name = 'OutputPayload';
GO

EXEC sp_addextendedproperty
    @name = N'MS_Description',
    @value = N'Failure detail when Status is Failed. Null for tasks that have not failed.',
    @level0type = N'SCHEMA', @level0name = '${flyway:defaultSchema}',
    @level1type = N'TABLE',  @level1name = 'Task',
...
EXEC sp_addextendedproperty
    @name = N'MS_Description',
    @value = N'The specific AI Agent Run that executed this task. Distinct from the conversation-level link: sibling tasks in one graph each get their own run, so this is what run-history and Gantt views should follow.',
    @level0type = N'SCHEMA', @level0name = '${flyway:defaultSchema}',
    @level1type = N'TABLE',  @level1name = 'Task',
...
EXEC sp_addextendedproperty
    @name = N'MS_Description',
    @value = N'Identifier of the dispatcher instance currently executing this task. Part of the compare-and-swap claim protocol; set only by the dispatcher, never by user-facing writes. Null when unclaimed. Human-assigned tasks never carry a claim.',
    @level0type = N'SCHEMA', @level0name = '${flyway:defaultSchema}',
    @level1type = N'TABLE',  @level1name = 'Task',
    @level2type = N'COLUMN', @level2name = 'ClaimedBy';
GO

EXEC sp_addextendedproperty
    @name = N'MS_Description',
    @value = N'When the current dispatcher claim lapses. Long-running tasks extend it by heartbeat; reconciliation treats an expired claim as an orphaned task and returns it to Pending.',
    @level0type = N'SCHEMA', @level0name = '${flyway:defaultSchema}',
    @level1type = N'TABLE',  @level1name = 'Task',
    @level2type = N'COLUMN', @level2name = 'ClaimExpiresAt';
GO
...
CREATE NONCLUSTERED INDEX [IX_Task_Status_ClaimExpiresAt]
    ON [${flyway:defaultSchema}].[Task] ([Status], [ClaimExpiresAt]);
GO

CREATE NONCLUSTERED INDEX [IX_Task_ParentID_Status]
    ON [${flyway:defaultSchema}].[Task] ([ParentID], [Status]);
GO
...
ALTER TABLE [${flyway:defaultSchema}].[AIAgentRunStep]
    DROP CONSTRAINT [CK_AIAgentRunStep_StepType];
GO
...
;WITH marked AS (
    SELECT
        [ID],
        [Description],
        CHARINDEX('__TASK_METADATA__' + CHAR(10), [Description]) AS MetaPos,
        CHARINDEX('__TASK_OUTPUT__'   + CHAR(10), [Description]) AS OutPos
    ...
    SELECT
        ...
        CASE WHEN MetaPos > 0 THEN
            LTRIM(RTRIM(SUBSTRING(
                [Description],
                MetaPos + LEN('__TASK_METADATA__') + 1,
                CASE WHEN OutPos > MetaPos
                     THEN OutPos - (MetaPos + LEN('__TASK_METADATA__') + 1)
...
trimmed AS (
    SELECT
        ...
        -- leaves the CR/LF pair that preceded the marker. LEN() ignores trailing SPACES,
        ...
        LTRIM(LEFT(
            RawDescription,
            LEN(REPLACE(REPLACE(RawDescription, CHAR(13), ' '), CHAR(10), ' '))
        )) AS CleanDescription
    FROM sliced
)
UPDATE t
SET
    [InputPayload] = CASE
        WHEN s.MetaJson IS NOT NULL AND ISJSON(s.MetaJson) = 1
        THEN JSON_QUERY(s.MetaJson, '$.inputPayload')
        ELSE t.[InputPayload]
    END,
    [OutputPayload] = CASE
        WHEN s.OutJson IS NOT NULL AND ISJSON(s.OutJson) = 1
        THEN JSON_QUERY(s.OutJson, '$.output')
    ...
    [Description] = CASE
        ...
        WHEN (s.MetaPos > 0 OR s.OutPos > 0)
         AND (s.MetaJson IS NULL OR ISJSON(s.MetaJson) = 1)
         AND (s.OutJson  IS NULL OR ISJSON(s.OutJson)  = 1)
        THEN NULLIF(s.CleanDescription, '')
        ELSE t.[Description]
    END
FROM [${flyway:defaultSchema}].[Task] t
INNER JOIN trimmed s ON s.[ID] = t.[ID];
GO
...
/* SQL text to insert 6 new entity field(s) */
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '90a53434-f817-472c-ab60-28db645385e2' OR (EntityID = '64AD3C8D-0570-48AF-AF4C-D0A2B173FDE1' AND Name = 'InputPayload')) BEGIN
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '0dbe401e-846b-4ba5-90f5-880f6002be3a' OR (EntityID = '64AD3C8D-0570-48AF-AF4C-D0A2B173FDE1' AND Name = 'OutputPayload')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '459fabc9-0114-4c9a-bded-ea8bfe3a01a1' OR (EntityID = '64AD3C8D-0570-48AF-AF4C-D0A2B173FDE1' AND Name = 'ErrorMessage')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '4bbbbcc7-939d-4263-8e9e-739734722f01' OR (EntityID = '64AD3C8D-0570-48AF-AF4C-D0A2B173FDE1' AND Name = 'AgentRunID')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '7248fce9-f49e-4e2a-92d8-a9415fc3e032' OR (EntityID = '64AD3C8D-0570-48AF-AF4C-D0A2B173FDE1' AND Name = 'ClaimedBy')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = 'd1672ab4-9c50-4948-8098-3c19847559b9' OR (EntityID = '64AD3C8D-0570-48AF-AF4C-D0A2B173FDE1' AND Name = 'ClaimExpiresAt')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
...
/* SQL text to insert entity field value with ID da03086d-1b74-4356-81ac-edaa33e2c3f3 */
INSERT INTO [${flyway:defaultSchema}].[EntityFieldValue]
                                       ([ID], [EntityFieldID], [Sequence], [Value], [Code], [__mj_CreatedAt], [__mj_UpdatedAt])
                                    VALUES
                                       ('da03086d-1b74-4356-81ac-edaa33e2c3f3', 'B04A327B-55BF-4914-9DCF-3552A5DD0293', 10, 'TaskGraph', 'TaskGraph', GETUTCDATE(), GETUTCDATE());

/* SQL text to update entity field value sequence */
UPDATE [${flyway:defaultSchema}].[EntityFieldValue] SET Sequence=11 WHERE ID='BA69DCBC-B743-4DED-930D-6F0DB4E8C01E';

/* SQL text to update entity field value sequence */
UPDATE [${flyway:defaultSchema}].[EntityFieldValue] SET Sequence=12 WHERE ID='61F9CF39-ECB3-4476-9AFC-7F037F5EB34E';

/* SQL text to update entity field value sequence */
UPDATE [${flyway:defaultSchema}].[EntityFieldValue] SET Sequence=13 WHERE ID='5E90D638-0329-4FF7-BA05-B38037474BF5';


/* Create Entity Relationship: MJ: AI Agent Runs -> MJ: Tasks (One To Many via AgentRunID) */
   IF NOT EXISTS (
      SELECT 1 FROM [${flyway:defaultSchema}].[EntityRelationship] WHERE [ID] = '1300ce73-c6a6-4050-a62e-cbfdf8ca46bb'
   )
   BEGIN
      INSERT INTO [${flyway:defaultSchema}].[EntityRelationship] ([ID], [EntityID], [RelatedEntityID], [RelatedEntityJoinField], [Type], [BundleInAPI], [DisplayInForm], [Sequence], [__mj_CreatedAt], [__mj_UpdatedAt])
                    VALUES ('1300ce73-c6a6-4050-a62e-cbfdf8ca46bb', '5190AF93-4C39-4429-BDAA-0AEB492A0256', '64AD3C8D-0570-48AF-AF4C-D0A2B173FDE1', 'AgentRunID', 'One To Many', 1, 1, 14, GETUTCDATE(), GETUTCDATE())
   END;
...
EXEC [${flyway:defaultSchema}].[spUpdateEntityFieldRelatedEntityNameFieldMap] @EntityFieldID='4BBBBCC7-939D-4263-8E9E-739734722F01', @RelatedEntityNameFieldMap='AgentRun';
...
/* SQL text to insert 1 new entity field(s) */
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '24928914-3408-43f5-b68a-83fbe325d603' OR (EntityID = '64AD3C8D-0570-48AF-AF4C-D0A2B173FDE1' AND Name = 'AgentRun')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
...
/* Update FieldCategoryInfo setting for entity */
               ...
               SET [Value] = '{"Task Execution Data":{"icon":"fa fa-terminal","description":"Technical data related to task processing, agent runs, and dispatcher claims"}}', [__mj_UpdatedAt] = GETUTCDATE()
               WHERE [EntityID] = '64AD3C8D-0570-48AF-AF4C-D0A2B173FDE1' AND [Name] = 'FieldCategoryInfo';

/* Update FieldCategoryIcons setting (legacy) */

               UPDATE [${flyway:defaultSchema}].[EntitySetting]
               SET [Value] = '{"Task Execution Data":"fa fa-terminal"}', [__mj_UpdatedAt] = GETUTCDATE()
