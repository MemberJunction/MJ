...
-- =====================================================
-- 5. SEED DEFAULT KEY SOURCES
-- Pre-populate with standard key sources
-- Using hardcoded UUIDs for cross-environment consistency
-- =====================================================
...
BEGIN
    ...
    VALUES
        ('38A961D2-022B-49C2-919F-1825A0E9C6F9',
         'Environment Variable',
         'Retrieves encryption keys from environment variables. Keys should be base64-encoded. For AES-256, generate with: openssl rand -base64 32',
         'EnvVarKeySource',
         '@memberjunction/encryption',
         '{"envVarName": "string - The name of the environment variable containing the base64-encoded key"}');
END;

-- Configuration File Key Source
IF NOT EXISTS (SELECT 1 FROM ${flyway:defaultSchema}.EncryptionKeySource WHERE Name = 'Configuration File')
BEGIN
    INSERT INTO ${flyway:defaultSchema}.EncryptionKeySource
        (ID, Name, Description, DriverClass, DriverImportPath, ConfigTemplate)
    VALUES
        ('CBF9632D-EF05-42E2-82F6-5BAC79FAA565',
         'Configuration File',
         'Retrieves encryption keys from the mj.config.cjs encryptionKeys section. Keys should be base64-encoded. Not recommended for production - use environment variables or a secrets manager instead.',
         'ConfigFileKeySource',
         '@memberjunction/encryption',
         '{"keyName": "string - The key name in the encryptionKeys section of mj.config.cjs"}');
END;

-- AWS KMS Key Source
IF NOT EXISTS (SELECT 1 FROM ${flyway:defaultSchema}.EncryptionKeySource WHERE Name = 'AWS KMS')
BEGIN
    INSERT INTO ${flyway:defaultSchema}.EncryptionKeySource
        (ID, Name, Description, DriverClass, DriverImportPath, ConfigTemplate)
    VALUES
        ('D8E4F521-3A7B-4C9E-8F12-6B5A4C3D2E1F',
         'AWS KMS',
         'Retrieves encryption keys from AWS Key Management Service (KMS). Uses envelope encryption - store the base64-encoded CiphertextBlob from GenerateDataKey as the lookup value. Requires @aws-sdk/client-kms package and AWS credentials.',
         'AWSKMSKeySource',
         '@memberjunction/encryption',
         '{"lookupValue": "string - Base64-encoded encrypted data key (CiphertextBlob from KMS GenerateDataKey)"}');
END;

-- Azure Key Vault Key Source
IF NOT EXISTS (SELECT 1 FROM ${flyway:defaultSchema}.EncryptionKeySource WHERE Name = 'Azure Key Vault')
BEGIN
    INSERT INTO ${flyway:defaultSchema}.EncryptionKeySource
        (ID, Name, Description, DriverClass, DriverImportPath, ConfigTemplate)
    VALUES
        ('A2B3C4D5-E6F7-8901-2345-6789ABCDEF01',
         'Azure Key Vault',
         'Retrieves encryption keys from Azure Key Vault secrets. Store the base64-encoded key as a secret value. Requires @azure/keyvault-secrets and @azure/identity packages and Azure credentials.',
         'AzureKeyVaultKeySource',
         '@memberjunction/encryption',
         '{"lookupValue": "string - Full secret URL (https://vault.vault.azure.net/secrets/name) or secret name if AZURE_KEYVAULT_URL is set"}');
END;

-- =====================================================
-- 6. SEED DEFAULT ALGORITHMS
-- Pre-populate with standard encryption algorithms
-- =====================================================
...
BEGIN
    ...
    VALUES
        ('B2E88E95-D09B-4DA6-B0AE-511B21B70952',
         'AES-256-GCM',
         'AES 256-bit encryption with Galois/Counter Mode. Provides authenticated encryption (AEAD) which detects tampering. RECOMMENDED for most use cases. Requires 32-byte key (openssl rand -base64 32).',
         'aes-256-gcm',
         256,
         12,
         1);
...
BEGIN
    INSERT INTO ${flyway:defaultSchema}.EncryptionAlgorithm
        (ID, Name, Description, NodeCryptoName, KeyLengthBits, IVLengthBytes, IsAEAD)
    VALUES
        ('00D7B08E-C0B8-4929-96CF-0CFF38B6B71C',
         'AES-256-CBC',
         'AES 256-bit encryption with Cipher Block Chaining. Does NOT provide authentication - use AES-256-GCM instead for new implementations. Requires 32-byte key.',
         'aes-256-cbc',
         256,
         16,
         0);
...
-- AES-128-GCM (Smaller key option)
IF NOT EXISTS (SELECT 1 FROM ${flyway:defaultSchema}.EncryptionAlgorithm WHERE Name = 'AES-128-GCM')
BEGIN
    INSERT INTO ${flyway:defaultSchema}.EncryptionAlgorithm
        (ID, Name, Description, NodeCryptoName, KeyLengthBits, IVLengthBytes, IsAEAD)
    VALUES
        ('4139AAB7-98E1-44BB-9841-C33F0B865F71',
         'AES-128-GCM',
         'AES 128-bit encryption with Galois/Counter Mode. Provides authenticated encryption (AEAD). Requires 16-byte key (openssl rand -base64 16). Use AES-256-GCM for higher security.',
         'aes-128-gcm',
         128,
         12,
         1);
...
-- =====================================================
-- 7. EXTENDED PROPERTIES (Table Descriptions)
-- =====================================================
...
-- =====================================================
-- 8. EXTENDED PROPERTIES (Column Descriptions)
-- =====================================================
...
EXEC sp_addextendedproperty @name = N'MS_Description', @value = N'When AllowDecryptInAPI is false: if true, send encrypted ciphertext; if false (default), send NULL. Most secure option is false.',
...
/* SQL text to insert new entity field */
      ...
      BEGIN
         ...
         (
            ...
            'When AllowDecryptInAPI is false: if true, send encrypted ciphertext; if false (default), send NULL. Most secure option is false.',
            'bit',
            1,
            1,
            0,
