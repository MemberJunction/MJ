-- =====================================================================================================
-- Record Process: add a general-purpose Configuration column. For WorkType='FieldRules' this holds the
-- serialized FieldRuleSet (the rules applied to each record); it is reserved for future work types that
-- need structured, work-specific configuration beyond the existing Input/Output mappings.
-- =====================================================================================================

ALTER TABLE ${flyway:defaultSchema}.RecordProcess ADD
    Configuration NVARCHAR(MAX) NULL;

EXEC sp_addextendedproperty
    @name = N'MS_Description',
    @value = N'JSON configuration for the process''s work, used by work types that need structured config beyond Input/Output mappings. For WorkType=''FieldRules'' this holds the serialized FieldRuleSet (the rules applied to each record). NULL for work types that do not use it.',
    @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}',
    @level1type = N'TABLE',  @level1name = N'RecordProcess',
...
--
--    Produced by `mj codegen` after the Configuration column above was applied: EntityField metadata +
--    the RecordProcess view + CRUD stored procedures regenerated to include it. Do NOT hand-edit.
--
-- ═════════════════════════════════════════════════════════════════════════════════════════════════════
-- ═════════════════════════════════════════════════════════════════════════════════════════════════════
...
/* SQL text to insert new entity field */
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '673e4c24-bdb7-4634-98ea-33303bee13f8' OR (EntityID = 'BDE34DF9-7B59-4921-9B80-E94BC013A5BB' AND Name = 'Configuration')) BEGIN
         ...
         VALUES
         (
            '673e4c24-bdb7-4634-98ea-33303bee13f8',
            'BDE34DF9-7B59-4921-9B80-E94BC013A5BB', -- Entity: MJ: Record Processes
            100067,
            'Configuration',
            'Configuration',
            'JSON configuration for the process''s work, used by work types that need structured config beyond Input/Output mappings. For WorkType=''FieldRules'' this holds the serialized FieldRuleSet (the rules applied to each record). NULL for work types that do not use it.',
            'nvarchar',
            -1,
            0,
            0,
...
/* Set field properties for entity */
               ...
               SET DefaultInView = 1
               WHERE ID = '39A27B5B-1D40-49C5-9A3B-0E6FB81EE0AA'
               AND AutoUpdateDefaultInView = 1;

               UPDATE [${flyway:defaultSchema}].[EntityField]
               SET DefaultInView = 1
               WHERE ID = '648AFE09-C3E4-44CF-9402-6498835B16CE'
               AND AutoUpdateDefaultInView = 1;

               UPDATE [${flyway:defaultSchema}].[EntityField]
               SET IncludeInUserSearchAPI = 1
               WHERE ID = '540F4489-76CF-4941-A776-1D2C1EA862A2'
...
SET 
   ...
   CodeType = 'Other'
WHERE 
   ID = '673E4C24-BDB7-4634-98EA-33303BEE13F8' AND AutoUpdateCategory = 1;
