-- =============================================================================
-- Skill Activation Governance & Observability (5.45)
-- =============================================================================
-- Live testing of the core skill library (5.44) surfaced two governance gaps:
--
--   1. Agents with AcceptsSkills='All' see their allowed skills as a prompt
--      catalog and may SELF-ACTIVATE any of them mid-run. Correct behavior,
--      but there was no dial to prevent autonomous expansion into
--      consequential skills (e.g. outbound communications) — a security and
--      governance concern ("skill leakage into an agent").
--   2. Observability: nothing recorded which skills were in effect on a
--      prompt turn, which skill granted an action/sub-agent the agent used,
--      why the agent chose to activate a skill, or whether a run executed
--      under plan mode.
--
-- This migration adds:
--   * A DOUBLE activation gate — self-activation requires 'Auto' on BOTH the
--     skill (AISkill.ActivationMode) AND the agent
--     (AIAgent.SkillActivationMode). Both default to 'RequestedOnly', so the
--     "super agent" posture (auto x auto) is always a deliberate double opt-in.
--     The explicit-user-request path (/skill mention →
--     ExecuteAgentParams.requestedSkillIDs) works under either mode. All
--     existing availability gates (AcceptsSkills, AISkill.Status,
--     AIAgentSkill assignment, user Run permission) apply unchanged on both
--     paths.
--   * AIAgent.RequirePlanMode — forces plan mode on every root run of the
--     agent regardless of the per-request toggle (SupportsPlanMode is
--     irrelevant when set).
--   * AIAgentRun.PlanMode — records that a run executed under plan mode.
--   * AIAgentRunStep.Skills — JSON array of skill-invocation records
--     (skill, activation type, provenance of authority, agent-stated reason)
--     populated on Skill/Prompt/Actions/Sub-Agent steps.
-- =============================================================================

-- AISkill: per-skill self-activation dial
ALTER TABLE ${flyway:defaultSchema}.AISkill ADD
    ActivationMode NVARCHAR(20) NOT NULL
        CONSTRAINT DF_AISkill_ActivationMode DEFAULT ('RequestedOnly')
        CONSTRAINT CK_AISkill_ActivationMode
            CHECK (ActivationMode IN ('Auto', 'RequestedOnly'));

EXEC sp_addextendedproperty
    @name = N'MS_Description',
    @value = N'Controls whether this skill may ever be self-activated by an agent. Auto: the skill may appear in accepting agents'' prompt catalogs and be activated mid-run on agent judgment — but only for agents whose own SkillActivationMode is also Auto (double gate). RequestedOnly (default): the skill is excluded from prompt catalogs entirely and can only be activated when the user explicitly requests it for the run (a /skill mention flowing through ExecuteAgentParams.requestedSkillIDs). All other activation gates (AcceptsSkills, skill Status, per-agent assignment, user Run permission) apply unchanged in both modes.',
...
-- It contains the framework plumbing for the new columns: EntityField metadata
-- inserts, regenerated base views, stored procedures (spCreate/spUpdate/
-- spDelete), permission grants, and related sp_addextendedproperty calls.
--
...
/* SQL text to insert new entity field */
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '861fe881-c9fa-497f-a092-c7b8c7c4f81f' OR (EntityID = '5190AF93-4C39-4429-BDAA-0AEB492A0256' AND Name = 'PlanMode')) BEGIN
         ...
         VALUES
         (
            '861fe881-c9fa-497f-a092-c7b8c7c4f81f',
            '5190AF93-4C39-4429-BDAA-0AEB492A0256', -- Entity: MJ: AI Agent Runs
            100116,
            'PlanMode',
            'Plan Mode',
            '1 when this run executed under plan mode (whether via the per-request planMode flag or the agent''s RequirePlanMode setting). Drives plan-mode indicators in the run UX and supports plan-drift auditing (comparing the approved plan against the steps that actually executed).',
            'bit',
            1,
            1,
            0,
...
/* SQL text to insert new entity field */
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '622048e5-0843-4e9d-a73c-c4bf70e5c1e0' OR (EntityID = 'CDB135CC-6D3C-480B-90AE-25B7805F82C1' AND Name = 'SkillActivationMode')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
         ...
         (
            ...
            'Controls whether this agent may ever self-activate skills from its prompt catalog. Auto: the agent sees its allowed skills whose own ActivationMode is Auto (double gate) and may activate them mid-run on its own judgment. RequestedOnly (default): the agent''s prompt catalog is empty and skills only enter a run via an explicit user request (/skill mention). Orthogonal to AcceptsSkills, which governs which skills are available at all; this governs who may pull the activation trigger.',
            'nvarchar',
            40,
            0,
            0,
...
/* SQL text to insert new entity field */
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '17fb1fc5-f4d5-4fb0-97e7-06da79c03a1d' OR (EntityID = 'CDB135CC-6D3C-480B-90AE-25B7805F82C1' AND Name = 'RequirePlanMode')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
         ...
         VALUES
         (
            '17fb1fc5-f4d5-4fb0-97e7-06da79c03a1d',
            'CDB135CC-6D3C-480B-90AE-25B7805F82C1', -- Entity: MJ: AI Agents
            100165,
            'RequirePlanMode',
            'Require Plan Mode',
            'When 1, every root-level run of this agent executes in plan mode regardless of the per-request planMode flag — the agent must present a plan and receive human approval before any Actions or Sub-Agent steps execute. SupportsPlanMode is irrelevant when this is set. Use for high-consequence agents (e.g. ones with outbound-communication capabilities) where human-in-the-loop review is mandatory.',
            'bit',
            1,
            1,
            0,
...
/* SQL text to insert new entity field */
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '8ac2445e-06c4-4e87-90e1-960a4df6af81' OR (EntityID = '1D52DE84-DD3F-4E46-8D2B-574B70080BB4' AND Name = 'ActivationMode')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
         ...
         (
            ...
            'ActivationMode',
            'Activation Mode',
            'Controls whether this skill may ever be self-activated by an agent. Auto: the skill may appear in accepting agents'' prompt catalogs and be activated mid-run on agent judgment — but only for agents whose own SkillActivationMode is also Auto (double gate). RequestedOnly (default): the skill is excluded from prompt catalogs entirely and can only be activated when the user explicitly requests it for the run (a /skill mention flowing through ExecuteAgentParams.requestedSkillIDs). All other activation gates (AcceptsSkills, skill Status, per-agent assignment, user Run permission) apply unchanged in both modes.',
            'nvarchar',
            40,
            0,
            0,
...
/* SQL text to insert new entity field */
      ...
      IF NOT EXISTS (SELECT 1 FROM [${flyway:defaultSchema}].[EntityField] WHERE ID = '22b0dd0e-5843-41cf-aff8-c23d3fb22c19' OR (EntityID = '99273DAD-560E-4ABC-8332-C97AB58B7463' AND Name = 'Skills')) BEGIN
         INSERT INTO [${flyway:defaultSchema}].[EntityField]
         (
            [ID],
            [EntityID],
         ...
         VALUES
         (
            '22b0dd0e-5843-41cf-aff8-c23d3fb22c19',
            '99273DAD-560E-4ABC-8332-C97AB58B7463', -- Entity: MJ: AI Agent Run Steps
            100049,
            'Skills',
            'Skills',
            'JSON array of skill-invocation records (AgentSkillInvocation[]) associating this step with the skills involved in it, or NULL when no skills are in play. Each record carries SkillID, SkillName, ActivationType (requested = user /skill mention; auto = agent self-activation), Provenance of authority (the gate values that admitted the skill: AcceptsSkills, both ActivationMode dials, and who requested it), and an optional agent-stated Reason when self-activated. Population: Skill steps record the activation(s) they performed; Prompt steps record the full set of skills in effect for that turn; Actions and Sub-Agent steps record the skill(s) through which the executed tool became available (NULL means the tool was a native grant).',
            'nvarchar',
            -1,
            0,
            0,
...
INSERT INTO [${flyway:defaultSchema}].[EntityFieldValue]
                                    ...
                                    VALUES
                                       ('322b9434-c2fe-42c8-a0c5-e2e57e08892c', '8AC2445E-06C4-4E87-90E1-960A4DF6AF81', 1, 'Auto', 'Auto', GETUTCDATE(), GETUTCDATE());

/* SQL text to insert entity field value with ID 6231c5e5-72dc-4ab8-9161-cfcb273be45a */
INSERT INTO [${flyway:defaultSchema}].[EntityFieldValue]
                                       ([ID], [EntityFieldID], [Sequence], [Value], [Code], [__mj_CreatedAt], [__mj_UpdatedAt])
                                    VALUES
                                       ('6231c5e5-72dc-4ab8-9161-cfcb273be45a', '8AC2445E-06C4-4E87-90E1-960A4DF6AF81', 2, 'RequestedOnly', 'RequestedOnly', GETUTCDATE(), GETUTCDATE());

/* SQL text to update ValueListType for entity field ID 8AC2445E-06C4-4E87-90E1-960A4DF6AF81 */
UPDATE [${flyway:defaultSchema}].[EntityField] SET ValueListType='List' WHERE ID='8AC2445E-06C4-4E87-90E1-960A4DF6AF81';

/* SQL text to insert entity field value with ID f238cef4-2fa7-49ba-90e1-c4baec517a0a */
INSERT INTO [${flyway:defaultSchema}].[EntityFieldValue]
                                       ([ID], [EntityFieldID], [Sequence], [Value], [Code], [__mj_CreatedAt], [__mj_UpdatedAt])
                                    VALUES
                                       ('f238cef4-2fa7-49ba-90e1-c4baec517a0a', '622048E5-0843-4E9D-A73C-C4BF70E5C1E0', 1, 'Auto', 'Auto', GETUTCDATE(), GETUTCDATE());

/* SQL text to insert entity field value with ID 633d7e9e-0319-4f09-8b10-358bcfa5eb23 */
INSERT INTO [${flyway:defaultSchema}].[EntityFieldValue]
                                       ([ID], [EntityFieldID], [Sequence], [Value], [Code], [__mj_CreatedAt], [__mj_UpdatedAt])
                                    VALUES
                                       ('633d7e9e-0319-4f09-8b10-358bcfa5eb23', '622048E5-0843-4E9D-A73C-C4BF70E5C1E0', 2, 'RequestedOnly', 'RequestedOnly', GETUTCDATE(), GETUTCDATE());

/* SQL text to update ValueListType for entity field ID 622048E5-0843-4E9D-A73C-C4BF70E5C1E0 */
UPDATE [${flyway:defaultSchema}].[EntityField] SET ValueListType='List' WHERE ID='622048E5-0843-4E9D-A73C-C4BF70E5C1E0';
...
/* Set field properties for entity */
               ...
               SET DefaultInView = 1
               WHERE ID = '8AC2445E-06C4-4E87-90E1-960A4DF6AF81'
...
SET 
   GeneratedFormSection = 'Category',
   DisplayName = 'Parent Step Context',
   ExtendedType = NULL,
   CodeType = NULL
WHERE 
...
SET 
   ...
   GeneratedFormSection = 'Category',
   DisplayName = 'Skills Used',
   ExtendedType = NULL,
   CodeType = NULL
WHERE 
   ID = '22B0DD0E-5843-41CF-AFF8-C23D3FB22C19' AND AutoUpdateCategory = 1;
...
SET 
   GeneratedFormSection = 'Category',
   DisplayName = 'Self-Read Payload Paths',
   ExtendedType = 'Code',
   CodeType = 'Other'
WHERE 
...
SET 
   GeneratedFormSection = 'Category',
   DisplayName = 'Self-Write Payload Paths',
   ExtendedType = 'Code',
   CodeType = 'Other'
WHERE 
...
SET 
   ...
   CodeType = NULL
WHERE 
   ID = '622048E5-0843-4E9D-A73C-C4BF70E5C1E0' AND AutoUpdateCategory = 1;
...
SET 
   ...
   CodeType = NULL
WHERE 
   ID = '17FB1FC5-F4D5-4FB0-97E7-06DA79C03A1D' AND AutoUpdateCategory = 1;
