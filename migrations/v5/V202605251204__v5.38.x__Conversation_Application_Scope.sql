/*
    ...
    Adds three columns to the Conversation table:
      ...
      2. ApplicationID (UNIQUEIDENTIFIER, NULL, FK -> Application.ID)
      ...
      3. DefaultAgentID (UNIQUEIDENTIFIER, NULL, FK -> AIAgent.ID)
...
EXEC sp_addextendedproperty
    ...
    @value     = N'Controls where this conversation surfaces in the UI. Global = appears in the main Chat app (no application binding). Application = scoped to a specific Application''s embedded chat surface (e.g. the Form Builder cockpit); hidden from the main chat list by default. Both = explicitly promoted to appear in BOTH the main chat list and the bound Application''s embedded surface. Defaults to Global so pre-existing conversations stay visible in main chat. Paired with ApplicationID via a cross-column CHECK constraint: Global => ApplicationID IS NULL; Application or Both => ApplicationID IS NOT NULL.',
    @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}',
    @level1type = N'TABLE',  @level1name = N'Conversation',
    @level2type = N'COLUMN', @level2name = N'ApplicationScope';
GO

EXEC sp_addextendedproperty
    @name      = N'MS_Description',
    @value     = N'Optional Application this conversation is bound to. Required when ApplicationScope is ''Application'' or ''Both''; must be NULL when ApplicationScope is ''Global''. Enforced by the CK_Conversation_ScopeAppBinding cross-column CHECK. Used by embedded chat surfaces (e.g. the Form Builder cockpit) to filter their conversation list to just their own application''s conversations.',
    @level0type = N'SCHEMA', @level0name = N'${flyway:defaultSchema}',
    @level1type = N'TABLE',  @level1name = N'Conversation',
