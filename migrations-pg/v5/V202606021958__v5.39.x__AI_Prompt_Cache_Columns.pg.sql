-- ============================================================================
-- MemberJunction PostgreSQL Migration — V202606021958__v5.39.x__AI_Prompt_Cache_Columns.sql
...
ALTER TABLE __mj."AIPromptRun"
  ADD COLUMN "TokensCacheReadRollup" INT NULL,
  ADD COLUMN "TokensCacheWriteRollup" INT NULL
 /* New prompt-cache columns for this release (the base AIPromptRun.TokensCacheRead/TokensCacheWrite */ /* counts already shipped in the earlier V202606021200 migration; do NOT re-add them here): */ /*   * AIPromptRun  — hierarchical ROLLUP variants of the cache token counts */ /*   * AIModelCost  — optional cache read/write per-unit pricing (priced like InputPricePerUnit) */ /*   * AIAgentRun   — agent-level cache token rollups (cache counterparts of TotalPromptTokensUsed) */ /* The *Rollup variants mirror TokensUsedRollup/TokensPromptRollup: leaf = own value; consolidated */ /* parent (parallel / multi-attempt / failover) = sum of attempts, so fan-out provider calls are not */ /* under-counted when aggregating up a prompt-run or agent-run hierarchy. */ /* AIModelCost cache rates are NULLABLE: NULL means "no distinct cache rate," and the cost calculator */ /* falls back to InputPricePerUnit for that bucket (so behavior is unchanged until rates are populated). */ /* Hand-written DDL + extended properties at top; CodeGen-generated metadata appended below. */;
ALTER TABLE __mj."AIModelCost"
  ADD COLUMN "CacheReadPricePerUnit" DECIMAL(18, 8) NULL,
  ADD COLUMN "CacheWritePricePerUnit" DECIMAL(18, 8) NULL;
ALTER TABLE __mj."AIAgentRun"
  ADD COLUMN "TotalCacheReadTokensUsed" INT NULL,
  ADD COLUMN "TotalCacheWriteTokensUsed" INT NULL;
...
COMMENT ON COLUMN __mj."AIModelCost"."CacheReadPricePerUnit" IS 'Optional price per unit for input tokens served from the AI provider''s prompt cache (cache reads / hits), expressed in the same currency and UnitType (e.g. per 1M tokens) as InputPricePerUnit. When NULL, cache-read tokens are priced at InputPricePerUnit. Cache reads are usually far cheaper than uncached input (e.g. ~0.1x for Anthropic/Gemini, ~0.5x for OpenAI).';

COMMENT ON COLUMN __mj."AIModelCost"."CacheWritePricePerUnit" IS 'Optional price per unit for input tokens written to the AI provider''s prompt cache (cache writes / creation), expressed in the same currency and UnitType as InputPricePerUnit. When NULL, cache-write tokens are priced at InputPricePerUnit. Populated for providers that bill cache creation separately (e.g. Anthropic, ~1.25x input); leave NULL for providers that do not (OpenAI, Gemini), which also report 0 cache-write tokens.';

COMMENT ON COLUMN __mj."AIAgentRun"."TotalCacheReadTokensUsed" IS 'Total input tokens served from the AI provider''s prompt cache (cache reads / hits) across this agent run, summed from child prompt runs'' TokensCacheReadRollup and sub-agent runs'' TotalCacheReadTokensUsed. Counts only; the cost impact (cache reads are billed at a steep discount) is reflected in TotalCost. The cache counterpart of TotalPromptTokensUsed.';

COMMENT ON COLUMN __mj."AIAgentRun"."TotalCacheWriteTokensUsed" IS 'Total input tokens written to the AI provider''s prompt cache (cache writes / creation) across this agent run, summed from child prompt runs'' TokensCacheWriteRollup and sub-agent runs'' TotalCacheWriteTokensUsed. Populated for providers that bill cache creation (e.g. Anthropic); 0 or NULL otherwise. The cache counterpart of TotalCompletionTokensUsed.';

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM __mj."EntityField" WHERE "ID" = 'caee3e16-509e-4f64-a4fd-6b5428d325be' OR ("EntityID" = '5190AF93-4C39-4429-BDAA-0AEB492A0256' AND "Name" = 'TotalCacheReadTokensUsed')) THEN
    INSERT INTO __mj."EntityField" ("ID", "EntityID", "Sequence", "Name", "DisplayName", "Description", "Type", "Length", "Precision", "Scale", "AllowsNull", "DefaultValue", "AutoIncrement", "AllowUpdateAPI", "IsVirtual", "IsComputed", "RelatedEntityID", "RelatedEntityFieldName", "IsNameField", "IncludeInUserSearchAPI", "IncludeRelatedEntityNameFieldInBaseView", "DefaultInView", "IsPrimaryKey", "IsUnique", "RelatedEntityDisplayType", "__mj_CreatedAt", "__mj_UpdatedAt") VALUES ('caee3e16-509e-4f64-a4fd-6b5428d325be', '5190AF93-4C39-4429-BDAA-0AEB492A0256' /* Entity: MJ: AI Agent Runs */, 100109, 'TotalCacheReadTokensUsed', 'Total Cache Read Tokens Used', 'Total input tokens served from the AI provider''s prompt cache (cache reads / hits) across this agent run, summed from child prompt runs'' TokensCacheReadRollup and sub-agent runs'' TotalCacheReadTokensUsed. Counts only; the cost impact (cache reads are billed at a steep discount) is reflected in TotalCost. The cache counterpart of TotalPromptTokensUsed.', 'int', 4, 10, 0, TRUE, NULL, FALSE, TRUE, FALSE, FALSE, NULL, NULL, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, 'Search', NOW(), NOW());
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM __mj."EntityField" WHERE "ID" = '3b815f5a-2c13-4fe8-9c8f-ed4a1022883b' OR ("EntityID" = '5190AF93-4C39-4429-BDAA-0AEB492A0256' AND "Name" = 'TotalCacheWriteTokensUsed')) THEN
    INSERT INTO __mj."EntityField" ("ID", "EntityID", "Sequence", "Name", "DisplayName", "Description", "Type", "Length", "Precision", "Scale", "AllowsNull", "DefaultValue", "AutoIncrement", "AllowUpdateAPI", "IsVirtual", "IsComputed", "RelatedEntityID", "RelatedEntityFieldName", "IsNameField", "IncludeInUserSearchAPI", "IncludeRelatedEntityNameFieldInBaseView", "DefaultInView", "IsPrimaryKey", "IsUnique", "RelatedEntityDisplayType", "__mj_CreatedAt", "__mj_UpdatedAt") VALUES ('3b815f5a-2c13-4fe8-9c8f-ed4a1022883b', '5190AF93-4C39-4429-BDAA-0AEB492A0256' /* Entity: MJ: AI Agent Runs */, 100110, 'TotalCacheWriteTokensUsed', 'Total Cache Write Tokens Used', 'Total input tokens written to the AI provider''s prompt cache (cache writes / creation) across this agent run, summed from child prompt runs'' TokensCacheWriteRollup and sub-agent runs'' TotalCacheWriteTokensUsed. Populated for providers that bill cache creation (e.g. Anthropic); 0 or NULL otherwise. The cache counterpart of TotalCompletionTokensUsed.', 'int', 4, 10, 0, TRUE, NULL, FALSE, TRUE, FALSE, FALSE, NULL, NULL, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, 'Search', NOW(), NOW());
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM __mj."EntityField" WHERE "ID" = '7a23bcb6-c173-46d4-a44d-881c0a18862d' OR ("EntityID" = '5A5CF9A9-EAE7-4ECB-B1B7-277BAAC73127' AND "Name" = 'CacheReadPricePerUnit')) THEN
    INSERT INTO __mj."EntityField" ("ID", "EntityID", "Sequence", "Name", "DisplayName", "Description", "Type", "Length", "Precision", "Scale", "AllowsNull", "DefaultValue", "AutoIncrement", "AllowUpdateAPI", "IsVirtual", "IsComputed", "RelatedEntityID", "RelatedEntityFieldName", "IsNameField", "IncludeInUserSearchAPI", "IncludeRelatedEntityNameFieldInBaseView", "DefaultInView", "IsPrimaryKey", "IsUnique", "RelatedEntityDisplayType", "__mj_CreatedAt", "__mj_UpdatedAt") VALUES ('7a23bcb6-c173-46d4-a44d-881c0a18862d', '5A5CF9A9-EAE7-4ECB-B1B7-277BAAC73127' /* Entity: MJ: AI Model Costs */, 100037, 'CacheReadPricePerUnit', 'Cache Read Price Per Unit', 'Optional price per unit for input tokens served from the AI provider''s prompt cache (cache reads / hits), expressed in the same currency and UnitType (e.g. per 1M tokens) as InputPricePerUnit. When NULL, cache-read tokens are priced at InputPricePerUnit. Cache reads are usually far cheaper than uncached input (e.g. ~0.1x for Anthropic/Gemini, ~0.5x for OpenAI).', 'decimal', 9, 18, 8, TRUE, NULL, FALSE, TRUE, FALSE, FALSE, NULL, NULL, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, 'Search', NOW(), NOW());
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM __mj."EntityField" WHERE "ID" = '1b169ea7-98a6-4533-91cb-e265c839ae06' OR ("EntityID" = '5A5CF9A9-EAE7-4ECB-B1B7-277BAAC73127' AND "Name" = 'CacheWritePricePerUnit')) THEN
    INSERT INTO __mj."EntityField" ("ID", "EntityID", "Sequence", "Name", "DisplayName", "Description", "Type", "Length", "Precision", "Scale", "AllowsNull", "DefaultValue", "AutoIncrement", "AllowUpdateAPI", "IsVirtual", "IsComputed", "RelatedEntityID", "RelatedEntityFieldName", "IsNameField", "IncludeInUserSearchAPI", "IncludeRelatedEntityNameFieldInBaseView", "DefaultInView", "IsPrimaryKey", "IsUnique", "RelatedEntityDisplayType", "__mj_CreatedAt", "__mj_UpdatedAt") VALUES ('1b169ea7-98a6-4533-91cb-e265c839ae06', '5A5CF9A9-EAE7-4ECB-B1B7-277BAAC73127' /* Entity: MJ: AI Model Costs */, 100038, 'CacheWritePricePerUnit', 'Cache Write Price Per Unit', 'Optional price per unit for input tokens written to the AI provider''s prompt cache (cache writes / creation), expressed in the same currency and UnitType as InputPricePerUnit. When NULL, cache-write tokens are priced at InputPricePerUnit. Populated for providers that bill cache creation separately (e.g. Anthropic, ~1.25x input); leave NULL for providers that do not (OpenAI, Gemini), which also report 0 cache-write tokens.', 'decimal', 9, 18, 8, TRUE, NULL, FALSE, TRUE, FALSE, FALSE, NULL, NULL, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, 'Search', NOW(), NOW());
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM __mj."EntityField" WHERE "ID" = '645dfa6e-9b83-4d3e-a0e8-4f4646e3e70e' OR ("EntityID" = '7C1C98D0-3978-4CE8-8E3F-C90301E59767' AND "Name" = 'TokensCacheReadRollup')) THEN
    INSERT INTO __mj."EntityField" ("ID", "EntityID", "Sequence", "Name", "DisplayName", "Description", "Type", "Length", "Precision", "Scale", "AllowsNull", "DefaultValue", "AutoIncrement", "AllowUpdateAPI", "IsVirtual", "IsComputed", "RelatedEntityID", "RelatedEntityFieldName", "IsNameField", "IncludeInUserSearchAPI", "IncludeRelatedEntityNameFieldInBaseView", "DefaultInView", "IsPrimaryKey", "IsUnique", "RelatedEntityDisplayType", "__mj_CreatedAt", "__mj_UpdatedAt") VALUES ('645dfa6e-9b83-4d3e-a0e8-4f4646e3e70e', '7C1C98D0-3978-4CE8-8E3F-C90301E59767' /* Entity: MJ: AI Prompt Runs */, 100191, 'TokensCacheReadRollup', 'Tokens Cache Read Rollup', 'Rollup of TokensCacheRead across this prompt run and all of its descendant prompt runs (e.g. the individual attempts behind a parallel / multi-attempt / failover consolidation). For a leaf run this equals TokensCacheRead. Use this (not TokensCacheRead) when aggregating cache reads up a prompt-run or agent-run hierarchy so fan-out provider calls are not under-counted.', 'int', 4, 10, 0, TRUE, NULL, FALSE, TRUE, FALSE, FALSE, NULL, NULL, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, 'Search', NOW(), NOW());
  END IF;
END $$;

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM __mj."EntityField" WHERE "ID" = '17bf5a60-74dc-41c8-81a5-3c49fec3016a' OR ("EntityID" = '7C1C98D0-3978-4CE8-8E3F-C90301E59767' AND "Name" = 'TokensCacheWriteRollup')) THEN
    INSERT INTO __mj."EntityField" ("ID", "EntityID", "Sequence", "Name", "DisplayName", "Description", "Type", "Length", "Precision", "Scale", "AllowsNull", "DefaultValue", "AutoIncrement", "AllowUpdateAPI", "IsVirtual", "IsComputed", "RelatedEntityID", "RelatedEntityFieldName", "IsNameField", "IncludeInUserSearchAPI", "IncludeRelatedEntityNameFieldInBaseView", "DefaultInView", "IsPrimaryKey", "IsUnique", "RelatedEntityDisplayType", "__mj_CreatedAt", "__mj_UpdatedAt") VALUES ('17bf5a60-74dc-41c8-81a5-3c49fec3016a', '7C1C98D0-3978-4CE8-8E3F-C90301E59767' /* Entity: MJ: AI Prompt Runs */, 100192, 'TokensCacheWriteRollup', 'Tokens Cache Write Rollup', 'Rollup of TokensCacheWrite across this prompt run and all of its descendant prompt runs. For a leaf run this equals TokensCacheWrite. Mirrors TokensUsedRollup/TokensPromptRollup; populated for providers that report cache writes (e.g. Anthropic), otherwise 0 or NULL.', 'int', 4, 10, 0, TRUE, NULL, FALSE, TRUE, FALSE, FALSE, NULL, NULL, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, 'Search', NOW(), NOW());
  END IF;
END $$;
...
WHERE
  "ID" = '0CDCEFDE-FBFE-44CD-ACAF-A1543F309EC4' AND "AutoUpdateIsNameField" = TRUE;
...
WHERE
  "ID" = '8EB9EB12-02C0-4D19-BC14-0DC706C9EE58'
  AND "AutoUpdateDefaultInView" = TRUE;
UPDATE __mj."EntityField" SET "UserSearchPredicateAPI" = 'BeginsWith'
WHERE
  "ID" = '5603B884-25A8-4D10-94A3-636E59F3E91C'
  AND "AutoUpdateUserSearchPredicate" = TRUE;
UPDATE __mj."EntityField" SET "UserSearchPredicateAPI" = 'BeginsWith'
WHERE
  "ID" = 'F1D62EEE-FEEF-4D0C-8955-7AB4442A9150'
  AND "AutoUpdateUserSearchPredicate" = TRUE;
UPDATE __mj."EntityField" SET "UserSearchPredicateAPI" = 'Exact'
WHERE
  "ID" = '206BDDB4-41C4-4CC4-8057-43BE145DFE13'
  AND "AutoUpdateUserSearchPredicate" = TRUE;

/* Set field properties for entity */
UPDATE __mj."EntityField" SET "IsNameField" = TRUE
WHERE
  "ID" = 'EFAFC399-58A9-42E6-AFF4-A31CBE7CAC16' AND "AutoUpdateIsNameField" = TRUE;
UPDATE __mj."EntityField" SET "IncludeInUserSearchAPI" = TRUE
WHERE
  "ID" = 'A6584C65-63F7-46CA-8A58-25BC5B6BFD54'
  AND "AutoUpdateIncludeInUserSearchAPI" = TRUE;
UPDATE __mj."EntityField" SET "IncludeInUserSearchAPI" = TRUE
WHERE
  "ID" = '4333EAE9-2185-403F-B742-B8FF9631C860'
  AND "AutoUpdateIncludeInUserSearchAPI" = TRUE;
UPDATE __mj."EntityField" SET "IncludeInUserSearchAPI" = TRUE
WHERE
  "ID" = 'BB204D9E-2D75-4DB7-8022-55E73A56CBCA'
  AND "AutoUpdateIncludeInUserSearchAPI" = TRUE;
UPDATE __mj."EntityField" SET "IncludeInUserSearchAPI" = TRUE
WHERE
  "ID" = 'EFAFC399-58A9-42E6-AFF4-A31CBE7CAC16'
  AND "AutoUpdateIncludeInUserSearchAPI" = TRUE;
UPDATE __mj."EntityField" SET "UserSearchPredicateAPI" = 'BeginsWith'
WHERE
  "ID" = 'EFAFC399-58A9-42E6-AFF4-A31CBE7CAC16'
...
WHERE
  "ID" = '5A5CF9A9-EAE7-4ECB-B1B7-277BAAC73127'
  AND "AutoUpdateAllowUserSearchAPI" = TRUE;

/* Set categories for 21 fields */ /* UPDATE Entity Field Category Info MJ: AI Model Costs.ID */
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Model Name', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Vendor Name', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Price Type', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Price Type Name', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Unit Type', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Unit Type Name', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "Category" = 'Pricing Details', "GeneratedFormSection" = 'Category', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
/* Set categories for 62 fields */ /* UPDATE Entity Field Category Info MJ: AI Agent Runs.ID */
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Run ID', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'External Reference', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Parent Run Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Last Run Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Scheduled Job Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Final Message', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Execution Data', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Sequence', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Agent Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Conversation Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'User Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Conversation Detail Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Total Tokens', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Prompt Tokens', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Completion Tokens', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Total Tokens (Rollup)', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Prompt Tokens (Rollup)', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Completion Tokens (Rollup)', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "Category" = 'Resource Usage & Cost', "GeneratedFormSection" = 'Category', "DisplayName" = 'Cache Read Tokens', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "Category" = 'Resource Usage & Cost', "GeneratedFormSection" = 'Category', "DisplayName" = 'Cache Write Tokens', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Configuration Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Override Model Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Override Vendor Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Test Run Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Primary Scope Entity Details', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "Category" = 'Scope & Multi-Tenant', "GeneratedFormSection" = 'Category', "DisplayName" = 'Company', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
  "ID" = '766B0728-BB2E-4827-B23A-7A4CA04FB7F6' AND "AutoUpdateCategory" = TRUE;

/* Set categories for 103 fields */ /* UPDATE Entity Field Category Info MJ: AI Prompt Runs.ID */
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Started At', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Rerun From', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Root Rerun Source', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Judge Prompt', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Provider Response Details', "ExtendedType" = 'Code', "CodeType" = 'Other'
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Tokens Used', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Currency', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Original Model ID', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Original Request Start', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "Category" = 'Performance & Cost Metrics', "GeneratedFormSection" = 'Category', "DisplayName" = 'Tokens Cache Read (Rollup)', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "Category" = 'Performance & Cost Metrics', "GeneratedFormSection" = 'Category', "DisplayName" = 'Tokens Cache Write (Rollup)', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Log Probs', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Top Log Probs', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Validation Attempts', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Successful Validations', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Validation Passed', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Max Retries', "ExtendedType" = NULL, "CodeType" = NULL
WHERE
...
UPDATE __mj."EntityField" SET "GeneratedFormSection" = 'Category', "DisplayName" = 'Validation Attempt Logs', "ExtendedType" = 'Code', "CodeType" = 'Other'
WHERE
...
-- ============================================================
CREATE INDEX IF NOT EXISTS "idx_auto_mj_fkey_ai_model_cost_model_id"
    ON __mj."AIModelCost" ("ModelID");

CREATE INDEX IF NOT EXISTS "idx_auto_mj_fkey_ai_model_cost_vendor_id"
    ON __mj."AIModelCost" ("VendorID");

CREATE INDEX IF NOT EXISTS "idx_auto_mj_fkey_ai_model_cost_price_type_id"
    ON __mj."AIModelCost" ("PriceTypeID");

CREATE INDEX IF NOT EXISTS "idx_auto_mj_fkey_ai_model_cost_unit_type_id"
    ON __mj."AIModelCost" ("UnitTypeID");
...
CREATE OR REPLACE FUNCTION __mj."spCreateAIModelCost"(
    ...
    p_inputpriceperunit decimal(18, 8) DEFAULT NULL,
    p_outputpriceperunit decimal(18, 8) DEFAULT NULL,
    p_unittypeid uuid DEFAULT NULL,
    p_processingtype text DEFAULT NULL,
    p_comments_clear boolean DEFAULT false,
    p_comments text DEFAULT NULL,
    p_cachereadpriceperunit_clear boolean DEFAULT false,
    p_cachereadpriceperunit decimal(18, 8) DEFAULT NULL,
    p_cachewritepriceperunit_clear boolean DEFAULT false,
    p_cachewritepriceperunit decimal(18, 8) DEFAULT NULL
...
BEGIN
    ...
    RETURN QUERY
    SELECT * FROM __mj."vwAIModelCosts"
    WHERE "ID" = v_new_id;
END;
$$ LANGUAGE plpgsql;
...
------------------------------------------------------------
CREATE OR REPLACE FUNCTION __mj."fn_trg_update_ai_model_cost"()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS "trg_update_ai_model_cost" ON __mj."AIModelCost";

CREATE TRIGGER "trg_update_ai_model_cost"
BEFORE UPDATE ON __mj."AIModelCost"
FOR EACH ROW
EXECUTE FUNCTION __mj."fn_trg_update_ai_model_cost"();
...
END;
$$ LANGUAGE plpgsql;
GRANT EXECUTE ON FUNCTION __mj."spDeleteAIModelCost" TO "cdp_Developer";
