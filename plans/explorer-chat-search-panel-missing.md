...
**Status:** ✅ Resolved — **Option A** implemented in this PR (see the resolution note at the bottom)
...
## Resolution (added when this analysis moved onto the fix branch)

The product decision came back as **Option A — wire the panel into Explorer**, not the
...
artifacts / collections / tasks — a different question with a different provider.

**On the asymmetry this document flagged as worth fixing independently:** it is now
resolved in the direction of parity. `ConversationWorkspaceComponent` still has zero
