...
| C8 | `EnrichSchemaConstraints` (DBAutoDoc-lite) + retire connector-validator | **DONE (deterministic core) + DEFERRED (value-overlap + AI)** | Shipped `EnrichSchemaConstraints.Enrich()` + `InferForeignKeys()` — the runtime LightweightConstraintDiscovery: deterministic, **naming-based, provable-only FK inference** (`<Sibling>Id` → object; field-name == sibling PK; singular/plural aware; ambiguous/self/PK skipped), wired into `IntegrationSchemaSync.PersistDiscoveredSchema` so inferred FKs ride the Declared/Discovered merge. AI-optional one-shot descriptions via injected `describeFn` (no-op without it → safe on AI-less BI). +9 tests; no AI dependency, no migration. **DEFERRED:** DBAutoDoc's value-overlap/cardinality PK/FK detectors need a live-data driver (`BaseAutoDocDriver`+`ColumnStatsCache`) over the synced tables — that's a heavier post-sync pass (its own focused effort), and the LLM-description wiring to a real AIEngine. Retiring `@memberjunction/connector-validator` → keep its invariants as the agent rubric (agent branch). |
