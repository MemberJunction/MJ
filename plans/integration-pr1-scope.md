...
| C3 | **Template formalization / multi-level FK-DAG traversal** | **PARTIAL → SCOPE** | Engine has Kahn FK-layering for sync order; connector-side `BaseRESTIntegrationConnector.FetchWithTemplateVars` is still **single-level**. This is the user's "support better templates." **Scope:** generalize to topological multi-level template-var resolution (trees/chains/DAGs), cycle-detection with named offending edge. No migration. Engine-only. **Candidate for a PR-1 follow-up commit.** |
...
| C4 | `MostRecent` conflict policy (real timestamp compare) | **PARTIAL → SCOPE** | Falls back to DestWins. **Scope:** compare a modified-at on snapshot vs external in the 3-way combine; pick the newer. Engine-only, no migration. |
