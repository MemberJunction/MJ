...
| D-S6 | **Tasks lose all top-level real estate.** Remove the W1 Tasks surface and the sidebar Tasks nav item. MJ-core tasking = agent operation-graph creation, now lower emphasis; user-facing tasking is the BizApps-Tasks OpenApp's job. Live-run visibility stays in-thread (Companion Rail) and in Front Door "what ran". Sub-question **RESOLVED 2026-07-22 (Matt ratified v4's P5 proposal)**: the Room's Workflows tab does NOT survive — it folds into an Overview **"Runs" section** (live run · recent completions · failures with Retry). Fold, don't reduce: the tab was empty most of the time and duplicated the Companion Rail (live) and Needs-you (failures); the Room stays four tabs calm (Overview · Conversations · Memory · Artifacts). Graduation path: if run volume grows, the section's "All runs" link can graduate back to a tab (two-way door). | Avoid depicting an agent-internal mechanism as a user-facing tasking product; simplifies the app. | Two-way (surfaces can return) | **Locked** 2026-07-16 (Amith); Workflows fold ratified 2026-07-22 (Matt) — strip P5's PROPOSAL badge in the next canvas pass |
