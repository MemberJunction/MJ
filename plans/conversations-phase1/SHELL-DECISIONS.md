...
| D-S7 | **Projects default visibility — CONTESTED, decision pending.** Amith's proposal (2026-07-16): default-OFF per user (omnibar-style Conversations-app setting), auto-enabled by two organic escalations — (a) being added to a project by a teammate, (b) an agent suggesting mid-chat that a conversation become a project (in-chat convert button). Matt's counter (2026-07-16): **visible by default** — one quiet nav item + a teaching empty state; the Settings toggle kept but as opt-OUT (plus optional org-level default). Grounds: hidden organizing models don't get adopted and the cost compounds (flat history accumulates); the sidebar clutter that motivated hiding was an Option A problem the composed shell already fixed; per-user appearing/disappearing nav creates mixed states org-wide (escalation (a) guarantees it) hurting screen-share/training/support; gating projects undercuts the P1.6+ roadmap built on them. **Both agree on**: the two organic escalation moments (reframed as onboarding, valuable in either model) and the existence of a per-user toggle. v4 renders BOTH states side-by-side to decide on pictures, not predictions; instrument project-creation rate post-launch as the tiebreaker. | The disagreement is mechanism, not goal — both positions target first-run calm; absence vs designed quietness. | One-way-ish (product identity for new users) | **OPEN** — comparison mockups in v4; Matt↔Amith to resolve |
