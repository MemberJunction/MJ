...
| **Artifacts (partial)** | share-modal person-to-person sharing via `MJ: Resource Permissions` (View/Edit/Owner); Sharing Center wired | internal sharing ✓ | **Public link is COSMETIC** (2026-07-07 deep-dive): the toggle string-builds a URL, writes nothing — `PublicLink` table exists but is never constructed; no token/expiry/enforcement. Publish flow + privilege gate = fully greenfield; no version-editing; no remix |
