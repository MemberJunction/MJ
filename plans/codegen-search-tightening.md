...
### Phase 3 — __mj schema hygiene as metadata source-of-truth

Move what the v5.33 migration did into `metadata/`. Once committed, `mj sync push` becomes the single mechanism for keeping __mj search settings frozen.

**Changes:**

1. **Augment `metadata/entities/.audit-related-entities.json`.** For every entity already in that file, add to its `fields` block:
   ```json
   "AllowUserSearchAPI": false,
   "AutoUpdateAllowUserSearchAPI": false
   ```
   Add any audit/log/run entities listed in the v5.33 migration but missing from this file (cross-reference [the migration's @DisableMjEntities table](../migrations/v5/V202605041250__v5.33.x__Search_Hygiene_For_Mj_Schema_And_Field_Types.sql#L33)).

2. **Create `metadata/entity-fields/`** — new directory.
   - `metadata/entity-fields/.mj-sync.json` configured for `MJ: Entity Fields` with `filePattern: "**/.*.json"`.
   - `metadata/entity-fields/.mj-narrative-fields-search-disabled.json` — array of records targeting narrative fields on __mj entities. Each record:
     ```json
     {
       "fields": {
       ...
       "primaryKey": {
         "ID": "@lookup:MJ: Entity Fields.Entity=MJ: AI Prompts&Name=Description"
       }
     }
     ```
     Generated by querying for narrative-field names on __mj.* entities. Scope: `Description`, `Comments`, `Notes`, `Body`, `Bio`, `Summary`, `Content`, `Remarks` columns across all `__mj` schema entities.
   - `metadata/entity-fields/.mj-pk-search-disabled.json` — disable + freeze `IncludeInUserSearchAPI` on every primary-key column system-wide (mirrors the migration's section 2).
   
   We may auto-generate these files via a one-shot script that queries the dev database for affected EntityField rows. Acceptable to do that locally and check the generated JSON in.

3. **Push step:** `npx mj sync push --dir=metadata --include="entities,entity-fields"` after authoring.

4. **Migration cleanup decision:** keep the existing v5.33 migration in place (it's already shipped; ripping it out breaks installations mid-upgrade). Going forward, the metadata files are the source-of-truth and the migration becomes the historical bootstrap. Document this in a comment at the top of the migration file: "Search hygiene is now maintained in metadata/entities/ and metadata/entity-fields/. This migration remains for historical bootstrap of pre-v5.33 installs."
...
- `metadata/entities/.audit-related-entities.json` — Phase 3 (augment)
- `metadata/entity-fields/.mj-sync.json` — Phase 3 (new)
- `metadata/entity-fields/.mj-narrative-fields-search-disabled.json` — Phase 3 (new)
- `metadata/entity-fields/.mj-pk-search-disabled.json` — Phase 3 (new)
- `migrations/v5/V202605041250__v5.33.x__Search_Hygiene...sql` — Phase 3 (header comment only)
...
- [ ] `metadata/entities/.audit-related-entities.json` carries `AllowUserSearchAPI=false` + `AutoUpdateAllowUserSearchAPI=false` for every audit/log/run entity from the v5.33 migration list.
- [ ] `metadata/entity-fields/` exists with files freezing narrative + PK search across __mj.
