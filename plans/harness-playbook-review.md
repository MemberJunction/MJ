...
4. **Corrective inference hardening** — repetition-loop detection, leaked-envelope recovery, charitable parameter repair (Lesson 7).
...
**MJ today.** The JSON repair cascade (CleanJSON → JSON5 → LLM repair), `stripWrappingCodeFence`, corrective retry prompts, and error-classified failover are strong. Missing: repetition-loop detection on streamed output (a degenerate run today burns tokens until a limit trips); leaked-envelope recovery (a JSON decision wrapped in prose costs a full `Retry` turn when it is often mechanically extractable); and typed coercion of LLM-supplied action params (`Record<string, unknown>` is converted to `ActionParam[]` and handed to the action — dialect mistakes surface as action-level errors rather than a uniform repair-or-retryable-error at the boundary).

**Proposed.** (1) A streaming repetition detector in `AIPromptRunner` (n-gram/window heuristic) that aborts and treats the attempt as a failover-classified error; (2) an envelope-extraction pass in `LoopAgentType` before declaring a parse failure — find the outermost valid JSON object matching the response shape inside prose; (3) framework-level param validation/coercion against the action's declared param types, with unambiguous repairs applied and a structured retryable message otherwise. All three reduce paid retry turns.
...
| 2 | H3, H4 | Runtime hardening: central budgets, job surface, cheaper retries |
