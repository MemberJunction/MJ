...
| 5a | Phase 5 remainder — Create Workflow front door in a new **Workflows** app + per-screen locked-mockup implementations (five design answers ratified by AN-BC 2026-08-07; `front-door-v1.html` is the contract). **Known gap, flagged not dropped:** Playwright structure/behavior checks + visual baselines for the three screens are in the 5a scope but NOT in #3648 (34 unit/DOM tests incl. a D18 vocabulary guard exist; nothing exercises the screens in a browser). | [#3648](https://github.com/MemberJunction/MJ/pull/3648) | 🟦 In review |
...
| 8a | Housekeeping surfaced by #3617 — **verified mostly already-resolved rather than assumed** (suite at 61 members, IT71 at sequence 33, #3251 invariant holding 0/0, tier 61/61). #3649 ships the one genuinely missing piece: a **guard** for the suite-membership/transport invariant, which was convention-only and had already been broken once — verified by reproducing the original IT71 mistake and confirming the guard fails it. | [#3649](https://github.com/MemberJunction/MJ/pull/3649) | 🟦 In review |
...
1. **Spec.** A new mutually-exclusive assignment on `TaskGraphSpecNode`: `externalRef?: { domain: string; ref?: string }` — `domain` names the completing driver's domain, `ref` optionally identifies the domain object. *(Updated for #3644: the spec now has three arms — `agentName` / `actionName` / `assignToUser` — so `externalRef` is the **fourth**.)* Validator: `AssignmentConflict`/`NoAssignment` learn the new arm; exactly one of the four must be set. Additive under Publish-No-Break.
