...
| C.1 | **Flow agents onto the engine** ([`plans/flow-agent-taskgraph-unification.md`](flow-agent-taskgraph-unification.md) v2 — gap found 2026-08-08: `__mj.Task` has never held a row; flows never touch the dispatcher). Phases C1.0 spec v2 (union + `External` + policy/layout + dependency `priority`/`exclusiveGroup` + RO wire) → C1.1 `FlowGraphCompiler` + differential suite + golden fixtures → C1.2 dispatcher (Skipped status + XOR + runners + claim fixes) → C1.3 calling contract (envelope run, `WaitForSettlement`, streaming bridge, HITL + `CompleteTask` RO) → C1.4 cutover (gated, oracle retained unrouted) → C1.5 Prompt loop bodies. Supersedes D6, amends D7, restates D8 (notes in the decision rows). In-run-executor **deletion belongs to Track R**. | `feat/workflows-single-editor` (pre-PR) | 🟨 **C1.0–C1.3 built, running end-to-end** (build note on #3456, 2026-08-09). **Three deviations read into the record:** (1) **no back-compat on `TaskGraphSpec` — §11.1 superseded by AN-BC ruling**: one clean discriminated union across all seven kinds, all producers converted in-change; verified safe because nothing persists a spec (`persistParent` writes derived rows only); `AssignmentConflict` deleted as unrepresentable. (2) **`Task` gains `StepType`, `PromptID`, `Configuration` (`ITaskStepConfiguration` JSONType) + `CK_Task_Assignment` counts `PromptID`** — the row could not represent Prompt/ForEach/While steps or carry mappings/policy; without it a ForEach became an unauthored human task stalling forever, and mappings dropped meant branch conditions evaluated against `undefined` (falsy, not erroneous). §3.6 schema table superseded accordingly. (3) **Mapping semantics lifted once into `ai-core-plus/task-graph/payload-mapping.ts`** (25 tests, both engines call it) — reimplementing in the dispatcher would have made the same workflow map differently per engine. **Verified:** from-scratch DB, XOR fork one-branch + `Skipped`, ForEach 5/5, migration replays standalone. **Walker refuses at its single choke point** — a workflow that runs provably ran on the new engine. **Open by design:** `Prompt`/`External` runners refused by name at submission (not persisted-and-stalled); Track R deferred. **Not cited in the verification: the C1.1 differential suite / golden fixtures** — flagged for the C1 PR review. |
