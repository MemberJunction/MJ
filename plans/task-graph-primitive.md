...
| 8 | Track A — substrate correctness (the unfinished #3408 runbook). **Still fully open despite #3617's title** (verified on `next` @ `478b88371`): transition filters (`OldValues`/`NewValues` contract + seeded `Field Changed`/`Field Changed To Value` rows — zero occurrences); the Validate-path scope/provenance/redaction bypass; the retention purge job (no `RetentionPeriod` reader); the `RunEntityAction` null-contract guard; an entity-action deterministic check bundle; **the D14 After\*-durability re-homing** (`OnAfterSaveExecute` still fire-and-forget); **Record Process `OnChangeEnabled` reconciliation** (still zero consumers; prerequisites shipped in #3610) | — | ⬜ Not started |
