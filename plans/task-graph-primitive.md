...
| 7a | Follow-up — schema bundle deferred from #3610: one-shot scheduling (`CronExpression` is `NOT NULL`), missed-run policy field, trigger vocabulary normalization, durable continuation-depth marker (unblocks `Reinvoke`), and **the live-signal decision** (fourth review appearance — committed to this slice) | — | ⬜ Not started |
