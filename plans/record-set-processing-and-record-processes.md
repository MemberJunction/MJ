...
- **P4 Infer selectability** — schema + processor DONE; **only `buildProcessor` wiring remains, after CodeGen.** The Record Set Processing migration now defines `RecordProcess.WorkType IN ('Action','Agent','Infer')` + a `PromptID` FK → `AIPrompt` (folded into the original CREATE TABLE; migration re-timestamped `V202606201145`). Write-back ownership decided **option A**: `InferProcessor` is infer-only (returns the prompt output) and the existing `WriteBackProcessor` wrapper applies OutputMapping uniformly. **Next step once CodeGen regenerates** `WorkType`'s union + the typed `PromptID`: add the `rp.WorkType === 'Infer'` branch to `RecordProcessExecutor.buildProcessor` → `new InferProcessor(rp.PromptID, inputMapping)` (the OutputMapping wrap already applies). Can't be written before CodeGen (typed `PromptID` / `'Infer'` literal don't exist yet).
