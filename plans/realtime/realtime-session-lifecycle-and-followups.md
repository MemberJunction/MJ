...
1. **✅ Tool / function-calling over the wire — RESOLVED at the protocol level (2026-07-12).** This was the load-bearing risk: `invoke-target-agent` delegation requires the model to emit function calls **over the realtime socket**. A source audit of the engine's own event reference (`src/speech_to_speech/api/openai_realtime/README.md`) confirms all three needed primitives, verbatim: `session.update` accepts a **`tools`** field; the server **emits `response.function_call_arguments.done`** ("Tool call with `call_id`, `name`, and JSON `arguments`"); and it **accepts `conversation.item.create` with `type: "function_call_output"`** (+ a following `response.create`) for the tool result. That is exactly the OpenAI-Realtime tool-calling contract `invoke-target-agent` needs, so the delegation path is viable. (Issue #120, "Realtime API with Tool Chaining," is about orchestration *UX*, not the absence of the primitives.) **Two things still to verify against a real box** — I couldn't run HF S2S in-sandbox (models/GPU): (a) a live end-to-end round-trip (voice → delegate → tool result voiced back); and (b) **driver conformance to HF's exact shape** — HF documents `response.function_call_arguments.**done**` (no streamed `.delta` for args), and the result goes back as `conversation.item.create` + `function_call_output` **then** an explicit `response.create`. Confirm `HuggingFaceRealtimeClient` emits/consumes precisely that (done-only args; explicit response trigger after the result), not an OpenAI-delta assumption.
