...
**Validation status (the biggest risk is now retired at the protocol level — see #1):**

1. **✅ Tool / function-calling over the wire — RESOLVED at the protocol level (2026-07-12).** This was the load-bearing risk: `invoke-target-agent` delegation requires the model to emit function calls **over the realtime socket**. A source audit of the engine's own event reference (`src/speech_to_speech/api/openai_realtime/README.md`) confirms all three needed primitives, verbatim: `session.update` accepts a **`tools`** field; the server **emits `response.function_call_arguments.done`** ("Tool call with `call_id`, `name`, and JSON `arguments`"); and it **accepts `conversation.item.create` with `type: "function_call_output"`** (+ a following `response.create`) for the tool result. That is exactly the OpenAI-Realtime tool-calling contract `invoke-target-agent` needs, so the delegation path is viable. (Issue #120, "Realtime API with Tool Chaining," is about orchestration *UX*, not the absence of the primitives.)
   - **✅ (b) Driver conformance to HF's exact shape — CONFIRMED + now regression-locked (2026-07-13).** Code audit of BOTH halves: the server session (`HuggingFaceRealtimeSession`) and the browser client (`HuggingFaceRealtimeClient`) consume `response.function_call_arguments.**done**` ONLY (no `.delta` handler — done-only args), and send the result as `conversation.item.create`(`function_call_output`) **then** an explicit `response.create`. The client's collision-safe path queues the `response.create` when a spoken response is still in flight and fires it once on `response.done`. This is now covered by unit tests: the server side (`packages/AI/Providers/HuggingFace/src/__tests__/huggingFaceRealtime.test.ts`) and a NEW client suite (`packages/AI/RealtimeClient/src/__tests__/huggingface-realtime-client.test.ts` — the client driver had shipped without one; it now asserts done-only args, the exact `function_call_output` → `response.create` ordering, the queued-then-flushed path, and that a `.delta` frame is NOT treated as a tool call).
   - **✅ (a) Live end-to-end round-trip VALIDATED (2026-07-13)** against a real HF `speech-to-speech` box (v0.2.10) running natively on Apple Silicon (M4 Max, MPS): Parakeet STT + Qwen3-TTS local, LLM via a tool-capable `chat-completions` backend (llama.cpp + Qwen2.5-3B). A realtime-wire harness drove the full path and observed `response.function_call_arguments.done` (done-only args, `invoke-target-agent`), accepted MJ's `conversation.item.create`(`function_call_output`) + `response.create` tool-result contract, and got the delegated result **voiced back** (525 audio deltas). `RESULT: TOOL_ROUNDTRIP_OK`.
   - **🐛 Bug found + fixed during live testing:** MJ's `HuggingFaceRealtime.BuildSessionObject` (and `RegisterTools`) omitted the GA-required `session.type: 'realtime'` discriminator, so this HF build **rejected every `session.update`** (`"Unknown or invalid event: session.update"`) — silently dropping the prompt AND tools (delegation would never work). Fixed: `type: 'realtime'` is now stamped on both `session.update` paths, with unit assertions. (The plan's "beta shape" audit missed this because the running engine implements the GA shape.)
   - **⚠️ Deployment caveat (not an MJ issue):** the HF pipeline's **in-process `mlx-lm` LLM backend does not emit tool calls** — a co-agent on it can voice but never delegate. Tool delegation requires a tool-capable backend (`--llm_backend chat-completions`/`responses-api` pointed at vLLM/llama.cpp, or the HF router). Worth surfacing in the provider README as the recommended production config.
2. **✅ 16 kHz sample-rate wiring — CONFIRMED (2026-07-13).** Both `HUGGINGFACE_DEFAULT_PCM_SAMPLE_RATE` constants (server + client) are `16000`, and `HuggingFaceRealtimeClient.Connect` drives `createMicCapture` AND `createPlayback` at the same resolved rate (default 16 kHz, `SessionConfig.sampleRate`-overridable) — verified by the new client test (asserts both directions = 16000, and the 24 kHz override path). Fixed two stale `24 kHz` JSDoc comments (residue of the AssemblyAI-plane copy) in `HuggingFaceRealtime.ResolveSampleRate` and the client's `parseSampleRate` so the documented default matches the actual default.
3. **✅ Housekeeping — DONE.** `next` merged in (branch no longer conflicted); RealtimeClient (348) + HuggingFace-provider (14) suites green; changeset present (`.changeset/huggingface-realtime-provider.md`). The `package-lock.json` in the diff is the merged v5.47.0 release state, not an unreconciled artifact.
