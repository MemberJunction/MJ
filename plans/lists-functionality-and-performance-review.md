...
| F10 | **Add-to-List dialog opens empty from grids AND the record-form toolbar** (confirmed at runtime 2026-07-14): both hosts bind `[visible]` before `[config]`, and Angular assigns inputs in template attribute order — the `visible` setter fired `initializeDialog()` while `config` was still undefined, `loadData()` bailed silently, and nothing re-triggered the load. Fixed in the dialog itself (order-independent init: a `_pendingInit` flag armed on closed→open, satisfied as soon as both `visible` and `config` are present), plus the dialog now threads its `Provider` input into `ListManagementService` per that service's documented multi-provider contract | [list-management-dialog.component.ts:45-90](../packages/Angular/Generic/list-management/src/lib/components/list-management-dialog/list-management-dialog.component.ts#L45-L90) | **High** — both add-to-list surfaces affected |
