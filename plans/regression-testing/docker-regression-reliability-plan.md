...
**Rebuilding db-setup to bake the DR-B4 CLI exposed a CASCADE of latent build breakages** — the regression images are very stale vs. the branch (they predate DR-C1's manifests-skeleton, the CU build-time-metadata dependency, and codegen changes), so a fresh `mj test regression build` currently cannot produce a working image. Three found so far; **the remaining Theme-C/E wave items (E5/C4/C5) are all gated on a fully-working rebuild**, i.e. on completing this build remediation (a distinct effort from Theme-B DB-lifecycle):
...
3. **OPEN (separate build-remediation) — codegen ClassFactory registrations + AdvancedGeneration LLM key.** After migrate, `mj codegen` (pass 1) fails in the rebuilt image: ClassFactory has ZERO registrations for `GraphQLServerGeneratorBase` / `EntitySubClassGeneratorBase` / `ActionSubClassGeneratorBase` (the concrete generators aren't imported/registered — a class-registration-manifest/build gap), and the CHECK-constraint field-validator step needs an AI key db-setup isn't given. Needs its own investigation; it does NOT affect the DB-lifecycle items already landed. The live stack was restored to healthy by RESTORE-ing the valid `33427d50` snapshot directly (proving the `.bak` is durable + usable) + bringing the 44h mjapi/mjexplorer back up.
