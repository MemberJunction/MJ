...
**Wave 4 status — LANDED (codified; the mechanism had already shipped).** The bind-mount half of the policy was already in place: DR-B1 bind-mounted `db-setup-entrypoint.sh` + `scripts/`, the runner already mounts both its entrypoints + `scripts/` + `targets/` + `archive/`, and DR-B5 mounts `metadata/` `:ro`. So DR-C4 was completed by **codifying** the rule in [ARCHITECTURE.md §3 "Bake-vs-Mount Policy"](../../docker/regression/ARCHITECTURE.md): *code is baked; entrypoints/scripts/metadata/targets are mounted `:ro`* — with the current-state table (already compliant), the intentional exceptions (published/standalone variants bake their entrypoints; `mj.config.cjs` is baked + role-specialized four ways, all starting from `docker/MJAPI/docker.config.cjs`), and the rule for new services. **`develop.watch` deliberately NOT added** (Simplicity-First): the paths it would watch are already bind-mounts, so a watch/sync block would be redundant — documented as such. The `mj.config.cjs` divergence is documented (not unified) — the four mechanisms are legitimately role-specific and unifying them carries risk for no functional gain.
...
**Wave 4 status — LANDED for the two co-located regression entrypoints (sourced-library form); single-file MODE-switch + published dispatcher deferred to DR-C6.** The five duplicated blocks — the `EXTRA_METADATA_DIRS` push loop, the archive-destination pre-flight, the `--oracles-module` arg builder, the screenshot+md+html report generation, and the `latest` symlink — now live once in [`scripts/lib/entrypoint-common.sh`](../../docker/regression/scripts/lib/entrypoint-common.sh) (`push_extra_metadata_dirs` / `run_archive_preflight` / `build_oracles_args` / `generate_standard_reports` / `finalize_latest_symlink`), sourced by both `test-runner-entrypoint.sh` (full) and `test-runner-remote-entrypoint.sh` (remote) — the same pattern already used for `archive-run.sh`. Full 332→286 lines, remote 175→134; the drift class between the two is gone. **Deliberately NOT a big-bang single-file `MODE={…}` rewrite:** these scripts drive the live suite and can't be runtime-verified in this environment (full = the LLM suite, currently behind the DR-C5 tripwire; remote = needs a target URL; dispatcher = the unbuilt published image), so a blind rewrite would be reckless right after the build stabilization — whereas a verbatim sourced-function extraction is behavior-preserving and `bash -n`-verifiable. The **published-image dispatcher** (`docker/agentic-test-runner/dispatcher.sh`) carries the same duplication but is a different image; the library is written mode-agnostically so it can adopt it under **DR-C6** (which vendors `scripts/` into that image) — exactly the tie-in this item's Risks note calls out. **Also fixed a latent bug:** the remote entrypoint's `latest` symlink used `run-${TIMESTAMP}`, which pointed at a nonexistent directory whenever the host minted `RUN_ID` (DR-F1); the shared `finalize_latest_symlink` keys on `$RUN_ID` in every mode. **Verified:** `bash -n` on the lib + both entrypoints; a sourced-lib unit pass (all five functions load; oracles/archive/extra-metadata branch behavior; the symlink→`$RUN_ID` fix); the defined-vs-called cross-check; downstream globals intact (`ORACLES_ARGS` → the suite invocation, `ARCHIVE_PREFLIGHT_OK` → `archive-run.sh`). **PENDING (infra-gated, flagged not done):** a live full-mode + remote-mode run to confirm end-to-end — the extraction is faithful and the scripts are bind-mounted (any issue is a one-line fix, not an image rebuild), but a real run is the last mile.
...
**Wave 5 status — LANDED (memory flags + banner + A4 formula); `--shm` skipped as inert; auto-apply left advisory.** `up` gains `--runner-memory` / `--db-memory` / `--api-memory`, validated up front (`parseMemoryToBytes` — a typo aborts instantly, not as a cryptic compose error) and forwarded to the existing `MJ_REGRESSION_*_MEM_LIMIT` env knobs. **No transient override file was needed** (Simplicity-First): those limits are already env-driven in the compose, and shell env wins over `--env-file` (the same mechanism `--workers`/`--retries` use) — so the env path achieves the plan's outcome without a redundant `-f` file. `--shm` was **deliberately skipped**: `shm_size` is hardcoded AND the compose comment marks it currently inert (ipc mode changed); adding override machinery for a dead knob failed the Simplicity test — trivially added later if ipc reverts. The **DR-A4 worker-derivation formula** (`suggestWorkers`, pure: `(runnerMem − 1GiB reserve) / 1.5GiB per Chromium worker`, clamped [1,12]) ships and is surfaced in a new **effective-config banner** (workers/retries/memory, with the A4 suggestion as advice) — advisory only, it never overrides an explicit `--workers`. Verified: 8 unit tests (`regression-sizing.test.ts` — memory parsing incl. g/m/k/bare-bytes/fractions/reject-bad; A4 formula incl. reserve, floor, min-1, clamp-12, custom budgets); `up --help` lists the flags; a bad `--runner-memory=big` aborts pre-docker with the precise message; full MJCLI suite 444 passed. **Note:** the `docker system info` auto-suggest source in the proposal was refined to the runner's own mem budget (the relevant constraint for browser workers), not host total.
...
**Wave 5 status — the JUnit-emitter carve-out LANDED; the full `ci` orchestration deferred (STATIC-ONLY).** The integrated `ci` command (build→preflight→up→export→compare→exit-code) is a release-gate *run* — verifiable only by the live LLM suite, so its wiring is deliberately NOT shipped blind. Its one net-new, pure, testable piece IS shipped: `scripts/generate-junit.cjs` (`buildJUnitXml`, pure) converts a run's `results.json` to JUnit XML for CI consumers — Failed→`<failure>`, the infra classes Error/Timeout→`<error>`, Skipped→`<skipped>`, per-test/total time in seconds, everything XML-escaped. It's wired into the shared `generate_standard_reports` (DR-E5 lib) as a **best-effort** step (guarded `|| WARNING` — a report failure never fails an otherwise-complete run), so every full + remote run now drops a `report.junit.xml`. Verified (no live suite): node-against-fixture assertions (tests/failures/errors/skipped counts, Error+Timeout→errors, the three body shapes, `&`/`<`/`>`/`"` escaping in suite + test names, total-time sum, XML structure), the empty-results case (valid empty suite, no crash), and an end-to-end `RUN_DIR` run writing a well-formed file; `node --check` + `bash -n` clean. **Deferred (needs the live gate run):** the `ci` command itself (arg assembly reuses the already-verified build/up/export/compare + F2 exit-code path), a strict host-side preflight (Wave-1-deferred), and a `--baseline` results-artifact convention.
...
**Wave 5 status — the safe path-hardening slice LANDED; the full run-from-subdir refactor + orchestration inversion deferred (with reason).** The two monorepo detectors are now unified on one walk-up resolver: new `findMonorepoRoot()` (checks the start dir through the filesystem root — fixing `isInsideMonorepo`'s old off-by-one that never tested the fs-root ancestor), with `isInsideMonorepo` reduced to `findMonorepoRoot(...) !== null`. `requireMonorepoRoot` now distinguishes **"in a subdirectory OF a monorepo"** (a precise `cd <root>` error) from **"not in a monorepo"** (the external message) — the old guard printed the generic message in both, so a wrong-cwd looked like a broken checkout. Verified: 4 unit tests (`regression-monorepo-root.test.ts` — root-from-root, walk-up-from-nested, null-when-absent, isInsideMonorepo parity); full MJCLI suite 448 passed. **Deliberately NOT done here** (documented, needs work I can't verify in this environment): (1) making the internal path CONSTANTS root-relative so commands actually *run* from a subdir — that would have to thread a resolved root through the path helpers rather than absolutize the module constants, because absolutizing them breaks cwd-dependent code added since (DR-C5's `formsFingerprintStatus` deliberately resolves the fingerprint file against cwd), and verifying it needs every `mj test regression *` command run live from a subdir; (2) the `compare.ts` `process.argv[1]` self-reinvoke → direct `CompareCommand` call (deferred to avoid colliding with concurrent DR-G6 work in the same testing-cli package); (3) the multi-PR orchestration inversion (Part 2), explicitly longer-term. The near-term win — a clear, actionable error instead of a silent path failure — is landed.
...
**Wave 5 status — LANDED (coarse tier: category + route); cross-run signature IDs + perceptual hashing deferred (need the archive DB).** A pure, dependency-free `scripts/lib/cluster-failures.cjs` groups FAILING results by `<category> @ <route-prefix>` — `category` mirrors `generate-summary.cjs`'s expression exactly (so clusters never drift from the `categories` tally), `route-prefix` is the first 2 id-normalized path segments (MJ's `/app/<app>` feature boundary; UUIDs / MJ composite keys / numbers collapse to `:id`). Clusters of ≥3 sharing a REAL route auto-flag `suspectedAppDefect` ("do not retry, file a bug"). Wired additively (existing per-test rows untouched) into `summary.json` (`failureClusters[]`), the markdown report (collapsible "Failure clusters" section), and the HTML report (collapsible blocks + red badge; matches the report's existing hardcoded-hex palette — a standalone report, not an Angular component, so the design-token rule doesn't apply). One impure companion, `enrichRoutesFromSteps`, recovers the last-visited URL from each failing test's `screenshots/<test>/steps.json` (real runs' oracle details usually omit `finalUrl`), keeping `clusterFailures` itself pure. Verified (no live suite/DB): pure-function assertions (grouping, id-normalization incl. `ID|<uuid>` + numbers, the ≥3-flag, no-route-never-flagged), `generate-summary.cjs` against a synthetic RUN_DIR (3 clusters / 1 flagged), the zero-failure case (all three generators exit 0, empty clusters), AND a real 44-test archived run — 27 failures collapse to 14 clusters with 2 auto-flagged (`/app/home` ×9, `/app/knowledge-hub` ×3), the intended hours→minutes triage payoff on real data.
...
**Wave 5 status — LANDED (duration/flake/pass-rate half); cost half plumbed-but-`n/a` (blocked on CU telemetry).** Both stubs are now real. `mj test history [--test|-t] [--suite|-s] [--limit|-l] [--format] [--output]` loads recent `MJ: Test Runs` (`RunView<T>` `ResultType:'simple'`, narrowed `Fields`) and renders per-test duration (min/median/max/last), pass/fail/skip counts, pass-rate, and flake-rate, flakiest-first. `mj test report [--suite] [--baseline] …` resolves a suite run, batches the trend window in one `TestSuiteRunID IN (...)` query, and reports per-run counts + pass-rate + total duration + a cross-run trend table (≥2 runs). All branching/math lives in a pure, DB-free `utils/history-aggregation.ts` (median ignores null durations; `flakeRate = min(pass,fail)/terminal` with Error/Timeout counted as fail, Skipped excluded). **Cost gap handled honestly:** `CostUSD`/`TotalCostUSD` are read + plumbed, but every value is 0 until CU-plan telemetry lands, so the display shows `n/a` + a `costCapturePending` note (JSON flag too) and auto-switches to real `$` once any row reports cost>0 — no invented numbers. **Flaky derived from cross-run outcome inconsistency**, not a DB column (DR-D8's `Flaky` column is blocked): `updateTestRun` never persists `flaky`, so per-run counts come from individual `TestRun.Status` rows and flakiness from a test both passing and failing across the window (single-run → `n/a — needs ≥2 runs`). Verified: 22 new unit tests over synthetic rows (`history-aggregation.test.ts` — duration stats, flake-rate, per-run aggregation, zero-cost) + 42 existing = **testing-cli 64 passed**; both packages build clean; `mj test history/report --help` render; and the `RunView` `Fields` (`TestID`/`Test`/`Status`/`DurationSeconds`/`CostUSD`/`TestSuiteRunID` on Test Runs; `Suite`/`SuiteID`/`StartedAt`/`Status` on Test Suite Runs) were cross-checked against the generated entity classes — the one runtime risk of a string field list, cleared. **NOT verified (infra-gated):** the live `RunView` execution against a populated DB (`--suite`/`--baseline` filters, the `IN (...)` batch, the `StartedAt` string-vs-Date shape — normalized defensively); the pure logic underneath is fully unit-tested.
