...
- ❌ Voice input/output (Phase 2 — Web Speech API doesn't exist in RN; Phase 2 reuses core's realtime stack via `@memberjunction/ai-realtime-client` — see the revised Voice mode subsection)
...
> **Updated 2026-07-02:** core now provides realtime/voice via `@memberjunction/ai-realtime-client` (the framework-agnostic `BaseRealtimeClient` + provider drivers) fronting the server's `StartRealtimeClientSession` GraphQL transport. The original "record → upload `audio/webm` → server Whisper round-trip" approach is **superseded** — we reuse the core realtime brain from RN instead of building a bespoke STT pipeline. See the revised wiring below.

- **Phase 1:** Tapping the mic from any thread navigates here and shows the visual mockup. The actual voice pipeline is stubbed. The screen is functional as a placeholder for the "voice-soon" experience but does not record.
- **Phase 2 wiring (revised — reuse core, don't rebuild):** This mirrors the doc's "one TS brain, three UIs" thesis. Since the plan was first written, core shipped a full realtime voice stack; the mobile app should *consume* it rather than hand-roll a Whisper round-trip:
  - **The brain — `@memberjunction/ai-realtime-client`.** A framework-agnostic (no Angular, no DOM-framework) TypeScript package: the `BaseRealtimeClient` contract plus provider drivers (`openai`, `gemini`, `elevenlabs`, `assemblyai`, `xai`, resolved through `MJGlobal.ClassFactory` by provider key). This is the *client-side mirror* of the server's `BaseRealtimeModel` — the same pluggability machinery (`@RegisterClass`, `ClassFactory`) we already rely on runs it under Hermes unchanged. It owns the full-duplex session: transport, the response state machine, barge-in, tool-call routing, transcript deltas/finals, and narration. **Prompt and tool authority stay server-side** — the client applies the server-built session config verbatim.
  - **The transport — `StartRealtimeClientSession` (MJAPI GraphQL).** The Angular host calls this mutation to have the server mint an *ephemeral, provider-native* session config (`ClientRealtimeSessionConfig`: provider key, model, a short-lived credential/signed URL, and an opaque `SessionConfig`). The client driver then opens the provider socket **directly** (client-direct topology — audio frames never transit MJAPI, so latency is minimal). Tool results and transcript persistence flow back through the companion `ExecuteRealtimeSessionTool` / `RelayRealtimeTranscript` mutations. This is exactly the flow the Explorer host uses today — see the reference host below.
  - **What's RN-safe as-is vs. what needs an adapter:** The driver *logic* (contract, state machine, event translation, tool routing) is pure TS and reuses unchanged. The **audio plane is the browser-coupled part** — `@memberjunction/ai-realtime-client`'s shipped mic capture and playback are built on the Web Audio API (`AudioContext` + `AudioWorkletNode`, an `<audio>` sink, `MediaStream`), which do **not** exist in RN/Hermes. The drivers expose this behind overridable `protected` seams (`createMicCapture` / `createPlayback`, and per-driver socket/peer seams), so the RN work is a **thin audio adapter**, not a fork: implement mic capture (PCM16 chunks) and gapless PCM playback via `expo-av` / `expo-audio`, and inject them through those seams. Pick a **WebSocket** driver (ElevenLabs is the simplest target — raw WS against a server-minted signed URL, PCM both ways, native context-notes, no WebRTC); Hermes has a global `WebSocket`, so the transport itself is fine. Avoid the OpenAI driver for the first cut — it uses `RTCPeerConnection`/WebRTC and would drag in `react-native-webrtc`.
  - **Result into chat:** final `normal`-kind transcripts persist as `MJ: Conversation Detail` turns on the active conversation (the host already routes this via `RelayRealtimeTranscript`); `narration`-kind transcripts stay ephemeral by contract (never persisted). Voice *output* is the model's own streamed PCM played back through the same RN playback adapter — no separate TTS round-trip needed. (If we ever want a non-realtime "speak this text" path, `@memberjunction/ai-elevenlabs`'s `ElevenLabsAudioGenerator` TTS or RN's `expo-speech` are fallbacks.)
  - **Reference host to mirror:** `RealtimeSessionService` in `@memberjunction/ng-conversations` (`packages/Angular/Generic/conversations`) is the production Angular consumer — read it for the policy layer (caption/transcript routing, narration pacing, mic-permission UX, tool relay). The RN "host" is the same policy code re-expressed as a hook/service; only the audio adapter and the permission/UX chrome differ.
- **RN gotchas (Phase 2, to validate):** background audio + lock-screen behavior (iOS audio session category, `UIBackgroundModes: audio`); WebSocket stability on Hermes across app-background transitions (reconnect on the `Fatal` error path the driver already surfaces); mic permission — already declared in `app.json` (`NSMicrophoneUsageDescription` / Android `RECORD_AUDIO`), so no new native config; and confirming `expo-av`/`expo-audio` can deliver/consume raw PCM16 at the negotiated sample rates (16/24 kHz) with acceptable latency.
- **Visual:** Status bar / Listening pill (top-left), conversation title (center), close (top-right). Center stage: animated breathing orb with double ripple. Below: waveform driven from the driver's audio-meter / amplitude data (the core package ships an `audioMeter`; the RN adapter feeds it from the capture stream). Below that: glass-effect transcript card showing live (interim delta) or final text from the driver's `OnTranscript`. Bottom: keyboard mode | stop (red) | menu, with hint "Tap to stop · swipe right for keyboard."
- **Open questions (Phase 2 / future PR):** (1) which provider driver ships first (ElevenLabs WS is the leading candidate for RN simplicity) and whether we standardize on one or expose the server's configured default; (2) whether `expo-av`/`expo-audio` PCM I/O meets the latency/echo-cancellation bar, or whether we need a small native module; (3) echo cancellation / half-duplex handling on-device; (4) whether the RN audio adapter should live in `packages/MobileApp` or be contributed back to `@memberjunction/ai-realtime-client` as an RN seam implementation for reuse.
