...
- **Watermark strict `>` can skip a boundary-tick row (R3).** A source row committed at exactly the captured watermark millisecond (same-tick, commit-ordering race) can be missed by the incremental pass. **Bounded** — the forced-full-rebuild cadence (§11.3, every 10 cycles) reconciles it. Choosing `>=` (idempotent reprocessing of the max-timestamp group) would close it at negligible cost; deferred.
- **No overlap-sweep lease (R4).** Two concurrent refreshers on one materialization (multi-node scheduler, or a slow sweep overlapping the next fire) share the fixed `__shadow` table name and can produce spurious refresh failures. Not data-corrupting (DDL locks serialize the swaps). A `Status='Refreshing'` in-flight marker is the follow-up.
