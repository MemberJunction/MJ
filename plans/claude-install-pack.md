...
| ~~`/generate-integration-actions`~~ | ~~Generate mj-sync action JSON for a user-authored connector~~ | ~~`.claude/commands/generate-integration-actions.md`~~ | **Dropped during PR review.** Originally planned to rewrite as a downstream-correct command driven by a new `mj codegen integration-actions` CLI, but the supporting CLI + runner extraction was out of scope for this PR. To be shipped as a follow-up alongside the dedicated CLI work. |
