...
- **The realtime-registered tool set is capability-gated: dynamic for supporting models, stable and target-independent for connect-bound providers.**
  - *Drivers with `SupportsDynamicToolSet = true` (OpenAI Realtime, OpenAI Live, xAI)*: Agents configured with `directActions.enabled: true` project allowed actions directly into the session's toolset. This avoids the 2–5 second latency roundtrip of spinning up a full sub-agent run (`invoke-target-agent`) for fast, single-step operations (e.g., lookups, queries), executing them via `ActionEngineServer.Instance.RunAction` in `RealtimeClientSessionService.executeNonTargetTool` on the client-direct relayed path with an explicit timeout. (Note: in the server-bridged topology, telephony/media/room sessions run via `RealtimeSessionRunner` do not currently project direct actions).
  - *Connect-bound and server-provisioned providers (ElevenLabs, Gemini)*: Do **not** declare `SupportsDynamicToolSet`. For these providers, tool sets are fixed, server-bound, or pre-provisioned at connect time. All target-specific work continues to execute inside the delegated agent's own run via the stable **`invoke-target-agent`** tool (the target is a runtime parameter *inside* that tool). This preserves the original invariant for providers whose connection topology cannot dynamically project arbitrary agent action sets.
