...
- **The realtime-registered tool set is capability-gated: dynamic for supporting models, stable and target-independent for connect-bound providers.**
  - *Drivers with `SupportsDynamicToolSet = true` (OpenAI Realtime, OpenAI Live, xAI)*: Agents configured with `directActions.enabled: true` project allowed actions directly into the session's toolset. This avoids the 2–5 second latency roundtrip of spinning up a full sub-agent run (`invoke-target-agent`) for fast, single-step operations (e.g., lookups, queries), executing them via `ActionEngine.RunAction` directly in the realtime broker with an explicit timeout.
