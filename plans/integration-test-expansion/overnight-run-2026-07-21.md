...
| # | Scope | Status |
|---|---|---|
| W1 | **Domain 0**: graduate `runview-matrix` rig → bundles; `runview-features`, `runquery-catalog`, `runquery-params`, `runquery-features` (incl. register items: `since` injection hole, ValidationFilters unenforced, CategoryPath misnomer — fix + pin) | pending |
