...
## 19. Card design foundation | ~3 days

*Depends on: O2 (CI gates). Should run before O20.*

| KR | Start | Target |
|----|-------|--------|
| Shared card CSS tokens extracted (`_card-tokens.scss`) | 0 | 1 |
| Card design inconsistencies resolved | 0 | 1 |

**Deep-dive findings** (May 27):
- 10 dedicated card components, 150+ CSS files with `.card` patterns, 523 files touch cards
- 5 visual archetypes: Horizontal Card (60%), KPI/Stat (20%, covered by O11), Collapsible Panel (10%), Compact List Item (5%), Artifact/Content (3%)
- **Design inconsistencies across all cards**:
  - Padding: 3 models (12-16px, 20px, 1.25rem) — standardize to `--mj-space-md`
  - Icon sizes: 4 variants (36px, 40px, 48px) — standardize to 40px icons, 36px avatars
  - Border-radius: 3 variants (8px, 12px, 16px) — standardize to `--mj-radius-md`
  - Hover effects: 3+ patterns (shadow-only, shadow+translateY, border-color) — create single mixin
- `.card-header`, `.card-icon`, `.card-title` defined independently 10+ times across components

**Scope**: Extract shared card CSS tokens and mixins. Standardize padding, border-radius, hover, and icon sizes across all 10 existing card components. No new components — CSS-only consolidation. Fix the visual inconsistencies without touching component architecture.

**Done when**: `_card-tokens.scss` committed and imported by all card components. All cards use consistent padding, radius, hover, and icon sizes. Light/dark validated.

---

## 20. Card component consolidation | ~5–7 days

*Depends on: O19 (card tokens established). Heaviest component objective.*

| KR | Start | Target |
|----|-------|--------|
| `<mj-card>` parametric component shipped | 0 | 1 |
| Archetype A card patterns migrated | 0 | 150 |

**Deep-dive findings** (May 27):
- Archetype A (Icon+Title+Fields+Action) accounts for **60% of all card usage** (~150 CSS files)
- Existing `MJEntityCardComponent` (436 lines, standalone, metadata-driven) is the most reusable starting point — already supports `compact`, `card`, and `detail` variants
- Domain-specific cards (IntegrationCard, SettingsCard) are appropriately specialized — don't consolidate
- Key shared structure: icon/avatar left (36-48px) + content section (flex:1, title+fields) + action button right (hover-visible)

**Scope**: Build parametric `<mj-card>` covering Archetype A. Slots: `[icon]`, `[header]`, `[meta]`, `[actions]`, default (body). Migrate the ~150 files using inline Archetype A card markup. Leave IntegrationCard, SettingsCard, and KPICard as domain-specific.

**Phased approach**: Start with `MJEntityCardComponent` as the base. Extend to cover ActionCard and CollectionArtifactCard patterns. Migrate inline card markup file by file.

**Done when**: Archetype A card patterns consolidated. Domain-specific cards documented as intentional exceptions. Light/dark validated.
