...
| Trust (match target) | The *candidate* is trusted; the **column it is matched against is not**. On the baseline seed `UI` is read-only on `MJ: Users` while `Developer` and `Integration` hold full update, and the shipped `newUserRoles` default **was** `['UI', 'Developer']` (`config.ts:666`) — so on a stock config every auto-provisioned user could update `User` rows, `Name` and `Type` included. Name-first ordering therefore rested on a column that grant made writable. Raised as issue #4260 and **fixed there**: the shipped default is now `['UI']`, which holds no write on `MJ: Users`. The underlying grant is unchanged — `Developer` and `Integration` still hold unfiltered update on `MJ: Users`, so a deployment that adds either to `newUserRoles` re-opens this exactly. The ordering must therefore still not be read as resting on an immutable column |
