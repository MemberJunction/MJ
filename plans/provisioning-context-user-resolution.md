...
| Trust (match target) | The *candidate* is trusted; the **column it is matched against was not — until this branch's second half made it so**. On the baseline seed `UI` is read-only on `MJ: Users` while `Developer` and `Integration` hold full update, and the shipped `newUserRoles` default **was** `['UI', 'Developer']` (`config.ts:666`) — so on a stock config every auto-provisioned user could update `User` rows, `Name` and `Type` included. Name-first ordering therefore rested on a column that grant made writable. Raised as issue #4260 and fixed in two parts: the shipped default is now `['UI']`, which holds no write on `MJ: Users` (closes reachability); and `MJUserEntityServer.Validate()` now refuses any non-Owner caller's attempt to change `Name` on an existing row — role-blind, so it holds regardless of what `newUserRoles` names, `Developer`/`Integration` included (closes capability; see `MJUserEntityServer.server.ts`, invariant 4). `Name` is therefore now genuinely immutable for a non-Owner caller, and the Name-first ordering rests on something the guard actively maintains, not merely on the config default holding |
