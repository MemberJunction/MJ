// ActionPanel Sub-component
function ActionPanel ({ data, isOpen, onClose, utilities, styles, components, callbacks, savedUserSettings, onSaveUserSettings }) {
  const [activeTab, setActiveTab] = useState('details');

  return (
    <div 
      className="actionpanel-panel"
      style={{
        position: 'fixed',
        top: 0,
    ...
    >
      <div style={{
        ...
        <h3 style={{ margin: 0 }}>{data?.title || 'ActionPanel'}</h3>
        <button 
          onClick={onClose}
          style={{
            background: 'none',
