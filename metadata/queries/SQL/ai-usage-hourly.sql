SELECT
    ...
    SUM(CASE WHEN IsPriced = 1 AND IsParallelParent = 0 THEN 1 ELSE 0 END) AS PricedRuns,
    SUM(CASE WHEN IsPriced = 0 AND IsParallelParent = 0 THEN 1 ELSE 0 END) AS UnpricedRuns,
    ...
    SUM(CASE WHEN IsParallelParent = 0 THEN TokensCacheRead ELSE 0 END) AS TokensCacheRead,
    SUM(CASE WHEN IsParallelParent = 0 THEN TokensCacheWrite ELSE 0 END) AS TokensCacheWrite,
