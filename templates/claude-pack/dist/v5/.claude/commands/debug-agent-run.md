...
**Agent Architecture**:
- All agents inherit from the `BaseAgent` class (exported from `@memberjunction/ai-agents`)
- Agents execute prompts using `AIPromptRunner` (exported from `@memberjunction/ai-core-plus`)
...
These commands are exposed by `@memberjunction/cli` and backed by the `AgentAuditService` in `@memberjunction/ai-cli`.
...
When debugging agent code issues, the relevant packages installed under your
`node_modules/` are:

- `@memberjunction/ai-agents` — Base agent implementation (`BaseAgent`) and built-in agent types
- `@memberjunction/ai-core-plus` — Prompt execution engine (`AIPromptRunner`)
- `@memberjunction/actions-base` — Action system (`BaseAction` and the action runner)

Inspect their `.d.ts` files for the surface area (`node_modules/@memberjunction/<pkg>/dist/index.d.ts`),
or read the source in the MJ repository at https://github.com/MemberJunction/MJ.
