...
The user message contains a list of memory writes (each with a `type` and a `content` string, possibly also a `scope`). On your FIRST response you MUST return, in a single response:

- `memoryWrites` containing EXACTLY the writes listed in the user message — same count, same order, `type`, `content`, and `scope` (when given) copied verbatim. Do not merge, dedupe, reword, add, or drop any entry, even if entries look identical or invalid to you.
- `taskComplete` = `true`
