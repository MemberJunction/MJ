...
The `Computer Use` TestType row itself stays in the normal `metadata/test-types/` tree (it is just a type definition, no test data). The pre-consolidation regression tests (the original `T01`–`T25` that shipped in `metadata/tests/regression/`) are removed from every instance by [`migrations/v6/V202609181937__v6.2.x__Prune_Pre_Consolidation_ComputerUse_Tests.sql`](../../migrations/v6/V202609181937__v6.2.x__Prune_Pre_Consolidation_ComputerUse_Tests.sql), which drops their suite memberships and then the test rows themselves. It is a migration rather than metadata delete records because a `deleteRecord` directive re-stamps `deletedAt` into its own file on every database where the row is still present, which leaves the committed tree dirty on every CI run.
...
│       ├── ...
│       └── .T155-deep-link-app-url-navigation.json  # 155 tests, contiguous T001–T155
└── test-suites/
...
1. **Pick the next free test number** (T001–T155 are taken; start at T156).
2. **Copy the closest existing test** that does something similar — it's almost always faster than starting from scratch.
   ```bash
   cp metadata-optional/regression-test/tests/regression/.T041-entity-form-edit-mode-dirty-field-banner.json \
      metadata-optional/regression-test/tests/regression/.T156-my-new-test.json
   ```
3. **Edit the new file**:
...
6. **Smoke test single-run**: `mj test run --name "T156 - My New Test"`
...
"Configuration": {
  "maxSteps": 35,
  "ReplayScript": "@file:regression/scripts/t001-login-smoke.json"
}
```

**The files are the source of truth, not the database.** The ephemeral docker DB is wiped between
runs, and the entrypoint pushes this folder on every startup — so a script that exists only in the
database is gone by the next run. Git is what makes a script durable, and it makes the diff
reviewable in a PR, which is where a change to how the suite drives the app belongs.

`tests/.mj-sync.json` is what wires this up:

```json
"externalizeFields": [
  { "field": "Configuration.ReplayScript", "pattern": "@file:regression/scripts/{Name}.json" },
  { "field": "Configuration.PendingReplayScript", "pattern": "@file:regression/scripts/{Name}.pending.json" }
]
```

The dotted path externalizes just that property, leaving the hand-authored `maxSteps` / `oracles` /
`judgeFrequency` keys inline and readable. Push resolves the reference on the way back in, so no
push-side configuration is needed.

Three things worth knowing:

- **A test that never recorded a script has no file and no `ReplayScript` key.** Pull skips absent
  properties entirely, so those test definitions stay byte-identical.
- **A re-derived script lands in `PendingReplayScript`, not `ReplayScript`.** Once a test has a
  promoted script, a run that produces a different one parks it in the review slot rather than
  silently rewriting how the suite drives the app. Use `mj test scripts` to see the drift, then
  `--promote` or `--discard`.
- **Pulling without those `externalizeFields` entries inlines every script.** Externalization is
  driven by the config, not by what the file already contains — so a pull with the entries removed
  writes the scripts into the test JSONs and orphans the files.
