/**
 * Post-publish back-merge: main -> next, then refresh pnpm-lock.yaml with the
 * @memberjunction/* versions publish.yml just pushed to npm.
 *
 * This runs as publish.yml's final step, AFTER the packages are on npm and the tag is
 * pushed. Every failure path below is shaped by that fact: nothing may push a tree it
 * cannot vouch for, and nothing may fail the job over a problem that leaves next intact.
 */
 ...
 *
 * The lockfile qualifies because it is *derived* — regenerated from the package.json
 * files that merged cleanly — so discarding both conflicting sides loses no information.
 * Nothing else belongs in this set: for a hand-authored file, picking a side by rule is
 ...
 *
 * Deliberately NOT `-X theirs`, which is what this used to do. Under a frozen next,
 * main's source and next's were identical so the strategy never actually fired. Now that
 ...
 * @returns {Promise<{lockfileConflicted: boolean}>} whether the caller must regenerate
 *   the lockfile before pushing.
 * @throws when anything else conflicts, or when the merge fails for a non-conflict
 *   reason. The working tree is left clean (the merge is aborted) and next is untouched.
 */
