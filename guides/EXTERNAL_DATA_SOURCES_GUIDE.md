...
- **`@databricks/sql` is an optional peer dependency.** Like `snowflake-sdk`, it's loaded via dynamic import only when the driver is actually used, and is declared `optional` in the package's `peerDependenciesMeta` — so a downstream consumer that installs the published package never pulls it in. (Within the MJ monorepo it's also a devDependency, since the SDK bundles the types this driver imports at build time, so it does install in the dev tree.) Install it where the driver runs: `npm install @databricks/sql`.
