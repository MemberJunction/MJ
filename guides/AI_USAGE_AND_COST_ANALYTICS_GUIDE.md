...
3. **A parallel parent has no own spend.**
   A parallel parent row carries `RunType='ParallelParent'`, `Cost = NULL`, and its execution arms are persisted as `ParallelChild` rows with their own `Cost`. "Parallel parent" means `RunType = 'ParallelParent'` and nothing else. Do **not** derive it from "has a child": `ParentID` is not parallel-only — the JSON-repair path and embedding runs set it on ordinary runs — so a has-a-child rule drops a real run's own cost from every sum while it still counts as priced. Parallel parents are excluded from both cost sums and unpriced counts. No backfill of historical `RunType` is possible (the children were never written), so historical parents keep `Cost` = selected arm and are treated as ordinary single runs.

4. **Rollups are derived from the hierarchy at query time.**
   Agent-level cost = `SUM(Cost)` over prompt runs whose resolved agent run (`vwAIUsageFacts.AgentRunID`) is in the run's subtree (recursive CTE bounded at depth 20, following the `CalculateRunCost` precedent), never `SUM(AIAgentRun.TotalCost)` over a set that can contain both a parent and its child.

5. **Tenant dimension = the scope pair `AIAgentRun.PrimaryScopeEntityID` / `PrimaryScopeRecordID`.**
   Carried onto the fact row through the prompt run's agent run. `AIPromptRun` holds no agent-run key — the agent layer owns that link as `AIAgentRunStep.TargetLogID`, and `vwAIUsageFacts` resolves it at query time (the step targeting the run itself, else its parent, else its grandparent), seeking `IX_AIAgentRunStep_TargetLogID`. It cascades to sub-agents (`base-agent.ts:7273-7274`), is indexed (`IX_AIAgentRun_PrimaryScope`), and is the tenant key enforced by downstream products. `CompanyID` is kept as the memory-scoping key it is documented as; sub-agents propagate it for memory scoping, and fact views expose it, but no new tenant column is added to run tables.
