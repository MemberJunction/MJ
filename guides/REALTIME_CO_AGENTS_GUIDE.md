...
| Per-driver voice plumbing for the effective config (`realtime.voice.providers`) | **Partially shipped** — provider-matched settings flow into the open `Config` bag (server-bridged: OpenAI spread, Gemini merge-last plus a `voice` → `speechConfig.voiceConfig.prebuiltVoiceConfig.voiceName` mapping on both topologies (mask-safe, so the mint token-locks it); AssemblyAI `voice` mapping; ElevenLabs `voice` → `tts.voice_id` on both topologies). **TODOs**: `OpenAIRealtime.CreateClientSession` does not fold `params.Config` into the minted client-direct session (clients apply `EffectiveConfigJson` instead); the native voice picker (`BuildRealtimeConfigOverridesJson` / `BuildRealtimeOverridesJson`) still hardcodes the `openai` provider key, so a runtime voice pick cannot target any other provider |
