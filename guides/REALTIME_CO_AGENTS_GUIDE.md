...
1. **The realtime-registered tool set is capability-gated.** Dynamic-capable drivers (`SupportsDynamicToolSet = true`: OpenAI Realtime, OpenAI Live, xAI) can project allowed direct actions from the target agent when `directActions.enabled: true`, executing them rapidly via `ActionEngine.RunAction` (with per-call timeouts) to avoid the 2–5s roundtrip of a full sub-agent run. For connect-bound, server-provisioned, or self-hosted compat providers (ElevenLabs Agents, Gemini Live, HuggingFace), toolsets are fixed or pre-provisioned at connect time, so they maintain the stable target-independent `invoke-target-agent` tool set (`INVOKE_TARGET_AGENT_TOOL_NAME` in `packages/AI/Agents/src/realtime/realtime-tool-broker.ts`) where all target-specific capabilities execute inside the delegated agent's own run. This keeps the provider contract identical across targets for fixed providers — and it is exactly what let the **ElevenLabs Agents** driver (a pre-provisioned, server-side-agent provider) fit the same model: its managed agent is created once with the stable tool set and reused across sessions (see [§2](#2-the-triple-registry-plugin-architecture)).
...
{ "realtime": {
    ...
    "voice": { "default": { "tone": "…", "speakingStyle": "…", "voice": "<voice id>",
                            "firstMessage": "<what the agent says first, verbatim>" },
...
- **`voice.providers.<provider>`** is matched to the resolved vendor's `DriverClass` by normalized prefix (`openai` ↔ `OpenAIRealtime`, …) and merged into the session's open **`Config` bag** — the same opaque per-driver pact every other config entry rides (a caller-supplied runtime `Config` wins per key). Server-bridged reach today: **OpenAI** spreads the bag into `session.update`; **Gemini** merges it last, consuming the neutral `voice` first and mapping it to `speechConfig.voiceConfig.prebuiltVoiceConfig.voiceName` (its `LiveConnectConfig` has no `voice` field, and the SDK's config converter is a path allowlist that would drop it silently — issue #3721); **AssemblyAI** maps `voice` → `output.voice`; **ElevenLabs** maps `voice` → the `tts.voice_id` entry of its `conversation_config_override` (on BOTH topologies — its managed agent enables the `tts.voiceId` override so each session can carry its own voice). **Client-direct reach**: `OpenAIRealtime.CreateClientSession` **does** fold `params.Config` into the minted session — `ExtractRealtimeFeatures` pulls the neutral `voice` out of the bag and `BuildAudioBlock` maps it to `audio.output.voice`, the same block used on the server-bridged path. The resolver additionally surfaces the full resolved config as **`EffectiveConfigJson`** (and the resolved vendor as **`DriverClass`**) on `StartRealtimeClientSessionResult`, so client drivers can apply provider settings browser-side and can report which vendor actually spoke.
...
| Agent speaks first (`realtime.voice.default.firstMessage`) | **Shipped** (#3557) for **ElevenLabs** (`agent.first_message` override, enabled on the managed agent — agents provisioned before the change are re-PATCHed on next use rather than silently dropping it) and **AssemblyAI** (`greeting`). **Remaining gap**: OpenAI and Gemini read the neutral `firstMessage` key nowhere, so a persona authoring one still opens silently on those vendors. |
| Per-driver voice plumbing for the effective config (`realtime.voice.default.voice` / `realtime.voice.providers`) | **Shipped for authoring** (#3530) — a voice is authored PROVIDER-AGNOSTICALLY as `realtime.voice.default.voice` and filed onto whichever vendor resolves, so a runtime pick is no longer OpenAI-only; both override builders emit that shape and `GetProviderVoiceSettings` resolves it after the model is chosen. Provider-matched bags still merge into the open `Config` bag (OpenAI spread, AssemblyAI/Inworld `output.voice`, HuggingFace `audio.output.voice`, ElevenLabs `voice` → `tts.voice_id` on both topologies, **Gemini** `voice` → `speechConfig.voiceConfig.prebuiltVoiceConfig.voiceName` on both topologies — mask-safe, so the ephemeral-token mint locks it, #3721), and `OpenAIRealtime.CreateClientSession` **does** fold `params.Config` (via `ExtractRealtimeFeatures` → `BuildAudioBlock`). **Remaining gap**: only `OpenAIRealtime`, `xAIRealtime` and `HuggingFaceRealtime` declare `SupportedVoices` (HF returns none), and the picker's dropdown is gated on that list — so for ElevenLabs/Gemini/Inworld/AssemblyAI the voice is reachable via agent metadata and programmatic hosts, but the picker still offers nothing. |
