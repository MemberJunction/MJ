...
| Can the agent **speak first**? | Not wired — the neutral `firstMessage` key is unread today | Not wired — the neutral `firstMessage` key is unread today | **Yes** — `agent.first_message` override (enabled on the managed agent). Without it the agent produces NO audio until it hears the user, whatever the prompt says (#3557) | **Yes** — the `greeting` field of the session object (fed by the neutral `firstMessage` key; the legacy `greeting` config key still works) |
