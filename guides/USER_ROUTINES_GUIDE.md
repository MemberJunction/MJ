...
| Live integration (deterministic, self-cleaning) | `packages/MJServer/integration-test-scripts/user-routines-tests.ts` — window edges, claim semantics, a full dispatcher pass over an Action-target fixture with run-row linkage; part of `npm run test:integration` (required before "done") |
