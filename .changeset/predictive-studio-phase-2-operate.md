...
**`@memberjunction/ng-dashboards`** — Predictive Studio "Models in Production" tab: after the Operate dialog runs / schedules / binds a model, reload the selected model's run history and scoring bindings so a just-created run appears immediately (the reactive model stream refreshes deploy state but not the on-demand run list). Also fixes the Operate dialog summary grammar and a stale idle-state caption.
