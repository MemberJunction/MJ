...
Rows now come from an `FKLookupStrategy` resolved through the class factory by `<HostEntity>.<Field>`, then `<RelatedEntity>`, then MJ's own default. The default ranks through `SearchEntity` (lexical, hybrid on request) and hydrates by ID, orders the browse list by the name field, applies the new `EntityField.RelatedEntityFilter` / `RelatedEntityOrderBy` metadata plus `[FKExtraFilter]` / `[FKOrderBy]` inputs, hides non-active rows behind an "Include inactive" toggle, escapes LIKE wildcards, and leads with the user's last picks for that field. A strategy can group its rows, give each a second line and chips, veto a pick and prefill the create form.
