...
- `VectorRecord` gains an optional `providerDirectives` field — an MJ-internal routing map set by ingestion and stripped before any external upsert.
...
- `providerDirectives` is stripped from each `VectorRecord` before any upsert call.
