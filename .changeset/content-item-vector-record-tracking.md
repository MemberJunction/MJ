...
- **`AutotagBaseEngine.VectorizeContentItems`** — after a successful upsert, persists the record ids it already generates: single-chunk items write `ContentItem.VectorRecordID`; multi-chunk items write ordered `ContentItemChunk` rows (replacing any prior rows first, atomically in a server-side transaction, so re-vectorization is idempotent). For multi-chunk items the item-level `VectorRecordID` is left null — the chunk table is the source of truth. Each chunk row is stamped `EmbeddingStatus='Complete'` with `LastEmbeddedAt` on creation.
