...
- **Search Engine Safeguards & 15% Zombie Leak Fix**:
  ...
  - In `migrations/v6/V202609171715__v6.1.x__Exclude_Content_Items_And_Zombie_Entities_From_User_Search.sql` and `metadata/entities/.entities.json`, sets `AllowUserSearchAPI = 0` and `AutoUpdateAllowUserSearchAPI = 0` for `MJ: Content Items` and 12 internal/zombie entities (`MJ: Magic Link Invites`, `MJ: Magic Link Invite Allowed Domains`, `MJ: Magic Link Invite Allowed Paths`, `MJ: Magic Link Invite Applications`, `MJ: Magic Link Invite Roles`, `MJ: Magic Link Redemptions`, `MJ: Materialized Results`, `MJ: Materialized Result Queries`, `MJ: RSU Pending Works`, `MJ: AI Skill Search Scopes`, `MJ: Cluster Analysis Clusters`, `MJ: Employees`).
