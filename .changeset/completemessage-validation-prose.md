...
`CompleteMessage` also no longer says the same thing twice. It seeds from `Message` and then appends `Error` and every `Errors` entry, so a producer that builds its `Message` OUT OF its `Errors` rendered everything twice — `_InnerSave` and `_InnerDelete` do exactly that on an IS-A parent failure (`Failed to save parent entity 'X': A; B`, with `Errors` then set to the same A and B). An entry is now skipped only when its text is already present in what has been built, so nothing a reader could not already see is dropped.
