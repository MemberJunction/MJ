...
- **DO** include a `primaryKey` on every new record, holding a UUID generated at the CLI with
  `uuidgen` — or with `crypto.randomUUID()` where `uuidgen` is unavailable, as it is in the research
  environment. The record then gets the same deterministic ID in every environment instead of a fresh
  one per database. Never invent a UUID by hand. (`metadata/CLAUDE.md` §1.)
- Do **NOT** include `sync` objects — those are written back by `mj sync push` and must never be
  hand-authored
...
Example of a properly formatted new model entry (a `uuidgen` `primaryKey` on every record, NO
`sync` anywhere):
...
Every `<uuidgen>` above is a distinct UUID — one per record, never reused. Verify uniqueness across
`metadata/` before committing.

**For NEW vendors** — append entries to `.ai-vendors.json` with the same rules (its own `uuidgen`
`primaryKey`, no `sync`):
