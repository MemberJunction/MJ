...
- Include a `primaryKey` with a freshly generated v4 UUID (uppercase): run `uuidgen | tr '[:lower:]' '[:upper:]'`, or `node -e "console.log(crypto.randomUUID().toUpperCase())"` where `uuidgen` is not installed. Never invent or infer a UUID by hand, and never reuse one. This is `metadata/CLAUDE.md` rule 1: a hardcoded key gives the record the same ID in every environment
- Do NOT include a `sync` object — the push writes it back
...
Example of a properly formatted new model entry (`primaryKey` present, NO sync):
...
**For NEW vendors** — append entries to `.ai-vendors.json` with the same rules (`uuidgen` primaryKey, no
sync):
